        ...
        void JustDied(Unit* killer)
        {
            ...
            if (instance)
            {
                if (IsHeroic())
                {
                    uint32 criteria = 0;
                    if (killer->ToPlayer())
                        criteria = killer->ToPlayer()->GetTeam() == TEAM_ALLIANCE ? 14402 : 14401;
                    else if (killer->GetOwner() && killer->GetOwner()->ToPlayer())
                        criteria = killer->GetOwner()->ToPlayer()->GetTeam() == TEAM_ALLIANCE ? 14402 : 14401;

                    if (criteria)
                        instance->CompleteGuildCriteriaForGuildGroup(criteria);
                }
