    ...
    public:
        ...
        struct instance_trial_of_the_crusader_InstanceMapScript : public InstanceScript
        {
            ...
            void OnPlayerEnter(Player* player) override
            {
                ...
                if (IsDone(BOSS_LICH_KING) && TrialCounter && !IsDone(BOSS_ANUBARAK))
                {
                        ...
                        switch (state)
                        {
                            ...
                            case SPECIAL:
                                if (GetBossState(BOSS_VALKIRIES) == SPECIAL)
                                    state = instance->IsHeroic() ? DONE_HM : DONE;
                                break;
                            case DONE:
                            case DONE_HM:
            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
                    case TYPE_COUNTER:
                        TrialCounter = data;
                        data = instance->IsHeroic() ? DONE_HM : DONE;
                        break;
                    case TYPE_EVENT:
                        EventStage = data;
                        data = NOT_STARTED;
                    ...
                    case TYPE_NORTHREND_BEASTS:
                        ...
                        switch (data)
                        {
                            ...
                            case ICEHOWL_DONE:
                                EventStage = 400;
                                SetData(TYPE_NORTHREND_BEASTS, instance->IsHeroic() ? DONE_HM : DONE);
                                SetBossState(BOSS_BEASTS, instance->IsHeroic() ? DONE_HM : DONE);
                                break;
                            case FAIL:
