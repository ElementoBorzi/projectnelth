                        ...
                        case EVENT_THROW:
                            for (uint8 i = 0; i < MAX_SNOBOLDS; ++i)
                            {
                                if (auto pSnobold = me->GetVehicleKit()->GetPassenger(i))
                                {
                                    ...
                                    pSnobold->ToCreature()->SetInCombatWithZone();
            ...
            void DoAction(int32 const action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_DISABLE_FIRE_BOMB:
                        _events.CancelEvent(EVENT_FIRE_BOMB);
                        _events.ScheduleEvent(EVENT_BATTER, 5*IN_MILLISECONDS);
                        _events.ScheduleEvent(EVENT_HEAD_CRACK, 1*IN_MILLISECONDS);
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
                        break;
                    default:
                        break;
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_BATTER:
                            ...
                            if (auto target = ObjectAccessor::GetPlayer(*me, _targetGUID))
                            {
                                ...
                                target->AddAura(SPELL_HEAD_CRACK, target);
                                if (!target->isAlive())
                                    BackToGornock();
                            }
                            _events.ScheduleEvent(EVENT_BATTER, 3*IN_MILLISECONDS);
                            return;
                        default:
                            return;
                    }
    ...
    public:
        ...
        struct boss_icehowlAI : public BossAI
        {
            ...
            void UpdateAI(uint32 const diff) override
            {
                ...
                if (me->HasUnitState(UNIT_STATE_CASTING) && _stage == 0)
                    return;
