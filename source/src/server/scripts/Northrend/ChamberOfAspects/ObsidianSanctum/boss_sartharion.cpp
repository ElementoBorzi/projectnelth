...
Waypoint sarthDummyDragonCommon[MAX_WAYPOINT] =
{
...
public:
    ...
    {
        boss_sartharionAI(Creature* creature) : BossAI(creature, DATA_SARTHARION_EVENT), summons(me)
        {
        ...
        uint32 m_uiLavaStrikeTimer_single;
        ...
        void Reset()
        {
            ...
            m_uiLavaStrikeTimer_single = urand(5000, 20000);
            ...
            if (instance)
            {
                ...
                if (pTenebron)
                {
                    ...
                    {
                        if (instance->GetData(DATA_TENEBRON_PREKILLED) == false)
                        {
                            pTenebron->Respawn();
                            pTenebron->GetMotionMaster()->MoveTargetedHome();
                ...
                if (pShadron)
                {
                    ...
                    {
                        if (instance->GetData(DATA_SHADRON_PREKILLED) == false)
                        {
                            pShadron->Respawn();
                            pShadron->GetMotionMaster()->MoveTargetedHome();
                ...
                if (pVesperon)
                {
                    ...
                    {
                        if (instance->GetData(DATA_VESPERON_PREKILLED) == false)
                        {
                            pVesperon->Respawn();
                            pVesperon->GetMotionMaster()->MoveTargetedHome();
        ...
        void EnterCombat(Unit* pWho)
        {
            ...
            for (std::list<Unit*>::iterator itr = targets.begin(); itr != targets.end(); itr++)
            {
                if (auto creature = (*itr)->ToCreature())
                {
                    if (me->getVictim())
                    {
                        if (creature->AI() && creature->CanAssistTo(me, me->getVictim(), false) && creature->GetEntry() != NPC_TENEBRON && creature->GetEntry() != NPC_SHADRON && creature->GetEntry() != NPC_VESPERON)
                        {
        ...
        void JustDied(Unit* pKiller)
        {
            ...
            if (instance)
            {
                ...
                if (pTenebron && pTenebron->isAlive())
                    pTenebron->Kill(pTenebron);
                if (pShadron && pShadron->isAlive())
                    pShadron->Kill(pShadron);
                if (pVesperon && pVesperon->isAlive())
                    pVesperon->Kill(pVesperon);
            }
        ...
        void FetchDragons()
        {
            ...
            if (pFetchTene && pFetchTene->isAlive() && !pFetchTene->getVictim())
            {
                ...
                pFetchTene->GetMotionMaster()->MovePoint(POINT_ID_INIT, sarthTene[0].m_fX, sarthTene[0].m_fY, sarthTene[0].m_fZ);
            ...
            if (pFetchShad && pFetchShad->isAlive() && !pFetchShad->getVictim())
            {
                ...
                pFetchShad->GetMotionMaster()->MovePoint(POINT_ID_INIT, sarthShad[0].m_fX, sarthShad[0].m_fY, sarthShad[0].m_fZ);
            ...
            if (pFetchVesp && pFetchVesp->isAlive() && !pFetchVesp->getVictim())
            {
                ...
                pFetchVesp->GetMotionMaster()->MovePoint(POINT_ID_INIT, sarthVesp[0].m_fX, sarthVesp[0].m_fY, sarthVesp[0].m_fZ);
                ...
                if (Creature *pTemp = Unit::GetCreature(*me, instance->GetData64(uiDataId)))
                {
                    if (pTemp->isAlive() && !pTemp->getVictim())
                    {
                        ...
                        case NPC_TENEBRON:
                            ...
                            pTemp->GetMotionMaster()->MovePoint(POINT_ID_LAND, sarthTene[1].m_fX, sarthTene[1].m_fY, sarthTene[1].m_fZ);
                            break;
                        case NPC_SHADRON:
                            iTextId = SAY_SARTHARION_CALL_SHADRON;
                            //            pTemp->AddAura(SPELL_POWER_OF_SHADRON, pTemp);
                            pTemp->GetMotionMaster()->MovePoint(POINT_ID_LAND, sarthShad[1].m_fX, sarthShad[1].m_fY, sarthShad[1].m_fZ);
                            break;
                        case NPC_VESPERON:
                            iTextId = SAY_SARTHARION_CALL_VESPERON;
                            //            pTemp->AddAura(SPELL_POWER_OF_VESPERON, pTemp);
                            pTemp->GetMotionMaster()->MovePoint(POINT_ID_LAND, sarthVesp[1].m_fX, sarthVesp[1].m_fY, sarthVesp[1].m_fZ);
                            break;
                        }
        ...
        void CastLavaStrike(bool addAura)
        {
            ...
            if (!m_bIsSoftEnraged)
            {
                advance(itr, rand() % pFireCyclonesList.size());
                if (addAura)
                {
            ...
            else
            {
                ...
                for (itr; itr != pFireCyclonesList.end(); ++itr)
                    (*itr)->CastSpell(*itr, SPELL_CYCLONE_AURA2_1, true);
            }
        }
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (!m_bIsBerserk && HealthBelowPct(35))
            {
            ...
            // Lavas Strike single
            if (m_uiLavaStrikeTimer_single <= uiDiff)
            {
                CastLavaStrike(false);
                m_uiLavaStrikeTimer_single = urand(5000, 20000);
            }
            else
                m_uiLavaStrikeTimer_single -= uiDiff;
        ...
        void playerHitByVolcano(uint64 guid)
        {
            players.erase(guid);
        }
    };
...
struct dummy_dragonAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        {
            case NPC_TENEBRON:
                instance->SetBossState(DATA_TENEBRON_EVENT, NOT_STARTED);
                break;
            case NPC_SHADRON:
                instance->SetBossState(DATA_SHADRON_EVENT, NOT_STARTED);
                break;
            case NPC_VESPERON:
                instance->SetBossState(DATA_VESPERON_EVENT, NOT_STARTED);
                break;
        }
    ...
    void EnterCombat(Unit*)
    {
        ...
        {
            case NPC_TENEBRON:
                instance->SetBossState(DATA_TENEBRON_EVENT, IN_PROGRESS);
                break;
            case NPC_SHADRON:
                instance->SetBossState(DATA_SHADRON_EVENT, IN_PROGRESS);
                break;
            case NPC_VESPERON:
                instance->SetBossState(DATA_VESPERON_EVENT, IN_PROGRESS);
                break;
        }
    }
    ...
    {
        if (!instance || uiType != POINT_MOTION_TYPE /*|| uiPointId == POINT_ID_HOME*/)
            return;

        //        sLog->outStaticDebug("dummy_dragonAI: %s reached point %u", me->GetName(), uiPointId);
        ...
        uint32 uiCommonWPCount = sizeof(sarthDummyDragonCommon) / sizeof(Waypoint);
    ...
    void JustDied(Unit* pKiller)
    {
        ...
        case NPC_TENEBRON:
            ...
            {
                if (instance->GetBossState(DATA_SARTHARION_EVENT) != IN_PROGRESS)
                    instance->SetData(DATA_TENEBRON_PREKILLED, 1);
                instance->SetBossState(DATA_TENEBRON_EVENT, DONE);
            }
            break;
        case NPC_SHADRON:
            ...
            if (instance)
                if (instance->GetBossState(DATA_SARTHARION_EVENT) != IN_PROGRESS)
                    instance->SetData(DATA_SHADRON_PREKILLED, 1);
            instance->SetBossState(DATA_SHADRON_EVENT, DONE);
            break;
        case NPC_VESPERON:
            ...
            if (instance)
                if (instance->GetBossState(DATA_SARTHARION_EVENT) != IN_PROGRESS)
                    instance->SetData(DATA_VESPERON_PREKILLED, 1);
            instance->SetBossState(DATA_VESPERON_EVENT, DONE);
            break;
        }
        ...
        if (m_bCanMoveFree && m_uiMoveNextTimer)
        {
            if (m_uiMoveNextTimer <= uiDiff)
            {
                if (m_uiWaypointId < MAX_WAYPOINT)
                    ...
                    sarthDummyDragonCommon[m_uiWaypointId].m_fX, sarthDummyDragonCommon[m_uiWaypointId].m_fY, sarthDummyDragonCommon[m_uiWaypointId].m_fZ);

                //    sLog->outStaticDebug("dummy_dragonAI: %s moving to point %u", me->GetName(), m_uiWaypointId);
                m_uiMoveNextTimer = 0;
            }
            else
            ...
            if (!m_bHasPortalOpen)
...

                {
                    ...
                    else
                    {
                        ...
                        if (instance)
                        {
                            Creature* pSartharion = Unit::GetCreature(*me, instance->GetData64(DATA_SARTHARION));
                            if (instance->GetBossState(DATA_SARTHARION_EVENT) == IN_PROGRESS)
                            {
                                if (pSartharion)
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (!m_bHasPortalOpen)
...

                {
                    ...
                    {
                        OpenPortal();
                        if (instance && instance->GetBossState(DATA_SARTHARION_EVENT) == IN_PROGRESS)
...
public:
    ...
    struct mob_acolyte_of_shadronAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer)
        {
            if (instance)
            {
                Creature* pShadron = instance->instance->GetCreature(instance->GetData64(DATA_SHADRON));

                if (pShadron)
                {
                    ((mob_shadron::mob_shadronAI*)pShadron->AI())->m_bHasPortalOpen = false;
                }
...
public:
    ...
    struct mob_twilight_eggsAI : public Scripted_NoMovementAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            if (instance)
            {
                Creature* tenebron = CAST_CRE(Unit::GetUnit(*me, instance->GetData64(DATA_TENEBRON)));
                if (!tenebron)
                    return;

                mob_tenebron::mob_tenebronAI* tenebronAI = CAST_AI(mob_tenebron::mob_tenebronAI, tenebron->AI());
                if (!tenebronAI)
                    return;

                tenebronAI->handleEggsDead();
            }
        }
        ...
        void SpawnWhelps()
        {
            ...
            Creature* pTenebron = instance->instance->GetCreature(instance->GetData64(DATA_TENEBRON));
            if (pTenebron)
            {
                if (instance->GetBossState(DATA_SARTHARION_EVENT) == IN_PROGRESS)
                {
                    if (Creature *creature = pTenebron->SummonCreature(NPC_SHARTHARION_TWILIGHT_WHELP, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300 * IN_MILLISECONDS))

                        if (Unit *pTarget = pTenebron->AI()->SelectTarget(SELECT_TARGET_RANDOM))
                            creature->AI()->AttackStart(pTarget);
                }
                else
                    if (Creature *creature = pTenebron->SummonCreature(NPC_TWILIGHT_WHELP, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300 * IN_MILLISECONDS))
                        if (Unit *pTarget = pTenebron->AI()->SelectTarget(SELECT_TARGET_RANDOM))
                            creature->AI()->AttackStart(pTarget);
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (m_uiHatchEggTimer <= uiDiff)
            {
                ...
                if (pTenebron)
                    ((mob_tenebron::mob_tenebronAI*)pTenebron->AI())->m_bHasPortalOpen = false;
                SpawnWhelps();
            }
            else
                m_uiHatchEggTimer -= uiDiff;
...
public:
    ...
    class spell_gen_lava_strike_57591_SpellScript : public SpellScript
    {
        ...
        void HandleOnHit()
        {
            ...
            if (instance)
            {
                Creature* sartharion = CAST_CRE(Unit::GetUnit(*GetCaster(), instance->GetData64(DATA_SARTHARION)));
                if (!sartharion)
                    return;

                boss_sartharion::boss_sartharionAI* sartharionAI = CAST_AI(boss_sartharion::boss_sartharionAI, sartharion->AI());
                if (!sartharionAI)
                    return;
...
public:
    ...
    class spell_gen_lava_strike_57572_SpellScript : public SpellScript
    {
        ...
        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_gen_lava_strike_57572_SpellScript::DoNothing, EFFECT_0, SPELL_EFFECT_SUMMON);
        }
    };
...
public:
    ...
    class spell_gen_lava_strike_57578_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            PreventHitDefaultEffect(effIndex);
            caster->CastSpell(hitUnit, SPELL_TRIGGERED, true);
        }
            ...
            bool Load()
        {
            if (auto caster = GetCaster())
            {
                if (caster->GetMap())
                {
                    ...
                    {
                        SetSpellValue(SPELLVALUE_BASE_POINT0, 3000);
                        SetSpellValue(SPELLVALUE_BASE_POINT1, 3000);
                    }
                    else
                    {
                        SetSpellValue(SPELLVALUE_BASE_POINT0, 1500);
                        SetSpellValue(SPELLVALUE_BASE_POINT1, 1500);
                    }
                }
            }
            return true;
...
class go_twilight_portal_sartharion : public GameObjectScript
{
public:
    go_twilight_portal_sartharion() : GameObjectScript("go_twilight_portal_sartharion") { }
    ...
    bool OnGossipHello(Player* pPlayer, GameObject* pGO)
    {
        ...
        if (pPlayer)
        {
            pPlayer->AddAura(SPELL_TWILIGHT_SHIFT_ENTER, pPlayer);
            pPlayer->AddAura(SPELL_TWILIGHT_SHIFT, pPlayer);
        }
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, GameObject* pGO)
    {
        ...
        if (pPlayer)
            pPlayer->CastSpell(pPlayer, SPELL_TWILIGHT_SHIFT_REMOVAL, true);
...
void AddSC_boss_sartharion()
{
    ...
    new go_twilight_portal_sartharion();
