    ...
    public:
        ...
        struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
        {
            instance_icecrown_citadel_InstanceMapScript(InstanceMap* map) : InstanceScript(map)
            {
                ...
                GunshipErsatzbossGUID = 0;
                GunshipErsatzboss = NOT_STARTED;
                CheckDeathTimer = 20000;
                addtargetGUID = 0;
                ICC_buff_state = 0;
            }
            ...
            void OnCreatureCreate(Creature* creature)
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_HORDE_GUNSHIP_CANNON:
                        creature->SetControlled(true, UNIT_STATE_ROOT); // wtf ?
                        break;
                    ...
                    case NPC_GUNSHIP_ERSAZBOSS:
                        GunshipErsatzbossGUID = creature->GetGUID();
                        break;
                    case NPC_GUNSHIP_START:
                        if (creature->isDead())
                            creature->Respawn();
                        break;
                    default:
            ...
            void OnGameObjectCreate(GameObject* go)
            {
                switch (go->GetEntry())
                {
                    ...
                    case GO_LADY_DEATHWHISPER_ELEVATOR:
                        ...
                        //if (IsDone(DATA_LADY_DEATHWHISPER))
                            //go->SetControlableTransport(false); Hyjarion: function makes no sense, deleted it
                        //else
                            //go->SetControlableTransport(true); Hyjarion: function makes no sense, deleted it
                        //break;
                    ...
                    case GO_SCIENTIST_AIRLOCK_DOOR_COLLISION:
                        ...
                        if (IsDone(DATA_FESTERGUT) && IsDone(DATA_ROTFACE))
                    ...
                    case GO_SINDRAGOSA_ENTRANCE_DOOR:
                        if (GetData(DATA_SINDRAGOSAS_GAUNTLET) == DONE || GetData(DATA_SINDRAGOSAS_GAUNTLET) == DONE_HM)
            ...
            uint32 GetData(uint32 type)  const
            {
                switch (type)
                {
                    ...
                    case DATA_BOSS_GUNSHIP_ERSATZ:
                        return GunshipErsatzboss;
                    case DATA_ZONE_BUFF_STATUS:
                        return ICC_buff_state;
                    default:
                        break;
                }
            ...
            uint64 GetData64(uint32 type)  const
            {
                switch (type)
                {
                    ...
                    case DATA_GUNSHIP_ERSATZBOSS:
                        return GunshipErsatzbossGUID;
            ...
            bool SetBossState(uint32 type, EncounterState state)
            {
                ...
                switch (type)
                {
                    ...
                    case DATA_ROTFACE:
                        ...
                        break;
                    case DATA_PROFESSOR_PUTRICIDE:
                        HandleGameObject(PlagueSigilGUID, state != DONE && state != DONE_HM);
                        if (state == DONE || state == DONE_HM)
                        ...
                        break;
                    case DATA_BLOOD_QUEEN_LANA_THEL:
                        HandleGameObject(BloodwingSigilGUID, state != DONE && state != DONE_HM);
                        if (state == DONE || state == DONE_HM)
                            CheckLichKingAvailability();
                        if (instance->IsHeroic())
                        {
                        ...
                        break;
                    case DATA_VALITHRIA_DREAMWALKER:
                        if ((state == DONE || state == DONE_HM) && sPoolMgr->IsSpawnedObject<Quest>(WeeklyQuestData[8].questId[instance->GetSpawnMode() & 1]))
                        ...
                        break;
                    case DATA_SINDRAGOSA:
                        HandleGameObject(FrostwingSigilGUID, state != DONE && state != DONE_HM);
                        if (state == DONE || state == DONE_HM)
                            CheckLichKingAvailability();
                        if (instance->IsHeroic())
                        {
            ...
            void SpawnGunship(Player *pPlayer = NULL)
            {
                if (!GunshipGUID)
                {
                    ...
                    if (Transport* gunship = sMapMgr->LoadTransportInMap(instance, gunshipEntry, 0))
                    {
                        gunship->BuildStopMovePacket(instance);
                        GunshipGUID = gunship->GetGUID();
                        gunship->LoadStaticPassengers();
                    }
                }
                else if (pPlayer)
                {
            ...
            bool CheckCrimsonHalls(uint32 bossId) const
            {
                switch (bossId)
                {
                    case DATA_THE_LICH_KING:
                        ...
                        // no break
                    case DATA_BLOOD_QUEEN_LANA_THEL:
                        if (!IsDone(DATA_BLOOD_PRINCE_COUNCIL))
                            return false;
                        break;
                    default:
                        break;
            ...
            bool CheckFrostwingHalls(uint32 bossId) const
            {
                switch (bossId)
                {
                    case DATA_THE_LICH_KING:
                        ...
                        // no break
                    case DATA_SINDRAGOSA:
                        if (!IsDone(DATA_VALITHRIA_DREAMWALKER))
                            return false;
                        break;
                    default:
                        break;
            ...
            bool CheckLowerSpire(uint32 bossId) const
            {
                switch (bossId)
                {
                    ...
                    case DATA_FESTERGUT:
                        ...
                        // no break
                    case DATA_DEATHBRINGER_SAURFANG:
                        if (!IsDone(DATA_ICECROWN_GUNSHIP_BATTLE))
                            return false;
                        // no break
                    case DATA_ICECROWN_GUNSHIP_BATTLE:
                        if (!IsDone(DATA_LADY_DEATHWHISPER))
            ...
            void ReadSaveDataMore(std::istringstream& data) override
            {
                ...
                data >> temp;
                GunshipErsatzboss = temp ? DONE : NOT_STARTED;
            }
        ...
        protected:
            ...
            uint64 GunshipErsatzbossGUID;
            uint64 IceShardGUID;
            uint64 IceShardGUID2;
            uint64 IceShardGUID3;
            uint64 IceShardGUID4;
            ...
            uint32 GunshipErsatzboss;
            uint16 BloodQuickeningMinutes;
            uint16 CheckDeathTimer;
            uint64 addtargetGUID;
            bool IsBonedEligible;
            bool IsOozeDanceEligible;
            bool IsNauseaEligible;
            bool IsOrbWhispererEligible;
            uint32 ICC_buff_state;
        };
