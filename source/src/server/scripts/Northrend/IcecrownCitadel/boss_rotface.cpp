...
enum Spells
{
    ...
    SPELL_MUTATED_INFECTION_SUMMON          = 69706,
    ...
    public:
        ...
        struct boss_rotfaceAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                ...
                me->setActive(true);
                Talk(SAY_AGGRO);
                if (auto professor = Unit::GetCreature(*me, instance->GetData64(DATA_PROFESSOR_PUTRICIDE)))
...
class npc_little_ooze : public CreatureScript
{
    public:
        ...
        struct npc_little_oozeAI : public ScriptedAI
        {
            ...
            {
                summon->CastSpell(summon, SPELL_STICKY_OOZE_BUFF, true);
            }
...
class npc_precious_icc : public CreatureScript
{
    public:
        ...
        struct npc_precious_iccAI : public ScriptedAI
        {
            ...
            {
                _summons.DespawnAll();
                if (auto rotface = Unit::GetCreature(*me, _instance->GetData64(DATA_ROTFACE)))
                    if (rotface->isAlive())
...
class spell_rotface_mutated_infection_summon : public SpellScriptLoader
{
public:
    spell_rotface_mutated_infection_summon() : SpellScriptLoader("spell_rotface_mutated_infection_summon")
    {
    }

    class spell_rotface_mutated_infection_summon_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_rotface_mutated_infection_summon_SpellScript);
        ...
        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_rotface_mutated_infection_summon_SpellScript::HandleSummon, EFFECT_0, SPELL_EFFECT_SUMMON);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_rotface_mutated_infection_summon_SpellScript();
    }
};
        ...
        class spell_rotface_large_ooze_combine_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Aura* unstable = GetCaster()->GetAura(SPELL_UNSTABLE_OOZE))
                {
                    ...
                    uint32 newStackAmount = unstable->GetStackAmount() + ((targetAura != NULL && targetAura->GetStackAmount() > 0) ? targetAura->GetStackAmount() : 1);
                    if (newStackAmount >= 5)
                        newStackAmount = 5;
                    unstable->SetStackAmount(newStackAmount);
                    if (newStackAmount >= 5)
                    {
                        if (auto instance = GetCaster()->GetInstanceScript())
                            if (auto rotface = Unit::GetCreature(*GetCaster(), instance->GetData64(DATA_ROTFACE)))
                                if (rotface->isAlive())
                                {
                                ...
                                if (auto cre = GetCaster()->ToCreature())
    ...
    public:
        ...
        class spell_rotface_large_ooze_buff_combine_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Aura* unstable = GetCaster()->GetAura(SPELL_UNSTABLE_OOZE))
                {
                    uint32 newStack = unstable->GetStackAmount() + 1;
        ...
        class spell_rotface_unstable_ooze_explosion_SpellScript : public SpellScript
        {
            ...
            void CheckTarget(SpellEffIndex effIndex)
            {
                ...
                if (auto script = GetCaster()->GetInstanceScript())
                    if (auto rotface = script->instance->GetCreature(script->GetData64(DATA_ROTFACE)))
...
void AddSC_boss_rotface()
{
    ...
    new spell_rotface_mutated_infection_summon();
