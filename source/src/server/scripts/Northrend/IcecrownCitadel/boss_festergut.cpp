...
enum Spells
{
    ...
    SPELL_GAS_SPORE_AURA        = 69279,
    SPELL_VILE_GAS_10_NH        = 69240,
    SPELL_VILE_GAS_25_NH        = 71218,
...
#define SPELL_VILE_GAS RAID_MODE<uint32>(SPELL_VILE_GAS_10_NH, SPELL_VILE_GAS_25_NH, SPELL_VILE_GAS_10_H, SPELL_VILE_GAS_25_H)
...
class boss_festergut : public CreatureScript
{
    public:
        ...
        struct boss_festergutAI : public BossAI
        {
            ...
            {
                if (me->GetPositionZ() < 360.0f)
                    me->SetPosition(me->GetPositionX(), me->GetPositionY(), 360.5f, me->GetOrientation());
                events.Reset();
                summons.DespawnAll();
    ...
    public:
        ...
        class spell_festergut_blighted_spores_AuraScript : public AuraScript
        {
            ...
            void ExtraEffect(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                uint32 auraId = sSpellMgr->GetSpellIdForDifficulty(SPELL_INOCULATED, GetCaster());
                uint32 effectId = sSpellMgr->GetSpellIdForDifficulty(69290, GetCaster());
                if (GetTarget()->ToPlayer()->HasSpellCooldown(auraId))
                    if (Aura* aura = GetTarget()->GetAura(auraId))
                    {
                        uint32 spellids = 0;
                        uint32 maxAura = GetTarget()->GetMap()->Is25ManRaid() ? 3 : 2;
                        if (GetTarget()->ToPlayer()->HasSpellCooldown(auraId))
                            if (Aura* auraEff = GetTarget()->GetAura(effectId))
                                if (auraEff->GetStackAmount() < maxAura)
                                    auraEff->SetStackAmount(auraEff->GetStackAmount() + 1);
                        return;
                    }
