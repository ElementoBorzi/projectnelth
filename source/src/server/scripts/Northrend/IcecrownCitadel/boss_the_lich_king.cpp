...
enum Spells
{
    ...
    SPELL_VILE_SPIRIT_PROC              = 70497,    // Workaround for frostmourne arena spawns
...
enum Events
{
    ...
    EVENT_EXIT_VEHICLE              = 64,

    // lk start fight
    EVENT_START_PHASE_1             = 65,
...
Position const OutroDead          = {508.9897f, -2124.561f, 840.8568f, 0.0f};
...
class StartMovementEvent : public BasicEvent
{
    public:
        ...
        bool Execute(uint64 /*time*/, uint32 /*diff*/)
        {
            ...
            if (auto target = _summoner->AI()->SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(_summoner)))
...
class VileSpiritActivateEvent : public BasicEvent
{
    public:
        ...
        bool Execute(uint64 /*time*/, uint32 /*diff*/)
        {
            _owner->AI()->DoAction(ACTION_VILE_SPIRIT_ATTACK);
            _owner->SetReactState(REACT_AGGRESSIVE);
    ...
    public:
        ...
        struct boss_the_lich_kingAI : public BossAI
        {
            ...
            void JustDied(Unit* /*killer*/) override
            {
                ...
                if (auto terenas = me->FindNearestCreature(NPC_TERENAS_MENETHIL_OUTRO, 100.0f, true))
                    terenas->DespawnOrUnsummon();

                if (auto frostmourne = me->FindNearestCreature(NPC_FROSTMOURNE_TRIGGER, 50.0f))
            ...
            void EnterEvadeMode()
            {
                ...
                _EnterEvadeMode();
                if (auto tirion = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_HIGHLORD_TIRION_FORDRING)))
            ...
            void JustSummoned(Creature* summon)
            {
                switch (summon->GetEntry())
                {
                    ...
                    case NPC_VILE_SPIRIT:
                    {
                        ...
                        summon->SetReactState(REACT_PASSIVE);
                        summon->SetSpeed(MOVE_FLIGHT, 0.9f);
                        summon->GetMotionMaster()->MoveRandom(10.0f);

                        if(summon->GetMap()->IsHeroic() && events.GetNextEventTime(EVENT_HARVEST_SOULS) - events.GetTimer() < 20000)
                           summon->m_Events.AddEvent(new VileSpiritActivateEvent(summon), summon->m_Events.CalculateTime(20000)); // last spawn on heroic needs a delay
                        else
            ...
            void MovementInform(uint32 type, uint32 pointId)
            {
                ...
                switch (pointId)
                {
                    ...
                    case POINT_CENTER_2:
                        ...
                        events.ScheduleEvent(EVENT_VILE_SPIRITS, 77500, EVENT_GROUP_VILE_SPIRITS, PHASE_THREE);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FINISH_INTRO:
                            if (auto tirion = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_HIGHLORD_TIRION_FORDRING)))
                                tirion->AddAura(SPELL_ICE_LOCK, tirion);
                            me->SetWalk(false);
                            me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
                            me->ApplySpellImmune(10326, IMMUNITY_STATE, SPELL_AURA_MOD_FEAR, true); //Turn Evil
                            me->ApplySpellImmune(57546, IMMUNITY_STATE, SPELL_AURA_MOD_FEAR, true); // Glyphed Turn Evil
                            me->SetReactState(REACT_AGGRESSIVE);
                            events.ScheduleEvent(EVENT_START_PHASE_1, 500);
                            break;
                        case EVENT_START_PHASE_1:
                        {
                            ...
                            events.ScheduleEvent(EVENT_NECROTIC_PLAGUE, urand(28000, 30000), 0, PHASE_ONE); // old 30 - 33 sec but #2 infest is casted after 31-33sec...
                        ...
                        case EVENT_NECROTIC_PLAGUE:
                            if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 1, NecroticPlagueTargetCheck(me, NECROTIC_PLAGUE_LK, NECROTIC_PLAGUE_PLR)))
                            {
                        ...
                        case EVENT_SOUL_REAPER:
                            ...
                            break;
                        case EVENT_DEFILE:
                            if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_HARVEST_SOUL_VALKYR))
                            {
                        ...
                        case EVENT_HARVEST_SOUL:
                            ...
                            if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, SpellTargetSelector(me, SPELL_HARVEST_SOUL)))
                        ...
                        case EVENT_START_ATTACK:
                            ...
                            if (events.IsInPhase(PHASE_FROSTMOURNE))
                            {
                                ...
                                if(IsHeroic())
                                {
                                    events.ScheduleEvent(EVENT_VILE_SPIRITS, 10000, EVENT_GROUP_VILE_SPIRITS, PHASE_THREE);
                                    events.ScheduleEvent(EVENT_HARVEST_SOULS, 60000, 0, PHASE_THREE);
                                }
                            }
                            break;
                        case EVENT_VILE_SPIRITS:
                            ...
                            events.ScheduleEvent(EVENT_VILE_SPIRITS, 30500, EVENT_GROUP_VILE_SPIRITS, PHASE_THREE);
                            break;
                        case EVENT_HARVEST_SOULS:
                            ...
                            me->SetReactState(REACT_PASSIVE);
                            me->AttackStop();
                            events.CancelEventGroup(EVENT_GROUP_VILE_SPIRITS); // event restarts after frostmourne arena
            ...
            void RelocateDeadPlayers()
            {
                ...
                if (!players.isEmpty())
                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                        if (auto player = itr->getSource())
                            if (!player->isAlive())
                                player->TeleportTo(631, 503.6282f, -2124.655f, 840.8569f, 0.0f);
            }
...
class npc_tirion_fordring_tft : public CreatureScript
{
    public:
        ...
        struct npc_tirion_fordringAI : public ScriptedAI
        {
            ...
            {
                _events.Reset();
                if (_instance->IsDone(DATA_THE_LICH_KING))
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                ...
                switch (id)
                {
                    case POINT_TIRION_INTRO:
                        ...
                        if (auto theLichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_THE_LICH_KING)))
            ...
            void UpdateAI(uint32 const diff)
            {
                if (!UpdateVictim() && !_events.IsInPhase(PHASE_INTRO) && !_events.IsInPhase(PHASE_OUTRO))
                    return;
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_OUTRO_REMOVE_ICE:
                            ...
                            if (auto lichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_THE_LICH_KING)))
                            {
...
class npc_valkyr_shadowguard : public CreatureScript
{
    public:
        ...
        struct npc_valkyr_shadowguardAI : public ScriptedAI
        {
            ...
            {
                me->ApplySpellImmune(0, IMMUNITY_ID, 10326, true);
            }
                ...
                switch (id)
                {
                    case POINT_DROP_PLAYER:
                        ...
                        */
                        ...
                        break;
                    case POINT_CHARGE:
                        if (auto target = ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
                        {
                            me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                            if (auto platform = ObjectAccessor::GetGameObject(*me, _instance->GetData64(DATA_ARTHAS_PLATFORM)))
                            {
                                std::list<Creature*> triggers;
    ...
    public:
        ...
        struct npc_strangulate_vehicleAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                ...
                _events.ScheduleEvent(EVENT_EXIT_VEHICLE, 4500);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_EXIT_VEHICLE:
                        {
                            ...
                            _events.ScheduleEvent(EVENT_TELEPORT, 1500);
                            break;
                        }
                        case EVENT_TELEPORT:
    ...
    public:
        ...
        struct npc_terenas_menethilAI : public ScriptedAI
        {
            npc_terenas_menethilAI(Creature* creature) : ScriptedAI(creature),
                _instance(creature->GetInstanceScript())
            {
                me->ApplySpellImmune(0, IMMUNITY_ID, SPELL_SPIRIT_BURST, true);
            }
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& damage)
            {
                if (damage >= me->GetHealth())
                {
                    ...
                    if (!me->HasAura(SPELL_TERENAS_LOSES_INSIDE) && !IsHeroic())
                    {
                        ...
                        if (auto warden = me->FindNearestCreature(NPC_SPIRIT_WARDEN, 20.0f))
                        {
                            Map::PlayerList const& players = me->GetMap()->GetPlayers();
                            if (!players.isEmpty())
                                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                    if (auto player = itr->getSource())
                                        if (player->HasAura(SPELL_IN_FROSTMOURNE_ROOM) && player->isAlive())
                                            me->DealDamage(player, player->GetMaxHealth());
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_OUTRO_TERENAS_TALK_2:
                            ...
                            if (auto lichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_THE_LICH_KING)))
                            {
                                ...
                                if (auto tirion = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_HIGHLORD_TIRION_FORDRING)))
...
class npc_spirit_warden : public CreatureScript
{
    public:
        ...
        struct npc_spirit_wardenAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
                if (auto terenas = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_TERENAS_MENETHIL)))
...
class npc_wicked_spirit : public CreatureScript
{
    public:
        ...
        struct npc_wicked_spiritAI : public CreatureAI
        {
            npc_wicked_spiritAI(Creature* creature) : CreatureAI(creature)
            {
            ...
            {
                me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                me->CastSpell((Unit*)NULL, SPELL_VILE_SPIRIT_DAMAGE_SEARCH, true);
            }
            void UpdateAI(uint32 const diff){}
        };
...
class spell_the_lich_king_quake : public SpellScriptLoader
{
    public:
        ...
        class spell_the_lich_king_quake_SpellScript : public SpellScript
        {
            ...
            {
                if (auto platform = ObjectAccessor::GetGameObject(*GetCaster(), GetCaster()->GetInstanceScript()->GetData64(DATA_ARTHAS_PLATFORM)))
        ...
        class spell_the_lich_king_valkyr_target_search_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
               ...
               // GetCaster()->CastSpell(GetHitUnit(), SPELL_CHARGE, true);
                if (GetCaster())
                    GetCaster()->GetMotionMaster()->MoveCharge(GetHitUnit()->GetPositionX(), GetHitUnit()->GetPositionY(), GetHitUnit()->GetPositionZ()+3.0f);
            }
...
class spell_the_lich_king_restore_soul : public SpellScriptLoader
{
    public:
        ...
        class spell_the_lich_king_restore_soul_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                if (auto lichKing = ObjectAccessor::GetCreature(*GetCaster(), _instance->GetData64(DATA_THE_LICH_KING)))
                    lichKing->AI()->DoAction(ACTION_TELEPORT_BACK);
                if (auto spawner = GetCaster()->FindNearestCreature(NPC_WORLD_TRIGGER_INFINITE_AOI, 50.0f))
    ...
    public:
        ...
        class spell_the_lich_king_trigger_vile_spirit_SpellScript : public SpellScript
        {
            ...
            void ActivateSpirit()
            {
                ...
                if (!target || target->GetEntry() == NPC_THE_LICH_KING)
                    return;
...
class spell_the_lich_king_mass_resurect : public SpellScriptLoader
{
    public:
        spell_the_lich_king_mass_resurect() : SpellScriptLoader("spell_the_lich_king_mass_resurect") { }

        class spell_the_lich_king_mass_resurect_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_the_lich_king_mass_resurect_SpellScript);

            void HandleScript(SpellEffIndex effIndex)
            {
                GetCaster()->CastSpell(GetHitUnit(), SPELL_MASS_RESURRECTION_REAL, true);
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_the_lich_king_mass_resurect_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_the_lich_king_mass_resurect_SpellScript();
        }
};
...
void AddSC_boss_the_lich_king()
{
    ...
    new spell_the_lich_king_mass_resurect();
