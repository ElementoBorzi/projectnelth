...
Position const SindragosaFlyPos    = {4474.683f, 2484.906f, 247.9887f, 3.141593f};
Position const SindragosaLandPos   = {4419.190f, 2484.570f, 203.3848f, 3.141593f};
Position const SindragosaAirPos    = {4474.683f, 2484.906f, 247.9887f, 3.141593f};
...
class boss_sindragosa : public CreatureScript
{
    public:
        ...
        struct boss_sindragosaAI : public BossAI
        {
            ...
            {
                me->ApplySpellImmune(0, IMMUNITY_ID, RAID_MODE(70127, 72528, 72529, 72530), true); // Mystic Buffet debuff
                me->ApplySpellImmune(0, IMMUNITY_ID, 53407, true); // Richturteil der Gerechtigkeit
            }
            ...
            void DoAction(int32 const action)
            {
                ...
                if (action == ACTION_ASPHYXIATION_CHECK)
                {
                    if (_isThirdPhase)
                    {
                            ...
                            return;
                        {
                            ...
                            if (threatlist.size())
                            {
                                ...
                                for (std::list<Unit*>::iterator it = targetList.begin(); it != targetList.end(); ++it)
                                {
                                    ...
                                    DoCast((*it), SPELL_FROST_BEACON);
                                    DoCast((*it), SPELL_ICE_TOMB_DUMMY, true);
                                }
                            }
            ...
            {
                if (uint32 spellId = sSpellMgr->GetSpellIdForDifficulty(70127, me) && target->GetTypeId() == TYPEID_PLAYER)
                    if (spellId == spell->Id)
                ...
                if (spell->Id == SPELL_FROST_BOMB_TRIGGER)
                {
                    ...
                    if (++_frostBombCount < 4)
                        events.ScheduleEvent(EVENT_FROST_BOMB, 0);
                    else
                        _frostBombCount = 0;
                }
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_AIR_PHASE:
                        {
                            ...
                            // events.DelayEvents(55000, EVENT_GROUP_LAND_PHASE);
                        ...
                        case EVENT_FROST_BOMB:
                        {
                            ...
                            if (TempSummon* summ = me->SummonCreature(NPC_FROST_BOMB, pos, TEMPSUMMON_TIMED_DESPAWN, 40000))
                            {
                                ...
                                float dist = me->GetDistance(summ->GetPositionX(), summ->GetPositionY(), summ->GetPositionZ());
                                float speed = dist / 5.5f; // 5.5 seconds to hit the target
                                me->CastCustomSpell(SPELL_FROST_BOMB_TRIGGER, SPELLVALUE_SPEED, (uint32)speed, summ, false);
                            }
                            break;
                        }
                        case EVENT_LAND:
        ...
        private:
            uint32 _mysticBuffetStack;
            uint8 _frostBombCount;
            bool _isInAirPhase;
            bool _firstAirPhaseDone;
            bool _isThirdPhase;
            bool _summoned;
            bool _isBlisteringCold;
        };
...
public:
    ...
    {
        return unit->ToUnit() && unit->ToUnit()->GetTypeId() != TYPEID_PLAYER;
    }
};
...
public:
    ...
    bool operator()(WorldObject* unit)
    {
        ...
        if (player && player->HasAura(SPELL_ICE_TOMB_DAMAGE))
            return true;

        for (std::list<Creature*>::const_iterator itr = _collisionList.begin(); itr != _collisionList.end(); ++itr)
            if ((*itr)->isAlive() && (*itr)->IsInBetween(_caster, player, 5.0f))
                return true;
