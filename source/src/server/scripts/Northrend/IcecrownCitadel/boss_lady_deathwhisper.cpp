...
enum Actions
{
    ACTION_SPAWN_REANIMATED_ADHERENT = NPC_REANIMATED_ADHERENT,
    ACTION_SPAWN_REANIMATED_FANATIC  = NPC_REANIMATED_FANATIC,
    ACTION_CANCEL_DARK_MARTYRDOM     = 3
};

#define ACHIEVEMENT_FULL_HOUSE RAID_MODE<uint32>(ACHIEVEMENT_FULL_HOUSE_10, ACHIEVEMENT_FULL_HOUSE_25)
        ...
        struct boss_lady_deathwhisperAI : public BossAI
        {
            ...
            void DamageTaken(Unit* /*damageDealer*/, uint32& damage)
            {
                ...
                if (events.IsInPhase(PHASE_ONE) && int32(damage) > me->GetPower(POWER_MANA))
                {
                    ...
                    summons.DoAction(ACTION_CANCEL_DARK_MARTYRDOM, pred);
                    EntryCheckPredicate predicate(NPC_CULT_ADHERENT);
                    summons.DoAction(ACTION_CANCEL_DARK_MARTYRDOM, predicate);
                    canTransformAdds = false;
                }
            }
            ...
            void DoActionSpawn(uint32 action, Position &pos)
            {
                Talk(SAY_ANIMATE_DEAD);
                me->SummonCreature(action, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
            }

            bool CanTransformAdds()
            {
                return canTransformAdds;
            }
...
class npc_cult_fanatic : public CreatureScript
{
    public:
        ...
        struct npc_cult_fanaticAI : public ScriptedAI
        {
            ...
            {
                if (action == ACTION_CANCEL_DARK_MARTYRDOM)
                {
                    Events.CancelEvent(EVENT_CULTIST_DARK_MARTYRDOM);
                    return;
                }
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CULTIST_DARK_MARTYRDOM:
                            ...
                            DoAction(ACTION_SPAWN_REANIMATED_FANATIC);
                            break;
                    }
                }
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CULTIST_DARK_MARTYRDOM:
                            ...
                            DoAction(ACTION_SPAWN_REANIMATED_ADHERENT);
                            break;
                    }
                }
