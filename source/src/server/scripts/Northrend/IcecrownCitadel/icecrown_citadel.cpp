...
enum EventTypes
{
    ...
    // Sindragosa's Gauntlet
    ACTION_START_SINDRAGOSA_GAUNTLET = 55,
    EVENT_SPAWN_WAVE_ONE = 56,
    EVENT_SPAWN_WAVE_TWO = 57,
    EVENT_SPAWN_WAVE_THREE = 58,
    EVENT_CHECK_FOR_NEXT_WAVE = 59,
    EVENT_SUMMON_NERUBAR_BROODLING = 60,
    EVENT_FINISH_GAUNTLET = 61,
};
...
Position const WebweaverPosition[4] =
{
...
Position const ChampionPosition[4] =
{
    { 4155.44f, 2442.21f, 211.033f, 0.89064f },
    { 4223.23f, 2456.34f, 211.033f, 2.36406f },
    { 4209.04f, 2526.87f, 211.033f, 4.07544f },
    { 4138.42f, 2512.19f, 211.033f, 5.55586f },
};

Position const FrostwardenWarrior[2] =
{
    { 4186.77f, 2400.8f, 211.033f, 1.60464f },
    { 4186.46f, 2568.6f, 211.033f, 4.70304f },
};

Position const FrostwardenSorceress[2] =
{
    { 4176.21f, 2400.5f, 211.033f, 1.6046f },
    { 4176.00f, 2568.7f, 211.033f, 4.7030f },
};

Position const NerubarBroodling[8] =
{
    { 4153.36f, 2525.18f, 211.033f, 5.3478f },
    { 4223.64f, 2512.48f, 211.033f, 3.8124f },
    { 4210.11f, 2442.05f, 211.033f, 2.3005f },
    ...
    public:
        ...
        struct npc_crok_scourgebaneAI : public npc_escortAI
        {
            ...
            _corpseDelay(creature->GetCorpseDelay())
            {
                ...
                _isEventDone = _instance->IsDone(DATA_SISTER_SVALNA);
...
class npc_arthas_teleport_visual : public CreatureScript
{
    public:
        ...
        struct npc_arthas_teleport_visualAI : public NullCreatureAI
        {
            ...
            {
                _events.Reset();
                if (_instance->IsDone(DATA_PROFESSOR_PUTRICIDE) &&
                    _instance->IsDone(DATA_BLOOD_QUEEN_LANA_THEL) &&
                    _instance->IsDone(DATA_SINDRAGOSA))
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            ...
            player->SEND_GOSSIP_MENU(15603, creature->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF + 2:
            player->SEND_GOSSIP_MENU(15605, creature->GetGUID());
            instance->SetData(DATA_ZONE_BUFF_STATUS, 1);
            instance->DoRemoveAurasDueToSpellOnPlayers(AURA_ZONE_PCT_BUFF_A_30);
            instance->DoRemoveAurasDueToSpellOnPlayers(AURA_ZONE_PCT_BUFF_H_30);
            break;
        }
        return true;
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->HasAura(AURA_ZONE_PCT_BUFF_A_30) || player->HasAura(AURA_ZONE_PCT_BUFF_H_30))
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->SEND_GOSSIP_MENU(15602, creature->GetGUID());
        }
        else player->SEND_GOSSIP_MENU(15605, creature->GetGUID());
...
public:
    ...
    class spell_icc_buff_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_icc_buff_SpellScript);
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_icc_buff_SpellScript::CheckCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_icc_buff_SpellScript();
    }
};


#define NPC_NERUBAR_RUBIAN_CHAMPION    37501
#define NPC_NERUBAR_WEBWAEVER          37502
#define NPC_NERUBAR_BROODLING          37232
#define NPC_FROSTWARDEN_WARRIOR        37228
#define NPC_FROSTWARDEN_SORCERESS      37229
...
class npc_sindragosas_ward : public CreatureScript
{
    public:
        ...
        {
            npc_sindragosas_wardAI(Creature* creature) : ScriptedAI(creature), summons(me), _instance(creature->GetInstanceScript()) {}
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                case ACTION_START_SINDRAGOSA_GAUNTLET:
                    summons.DespawnAll();
                    Talk(SAY_WARDEN_INTRO);
                    _instance->SetData(DATA_SINDRAGOSAS_GAUNTLET, IN_PROGRESS);
                    _events.ScheduleEvent(EVENT_SPAWN_WAVE_ONE, 2000);
                    _events.ScheduleEvent(EVENT_SUMMON_NERUBAR_BROODLING, 5000);
                    _events.ScheduleEvent(EVENT_CHECK_FOR_NEXT_WAVE, 3000);
                    break;
                }
            }
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                    case EVENT_SPAWN_WAVE_ONE:
                        for (int i = 0; i < 4; ++i)
                        {
                            DoSummon(NPC_NERUBAR_WEBWAEVER, WebweaverPosition[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                            DoSummon(NPC_NERUBAR_RUBIAN_CHAMPION, ChampionPosition[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                        }
                        break;
                    case EVENT_SPAWN_WAVE_TWO:
                        for (int i = 0; i < 2; ++i)
                        {
                            DoSummon(NPC_FROSTWARDEN_WARRIOR, FrostwardenWarrior[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                            DoSummon(NPC_FROSTWARDEN_SORCERESS, FrostwardenSorceress[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                        }
                        break;
                    case EVENT_SPAWN_WAVE_THREE:
                        for (int i = 0; i < 4; ++i)
                        {
                            DoSummon(NPC_NERUBAR_WEBWAEVER, WebweaverPosition[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                            DoSummon(NPC_NERUBAR_RUBIAN_CHAMPION, ChampionPosition[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                        ...
                        break;
                    case EVENT_SUMMON_NERUBAR_BROODLING:
                        if (me->SelectNearestPlayer(50.0f)) // Prevent spawning broodlings if raid is wiped because the event does not reset (blizzlike)
                        for (int i = 0; i < 8; ++i)
                            DoSummon(NPC_NERUBAR_BROODLING, NerubarBroodling[i], 60000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);

                        _events.RescheduleEvent(EVENT_SUMMON_NERUBAR_BROODLING, 10000);
                        break;
                    case EVENT_CHECK_FOR_NEXT_WAVE:
                        if (!me->FindNearestCreature(NPC_NERUBAR_WEBWAEVER, 100.0f, true) && !me->FindNearestCreature(NPC_NERUBAR_RUBIAN_CHAMPION, 100.0f, true) && IsWaveOne)
                        {
                            _events.ScheduleEvent(EVENT_SPAWN_WAVE_TWO, 500);
                            _events.RescheduleEvent(EVENT_CHECK_FOR_NEXT_WAVE, 3000);
                            _events.CancelEvent(EVENT_SUMMON_NERUBAR_BROODLING);
                            IsWaveOne = false;
                            IsWaveTwo = true;
                        }
                        else if (!me->FindNearestCreature(NPC_FROSTWARDEN_WARRIOR, 100.0f, true) && !me->FindNearestCreature(NPC_FROSTWARDEN_SORCERESS, 100.0f, true) && IsWaveTwo)
                        {
                            _events.ScheduleEvent(EVENT_SPAWN_WAVE_THREE, 500);
                            _events.RescheduleEvent(EVENT_CHECK_FOR_NEXT_WAVE, 3000);
                            _events.ScheduleEvent(EVENT_SUMMON_NERUBAR_BROODLING, 5000);
                            IsWaveTwo = false;
                            IsWaveThree = true;
                        }
                        else if (!me->FindNearestCreature(NPC_NERUBAR_WEBWAEVER, 100.0f, true) && !me->FindNearestCreature(NPC_NERUBAR_RUBIAN_CHAMPION, 100.0f, true) && IsWaveThree)
                        {
                            _events.ScheduleEvent(EVENT_FINISH_GAUNTLET, 500);
                            _events.CancelEvent(EVENT_CHECK_FOR_NEXT_WAVE);
                            _events.CancelEvent(EVENT_SUMMON_NERUBAR_BROODLING);
                        }
                        _events.RescheduleEvent(EVENT_CHECK_FOR_NEXT_WAVE, 3000);
                        break;
                    case EVENT_FINISH_GAUNTLET:
                        _instance->SetData(DATA_SINDRAGOSAS_GAUNTLET, DONE);
...
class at_icc_sindragosa_gauntlet : public AreaTriggerScript
{
    public:
        at_icc_sindragosa_gauntlet() : AreaTriggerScript("at_icc_sindragosa_gauntlet") { }
...
void AddSC_icecrown_citadel()
{
    ...
    new at_icc_sindragosa_gauntlet();
}
