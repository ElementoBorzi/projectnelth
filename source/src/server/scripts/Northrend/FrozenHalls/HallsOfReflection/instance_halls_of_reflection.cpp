...
public:
        ...
        void OnGameObjectCreate(GameObject* go)
        {
            switch (go->GetEntry())
            {
                ...
                case GO_ORGRIM_HAMMER:
                    _gunshipOrGUID = go->GetGUID();
                    go->SetPhaseMask(2, true);
                    break;
                case GO_SKYBREAKER:
                    _gunshipSkGUID = go->GetGUID();
                    go->SetPhaseMask(2, true);
                    break;
            }
        }
                    ...
                    if (_waveCount % 5)
                    {
                        ...
                        for (std::set<uint64>::const_iterator itr = waveGuidList[internalWaveId].begin(); itr != waveGuidList[internalWaveId].end(); ++itr)
                        {
                            if (auto temp = instance->GetCreature(*itr))
                            {
                                ...
                                if (temp->getVictim())
                                    temp->GetMotionMaster()->MoveChase(temp->getVictim(), 0.0f, 0.0f);
                            }
                        }
                    }
                    else
        ...
        uint64 GetData64(uint32 type) const
        {
            switch (type)
            {
                ...
                case DATA_CAVE_IN:
                    return _caveGUID;
                case GO_SKYBREAKER:
                    return _gunshipSkGUID;
                case GO_ORGRIM_HAMMER:
                    return _gunshipOrGUID;
                default:
                    break;
            }
    ...
    private:
        ...
        uint64 _caveGUID;
        uint64 _gunshipSkGUID;
        uint64 _gunshipOrGUID;
