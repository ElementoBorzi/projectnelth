...
enum Actions
{
    ...
    ACTION_END_COMBAT       = 4,
    ACTION_START_AMBUSH,
    ACTION_START_GAUNLET,
    POINT_ENTRANCE_EVENT,
    POINT_GAUNLET,
    POINT_END,
};
    ...
    public:
        ...
        struct boss_tyrannusAI : public BossAI
        {
            ...
            void InitializeAI()
            {
                ...
                else if (!instance->IsDone(DATA_TYRANNUS))
                    Reset();
                else
            ...
            void DoAction(const int32 actionId)
            {
                ...
                if (actionId == ACTION_START_AMBUSH)
                {
                    if (!entranceEvent)
                    {
                        events.SetPhase(PHASE_PRE_EVENT);
                        if (Vehicle *veh = me->GetVehicle())
                            if (Creature *rim = veh->GetBase()->ToCreature())
                            {
                                rim->SetSpeed(MOVE_FLIGHT, 5.0f);
                                rim->GetMotionMaster()->MovePoint(POINT_ENTRANCE_EVENT, 540.66f, 221.56f, 549.59f);
                            }
                        TalkWithDelay(30000, SAY_AMBUSH_1);
                        events.ScheduleEvent(EVENT_ENTRANCE_INTRO, 45000);
                    }
                    entranceEvent = true;
                }
                if (actionId == ACTION_START_GAUNLET)
                {
                    if (!gaunletEvent)
                    {
                        if (Vehicle *veh = me->GetVehicle())
                            if (Creature *rim = veh->GetBase()->ToCreature())
                                rim->GetMotionMaster()->MovePoint(POINT_GAUNLET, 922.05f, -78.18f, 654.97f);
                        events.SetPhase(PHASE_PRE_EVENT);
                        TalkWithDelay(5000, SAY_GAUNTLET_START);
                        events.ScheduleEvent(EVENT_GAUNLET_START, 15000);
                    }
                    gaunletEvent = true;
                }
            }

            void UpdatePreEvents(const uint32 diff)
            {
                if (events.IsInPhase(PHASE_PRE_EVENT))
                {
                    events.Update(diff);
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            case EVENT_ENTRANCE_INTRO:
                                Talk(SAY_AMBUSH_2);
                                if (Vehicle *veh = me->GetVehicle())
                                    if (Creature *rim = veh->GetBase()->ToCreature())
                                        rim->GetMotionMaster()->MovePoint(POINT_GAUNLET, 922.05f, -78.18f, 654.97f);
                                break;
                            case EVENT_GAUNLET_START:
                                if (Vehicle *veh = me->GetVehicle())
                                    if (Creature *rim = veh->GetBase()->ToCreature())
                                    {
                                        rim->GetMotionMaster()->MovePoint(POINT_END, _homePos);
                                        rim->DespawnOrUnsummon(5000);
...
// 5573
class at_tyrannus_en_event_starter : public AreaTriggerScript
{
    public:
            ...
            if (auto tyrannusEvent = ObjectAccessor::GetCreature(*player, instance->GetData64(DATA_TYRANNUS_EVENT)))
                if (tyrannusEvent->GetVehicleKit())
                    if (tyrannusEvent->GetVehicleKit()->GetPassenger(0))
                        ...
                        {
                            tyrannus->AI()->DoAction(ACTION_START_AMBUSH);
                            return true;
                        }
...
// 5579
class at_tyrannus_ga_event_starter : public AreaTriggerScript
{
    public:
            ...
            if (auto tyrannusEvent = ObjectAccessor::GetCreature(*player, instance->GetData64(DATA_TYRANNUS_EVENT)))
                if (tyrannusEvent->GetVehicleKit())
                    if (tyrannusEvent->GetVehicleKit()->GetPassenger(0))
                        if (Creature *tyrannus = tyrannusEvent->GetVehicleKit()->GetPassenger(0)->ToCreature())
                        {
                            tyrannus->AI()->DoAction(ACTION_START_GAUNLET);
                            return true;
                        }
...
public:
    ...
    struct npc_tyrannus_icicleAI : public ScriptedAI
    {
        ...
        {
            if (summoned->GetEntry() == 33169)
            {
                summoned->CastWithDelay(3000, summoned, SPELL_FALL_DAMAGE, true);
                me->CastSpell(me, SPELL_ICICLE_FALL, true);
            }
        }
...
void AddSC_boss_tyrannus()
{
    ...
    new at_tyrannus_en_event_starter();
    new at_tyrannus_ga_event_starter();
