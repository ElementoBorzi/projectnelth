...
enum mimickingNaturesCallQuest
{
    NPC_FROSTGORE                   = 24173,
    QUEST_MIMICKING_NATURES_CALL    = 11281,
    TALK_INTERRUPTED_DINNER         = -22876,
    TALK_FEMALE_YETI                = -22877,
    POINT_TALK                      = 1,
    EVENT_REMOVE_FLAGS              = 2,
};

Position const yetiPositions[2] =
{
    { 2921.633f, -4491.154f, 282.350f, 4.6340f }, // summon pos
    { 2938.535f, -4549.590f, 273.791f, 4.9090f }  // move pos
};

class spell_test_horn : public SpellScriptLoader
{
public:
    spell_test_horn() : SpellScriptLoader("spell_test_horn") { }

    class spell_test_horn_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_test_horn_SpellScript);

        void HandleSendEvent(SpellEffIndex /*effIndex*/)
        {
            if (GetCaster()->GetTypeId() != TYPEID_PLAYER || GetCaster()->ToPlayer()->GetQuestStatus(QUEST_MIMICKING_NATURES_CALL) != QUEST_STATUS_INCOMPLETE)
                return;

            if (!GetCaster()->FindNearestCreature(NPC_FROSTGORE, 100.00f))
                GetCaster()->SummonCreature(NPC_FROSTGORE, yetiPositions[0], TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000);
        }

        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_test_horn_SpellScript::HandleSendEvent, EFFECT_0, SPELL_EFFECT_SEND_EVENT);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_test_horn_SpellScript();
    }
};

class npc_frostgore : public CreatureScript
{
public:
    npc_frostgore() : CreatureScript("npc_frostgore") { }

    struct npc_frostgoreAI : public ScriptedAI
    {
        npc_frostgoreAI(Creature* creature) : ScriptedAI(creature) { }

        void IsSummonedBy(Unit* summoner)
        {
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC);
            me->GetMotionMaster()->MovePoint(POINT_TALK, yetiPositions[1]);
        }
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            ...
            if (id == POINT_TALK)
            {
                Talk(TALK_INTERRUPTED_DINNER);
                events.ScheduleEvent(EVENT_REMOVE_FLAGS, 4000);
            }
        }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_frostgoreAI(creature);
    }
};
