...
#include "../../../../game/Movement/Spline/MoveSplineInit.h"
...
Position const posAlexEvent         = { 788.070f, 1276.09f, 246.900f, 0.0f };
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_ARCANE_BREATH:
                        ...
                        if (auto v = me->getVictim()) me->CastSpell(v, SPELL_ARCANE_BREATH);
                        events.ScheduleEvent(EVENT_ARCANE_BREATH, 20*IN_MILLISECONDS, 0, PHASE_ONE);
                        break;
        ...
        class spell_malygos_random_portal_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                if (Unit *caster = GetCaster())
                {
                    targets.clear();
                    std::list<Creature*> temp;
                    caster->GetCreatureListWithEntryInGrid(temp, NPC_PORTAL_TRIGGER, 200.0f);
                    for (std::list<Creature*>::iterator itr = temp.begin(); itr != temp.end(); ++itr)
                        if (Creature *spark = *itr)
                            if (caster->GetDistance(spark) >= 30)
                                targets.push_back(spark);
                    if (targets.size() > 1)
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_malygos_random_portal_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
...
class spell_malygos_surge_of_power_P2 : public SpellScriptLoader
{
    public:
        ...
        public:
            ...
            {
                return (unit->GetTypeId() == TYPEID_PLAYER && unit->ToPlayer()->GetVehicle());
            }
        };
        ...
        class spell_malygos_surge_of_power_P2_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_malygos_surge_of_power_P2_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
            }
        };
