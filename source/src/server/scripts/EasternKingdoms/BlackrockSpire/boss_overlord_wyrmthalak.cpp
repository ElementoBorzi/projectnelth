    ...
    struct boss_overlordwyrmthalakAI : public BossAI
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (!Summoned && HealthBelowPct(51))
            {
                ...
                {
                    if (auto warlord = me->SummonCreature(NPC_SPIRESTONE_WARLORD, SummonLocation1, TEMPSUMMON_TIMED_DESPAWN, 300 * IN_MILLISECONDS))
                        warlord->AI()->AttackStart(target);
                    if (auto berserker = me->SummonCreature(NPC_SMOLDERTHORN_BERSERKER, SummonLocation2, TEMPSUMMON_TIMED_DESPAWN, 300 * IN_MILLISECONDS))
