...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if ((player->GetQuestStatus(24535) == QUEST_STATUS_INCOMPLETE) || (player->GetQuestStatus(24563) == QUEST_STATUS_INCOMPLETE))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Examiner les restes.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
    ...
    struct npc_thalorienAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (auto pMorlen = me->GetCreature(*me, uiMorlen))
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            switch(events.ExecuteEvent())
            {
                case 1:
                    // spawn all creatures
                    if (auto pThalorien = me->SummonCreature(37205, 11797.0f, -7074.06f, 26.379f, 0.242908f, TEMPSUMMON_CORPSE_DESPAWN, 0))
                    {
                        pThalorien->SetPhaseMask(2,true);
                        uiThalorien = pThalorien->GetGUID();
                    }

                    if (auto pGuard = me->SummonCreature(37211, 11807.0f, -7070.37f, 25.372f, 3.218f, TEMPSUMMON_TIMED_DESPAWN, 30000))
                        pGuard->SetPhaseMask(2,true);
                    if (auto pGuard = me->SummonCreature(37211, 11805.7f, -7073.64f, 25.5694f, 3.218f, TEMPSUMMON_TIMED_DESPAWN, 30000))
                        pGuard->SetPhaseMask(2,true);
                    if (auto pGuard = me->SummonCreature(37211, 11805.6f, -7077.0f, 25.9643f, 3.218f, TEMPSUMMON_TIMED_DESPAWN, 30000))
                        pGuard->SetPhaseMask(2,true);
                    if (auto pGuard = me->SummonCreature(37211, 11806.0f, -7079.71f, 26.2067f, 3.218f, TEMPSUMMON_TIMED_DESPAWN, 30000))
                        pGuard->SetPhaseMask(2,true);

                    if (auto pMorlen = me->SummonCreature(37542, 11776.8f, -7050.72f, 25.2394f, 5.13752f, TEMPSUMMON_CORPSE_DESPAWN, 0))
                    {
                        pMorlen->Mount(25678);
                    ...
                    break;
                case 2:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_1);
                    events.ScheduleEvent(3,5000);
                    break;
                case 3:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_2);
                    events.ScheduleEvent(4,5000);
                    break;
                case 4:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_3);
                    events.ScheduleEvent(5,10000);
                    break;
                case 5:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_4);
                ...
                case 7:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_5);
                    events.ScheduleEvent(8,9000);
                    break;
                case 8:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_6);
                    if (auto pMorlen = me->GetCreature(*me, uiMorlen))
                ...
                case 12:
                    ...
                    events.ScheduleEvent(13, 1000);
                    break;
                    // Outro
                case 13:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_7);
                    events.ScheduleEvent(14, 3500);
                    break;
                case 14:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_8);
                    events.ScheduleEvent(15, 3500);
                    break;
                case 15:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_9);
                    events.ScheduleEvent(16, 3000);
                    break;
                case 16:
                    if (auto pThalorien = me->GetCreature(*me, uiThalorien))
                        pThalorien->AI()->Talk(SAY_THALORIEN_10);
        ...
        void SummonWave(uint32 entry)
        {
            if (auto pThalorien = me->GetCreature(*me, uiThalorien))
            {
                if (auto pZombie = me->SummonCreature(entry, 11768.7f, -7065.83f, 24.0971f, 0.125877f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0))
                {
                    pZombie->SetPhaseMask(2,true);
                    pZombie->AddThreat(pThalorien, 100);
                    pZombie->AI()->AttackStart(pThalorien);
                }
                if (auto pZombie = me->SummonCreature(entry, 11769.5f, -7063.83f, 24.1399f, 6.06035f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0))
                {
                    pZombie->SetPhaseMask(2,true);
                    pZombie->AddThreat(pThalorien, 100);
                    pZombie->AI()->AttackStart(pThalorien);
                ...
                if (auto pZombie = me->SummonCreature(entry, 11769.8f, -7061.41f, 24.2536f, 6.06035f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0))
                {
                    pZombie->SetPhaseMask(2,true);
                    pZombie->AddThreat(pThalorien, 100);
                    pZombie->AI()->AttackStart(pThalorien);
                }
                if (auto pZombie = me->SummonCreature(entry, 11769.4f, -7057.86f, 24.4624f, 0.00335493f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0))
                {
                    pZombie->SetPhaseMask(2,true);
                    pZombie->AddThreat(pThalorien, 100);
                    pZombie->AI()->AttackStart(pThalorien);
                ...
                if (auto pZombie = me->SummonCreature(entry, 11769.4f, -7054.56f, 24.6869f, 0.00335493f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0))
                {
                    pZombie->SetPhaseMask(2,true);
                    pZombie->AddThreat(pThalorien, 100);
                    pZombie->AI()->AttackStart(pThalorien);
...
public:
    ...
    {
        player->PrepareGossipMenu(creature, 0);
        if ((player->GetQuestStatus(24564) == QUEST_STATUS_INCOMPLETE) || (player->GetQuestStatus(24553) == QUEST_STATUS_INCOMPLETE))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "I\'m ready to enter the Sunwell.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
    ...
    struct npc_warden_sunwellAI : public ScriptedAI
    {
        npc_warden_sunwellAI(Creature* c) : ScriptedAI(c) {}
