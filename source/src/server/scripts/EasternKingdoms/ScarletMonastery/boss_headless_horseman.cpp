...
{
    SAY_LOST_HEAD               = -22415,
    SAY_PLAYER_DEATH            = -40546,

    SAY_ENTRANCE                = -22261,
    SAY_REJOINED                = -22271,
    SAY_CONFLAGRATION           = -22587,
    SAY_SPROUTING_PUMPKINS      = -23861,
    SAY_DEATH                   = -23455,
};
...
uint32 const HorsemanFlyPathSize = 21;
const G3D::Vector3 HorsemanFlyPath[HorsemanFlyPathSize] =
{
...
public:
    ...
    struct boss_headless_horsemanAI : public ScriptedAI
    {
        ...
        {
            if (type != SPLINE_MOTION_TYPE || !IsFlying)
                return;
        ...
        Player* SelectRandomPlayer(float range = 0.0f, bool checkLoS = true)
        {
            ...
            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                if ((me->IsWithinLOSInMap(i->getSource()) || !checkLoS) && me->getVictim() != i->getSource() &&
                    me->IsWithinDistInMap(i->getSource(), range) && i->getSource()->isAlive() && !i->getSource()->isGameMaster())
            ...
            if (withhead)
            {
                switch (Phase)
                {
                    case 0:
                    {
                        ...
                        else if (!flyPathStarted)
                        {
                            flyPathStarted = true;
                            me->GetMotionMaster()->MoveSmoothPath(HorsemanFlyPath, HorsemanFlyPathSize);
                        }
                    }
                    break;
                    case 1:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        if (InstanceScript *instance = go->GetInstanceScript())
            if (instance->GetBossState(DATA_HORSEMAN_EVENT) != IN_PROGRESS && instance->GetBossState(DATA_HORSEMAN_EVENT) != DONE)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Call the Headless Horseman...", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
void npc_head::npc_headAI::Disappear()
{
    ...
    if (bodyGUID)
    {
        ...
        if (body && body->isAlive())
        {
            ...
            DoCast(body, SPELL_FLYING_HEAD, true);
            me->SetFullHealth();
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
