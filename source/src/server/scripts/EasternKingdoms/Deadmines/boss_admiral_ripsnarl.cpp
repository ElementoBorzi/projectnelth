        ...
        void DamageTaken(Unit* caster, uint32& damage)
        {
                ...
                (HealthBelowPct(25) && !phase3))
            {
                ...
                if (!phase1)
                {
                    ...
                    events.ScheduleEvent(EVENT_VAPOR_SUMMON, urand(4000, 6000));
                    phase1 = true;
            ...
            {
                /*
                    this alert of mists appearring was not found in any retail footage, so it has been disabled.
                */
                //Talk(5);
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SWIPE:
                        ...
                        events.ScheduleEvent(EVENT_SWIPE, 5000);
                        if (!events.HasEvent(EVENT_COMBAT_NOISE))
                        {
                            //Talk(8);
                            me->PlayDistanceSound(RAND(19881, 19878));
                            events.ScheduleEvent(EVENT_COMBAT_NOISE, 12000);
                        }
                    ...
                    case EVENT_GO_FOR_THE_THROAT:
                        if (me->HasReactState(REACT_PASSIVE))
                        {
                            ...
                            events.ScheduleEvent(EVENT_VAPOR_SUMMON, urand(10000, 14000));
                    ...
                    case EVENT_GROUP_VAPOR:
                    {
                        ...
                        if (list_of_players.size())
                            for (auto itr = list_of_players.begin(); itr != list_of_players.end();)
                                if (auto t = (*itr))
                                    if (t->isGameMaster())//filter condition
                                    {
                        ...
                        {
                            Position mist_spawn (* random_target);
                            random_target->MoveBlink(mist_spawn, frand(-5.f, 5.f), frand(0.f, M_PI*2.f));
                            me->SummonCreature(47714, mist_spawn);
                        }
                        if (auto random_target = Trinity::Containers::SelectRandomContainerElement(list_of_players))
                        {
                            Position mist_spawn(*random_target);
                    ...
                    case EVENT_VAPOR_SUMMON:
                    {
                        ...
                        if (canAttack)
                            events.ScheduleEvent(EVENT_VAPOR_SUMMON, urand(10000, 14000));
                        else events.ScheduleEvent(EVENT_VAPOR_SUMMON, urand(4000, 6000));
                        break;
                    }
                    case EVENT_COMBAT_NOISE:
                        //do nothing, this is just used as a cooldown.
                        break;
                    default:
                        break;
                }
