                ...
                switch (eventId)
                {
                    ...
                    case EVENT_DEVOURING_FLAMES:
                    {
                        if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, ShadowPhaseCheck(true, me)))
                        {
                            ...
                            Position tracerPos;
                            me->GetNearPoint2D(tracerPos.m_positionX, tracerPos.m_positionY, -2.f, me->GetAngle(target));
                            tracerPos.m_positionZ = me->m_positionZ;
                        ...
                        for (auto itr = targets.begin(); itr != targets.end();)
                            if (auto t = (*itr))
                                if (c->GetRelativeAngleOffset(t) > 0.f //to the left
                                    ?
                                    (c->GetRelativeAngleOffset(t) < (M_PI * 0.3334f))//less than 60 degrees away
                                    :
                                    (c->GetRelativeAngleOffset(t) > -(M_PI * 0.3334f))//less than 60 degrees away
                                    )
                                {
...
class spell_devouring_flames_periodic : public SpellScriptLoader // 86840
{
public:
    spell_devouring_flames_periodic() : SpellScriptLoader("spell_devouring_flames_periodic") {}

    class spell_devouring_flames_periodic_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_devouring_flames_periodic_AuraScript);
        ...
        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_devouring_flames_periodic_AuraScript::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_devouring_flames_periodic_AuraScript();
    }
};
...
void AddSC_boss_valiona_theralion()
{
    ...
    new npc_unstable_twilight_portal();
    new spell_devouring_flames();
    new spell_devouring_flames_periodic();
    ...
    new spell_twiligh_sentry_summon_periodic();
    new spell_tank_twilight_shift_heroic();
