    ...
    public:
        ...
        struct instance_bastion_of_twilight_InstanceMapScript : public InstanceScript
        {
            ...
            void OnCreatureCreate(Creature* pCreature)
            {
                switch (pCreature->GetEntry())
                {
                    ...
                    case NPC_TELEPORTER:
                        ...
                        break;
                    case 47302://crimson flames trigger
                        pCreature->AddUnitState(UNIT_STATE_ROOT);
                        pCreature->SetReactState(REACT_PASSIVE);
                        pCreature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        break;
                    case NPC_CHOSEN_SEER:
                    case NPC_CHOSEN_WARRIOR:
                        if (pCreature->GetDBTableGUIDLow())
                        if (pCreature->isAlive())
                        pCreature->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_READY1H_ALLOW_MOVEMENT);
                        break;
                }
            }
            ...
            void OnUnitDeath(Unit* unit)
            {
                ...
                case NPC_CHOSEN_WARRIOR:
                case NPC_CHOSEN_SEER:
                        if (auto chogall = instance->GetCreature(chogallHalfusGUID))
                        {
                            if (unit->ToCreature()->GetDBTableGUIDLow())
                                chogall->AI()->DoAction(55);
                        }
                    break;
                default:
                    break;
