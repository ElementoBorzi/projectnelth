...
public:
    ...
    struct mob_valiona_gbAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*pWho*/) 
        {
            if (!events.HasEvent(EVENT_SHREDDING_SWIPE))
                events.ScheduleEvent(EVENT_SHREDDING_SWIPE, 15000);
        }
            ...
            switch (id)
            {
                case POINT_VALIONA_FLY_IN_THE_AIR:
                    ...
                    if (currentWaypoint < 3)
                        ...
                        me->GetMotionMaster()->MoveLand(POINT_VALIONA_LAND, position[3]);
                    break;
