...
enum Spells
{
    ...
    SPELL_ENGULFING_DARKNESS_TRIGGER = 92787,
    // Black Vial
        ...
        struct boss_maloriakAI : public BossAI
        {
            ...
            void KilledUnit(Unit* victim)
            {
                if (victim->GetTypeId() == TYPEID_PLAYER)
                    Talk(urand(SAY_SLAY, SAY_SLAY_2), NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
            }
            ...
            void EnterCombat(Unit* /*who*/)
            {
                ...
                Position Nefarian{ -105.9514f, -494.0278f, 89.33157f, 1.605703f };

                if (IsHeroic())
                    if (auto nef = instance->instance->SummonCreature(NPC_NEFARIUS_MALORIAK, Nefarian))
                        if (auto c = nef->ToCreature())
                        {

                            nefGUID = c->GetGUID();
                            c->AI()->DoAction(SAY_AGGRO);
                        }
            }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_RELEASE_ABERRATIONS:
                        if (phase != PHASE_GREEN && phase != PHASE_BLACK)
                        {
                            ...
                            Talk(SAY_RELEASE_ABBS, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                        }
                        break;
                    ...
                    case EVENT_ENGULFING_DARKNESS:
                    {
                        _engDar++;
                        if (_engDar <= 7)
                            DoCast(SPELL_ENGULFING_DARKNESS);
                        events.ScheduleEvent(EVENT_ENGULFING_DARKNESS, 11000, EVENT_GROUP_BLACK);
                        break;
                    }
                    case EVENT_VIL_RATA:
...
class npc_maloriak_nefarian : public CreatureScript
{
public:
    npc_maloriak_nefarian() : CreatureScript("npc_maloriak_nefarian") { }

    struct npc_maloriak_nefarianAI : public ScriptedAI
    {
        npc_maloriak_nefarianAI(Creature* creature) : ScriptedAI(creature)
        {
            instance = me->GetInstanceScript();
            me->CastSpell(me, SPELL_TELEPORTATION_VISUAL, true);
        ...
        {
            if (action == SAY_AGGRO)
                events.ScheduleEvent(SAY_AGGRO+1, 9000);

            if (action == PHASE_BLACK)
                Talk(1, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);

            if (action == SAY_DEATH)
            {
                events.ScheduleEvent(SAY_DEATH, 6000);
            }
        }
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
                ...
                {
                case SAY_AGGRO+1:
                    Talk(SAY_AGGRO, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                    break;
                case SAY_DEATH:
                {
                    uint32 teletimer{ IsHeroic() ? uint32(18000) : uint32(5000) };
                    //TC_LOG_ERROR("sql.sql", "say-death called with teletimer %u", teletimer);
                    Talk(IsHeroic() ? 7 : 8, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                    me->CastWithDelay(teletimer, me, SPELL_TELEPORTATION_VISUAL, true);
                    me->DespawnOrUnsummon(teletimer + uint32(1000));

                    if (IsHeroic())
                        events.ScheduleEvent(105, 5000);
                    break;
                }
                case 105:
                {
                    ...
                    instance->DoCompleteAchievement(5108);
                    CharTitlesEntry const* titleInfo = sCharTitlesStore.LookupEntry(188);
                    if (list_of_players.size())
                        for (auto current_target_within_list : list_of_players)
                        {
                            current_target_within_list->SetTitle(titleInfo);
                        }
                    break;
                }
                }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_maloriak_nefarianAI(creature);
    }
};
