...
enum Spells
{
    ...
    // heroic
    SPELL_ENCASING_SHADOWS         = 92023, // casted by NPC_NEFARIUS_OMNOTRON (49226) on tank
...
enum Texts
{
    ...
    SAY_NEF_AGGRO = 0,
    SAY_NEF_ENCASING_SHADOW,
    SAY_NEF_OVERCHARGED_POWER,
    SAY_NEF_SHADOW_INFUSION_OR_CONDUCTOR,
    SAY_NEF_GRIP_OF_DEATH,
    SAY_NEF_DEATH_NM,
    SAY_NEF_DEATH_HC
};
...
struct generic_botAI : public BossAI
{
    ...
    {
        if (instance->GetBossState(DATA_OMNOTRON_DEFENSE_SYSTEM) != (IsHeroic() ? DONE_HM : DONE))
    ...
    public:
        ...
        struct npc_electronAI : public generic_botAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                generic_botAI::EnterCombat(who);
                events.Reset();
                events.ScheduleEvent(EVENT_CONDUCTOR, urand(22000, 25000));
    ...
    public:
        ...
        struct npc_toxitronAI : public generic_botAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_CHEMICAL_BOMB, urand(25000, 11000));
                //events.ScheduleEvent(EVENT_POISON_PROTOCOL, IsHeroic() ? 15000 : 21000);
                events.ScheduleEvent(EVENT_POISON_PROTOCOL, urand(15000, 21000));
            }
            ...
            void DoAction(int32 const action)
            {
                ...
                {
                    //events.ScheduleEvent(EVENT_CHEMICAL_BOMB, IsHeroic() ? 15000 : 21000);
                    events.ScheduleEvent(EVENT_CHEMICAL_BOMB, urand(25000, 11000));
                    //events.ScheduleEvent(EVENT_POISON_PROTOCOL, IsHeroic() ? 15000 : 21000);
                    events.ScheduleEvent(EVENT_POISON_PROTOCOL, urand(15000, 21000));
                }
    ...
    SAY_NEF_DEATH_HC
                        */
                        case EVENT_INTRO:
                            ...
                            break;
                        case EVENT_DEATH:
                            Talk(IsHeroic() ? SAY_NEF_DEATH_HC : SAY_NEF_DEATH_NM, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                            me->DespawnOrUnsummon(5000);
                            break;
                        case EVENT_OVERCHARGE_GENERATOR:
                            Talk(SAY_NEF_OVERCHARGED_POWER, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                            DoCast(SPELL_NEFARIUS_SURCHARGE);
                            break;
                        case EVENT_GRIP:
                            Talk(SAY_NEF_GRIP_OF_DEATH, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                        ...
                        case EVENT_ENCASTING_SHADOW:
                            Talk(SAY_NEF_ENCASING_SHADOW, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                            DoCast(SPELL_ENCASING_SHADOWS);
                            break;
                        case EVENT_SHADOW_INFUSION:
                            Talk(SAY_NEF_SHADOW_INFUSION_OR_CONDUCTOR, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
...
class spell_poison_soaked_shield : public SpellScriptLoader
{
public:
    spell_poison_soaked_shield() : SpellScriptLoader("spell_poison_soaked_shield") { }

    class spell_poison_soaked_shield_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_poison_soaked_shield_AuraScript);
        ...
        uint32 SoakedInPoison()
        {
            switch (GetSpellInfo()->Id)
            {
                case 79835: return 80011;   break;
                case 91501: return 91504;   break;
                case 91502: return 91505;   break;
                case 91503: return 91506;   break;
            }
            return 80011;
        }
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            //TC_LOG_ERROR("sql.sql", "spell %u proccing spell %u", GetSpellInfo()->Id, SoakedInPoison());
            if (auto target = eventInfo.GetProcTarget())
            {
                //TC_LOG_ERROR("sql.sql", "target: %u", target->GetGUID());
                if (auto aura = target->GetAura(SoakedInPoison()))
                {
                    aura->ModStackAmount(1);
                    aura->SetDuration(aura->GetMaxDuration());
                }
                else if (auto aura = target->AddAura(SoakedInPoison(), target))
                {
        ...
        void Register()
        {
            OnEffectProc += AuraEffectProcFn(spell_poison_soaked_shield_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_poison_soaked_shield_AuraScript();
    }
};
...
{
    new boss_omnotron();
    new spell_poison_soaked_shield();
    new npc_arcanotron();
    new npc_electron();
    new npc_magmatron();
    new npc_toxitron();
    ...
    new npc_test();
    new spell_electron_lighting_charge();
    ...
    new spell_nefraius_lighting_infusion();
    new spell_nefarius_lighting_infusion_trigger();
}
