    ...
    struct boss_atramedesAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            else
            {
                ...
                me->GetMotionMaster()->MovePoint(112, { 283.858063f, -226.115891f, 126.039467f, 3.134910f }, false, 10.f, true);
                Flight(true);
            }
            ...
            _Reset();
            ResetShields();
            me->DespawnCreaturesInArea(NPC_ROARING_FLAME, 250.f);
            me->DespawnCreaturesInArea(NPC_SONAR_PULSE, 250.f);
            


            instance->DoRemoveAurasDueToSpellOnPlayers(78897);
            instance->DoRemoveAurasDueToSpellOnPlayers(78092);
            /*
            auto play_refs = instance->instance->GetPlayers();
            for (auto pl_r : play_refs)
                if (auto player = pl_r.getSource())
                    if (!player->HasAura(78092))
            */
        }
        ...
        void ResetShields()
        {
            ...
            if (gongList.size())
            for (auto itr = gongList.begin(); itr != gongList.end(); ++itr)
            if (auto gong = (*itr))
            {
                gong->Respawn(true);
                gong->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_HIDE_NAMEPLATE | UNIT_FLAG2_UNK2);
                gong->RemoveAllAuras();
                gong->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                gong->SetCorpseDelay(3600);
            }
        }
        ...
        void EnterCombat(Unit * /*who*/)
        {
            ...
            if (IsHeroic())
            {
                if (auto c = instance->instance->SummonCreature(NPC_NEFARIUS_ATRAMEDES, { 98.551704f, -225.954895f, 98.597633f, 0.0f }))
                    if (auto nef = c->ToCreature())
                    {
                        nefarianGUID = nef->GetGUID();
                        nef->TalkWithDelay(5000, 2, NULL, CHAT_MSG_MONSTER_YELL);
                    }
        ...
        {
            //TC_LOG_ERROR("sql.sql", "Movement Inform: type: %u, id: %u", type, id);
            ////me->MonsterYell("void MovementInform(uint32 type, uint32 id)", LANG_UNIVERSAL, 0);
            if (type == POINT_MOTION_TYPE && id == me->GetEntry())
            {
            ...
            else if (type == POINT_MOTION_TYPE && id == 112)
            {

                me->GetMotionMaster()->MoveLand(108, { 174.527237f, -225.555634f, 75.453415f, 3.134934f });

                DoCast(SPELL_FLAME_BREATH);
                if (auto a = me->GetAura(92815))
                {
            ...
            else if (type == EFFECT_MOTION_TYPE && id == 108)
            {
                introTimer = 0;
            }
        }
        ...
        void Flight(bool flying)
        {
            me->DisableMovementFlagUpdate(true);
            me->SetHover(flying);
            me->SetCanFly(flying);
            me->SetDisableGravity(flying);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (!me->HasUnitState(UNIT_STATE_CASTING))
                ...
                //else TC_LOG_ERROR("sql.sql", "has react passive");


            if (!me->HasUnitState(UNIT_STATE_CASTING))
            if (me->HasUnitState(UNIT_STATE_ROOT))
                if (!me->GetCurrentSpell(CURRENT_GENERIC_SPELL) || (me->GetCurrentSpell(CURRENT_GENERIC_SPELL)->m_spellInfo->Id != SPELL_SONIC_BREATH && me->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->m_spellInfo->Id != SPELL_SONIC_BREATH))
                {

                    me->SetReactState(REACT_AGGRESSIVE);
                    me->ClearUnitState(UNIT_STATE_ROOT);
                    //TC_LOG_ERROR("sql.sql", "clearing unit state root");
                }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    case EVENT_AGGRESSIVE:
                        //me->MonsterYell("EVENT_AGGRESSIVE", LANG_UNIVERSAL, 0);
                        me->SetReactState(REACT_AGGRESSIVE);
                        break;
                    case EVENT_MODULATION:
                        //me->MonsterYell("EVENT_MODULATION", LANG_UNIVERSAL, 0);
                        DoCast(SPELL_MODULATION);
                        events.ScheduleEvent(EVENT_MODULATION, urand(20000, 25000));
                        break;
                    case EVENT_SONIC_BREATH:
                        //me->MonsterYell("EVENT_SONIC_BREATH", LANG_UNIVERSAL, 0);
                    ...
                    {
                        //me->MonsterYell("EVENT_SONAR_PULSE", LANG_UNIVERSAL, 0);
                    ...
                    case EVENT_SEARING_FLAME:
                        //me->MonsterYell("EVENT_SEARING_FLAME", LANG_UNIVERSAL, 0);
                    ...
                    case EVENT_AIR_PHASE:
                        //me->MonsterYell("EVENT_AIR_PHASE", LANG_UNIVERSAL, 0);
                        airPhase = true;
                        events.Reset();
                        me->SetReactState(REACT_PASSIVE);
                        ...
                        me->GetMotionMaster()->MoveIdle();
                        me->GetMotionMaster()->MovePoint(me->GetEntry(), centerPos);
                        break;
                    case EVENT_SET_FLYING:
                        //me->MonsterYell("EVENT_SET_FLYING", LANG_UNIVERSAL, 0);
                        Flight(true);
                        me->NearTeleportTo(centerPos.GetPositionX(), centerPos.GetPositionY(), centerPos.GetPositionZ() + 20.0f, M_PI);
                        break;
                    case EVENT_SONAR_BOMB:
                        //me->MonsterYell("EVENT_SONAR_BOMB", LANG_UNIVERSAL, 0);
                    ...
                    {
                        Talk(SAY_AIR_PHASE);
                        //me->MonsterYell("EVENT_ROARING_FLAME_BREATH", LANG_UNIVERSAL, 0);
                    ...
                    case EVENT_LAND:
                    {
                        //me->MonsterYell("EVENT_LAND", LANG_UNIVERSAL, 0);
                        me->GetMotionMaster()->MoveLand(me->GetEntry(), centerPos);
                        events.ScheduleEvent(EVENT_GROUND_PHASE, 5000);
                        break;
                    }
                case EVENT_GROUND_PHASE:
                    //me->MonsterYell("EVENT_GROUND_PHASE", LANG_UNIVERSAL, 0);
                    airPhase = false;
                    if(Creature * flame = me->GetCreature(*me, flameGUID))
...
public:
    ...
    struct npc_ancient_dwarven_shieldAI : public PassiveAI
    {
        ...
        void OnSpellClick(Unit* player, bool& result)
        {
            ...
            me->Kill(me);
            instance->DoRemoveAurasDueToSpellOnPlayers(78897);
            if (auto atr = player->FindNearestCreature(NPC_ATRAMEDES, 200.f))
                atr->PlayDirectSound(urand(0,1) ? 20828 : 20824);
    ...
    struct npc_victor_nefarius_atramedesAI : public ScriptedAI
    {
        ...
        void DoAction(const int32 action)
        {
            ...
            case ACTION_GONG:
                ...
                break;
            case ACTION_DEATH:
                TalkWithDelay(3000, IsHeroic() ? SAY_DEATH_HC : SAY_DEATH);
    ...
    public:
        ...
        bool Execute(uint64 /*eventTime*/, uint32 /*diff*/)
        {
            if (Player * target = m_owner.GetPlayer(m_owner, m_targetGUID))
            {
                ...
                //m_owner.SetSpeed(MOVE_WALK, target->GetSpeedRate(MOVE_RUN) * 1.25, true);
            }
            return true;
        }
            ...
            if (!start)
            {
                if (mui_start <= diff)
                {
                        ...
                        && !me->FindNearestCreature(43130, 100, true) && !me->FindNearestCreature(43126, 100, true))
                    {
                        ...
                        {
                            nefarius->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            ...
            if (canTalk)
            {
                if (textCooldown <= diff)
                {
                    ...
                    case 9:
                        ...
                        Talk(11, NULL, CHAT_MSG_MONSTER_EMOTE, TEXT_RANGE_MAP);
                        textCooldown = 9000;
                    ...
                    case 16:
                        Talk(8);
                        textCooldown = 3000;
                        TalkWithDelay(2500, 12, NULL, CHAT_MSG_MONSTER_EMOTE);
                        break; // 22:27:05.723
                    case 17:
