    ...
    struct boss_chimaeronAI : public BossAI
    {
        ...
        void DamageTaken(Unit* /*done_by*/, uint32 &damage)
        {
            if (me->HealthBelowPctDamaged(phaseSwitch, damage) && events.IsInPhase(PHASE_NORMAL) || me->HealthBelowPctDamaged(phaseSwitch, damage) && events.IsInPhase(PHASE_FEUD))
            {
                ...
                Talk(4, NULL, CHAT_MSG_RAID_BOSS_EMOTE, TEXT_RANGE_MAP);
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            Talk(5, NULL, CHAT_MSG_RAID_BOSS_EMOTE, TEXT_RANGE_MAP);
            events.ScheduleEvent(EVENT_ENRAGE, 450000);
            ScheduleEvents();
            Position nefar{ -113.135f, 43.535f, 80.136f, 4.680680f };
            if (auto nefarian = instance->instance->SummonCreature(NPC_NEFARIUS_CHIMAERON, nefar))
                if (auto nef = nefarian->ToCreature())
            ...
            if (me->getVictim())
            {
                if (me->isAttackReady() && !me->IsNonMeleeSpellCasted(false))
                {
                    if (me->IsWithinMeleeRange(me->getVictim()))
                    {
                        if (canDbleAttack)
                        {
                            DoCast(me, SPELL_DOUBLE_ATTACK_EFF, true);
                    ...
                    if (roll_chance_i(40 + massacreCnt) && massacreCnt > 0)
                    {
                        ...
                        if (IsHeroic())
                        {
                            if (auto nefarian = me->GetCreature(*me, nefarianGUID))
                            {
                                ...
                                events.ScheduleEvent(EVENT_DOUBLE_ATTACK, 2500, EVENT_GROUP_NORMAL, PHASE_FEUD);
                            }
                        }
                ...
                case EVENT_MASSACRE_FEUD:
                    if (!me->HasAura(SPELL_MORTALITY_SELF))
                    {
                        ...
                        if (auto bot = me->GetCreature(*me, botGUID))
                        {
                            ...
                            Talk(6, NULL, CHAT_MSG_MONSTER_EMOTE, TEXT_RANGE_MAP);
                        }
    ...
    public:
        ...
        struct npc_finkle_einhornAI : public ScriptedAI
        {
            ...
            void DoAction(int32 const action)
            {
                if (action == 55)
                {
                    me->GetMotionMaster()->MovePath(214325, false);
                    me->DespawnOrUnsummon(15000);
                    Talk(4, NULL, CHAT_MSG_MONSTER_SAY, TEXT_RANGE_MAP);
                    return;
                }
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (talkTimer <= diff)
                {
                    if (auto p = me->FindNearestPlayer(80.f))
...
class npc_victor_nefarius_chimaeron : public CreatureScript
{
    public:
        ...
        struct npc_victor_nefarius_chimaeronAI : public ScriptedAI
        {
            ...
            {
                TalkWithDelay(2000, SAY_AGGRO, NULL, CHAT_MSG_MONSTER_SAY);
            }
            ...
            void DoAction(int32 const action)
            {
                ...
                case ACTION_DEATH:
                    ...
                    break;
                case ACTION_FEUD:
                    Talk(SAY_FEUD, NULL, CHAT_MSG_MONSTER_SAY, TEXT_RANGE_MAP);
                    me->CastWithDelay(2000, me, SPELL_SHADOW_WHIP, true);
                    break;
                case ACTION_PHASE_2:
                    Talk(SAY_MOCKING_SHADOWS, NULL, CHAT_MSG_MONSTER_SAY, TEXT_RANGE_MAP);
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (deathTimer <= diff)
                {
                    ...
                    me->CastWithDelay(6000, me, SPELL_TELEPORTATION_VISUAL);
...
class go_chimaeron_cage : public GameObjectScript
{
public:
    go_chimaeron_cage() : GameObjectScript("go_chimaeron_cage") { }

    bool OnGossipHello(Player* pPlayer, GameObject* go)
    {
        if (!unlocked)
        if (pPlayer->HasItemCount(64663, 1))
            if (auto finkle = go->FindNearestCreature(NPC_FINKLE_EINHORN, 100.f))
            {
                if (finkle->isAlive())
                {
                    pPlayer->DestroyItemCount(64663, 1, true);
                    unlocked = true;
                    finkle->AI()->DoAction(55);
                    return false;
                }
            }
            return true;
    ...
    struct go_chimaeron_cageAI : public GameObjectAI
    {
        go_chimaeron_cageAI(GameObject* go) : GameObjectAI(go)
        {
        }
    ...
    {
        return new go_chimaeron_cageAI(go);
    }
};
