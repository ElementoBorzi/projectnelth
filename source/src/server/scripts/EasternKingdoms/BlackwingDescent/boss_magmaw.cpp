...
enum Misc
{
    ...
    ACTION_NEFARIAN_RP_INTRO        = 100,
    ACTION_NEFARIAN_ASSIST_1        = 101,
    ACTION_NEFARIAN_ASSIST_2        = 102,
    ACTION_NEFARIAN_OUTRO           = 103,
};
...
static const Position nefarianPos = { -437.184875f, 24.247477f, 243.747971f, 6.206650f};
static const Position movePos = { -324.588348f, 0.520290f, 249.471802f, 4.751662f };
...
public:
    ...
    struct boss_magmawAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            //me->PlayOneShotAnimKit(ANIM_KIT_SUBMERGE);

            summons.DespawnAll();
            me->DespawnCreaturesInArea(42321);
            me->DespawnCreaturesInArea(NPC_HEROIC_NEFARIAN);
            ...
            if (!instance->IsDone(DATA_MAGMAW))
                _Reset();
            if (!events.HasEvent(999))
                events.ScheduleEvent(999, 2000);
            me->SetFacingTo(4.2f);
        }
        ...
        void ChainUp()
        {
            ...
            GetCreatureListWithEntryInGrid(list_of_npcs, me, 42362, 50.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, 41767, 50.0f);
                ...
                for (auto c : list_of_npcs)
                    if (c->isAlive())
                        if (!c->isInCombat())
                        {
                            ...
                            {
                            case 207392:
                                c->LoadEquipment(0, true);
                                c->SetSheath(SHEATH_STATE_UNARMED);
                                c->HandleEmoteCommand(EMOTE_STATE_READY2H);
                                c->AddAura(SPELL_CHAIN_L, me);
                                break;
                            case 207393://channel
                                c->HandleEmoteCommand(EMOTE_CHANNEL);
                                break;
                            case 203561:
                                //TC_LOG_ERROR("sql.sql", "chain post found.");
                                c->AddAura(SPELL_CHAIN_R, me);
                            default:
                                break;
                            }
                        }
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if (auto nefarian = me->FindNearestCreature(NPC_HEROIC_NEFARIAN, 200.f))
                nefarian->AI()->DoAction(ACTION_NEFARIAN_OUTRO);
        }
        ...
        void EnterCombat(Unit* who)
        {
            ...
            //setPlayerVehicle(true);
                if (auto nefarian = me->SummonCreature(NPC_HEROIC_NEFARIAN, nefarianPos))
                {
                    nefarian->SetCanFly(true);
                    nefarian->SetHover(true);
                    nefarian->SetReactState(REACT_PASSIVE);
                    nefarian->NearTeleportTo(nefarian->GetHomePosition());
                    nefarian->SetHomePosition(movePos);
                    nefarian->GetMotionMaster()->MovePoint(1, movePos, false, 10.f);
                    //nefarian->MonsterYell("NEFARIAN HERE", LANG_UNIVERSAL, 0);
                }
        ...
        {
            if (!initial_head_reset)
            if (who->ToPlayer())
                if (auto v = me->GetVehicleKit())
                {
                    initial_head_reset = true;
                    v->InstallAllAccessories(false);
                }
        }
        ...
        void DamageTaken(Unit* /*done_by*/, uint32 &damage)
        {
            if (!below30 && me->HealthBelowPctDamaged(30, damage))
            {
                ...
                if (auto nefarian = me->FindNearestCreature(NPC_HEROIC_NEFARIAN, 200.f))
                    nefarian->AI()->DoAction(ACTION_NEFARIAN_ASSIST_2);
            }
        }
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
                ...
                case 999:
                    ChainUp();
                    //TC_LOG_ERROR("sql.sql", "executing chain up.");
                    events.ScheduleEvent(999, 5000);
                    break;
    ...
    struct npc_drakonid_drudgeAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit * who)
        {
            me->SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);
                me->RemoveAurasDueToSpell((me->GetDBTableGUIDLow() == 207393 ? SPELL_CHAIN_VISUAL_RIGHT : SPELL_CHAIN_VISUAL_LEFT));
            DoStartMovement(who);
            if (auto worm = me->FindNearestCreature(NPC_MAGMAW, 100.f, true))
                if (auto aura = worm->GetAura(SPELL_CHAIN_L, GetGUID()))
                    aura->Remove();
        }
...
class npc_MagMaw_Nefarian_extra : public CreatureScript
{
public:
    npc_MagMaw_Nefarian_extra() : CreatureScript("npc_MagMaw_Nefarian_extra") { }

    struct npc_MagMaw_Nefarian_extraAI : public ScriptedAI
    {
        npc_MagMaw_Nefarian_extraAI(Creature* creature) : ScriptedAI(creature)
        {
            instance = me->GetInstanceScript();
            summonerGUID = 0;
        }
        ...
        void MovementInform(uint32 type, uint32 id) 
        {
            if (type == POINT_MOTION_TYPE)
                if (id == 1)
                {
                    DoAction(ACTION_NEFARIAN_RP_INTRO);
                    me->SetFacingTo(4.7f);
                }
        }
        ...
        {
            if (act == ACTION_NEFARIAN_RP_INTRO)
            {
                //https://www.wowhead.com/sound=23367/vo-bd-nefarian-magmawintro01
                Talk(1, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                events.ScheduleEvent(1, 16000);
            }

            if (act == ACTION_NEFARIAN_ASSIST_2)
            {
                Talk(3, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
            }

            if (act == ACTION_NEFARIAN_OUTRO)
            {
                Talk(IsHeroic() ? 4 : 5, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                ...
                me->GetMotionMaster()->MovePoint(2, nefarianPos, false, 5.f);
                me->DespawnOrUnsummon(10000);
            }
        }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_MagMaw_Nefarian_extraAI(creature);
    }
};
