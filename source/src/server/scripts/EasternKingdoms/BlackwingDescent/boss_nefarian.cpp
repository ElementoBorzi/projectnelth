    ...
    struct boss_bwd_nefarianAI : public BossAI
    {
        ...
        void changePhase(EncounterPhases phase)
        {
            ...
            case PHASE_2:
                ...
                Talk(3);
                instance->SetData(DATA_NEFARIAN_ELEVATOR, GO_STATE_TRANSPORT_SPEC);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_START_PHASE_1:
                    events.ScheduleEvent(EVENT_DELAY_LAND, 12 * IN_MILLISECONDS);
                    //me->SetHover(false);
                    //me->SetCanFly(false);
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
                    me->GetMotionMaster()->MoveIdle();
...
public:
    ...
    struct npc_nefarius_nefarian_introAI : public ScriptedAI
    {
        npc_nefarius_nefarian_introAI(Creature* creature) : ScriptedAI(creature),
            _instance(creature->GetInstanceScript())
        {
            if (_instance->GetBossState(DATA_NEFARIAN) != DONE)
            {
                if (me->isDead())
                    me->Respawn(true);
                me->SetVisible(true);
        ...
        void Reset()
        {
            ...
            if (auto c3 = me->SummonCreature(144951, -27.928150f, -233.461151f, 69.838760f, 6.195622f))
            if (auto c4 = me->SummonCreature(144951, -28.930147f, -218.672455f, 74.262772f, 6.227029f))
            if (auto c5 = me->SummonCreature(144951, -28.330013f, -227.791748f, 74.262772f, 0.014528f))
                c1->CastSpell(c1, SPELL_DRAGON_ORB, true);
            // me->SetVisible(false);
            start = false;
            me->SetReactState(REACT_PASSIVE);
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                case EVENT_LEAVE_UPSTAIRS:
                    DoCast(me, 35517, true);
                    _events.ScheduleEvent(EVENT_LEAVE_UPSTAIRS + 1, 600);
                    break;
                case EVENT_LEAVE_UPSTAIRS + 1:
                    me->NearTeleportTo(at_onyxia, true);
                    me->SetHomePosition(at_onyxia);
                    break;
                case EVENT_START_INTRO:
                    me->SummonCreature(BOSS_NEFARIAN, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()+5.f, me->GetOrientation());
        ...
        {
            if (!gate_introduction)
                if (who->ToPlayer())
                    if (me->GetPositionZ() > 100.f)
                        if (auto door = me->FindNearestGameObject(GO_WING_DOOR, 50.f))
                        {
                            if (_instance->IsDone(DATA_NEFARIAN) || (door->GetGoState() == GO_STATE_ACTIVE))
                            {
                                me->NearTeleportTo(at_onyxia, true);
                                gate_introduction = true;
                                return;
                            }
                            ...
                            if (players.size() > 1)
                                ...
                                {
                                    Talk(3, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                                    gate_introduction = true;
                                    _events.ScheduleEvent(EVENT_LEAVE_UPSTAIRS, 15000);
                                }
                        }
        }
    ...
    private:
        ...
        InstanceScript* _instance;
        bool start;
        bool gate_introduction{ false };
        Position at_onyxia{ -187.034958f, -223.589493f, 43.804321f, 6.262542f };
    };
...
#define GOSSIP_DRAGON_ORB "<Place my hand on the orb>"
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go)
    {
        ...
        player->SEND_GOSSIP_MENU(7154, go->GetGUID());
        return true;
    }
    ...
    bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/, uint32 action)
    {
        ...
        GetCreatureListWithEntryInGrid(list_of_npcs, go, 144951, 20.0f);
...
class spell_orb_cosmetic_green_ribbons : public SpellScriptLoader
{
public:
    spell_orb_cosmetic_green_ribbons() : SpellScriptLoader("spell_orb_cosmetic_green_ribbons") { }

    class spell_orb_cosmetic_green_ribbons_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_orb_cosmetic_green_ribbons_AuraScript);

        void HandleTriggerSpellPlayers(AuraEffect const* aurEff)
        {
            //TC_LOG_ERROR("sql.sql", "casting ribbons on entries nearby.");
            PreventDefaultAction();
            if (auto me = GetOwner()->ToUnit())
            {
                auto list_of_players = me->GetPlayersInRange(15.f, true);
                if (list_of_players.size())
                    if (auto random_target = Trinity::Containers::SelectRandomContainerElement(list_of_players))
                        me->CastSpell(random_target, 78220, true);
            }
        }
            ...
            if (auto me = GetOwner()->ToUnit())
            {
                ...
                if (list_of_npcs.size())
                    for (auto itr = list_of_npcs.begin(); itr != list_of_npcs.end();)
                        if (auto t = (*itr))
                            if (t->GetGUID() == me->GetGUID() || t->GetGUID() == target_guid)//filter condition
                            {
                ...
                {
                    me->CastSpell(random_target, 78219, true);
                    target_guid = random_target->GetGUID();
                }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_orb_cosmetic_green_ribbons_AuraScript::HandleTriggerSpellPlayers, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_orb_cosmetic_green_ribbons_AuraScript::HandleTriggerSpell, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    private:
    ...
    AuraScript* GetAuraScript() const
    {
        return new spell_orb_cosmetic_green_ribbons_AuraScript();
    }
};
...
void AddSC_boss_bwd_nefarian()
{
    ...
    new spell_orb_cosmetic_green_ribbons();
}