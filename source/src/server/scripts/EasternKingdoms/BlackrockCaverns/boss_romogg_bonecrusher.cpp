    ...
    public:
        ...
        struct boss_romogg_bonecrusherAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                ...
                AttackStart(who);
                if (IsHeroic())
                    me->CastWithDelay(200, me, SPELL_CALL_FOR_HELP);
            ...
            {
                if (spell->Id == SPELL_THE_SKULLCRACKER || spell->Id == 93454/*Heroic*/)
                {
                    events.CancelEvent(EVENT_SPELLCAST_RECOVER);
                    events.ScheduleEvent(EVENT_SPELLCAST_RECOVER, 1);
                }
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                    switch (eventId)
                    {
                        case EVENT_QUAKE:
                            ...
                            {
                                /*
                                    We've juset set a couple things that cannot remain to ensure that this spellcast goes off without an issue.
                                    We NEED the recovery event to be roughly a second after the cast finishes.
                                */
                                me->SetReactState(REACT_PASSIVE);
                                me->AddUnitState(UNIT_STATE_ROOT);
                                events.ScheduleEvent(EVENT_SPELLCAST_RECOVER, 5000);    
                        ...
                        case EVENT_SKULLCRACKER:
                            ...
                            else
                            {
                                ...
                                */
                                me->SetReactState(REACT_PASSIVE);
                                me->AddUnitState(UNIT_STATE_ROOT);
                                events.ScheduleEvent(EVENT_SPELLCAST_RECOVER, 13000);

                                Talk(TALK_SKULLCRACKER);
                                DoCastAOE(SPELL_THE_SKULLCRACKER);
                            }
                            break;
                        case EVENT_SPELLCAST_RECOVER:
...
class npc_raz_the_crazed : public CreatureScript
{
    public:
        ...
        struct npc_raz_the_crazedAI : public ScriptedAI
        {
            ...
            {
                timerEvent = 1000;
                events.ScheduleEvent(EVENT_FURIOUS_RAGE, 1000);
...
class spell_romogg_skull_crack : public SpellScriptLoader
{
public:
    spell_romogg_skull_crack() : SpellScriptLoader("spell_romogg_skull_crack") { }

    class spell_romogg_skull_crack_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_romogg_skull_crack_SpellScript);
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_romogg_skull_crack_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_romogg_skull_crack_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_romogg_skull_crack_SpellScript();
    }
};
