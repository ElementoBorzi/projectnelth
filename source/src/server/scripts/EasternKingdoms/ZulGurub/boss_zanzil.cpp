...
{
    {-11541.0f, -1299.15f, 89.2326f, 2.13874f},
    {-11603.6f, -1233.00f, 85.3652f, 5.00108f},
    {-11544.0f, -1240.56f, 85.5043f, 3.62699f},
};
...
class boss_zanzil : public CreatureScript
{
    ...
    private:
        struct boss_zanzilAI : public BossAI
        {
            ...
            void InitializeAI()
            {
                me->ApplySpellImmune(SPELL_VODOO_BOLT,  IMMUNITY_MECHANIC,      MECHANIC_INTERRUPT,     false);
                Reset();
            }
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                ...
                switch (id)
                {
                    case POINT_BLUE_CAULDRON:
                    {
                        ...
                        events.ScheduleEvent(RAND(EVENT_DRAIN_RED_CAULDRON, EVENT_DRAIN_GREEN_CAULDRON), 30000);
                        break;
                    }
            ...
            void JustSummoned(Creature* summoned)
            {
                switch (summoned->GetEntry())
                {
                    ...
                    case NPC_ZANZILI_BERSERKER:
                        summoned->DisableMovementFlagUpdate(true);
                        summoned->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        summoned->SetReactState(REACT_PASSIVE);
                        summoned->SetCanFly(true);
                        summoned->SetDisableGravity(true, true);
                        summoned->SetHover(true);
                        summoned->SetUInt32Value(UNIT_NPC_EMOTESTATE, 398);
                        break;
            ...
            void EnterCombat(Unit* /*who*/)
            {
                ...
                events.ScheduleEvent(WIPE_CHECK, 5000);
            }
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            me->UpdateOrientation(me->GetAngle(me->getVictim()));
                            //me->AddUnitState(UNIT_STATE_CANNOT_TURN);
                            DoCastVictim(SPELL_ZANZILI_FIRE);
                            events.ScheduleEvent(EVENT_ZANZILI_FIRE, urand(15000, 20000));
                            //events.ScheduleEvent(EVENT_ZANZILI_FIRE_RECOVER, 4000);
                            break;
                        }
                        case EVENT_ZANZILI_FIRE_RECOVER:
                        {
                            me->ClearUnitState(UNIT_STATE_CANNOT_TURN);
                            break;
                        }
                        ...
                        {
                            DoCastVictim(SPELL_VODOO_BOLT);
                            events.ScheduleEvent(EVENT_VOODOO_BOLT, urand(2000, 5000));
                            break;
                        }
                        case WIPE_CHECK:
                        {
                            //TC_LOG_ERROR("sql.sql", "Checking For Wipe.");
                            if (checkWipe())
                            {
                                //TC_LOG_ERROR("sql.sql", "Group wiped, clearing combat.");
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_PURSUIT:
                    {
                        ...
                        if (auto target = Trinity::Containers::SelectRandomContainerElement(list_of_players))
                        {
                            if (list_of_players.size())
                                for (auto player_nearby : list_of_players)
                                    if (auto session = player_nearby->GetSession())
                                    {
                                        if (player_nearby->GetGUID() == target->GetGUID())
                                        {
                                            WorldPacket data2(SMSG_MESSAGECHAT, 200);
                                            std::stringstream warning_to_send2{ "" };
                                            warning_to_send2 << "Gurubashi Berserker is chasing you! Run!";
                                            auto text_pre2{ warning_to_send2.str() };
                                            auto text2{ text_pre2.c_str() };

                                            auto loc_idx{ session->GetSessionDbLocaleIndex() };
                                            me->BuildMonsterChat(&data2, CHAT_MSG_RAID_BOSS_EMOTE, text2, LANG_UNIVERSAL, me->GetNameForLocaleIdx(loc_idx), player_nearby->GetGUID());
                                            //ChatHandler(session).PSendSysMessage("%s", text_pre2.c_str());
                                            //TC_LOG_ERROR("sql.sql", "sending chasing packet to chasing target");
                                            session->SendPacket(&data2);
                                        }
                                        else
                                        {
                                            WorldPacket data(SMSG_MESSAGECHAT, 200);
                                            std::stringstream warning_to_send{ "" };
                                            warning_to_send << "Gurubashi Berserker is chasing " << target->GetName() << "!";
                                            auto text_pre1{ warning_to_send.str() };
                                            ...
                                            //ChatHandler(session).PSendSysMessage("%s", text_pre1.c_str());
...
class spell_zanzili_fire : public SpellScriptLoader
{
    ...
    private:
        class spell_zanzili_fire_AuraScript : public AuraScript
        {
            ...
            Position start_pos {    0.f, 0.f, 0.f, 0.f  };
...
enum witch_doctor
{
    ...
    SPELL_SHRINK = 97397,
    SPELL_RELEASE_TOADS = 97398,
    SPELL_SHADOW_BOLT = 96416,
    SPELL_SHADOW_CHANNELING = 45104,
    SPELL_SUMMON_VOODOO = 96418,
};

class npc_Hakkari_Witch_Doctor : public CreatureScript
{

public:
    npc_Hakkari_Witch_Doctor() : CreatureScript("npc_Hakkari_Witch_Doctor") { }

    struct npc_Hakkari_Witch_DoctorAI : public ScriptedAI
    {
        npc_Hakkari_Witch_DoctorAI(Creature* creature) : ScriptedAI(creature), summons(creature) {}

        void InitializeAI()
        {
            events.ScheduleEvent(EVENT_START_CHANNEL, 5000);
        }

        void EnterCombat(Unit* /*who*/) {
            me->ClearUnitState(UNIT_STATE_CASTING);
            events.ScheduleEvent(EVENT_SHRINK, urand(7000, 30000));
            events.ScheduleEvent(EVENT_SHADOWBOLT, urand(3000, 7000));
            events.ScheduleEvent(EVENT_TOADS, urand(24000, 26000));
        }
        void Reset()
        {
            events.Reset();
        ...
        {
            summons.DespawnAll();
            me->CastSpell(me, SPELL_SUMMON_VOODOO, true);
        }
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
                ...
                case EVENT_SHADOWBOLT:
                    ...
                    break;
                case EVENT_TOADS:
                    spell = SPELL_RELEASE_TOADS;    //Spell ID
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_Hakkari_Witch_DoctorAI(creature);
    }
};


class spell_shrink_aura : public SpellScriptLoader
{
public:
    spell_shrink_aura() : SpellScriptLoader("spell_shrink_aura") { }

    class spell_shrink_aura_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_shrink_aura_AuraScript);
        ...
        void Register()
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_shrink_aura_AuraScript::HandleOnEffectRemove, EFFECT_1, SPELL_AURA_MOD_SCALE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_shrink_aura_AuraScript();
    }
};
...
void AddSC_boss_zanzil()
{
    ...
    new npc_Hakkari_Witch_Doctor();
    new spell_shrink_aura();
}
