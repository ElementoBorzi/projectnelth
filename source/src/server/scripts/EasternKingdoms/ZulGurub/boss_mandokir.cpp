...
enum Events
{
    ...
    EVENT_DEVASTATING_SLAM_RECOVER,
    EVENT_SLAM_FIX_ORIENTATION,
    EVENT_BLOODLORD_AURA,
    EVENT_CHECK_VALID_PLAYERS,
};
    ...
    public:
        ...
        struct boss_mandokirAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                for (uint8 i = 0; i < 8; ++i)
                    if (auto spirit = me->SummonCreature(NPC_CHAINED_SPIRIT, ChainedSpiritsSpawnPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000))
                        spirit->setFaction(35);
                ...
                switch (summoned->GetEntry())
                {
                    ...
                    case NPC_DEVASTATING_SLAM:
                    {
                        ...
                        for (uint8 i = 0; i <= 30; ++i)
                        {
                            ...
                            {
                            case 0:
                                me->GetNearPoint2D(x, y, 0.5f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 1:
                            case 2:
                                me->GetNearPoint2D(x, y, 5.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 3:
                            case 4:
                            case 5:
                                me->GetNearPoint2D(x, y, 9.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 6:
                            case 7:
                            case 8:
                            case 9:
                                me->GetNearPoint2D(x, y, 15.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 10:
                            case 11:
                            case 12:
                            case 13:
                            case 14:
                                me->GetNearPoint2D(x, y, 20.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 15:
                            case 16:
                            case 17:
                            case 18:
                            case 19:
                                me->GetNearPoint2D(x, y, 25.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 20:
                            case 21:
                            case 22:
                            ...
                            case 24:
                            case 25:
                                me->GetNearPoint2D(x, y, 30.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            case 26:
                            case 27:
                            case 28:
                            case 29:
                            case 30:
                                me->GetNearPoint2D(x, y, 35.f, me->NormalizeOrientation(me->GetOrientation() + frand(-M_PI / 6, M_PI / 6)));
                                break;
                            }
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    case ACTION_OHGAN_IS_DEATH:
                    {
                        ...
                        events.ScheduleEvent(EVENT_REANIMATE_OHGAN, LeftTime + urand(10000, 15000));
                        break;
                    }
                    case ACTION_EVADE:
                        CanEvade = true;
            ...
            {
                if (target->ToPlayer())
                    if (spell->Id == SPELL_DECAPITATE_DAMAGE)
                        if (target->GetHealthPct() <= 0.f)
                            RevivePlayer(target->ToPlayer());
            }


            void OnSpellCastFinished(SpellInfo const* spell, SpellFinishReason reason) 
            { 
                //if (spell->Id == SPELL_DEVASTATING_SLAM)
                    //TC_LOG_ERROR("sql.sql", "DEVASTATING SLAM FINISHED: %u", uint32(reason));
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                    switch (eventId)
                    {
                        case EVENT_BLOODLORD_AURA:
                            if (!me->HasAura(SPELL_BLOODLORD_AURA))
                                me->AddAura(SPELL_BLOODLORD_AURA, me);
                            if (SummonedSpirit.empty())
                            {
                                if (auto ohgan = me->FindNearestCreature(NPC_OHGAN, 200.f, true))
                                    if (!ohgan->HasAura(SPELL_PERMANENT_FEIGN_DEATH))
                                        if (!ohgan->getVictim())
                                        if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.f, true))
                                        {
                                            ohgan->SetReactState(REACT_AGGRESSIVE);
                                            ohgan->AI()->AttackStart(target);
                                        }
                            }
                                events.ScheduleEvent(EVENT_BLOODLORD_AURA, 5000);
                            break;
                        ...
                        case EVENT_DEVASTATING_SLAM:
                            ...
                            {
                                Talk(3);
                                slamOrientation = me->GetOrientation();
                                me->SetReactState(REACT_PASSIVE);
                                me->StopMoving();
                                Position pos(*me);
                                me->MoveBlink(pos, 5.f, 0.f);
                                me->SummonCreature(NPC_DEVASTATING_SLAM, pos, TEMPSUMMON_TIMED_DESPAWN, 8000);
                                me->AttackStop();
                                me->CastWithDelay(200, me, SPELL_DEVASTATING_SLAM);
                                me->CastSpell(me, SPELL_DEVASTATING_SLAM_DUMMY, false);
                                events.ScheduleEvent(EVENT_DEVASTATING_SLAM_RECOVER, 5000);
                                events.ScheduleEvent(EVENT_SLAM_FIX_ORIENTATION, 250);
                                events.ScheduleEvent(EVENT_DEVASTATING_SLAM, 30000);
                                if (events.HasEvent(EVENT_REANIMATE_OHGAN))
                                    events.DelayEvent(EVENT_REANIMATE_OHGAN, 5000);
                            }
                            break;
                        case EVENT_SLAM_FIX_ORIENTATION:
                            //TC_LOG_ERROR("sql.sql", "EVENT_SLAM_FIX_ORIENTATION");
                            me->SetFacingTo(slamOrientation);
                            me->AddUnitState(UNIT_STATE_CANNOT_TURN);
                            break;
                        case EVENT_DEVASTATING_SLAM_RECOVER:
                            //TC_LOG_ERROR("sql.sql", "EVENT_DEVASTATING_SLAM_RECOVER");
        ...
        struct npc_ohganAI : public ScriptedAI
        {
            ...
            void SetGUID(uint64 guid, int32 type)
            {
                if (Unit *target = Unit::GetUnit(*me, guid))
                {
                    ...
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_NON_ATTACKABLE);
                    instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, me);
                }
            }
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    case ACTION_REANIMATE_OHGAN:
                        //TC_LOG_ERROR("sql.sql", "Recognized ACTION_REANIMATE_OHGAN");
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& damage)
            {
                ...
                {
                    me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_NON_ATTACKABLE);
                    damage = 0;
            ...
            void HandleAfterCast()
            {
                if (auto c = GetCaster())
                    if (auto cr = c->ToCreature())
                        if (auto ohgan = c->FindNearestCreature(NPC_OHGAN, 200.f, true))
                            ohgan->AI()->DoAction(ACTION_REANIMATE_OHGAN);
            }

            void Register()
            {
                AfterCast += SpellCastFn(spell_mandokir_reanimate_ohgan_SpellScript::HandleAfterCast);
            }
        };
    ...
    private:
        class spell_ohgan_permanent_feign_deathAuraScript : public AuraScript
        {
            ...
            void EffectRevive(AuraEffect const * /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (auto c = GetTarget())
                {
                    c->RemoveFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_FEIGN_DEATH);
                    c->RemoveFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_DEAD);
                    
                    if (auto m = c->FindNearestCreature(NPC_MANDOKIR, 100.f, true))
                        c->NearTeleportTo(*m);
                }
            }
