...
#define PATH_EFUSION_LEFT 5230200 //east 5230200
#define PATH_EFUSION_RIGHT 5230300//west
        ...
        struct boss_venoxisAI : public BossAI
        {
            ...
            void Punish_Venom_Maze()
            {
                ...
                if (players.size())
                    for (Player* player : players)
                        if (auto gas = player->FindNearestCreature(NPC_VENOMOUS_EFFUSION, 0.25f, true))
                            if (gas->HasAura(SPELL_VENOMOUS_EFFUSION_AURA))
                                gas->AI()->DoCast(player, IsHeroic() ? 96685 : 97338, true);
            }
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                    switch (eventId)
                    {
                    case EVENT_VENOM_GAS_TICK:
                            Punish_Venom_Maze();
                            events.ScheduleEvent(EVENT_VENOM_GAS_TICK, 500);
                        break;
                        case EVENT_PHASE_1:
                        {
                            ...
                            else
                            {
                                //  22:11:11.000
                                events.Reset();
                                events.ScheduleEvent(EVENT_VENOM_GAS_TICK, 1000);
