        ...
        struct boss_hex_lord_malacrassAI : public BossAI
        {
            ...
            void DoAction(const int32 action) override
            {
                ...
                case 2:
                    if (Introduction2)
                        return;
                    sCreatureTextMgr->SendChat(me, HEXLORD_TALK_FEAST_TONIGHT, 0, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_MAP);
                    Introduction2 = true;
                    break;
                case 3:
                    if (Introduction3)
                        return;
                    //TC_LOG_ERROR("sql.sql", "sacrifice state: %u", instance->GetData(DATA_SACRIFICES_STATE));
                    sCreatureTextMgr->SendChat(me, (instance->GetData(DATA_SACRIFICES_STATE) == STATE_SACRIFICES_ALL_SAFE ? HEXLORD_TALK_HOSTAGES_RESCUED : HEXLORD_TALK_HOSTAGES_FAILED), 0, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_MAP);
                    Introduction3 = true;
                    break;
                }
            }
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_SIPHON_SOUL:
                        {
                            ...
                            if (auto random_target = Trinity::Containers::SelectRandomContainerElement(list_of_players))
                               DoCast(random_target, SPELL_SIPHON_SOUL, true);
                            me->ClearUnitState(UNIT_STATE_CASTING);
                        ...
                        case EVENT_DK_PLAGUE_STRIKE:
                            ...
                            {
                                if (auto v = me->getVictim()) me->CastSpell(v, SPELL_DK_PLAGUE_STRIKE, false);
                        ...
                        case EVENT_MG_FROSTBOLT:
                            ...
                            else
                            {
                                ...
                                events.ScheduleEvent(EVENT_MG_FROSTBOLT, 4000);
                            }
                            break;
        ...
        struct npc_blood_wormsAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (TempSummon* summ = me->ToTempSummon())
                {
                    ...
                    {
                        if (me->IsWithinDistInMap(summoner, 2.0f))
                        {
...
class go_za_hexlord_exitDoor : public GameObjectScript
{
public:
    go_za_hexlord_exitDoor() : GameObjectScript("go_za_hexlord_exitDoor") {}

    struct go_za_hexlord_exitDoorAI : public GameObjectAI
    {
        go_za_hexlord_exitDoorAI(GameObject* go) : GameObjectAI(go) 
        {
        ...
        bool GossipHello(Player* player) override
        {
            ...
            if (auto p = player->ToUnit())
            {
                ...
                GetCreatureListWithEntryInGrid(units, player, 23889, 150.f);
                if (!units.empty())
                {
                    for (std::list<Creature*>::iterator itr = units.begin(); itr != units.end(); ++itr)
                        if ((*itr)->isAlive())
                        {
                            (*itr)->AddThreat(p, 10000.f);
    ...
    {
        return new go_za_hexlord_exitDoorAI(go);
    }
};

class spell_hexlord_drain_power : public SpellScriptLoader
{
public:
    spell_hexlord_drain_power() : SpellScriptLoader("spell_hexlord_drain_power") { }

    class spell_hexlord_drain_power_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hexlord_drain_power_AuraScript);
        ...
        void OnPeriodic(AuraEffect const* aurEff)
        {
            if (auto c = GetCaster())
                ...
                if (c->HasUnitState(UNIT_STATE_CASTING))
                {
                    bool reset{ true };
                    for (auto i = 0; i <= 3; ++i)
                        if (auto spell_in_slot = c->GetCurrentSpell(i))
                            if (spell_in_slot->m_spellInfo->Id != GetId())
                                reset = false;

                    if (reset)
                    {
                        //TC_LOG_ERROR("sql.sql", "resetting to show channeling on target.");
                        c->ClearUnitState(UNIT_STATE_CASTING);
                        c->SetUInt32Value(UNIT_CHANNEL_SPELL, GetId());
                        c->SetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT, o->GetGUID());
                    }
                }
        }
        ...
        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_hexlord_drain_power_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_hexlord_drain_power_AuraScript();
    }
};
...
void AddSC_boss_hex_lord_malacrass()
{
    ...
    new go_za_hexlord_exitDoor();
    new spell_hexlord_drain_power();
}
