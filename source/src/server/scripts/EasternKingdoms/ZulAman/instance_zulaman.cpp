    ...
    public:
        ...
        struct instance_zulaman_InstanceMapScript : public InstanceScript
        {
            ...
            void OnPlayerEnter(Player* player)
            {
                ...
                if (!HexlordGUID)
                    instance->SummonCreature(NPC_HEXLORD, ZA_Spawns[TRIGGER_HEXLORD_SPAWN]);
            }
            ...
            void OnCreatureCreate(Creature* creature)
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_PROTECTIVE_WARD:
                        creature->AddUnitState(UNIT_STATE_NOT_MOVE);
                        //creature->StopMoving();
            ...
            void CheckInstanceStatus()
            {
                if (IsDone(DATA_NALORAKKEVENT) && IsDone(DATA_AKILZONEVENT) && IsDone(DATA_JANALAIEVENT) && IsDone(DATA_HALAZZIEVENT))
                {
                    if (QuestMinute > 0)
                    {
                        if (GetData(DATA_SACRIFICES_STATE) != STATE_SACRIFICES_ALL_SAFE)
                        {
                    ...
                    {
                        DoUpdateWorldState(WORLDSTATE_SHOW_TIMER, 0);
                        if (GetData(DATA_SACRIFICES_STATE) != STATE_SACRIFICES_FAILED)
                        {
                            SetData(DATA_SACRIFICES_STATE, STATE_SACRIFICES_FAILED);
                        }
                    }
            ...
            void SetData(uint32 type, uint32 data)
            {
                switch (type)
                {
                    ...
                    case TYPE_RAND_VENDOR_2:
                        RandVendor[1] = data;
                        break;
                    case DATA_SACRIFICES_STATE:
                        HostagesActiveState = data;
                        break;
                }
            }
            ...
            uint32 GetData(uint32 type) const
            {
                switch (type)
                {
                    ...
                    case TYPE_RAND_VENDOR_2:
                        return RandVendor[1];
                    case DATA_SACRIFICE_MINUTES:
                        return uint32(QuestMinute);
                        break;
                    case DATA_SACRIFICES_STATE:
                        return HostagesActiveState;
                        break;
                    default:
                        return 0;
                }
            ...
            {
                if (QuestMinute && GetData(DATA_SACRIFICES_STATE) == STATE_SACRIFICES_IN_DANGER)
                {
                    if (QuestTimer <= diff)
                    {
                        QuestMinute--;
            ...
            uint64 GetData64(uint32 type) const
            {
                switch (type)
                {
                    ...
                    case NPC_HEXLORD:           return HexlordGUID;
                }
