...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "add",            SEC_CONSOLE,     false, &HandleWpAddCommand,                "" },
            { "event",          SEC_CONSOLE,     false, &HandleWpEventCommand,              "" },
            { "load",           SEC_CONSOLE,     false, &HandleWpLoadCommand,               "" },
            { "modify",         SEC_CONSOLE,     false, &HandleWpModifyCommand,             "" },
            { "unload",         SEC_CONSOLE,     false, &HandleWpUnLoadCommand,             "" },
            { "reload",         SEC_CONSOLE,  false, &HandleWpReloadCommand,             "" },
            { "show",           SEC_CONSOLE,     false, &HandleWpShowCommand,               "" },
        };
        static std::vector<ChatCommand> commandTable =
        {
            { "wp",             SEC_CONSOLE,     false, NULL,                     "", wpCommandTable },
        };
        return commandTable;
    }
    /**
    ...
    */
    static bool HandleWpAddCommand(ChatHandler* handler, const char* args)
    {
        ...
        TC_LOG_INFO("sql.dev", "INSERT INTO waypoint_data (id, point, position_x, position_y, position_z, delay) VALUES "
                               "(%u, %u, %f, %f, %f, 0);",
    ...
    static bool HandleWpLoadCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (result)
        {
            ...
            TC_LOG_INFO("sql.dev", "UPDATE creature_addon SET path_id = %u WHERE guid = %u;", pathid, guidLow);
        }
        else
        {
            ...
            TC_LOG_INFO("sql.dev", "INSERT INTO creature_addon (guid, path_id) VALUES (%u, %u);", guidLow, pathid);
        }
        ...
        TC_LOG_INFO("sql.dev", "UPDATE creature SET MovementType = %u WHERE guid = %u", uint8(WAYPOINT_MOTION_TYPE), guidLow);
    ...
    static bool HandleWpModifyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (show == "del")
        {
            ...
            if (wpGuid != 0)
                if (auto wpCreature = handler->GetSession()->GetPlayer()->GetMap()->GetCreature(MAKE_NEW_GUID(wpGuid, VISUAL_WAYPOINT, HIGHGUID_UNIT)))
                {
        ...
        if (show == "move")
        {
            ...
            Map* map = chr->GetMap();
            {
                ...
                if (wpGuid != 0)
                {
                    if (auto wpCreature = map->GetCreature(MAKE_NEW_GUID(wpGuid, VISUAL_WAYPOINT, HIGHGUID_UNIT)))
                    {
                        wpCreature->CombatStop();
                        wpCreature->DeleteFromDB();
                        wpCreature->AddObjectToRemoveList();
