...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "cinematic",      SEC_CONSOLE,      false, &HandleDebugPlayCinematicCommand,   "" },
            { "movie",          SEC_CONSOLE,      false, &HandleDebugPlayMovieCommand,       "" },
            { "sound",          SEC_CONSOLE,      false, &HandleDebugPlaySoundCommand,       "" },
        };
        static std::vector<ChatCommand> debugSendCommandTable =
        {
            { "buyerror",       SEC_CONSOLE,  false, &HandleDebugSendBuyErrorCommand,       "" },
            { "channelnotify",  SEC_CONSOLE,  false, &HandleDebugSendChannelNotifyCommand,  "" },
            { "chatmmessage",   SEC_CONSOLE,  false, &HandleDebugSendChatMsgCommand,        "" },
            { "equiperror",     SEC_CONSOLE,  false, &HandleDebugSendEquipErrorCommand,     "" },
            { "largepacket",    SEC_CONSOLE,  false, &HandleDebugSendLargePacketCommand,    "" },
            { "opcode",         SEC_CONSOLE,  false, &HandleDebugSendOpcodeCommand,         "" },
            { "qpartymsg",      SEC_CONSOLE,  false, &HandleDebugSendQuestPartyMsgCommand,  "" },
            { "qinvalidmsg",    SEC_CONSOLE,  false, &HandleDebugSendQuestInvalidMsgCommand, "" },
            { "sellerror",      SEC_CONSOLE,  false, &HandleDebugSendSellErrorCommand,      "" },
            { "setphaseshift",  SEC_CONSOLE,  false, &HandleDebugSendSetPhaseShiftCommand,  "" },
            { "spellfail",      SEC_CONSOLE,  false, &HandleDebugSendSpellFailCommand,      "" },
        };
        static std::vector<ChatCommand> debugCommandTable =
        {
            { "setbit",         SEC_CONSOLE,  false, &HandleDebugSet32BitCommand,        "" },
            { "threat",         SEC_CONSOLE,  false, &HandleDebugThreatListCommand,      "" },
            { "hostil",         SEC_CONSOLE,  false, &HandleDebugHostileRefListCommand,  "" },
            { "anim",           SEC_CONSOLE,     false, &HandleDebugAnimCommand,            "" },
            { "arena",          SEC_CONSOLE,  false, &HandleDebugArenaCommand,           "" },
            { "bg",             SEC_CONSOLE,  false, &HandleDebugBattlegroundCommand,    "" },
            { "joinrbg",        SEC_CONSOLE,  false, &HandleDebugJoinRbgCommand,         "" },
            { "getitemstate",   SEC_CONSOLE,  false, &HandleDebugGetItemStateCommand,    "" },
            { "lootrecipient",  SEC_CONSOLE,     false, &HandleDebugGetLootRecipientCommand, "" },
            { "getvalue",       SEC_CONSOLE,  false, &HandleDebugGetValueCommand,        "" },
            { "getitemvalue",   SEC_CONSOLE,  false, &HandleDebugGetItemValueCommand,    "" },
            { "Mod32Value",     SEC_CONSOLE,  false, &HandleDebugMod32ValueCommand,      "" },
            { "play",           SEC_CONSOLE,      false, NULL,              "", debugPlayCommandTable },
            { "send",           SEC_CONSOLE,  false, NULL,              "", debugSendCommandTable },
            { "setaurastate",   SEC_CONSOLE,  false, &HandleDebugSetAuraStateCommand,    "" },
            { "setitemvalue",   SEC_CONSOLE,  false, &HandleDebugSetItemValueCommand,    "" },
            { "setvalue",       SEC_CONSOLE,  false, &HandleDebugSetValueCommand,        "" },
            { "spawnvehicle",   SEC_CONSOLE,  false, &HandleDebugSpawnVehicleCommand,    "" },
            { "setvid",         SEC_CONSOLE,  false, &HandleDebugSetVehicleIdCommand,    "" },
            { "entervehicle",   SEC_CONSOLE,  false, &HandleDebugEnterVehicleCommand,    "" },
            { "uws",            SEC_CONSOLE,  false, &HandleDebugUpdateWorldStateCommand, "" },
            { "update",         SEC_CONSOLE,  false, &HandleDebugUpdateCommand,          "" },
            { "itemexpire",     SEC_CONSOLE,  false, &HandleDebugItemExpireCommand,      "" },
            { "areatriggers",   SEC_CONSOLE,  false, &HandleDebugAreaTriggersCommand,    "" },
            { "los",            SEC_CONSOLE,      false, &HandleDebugLoSCommand,             "" },
            { "moveflags",      SEC_CONSOLE,  false, &HandleDebugMoveflagsCommand,       "" },
            { "phase",          SEC_CONSOLE,      false, &HandleDebugPhaseCommand,           "" },
            { "unroot",         SEC_CONSOLE,      false, &HandleDebugUnRootCommand,          "" },
            { "combat",         SEC_CONSOLE,      false, &HandleDebugCombatCommand,          "" },
            { "mapz",           SEC_CONSOLE,      false, &HandleMapZCommand,                 "" },
        };
        static std::vector<ChatCommand> commandTable =
        {
            { "debug",          SEC_CONSOLE,      true,  NULL,                  "", debugCommandTable },
            { "wpgps",          SEC_CONSOLE,  false, &HandleWPGPSCommand,                "" },
        };
        return commandTable;
    }
    ...
    static bool HandleDebugJoinRbgCommand(ChatHandler* handler, char const* /*args*/)
    {
        Player* target = handler->getSelectedUnit()->ToPlayer();
        if (!target)
            return false;
        ...
        PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), target->getLevel());
        if (!bracketEntry)
            return false;
        ...
        if (grp->GetLeaderGUID() != target->GetGUID())
            return false;
        ...
        for (auto itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            {
                WorldPacket data;
                sBattlegroundMgr->BuildStatusFailedPacket(&data, bg, target, 0, err);
    ...
    static bool HandleDebugSetVehicleIdCommand(ChatHandler* handler, char const* args)
    {
        ...
        target->CreateVehicleKit(id, target->GetEntry());
    ...
    static bool HandleMapZCommand(ChatHandler* handler, char const* /*args*/)
    {
        Player* player = handler->GetSession()->GetPlayer();

        float dz = player->GetPositionZ();
        player->UpdateGroundPositionZ(player->GetPositionX(), player->GetPositionY(), dz);

        handler->PSendSysMessage("ground : %f", dz);
        return true;
    }
};
