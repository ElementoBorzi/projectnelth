...
public:
    ...
    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        ...
        {
            if (auto nether = player->SummonCreature(22408, player->GetPositionX(), player->GetPositionY()+20, player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 180000))
...
public:
    ...
    bool OnUse(Player* player, Item* item, const SpellCastTargets & /*targets*/)
    {
        ...
        {
            if (auto pLeviroth = player->FindNearestCreature(NPC_LEVIROTH, 10.0f)) // spell range
            {
...
class item_charged_decoder_ring : public ItemScript
{
public:
    item_charged_decoder_ring() : ItemScript("item_charged_decoder_ring") { }

    bool OnExpire(Player* player, ItemTemplate const* /*pItemProto*/)
    {
        ItemPosCountVec dest;
        uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 74748, 1); // Charged Decoder Ring
        if (msg == EQUIP_ERR_OK)
            player->StoreNewItem(dest, 74748, true);
...
// spell 82580
class item_bravo_company_field_kit_60384 : public ItemScript
{
public:
    item_bravo_company_field_kit_60384() : ItemScript("item_bravo_company_field_kit_60384") { }

    bool OnUse(Player* player, Item* item, const SpellCastTargets& /*targets*/)
    {
        if (player->GetQuestStatus(QUEST_PRISONERS_OF_WAR) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_PRISONERS_OF_WAR) == QUEST_STATUS_COMPLETE)
        {
            if (!player->HasAura(SPELL_BRAVO_COMPANY_FIELD_KIT_1))
                player->CastSpell(player, SPELL_BRAVO_COMPANY_FIELD_KIT_1);
            else
...
// spell 82587
class item_bravo_company_field_kit_60385 : public ItemScript
{
public:
    item_bravo_company_field_kit_60385() : ItemScript("item_bravo_company_field_kit_60385") { }

    bool OnUse(Player* player, Item* item, const SpellCastTargets& /*targets*/)
    {
        if (player->GetQuestStatus(QUEST_TO_WIN_A_WAR) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_TO_WIN_A_WAR) == QUEST_STATUS_COMPLETE)
        {
            if (!player->HasAura(SPELL_BRAVO_COMPANY_FIELD_KIT_2))
                player->CastSpell(player, SPELL_BRAVO_COMPANY_FIELD_KIT_2);
            else
...
void AddSC_item_scripts()
{
    ...
    new item_charged_decoder_ring();
    new item_bravo_company_field_kit_60384();
    new item_bravo_company_field_kit_60385();
}
