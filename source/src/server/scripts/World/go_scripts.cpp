    ...
    bool OnGossipHello(Player* pPlayer, GameObject *pGo)
    {
        if (pGo->GetMap()->IsBattleArena())
        {
            ...
            if (!pPlayer->_markedReady)
            {
                ...
                uint8 neededPlayers = bg->isSoloQueueArena() ? 6 : bg->GetArenaType() * 2;
...
enum BarbecueSpell
{
    SPELL_GOBLIN_BARBECUE_SUMMON        = 84351,
    SPELL_GOBLIN_BARBECUE               = 84352,
    SPELL_GOBLIN_BARBECUE_FEAST_SUMMON  = 87915,
    SPELL_GOBLIN_BARBECUE_FEAST         = 87916
};

class go_goblin_barbecue : public GameObjectScript
{
public:
    go_goblin_barbecue() : GameObjectScript("go_goblin_barbecue") { }

    bool OnGossipHello(Player* player, GameObject* go)
    {
        uint32 spellid = 0;

        switch (go->GetSpellId())
        {
        case SPELL_GOBLIN_BARBECUE_SUMMON:
            spellid = SPELL_GOBLIN_BARBECUE;
            break;
        case SPELL_GOBLIN_BARBECUE_FEAST_SUMMON:
