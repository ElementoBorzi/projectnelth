...
enum RogueSpells
{
    ...
    SPELL_ROGUE_SMOKE_BOMB_AREA_TRIGGER          = 76577,
    SPELL_ROGUE_SMOKE_BOMB_VISIBLE_AURA          = 88611,
    SPELL_ROGUE_SMOKE_BOMB_HOSTILES_ONLY         = 107752,
};
        ...
        class spell_rog_smoke_AuraScript : public AuraScript
        {
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
            {
                if (DynamicObject* dyn = GetTarget()->GetDynObject(aurEff->GetId()))
                    GetTarget()->CastSpell(dyn->GetPositionX(), dyn->GetPositionY(), dyn->GetPositionZ(), SPELL_ROGUE_SMOKE_BOMB_VISIBLE_AURA, true);
            }
...
public:
    ...
    class spell_rog_vendetta_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_rog_vendetta_AuraScript);
    ...
    private:
        ...
        void Register()
        {
            OnEffectUpdate += AuraEffectUpdateFn(spell_rog_vendetta_AuraScript::AuraUpdate, EFFECT_0, SPELL_AURA_MOD_DAMAGE_FROM_CASTER);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_rog_vendetta_AuraScript();
    }
};
...
void AddSC_rogue_spell_scripts()
{
    ...
    new spell_rog_t12_set_proc();
    new spell_rog_t12_helper();
    new spell_rog_combat_readiness();
    new spell_rog_bandits_guile();
    new spell_rog_combat_potency();
    new spell_rog_legendary_dagger_proc();
