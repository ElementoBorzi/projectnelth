                ...
                for (std::list<WorldObject*>::iterator itr = targets.begin(); itr != targets.end();)
                            ...
                            if (auto casterPlayer = caster->ToPlayer())
                    {
                        if ((casterPlayer && casterPlayer->getClass() == CLASS_MAGE && !caster->HasSpell(116)) //Player is a mage and does not have ice lance known. this would be done as a hardcore attempt to exploit.
                            ...
                            || (casterPlayer && casterPlayer->getClass() == CLASS_DEATH_KNIGHT && caster->GetSpellCooldownDelay(45477) > 0) //death knight Icy touch is on cooldown (never happens unless interrupted)
                            || caster->HasAura(33786) //caster is cycloned
                            || unit->HasAura(SPELL_MAGE_RING_OF_FROST_DUMMY) //target is ringed
                            || unit->HasAura(SPELL_MAGE_RING_OF_FROST_FREEZE) //target is ringed
                            || unit->GetExactDist(GetExplTargetDest()) > outRadius //target is outside ring
                            || unit->GetExactDist(GetExplTargetDest()) < inRadius)  //target is inside ring
                        {
    ...
    public:
        ...
        class spell_mage_cauterize_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* aurEff, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                ...
                // If damage kills us
                if (remainingHealth <= 1 && !victim->ToPlayer()->HasSpellCooldown(MAGE_SPELL_CAUTERIZE_HEAL))
                {
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                if (eventInfo.GetSpellInfo())
                {
                    ...
                    {
                        case 93424/*Baron Ashbury's asphyxiate*/: return false;   break;
                    }
                }
                return true;
            }
            ...
            void Register() override
            {
                 DoCheckProc += AuraCheckProcFn(spell_mage_cauterize_AuraScript::CheckProc);
...
public:
    ...
    class spell_mage_fingers_of_frost_AuraScript : public AuraScript
    {
        ...
        {
            /*
            https://wowpedia.fandom.com/wiki/Frost_Armor
            "Cataclysm Patch 4.1.0 (2011-04-26): Has been reworked:
            ...
            */
            if (eventInfo.GetSpellInfo())
            {
                if (eventInfo.GetSpellInfo()->Id)
                 if (
                    (eventInfo.GetSpellInfo()->Id == 7321)
                 || (eventInfo.GetSpellInfo()->Id == 7302)
                 || (eventInfo.GetSpellInfo()->Id == 133)
                 )
                 return false;
            }
...
class spell_mage_frost_armor_debuff : public SpellScriptLoader
{//INSERT INTO `spell_script_names` (`spell_id`, `ScriptName`) VALUES ('7302', 'spell_mage_frost_armor_debuff');
public:
    spell_mage_frost_armor_debuff() : SpellScriptLoader("spell_mage_frost_armor_debuff") { }

    class spell_mage_frost_armor_debuff_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_mage_frost_armor_debuff_AuraScript);

        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (eventInfo.GetSpellInfo()) { //spell info must exist
                if (eventInfo.GetSpellInfo()->Id) { //spell info ID must exist
                    //TC_LOG_ERROR("entities.unit", "Frost armor procced by spell: %u", eventInfo.GetSpellInfo()->Id);
                    switch (eventInfo.GetSpellInfo()->Id)
                    {
                    case 57755: // heroic throw (not a melee attack)
                    case 54158: // Judgement (magic, based off weapon damage)
                    case 20424: // seals of command (magic)
                    case 24275: // hammer of wrath (magic, based off weapon damage) https://youtu.be/adeol-Io-Ho?t=154
                        return false;
                        break;
                    default:
        ...
        void Register()
        {
            DoCheckProc += AuraCheckProcFn(spell_mage_frost_armor_debuff_AuraScript::CheckProc);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_mage_frost_armor_debuff_AuraScript();
    }
};
    ...
    class spell_mage_hot_streak_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            if (auto caster = GetCaster())
            {
                ...
                if (roll_chance_i(GetSpellInfo()->Effects[EFFECT_0].BasePoints + t12bonus))
                    switch (eventInfo.GetSpellInfo()->Id)
                    {
    ...
    class spell_mage_arcane_missiles_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckCast()
        {
            if (auto caster = GetCaster()->ToPlayer())
            {
                ...
                {
                    if (caster->HasAura(44546)  //brain freeze rank 1 talent
                        || caster->HasAura(44548)  //brain freeze rank 2 talent
                        || caster->HasAura(44549)  //brain freeze rank 3 talent
                        || caster->HasAura(44445)  //hot streak rank 1 talent
                        || caster->HasAura(44446)  //hot streak rank 2 talent
                        || caster->HasAura(44448))  //hot streak rank 3 talent
                ...
                return SPELL_CAST_OK; // In case some npc uses same id?
        }

        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_mage_arcane_missiles_SpellScript::CheckCast);
        }
    };
...
void AddSC_mage_spell_scripts()
{
    ...
    new spell_mage_frost_armor_debuff();
    new spell_mage_t13_2p();
