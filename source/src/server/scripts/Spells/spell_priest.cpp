        ...
        class spell_pri_lightwell_renew_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                if (auto aura = GetAura())
                    if (auto dmg_info = eventInfo.GetDamageInfo())
                        if (auto target = dmg_info->GetVictim())
                            if (auto damage = dmg_info->GetDamage())
            ...
            void Register()
            {
                    DoCheckProc += AuraCheckProcFn(spell_pri_lightwell_renew_AuraScript::CheckProc);
    ...
    public:
        ...
        class spell_pri_power_word_shield_AuraScript : public AuraScript
        {
            ...
            void HandleEffectRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                // Rapture
                if (GetTargetApplication()->GetRemoveMode() != AURA_REMOVE_BY_ENEMY_SPELL && GetTargetApplication()->GetRemoveMode() != AURA_REMOVE_BY_ENEMY_SPELL)
                    return;
...
class spell_pri_shadow_word_death : public SpellScriptLoader
{
    public:
        ...
        class spell_pri_shadow_word_death_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnHit += SpellHitFn(spell_pri_shadow_word_death_SpellScript::HandleGlyph);
...
public:
    ...
    class spell_pri_pw_sanctuary_heal_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pri_pw_sanctuary_heal_AuraScript);
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pri_pw_sanctuary_heal_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
        }
    };
    ...
    AuraScript* GetAuraScript() const
    {
        return new spell_pri_pw_sanctuary_heal_AuraScript();
    }
};
    ...
    public:
        ...
        class spell_pri_friendly_dispel_SpellScript : public SpellScript
        {
            ...
            void HandleOnDispel()
            {
                ...
                if (auto c = GetCaster())
                if (auto h = GetHitUnit())
                if (AuraEffect* aura = c->GetAuraEffect(55677, EFFECT_0))
                {
                    int32 bp0 = int32(h->CountPctFromMaxHealth(aura->GetAmount()));
                    c->CastCustomSpell(h, 56131, &bp0, NULL, NULL, true);
                }
            }
...
//  Twin Disciplines 88625, 14914, 585, 15237, 23455, 2060
class spell_pri_twin_disiplines : public SpellScriptLoader
{
public:
    spell_pri_twin_disiplines() : SpellScriptLoader("spell_pri_twin_disiplines") { }

    class spell_pri_twin_disiplines_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pri_twin_disiplines_AuraScript);
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pri_twin_disiplines_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_pri_twin_disiplines_AuraScript();
    }
};

// 2096 Mind Vision
#define EVENT_CHECK_STEALTH 1
class spell_pri_mind_vision : public SpellScriptLoader
{
public:
    spell_pri_mind_vision() : SpellScriptLoader("spell_pri_mind_vision") { }

    class spell_pri_mind_vision_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pri_mind_vision_AuraScript);

        bool Load()
        {
            Unit* caster = GetCaster();
            if (caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->IsInRatedBattleground())
            {
                events.ScheduleEvent(EVENT_CHECK_STEALTH, 500);
                return true;
            }
            else
                return false;
        ...
        void OnUpdate(AuraEffect* /*aurEff*/, const uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_CHECK_STEALTH:
                    {
                        Unit* target = GetUnitOwner();
                        Unit* caster = GetCaster();

                        if (target->HasAura(2096, caster->GetGUID())) // Mind Vision cast my self
                            if (target->HasAura(1784) || target->HasAura(58984) || target->HasAura(5215) || target->HasAura(80325) || target->HasAura(32612) || target->HasAura(11392)) // Stealth, Shadowmeld, Prowl, CamoStealth, Invis, Invis potion
                                caster->CastStop();
        ...
        void Register()
        {
            OnEffectUpdate += AuraEffectUpdateFn(spell_pri_mind_vision_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_BIND_SIGHT);
            AfterEffectRemove += AuraEffectRemoveFn(spell_pri_mind_vision_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_BIND_SIGHT, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_pri_mind_vision_AuraScript();
    }
};

// Devouring Plague 63675
...
public:
    ...
    class spell_pri_improved_devouring_plague_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/)
        {
            if (!sSpellMgr->GetSpellInfo(PRIEST_IMPROVED_DEVOURING_PLAGUE_DAMAGE))
                return false;
            return true;
        }
...
public:
    ...
    class spell_pri_mind_control_AuraScript : public AuraScript
    {
        ...
        void Register()
        {
            OnEffectUpdate += AuraEffectUpdateFn(spell_pri_mind_control_AuraScript::OnUpdateDummy, EFFECT_1, SPELL_AURA_DUMMY);
            OnEffectUpdate += AuraEffectUpdateFn(spell_pri_mind_control_AuraScript::OnUpdate, EFFECT_2, SPELL_AURA_MOD_MELEE_HASTE);
            OnEffectRemove += AuraEffectRemoveFn(spell_pri_mind_control_AuraScript::OnRemove, EFFECT_2, SPELL_AURA_MOD_MELEE_HASTE, AURA_EFFECT_HANDLE_REAL);
        }
    };
    ...
    class spell_pri_mind_control_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            BeforeCast += SpellCastFn(spell_pri_mind_control_SpellScript::HandleBeforeCast);
        }
    };
...
void AddSC_priest_spell_scripts()
{
    ...
    new spell_pri_pw_sanctuary_heal();
    new spell_pri_fear();
    new spell_pri_friendly_dispel();
    new spell_pri_blessed_resilience();
    new spell_pri_chakra();
    new spell_priest_spirit_of_redemption();
    new spell_pri_holy_nova();
    new spell_pri_devouring_plague();
    new SpellScriptLoaderEx<spell_pri_shadow_orb_SpellScript>("spell_pri_shadow_orb");
    new spell_pri_temporal_boon();
    new spell_pri_mind_vision();
    new spell_pri_improved_devouring_plague();
    new spell_pri_twin_disiplines();
}
