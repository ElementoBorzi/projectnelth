    ...
    class spell_hun_mend_pet_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckState()
        {
            if (auto caster = GetCaster())
            {
                ...
                {
                    if (caster->GetDistance(pPet) <= 45.00f) // 45 yard range between pet and hunter
                    {
                        return SPELL_CAST_OK;
                    }
                    else
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_hun_mend_pet_SpellScript::CheckState);
        }
    };
    SpellScript* GetSpellScript() const
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            for (std::list<WorldObject*>::const_iterator itr = targets.begin(); itr != targets.end(); itr++)
            {
                ...
                if (exactDist < closestDistance 
                    ...
                    && !((*itr)->ToUnit() && (*itr)->ToUnit()->HasAura(31661)))
                {
...
class spell_warl_pet_felguard_legion_strike : public SpellScriptLoader
{
public:
    spell_warl_pet_felguard_legion_strike() : SpellScriptLoader("spell_warl_pet_felguard_legion_strike") { }

    class spell_warl_pet_felguard_legion_strike_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warl_pet_felguard_legion_strike_SpellScript);

        void CountTargets(std::list<WorldObject*>& targetList)
        {
            _targetCount = targetList.size();
            //TC_LOG_ERROR("sql.sql", "target count set to %u", _targetCount);
        }

        void ChangeDamage(SpellEffIndex /*effIndex*/)
        {
            //TC_LOG_ERROR("sql.sql", "ChangeDamage executing.");
            if (auto caster = GetCaster())
            if (auto target = GetHitUnit())
            if (_targetCount > 0)
            {
                int32 damage = GetHitDamage();
                //TC_LOG_ERROR("sql.sql", "damage before split: %u, target count: %u, new damage: %u", damage, _targetCount, (damage / _targetCount));
                damage /= _targetCount;
                SetHitDamage(damage);
            }
        }
    private:
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_warl_pet_felguard_legion_strike_SpellScript::CountTargets, EFFECT_1, TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(spell_warl_pet_felguard_legion_strike_SpellScript::ChangeDamage, EFFECT_1, SPELL_EFFECT_WEAPON_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_warl_pet_felguard_legion_strike_SpellScript();
    }
};
...
void AddSC_pet_spell_scripts()
{
    ...
    new spell_warl_pet_felguard_legion_strike();
}
