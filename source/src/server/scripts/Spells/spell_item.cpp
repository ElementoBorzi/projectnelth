        ...
        class spell_item_noggenfogger_elixir_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                if (auto caster = GetCaster())
                    caster->CastSpell(caster, decision(), true, NULL);
            }
            ...
            uint32 decision()
            {
                ...
                {
                    if
                        ((!caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED1) && !caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED2) && !caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED3)) //has none of the auras
                            ||
                            (caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED1) && caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED2) && caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED3))) //has all of the auras
                    {//pick a random aura
                    ...
                    {
                        if (!caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED3))
                            return SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED3;
                        if (!caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED2))
                            return SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED2;
                        if (!caster->HasAura(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED1))
                            return SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED1;
                    }
                    return SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED1;
                }
...
enum SPELL_ILLUSION_ELIXIR_EFFECTS
{
    SPELL_ILLUSION_ELIXIR_TRIGGERED1 = 80396, //scripted morph into nearby player
    SPELL_ILLUSION_ELIXIR_TRIGGERED2 = 94632, //pirate
    SPELL_ILLUSION_ELIXIR_TRIGGERED3 = 94514, //ninja
    SPELL_ILLUSION_CLONE = 45204
};

class spell_item_potion_of_illusion : public SpellScriptLoader
{//INSERT INTO `spell_script_names` (`spell_id`, `ScriptName`) VALUES ('80265', 'spell_item_potion_of_illusion'); -- Potion of illusion cast spell
public:
    ...
    class spell_item_potion_of_illusion_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* /*spellEntry*/)
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_ILLUSION_ELIXIR_TRIGGERED1)
                || !sSpellMgr->GetSpellInfo(SPELL_ILLUSION_ELIXIR_TRIGGERED2)
                || !sSpellMgr->GetSpellInfo(SPELL_ILLUSION_ELIXIR_TRIGGERED3))
                return false;
            return true;
        }

        Unit* NearbyMorphTarget()
        {
            //list of unfriendly players within 10 yards.
            std::list<Unit*> targets;
            Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(GetCaster(), GetCaster(), 5.0f);
            Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(GetCaster(), targets, u_check);
            GetCaster()->VisitNearbyObject(5.0f, searcher);

            //list of friendly players within 10 yards.
            std::list<Unit*> targets2;
            Trinity::AnyFriendlyUnitInObjectRangeCheck u_check2(GetCaster(), GetCaster(), 5.0f);
            Trinity::UnitListSearcher<Trinity::AnyFriendlyUnitInObjectRangeCheck> searcher2(GetCaster(), targets2, u_check2);
            GetCaster()->VisitNearbyObject(5.0f, searcher2);
            ...
            for (auto itr = targets.begin(); itr != targets.end();)
            {
                ...
                itr++;
                if (temp != NULL)
                    if ((temp)->ToPlayer() && (temp->GetGUID() != GetCaster()->GetGUID()))
                        return temp;//a nonfriendly player is nearby
            }
            ...
            for (auto itr = targets2.begin(); itr != targets2.end();)
            {
                ...
                if (temp != NULL)
                    if ((temp)->ToPlayer() && (temp->GetGUID() != GetCaster()->GetGUID()))
                        return temp;//a friendly player is nearby
            }
            return NULL;//even though units were nearby, none were viable player targets to copy as a morph
        }
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (auto caster = GetCaster())
            {
                caster->RemoveAurasDueToSpell(SPELL_ILLUSION_ELIXIR_TRIGGERED1);
                caster->RemoveAurasDueToSpell(SPELL_ILLUSION_ELIXIR_TRIGGERED2);
                caster->RemoveAurasDueToSpell(SPELL_ILLUSION_ELIXIR_TRIGGERED3);

                if (auto spell = decision())
                {
                    if (spell == SPELL_ILLUSION_ELIXIR_TRIGGERED1)
                    {
                        if (auto clone_target = NearbyMorphTarget())
                            clone_target->CastSpell(caster, spell, true, NULL);
                    }
                    else
                        caster->CastSpell(caster, spell, true, NULL);
                }
            }
        }
        ...
        uint32 decision()
        {
            ...
            {
                uint32 spellId;
                if (NearbyMorphTarget())
                {
                    //TC_LOG_ERROR("sql.sql", "morph target nearby, GUID: %u", NearbyMorphTarget()->GetGUID());
                    spellId = SPELL_ILLUSION_ELIXIR_TRIGGERED1;
                    switch (urand(1, 3))
                    {
                    case 1: spellId = SPELL_ILLUSION_ELIXIR_TRIGGERED3; break;
                    case 2: spellId = SPELL_ILLUSION_ELIXIR_TRIGGERED2; break;
                    }
                    return spellId;
                }
                else
                {
                    //TC_LOG_ERROR("sql.sql", "no morph target.");
                    spellId = SPELL_ILLUSION_ELIXIR_TRIGGERED2;
                    switch (urand(1, 2))
                    {
                    case 1: spellId = SPELL_ILLUSION_ELIXIR_TRIGGERED2; break;
                    case 2: spellId = SPELL_ILLUSION_ELIXIR_TRIGGERED3; break;
                    }
                    return spellId;
                }
            }
            return SPELL_ILLUSION_ELIXIR_TRIGGERED3;
        }

        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_item_potion_of_illusion_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
class spell_item_illusion_player_morph : public SpellScriptLoader
{
public:
    spell_item_illusion_player_morph() : SpellScriptLoader("spell_item_illusion_player_morph") { }
    class spell_item_illusion_player_morph_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_illusion_player_morph_AuraScript);

        Unit* NearbyMorphTarget()
        {
            //list of unfriendly players within 5 yards.
            std::list<Unit*> targets;
            Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(GetCaster(), GetCaster(), 5.0f);
            Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(GetCaster(), targets, u_check);
            GetCaster()->VisitNearbyObject(5.0f, searcher);

            //list of friendly players within 5 yards.
            std::list<Unit*> targets2;
            Trinity::AnyFriendlyUnitInObjectRangeCheck u_check2(GetCaster(), GetCaster(), 5.0f);
            Trinity::UnitListSearcher<Trinity::AnyFriendlyUnitInObjectRangeCheck> searcher2(GetCaster(), targets2, u_check2);
            GetCaster()->VisitNearbyObject(5.0f, searcher2);
        ...
        void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            /*
            TC_LOG_ERROR("sql.sql", "player morph: started");
            if (auto caster = GetCaster())
            if (auto target = NearbyMorphTarget())
            {
                    target->CastSpell(caster, SPELL_ILLUSION_CLONE, true);
                    TC_LOG_ERROR("sql.sql", "player morph: morph target nearby, GUID: %u", NearbyMorphTarget()->GetGUID());
            }
            else
            {
                    TC_LOG_ERROR("sql.sql", "player morph: no target nearby");
            }
            */
        }
        ...
        void HandleRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (auto caster = GetCaster())
            {
                TC_LOG_ERROR("sql.sql", "player morph: player demorphing.");
        ...
        void Register()
        {
            OnEffectApply += AuraEffectRemoveFn(spell_item_illusion_player_morph_AuraScript::HandleApply, EFFECT_0, SPELL_AURA_CLONE_CASTER, AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(spell_item_illusion_player_morph_AuraScript::HandleRemove, EFFECT_0, SPELL_AURA_CLONE_CASTER, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_item_illusion_player_morph_AuraScript();
    }
};
    ...
    public:
        ...
        class spell_item_demon_broiled_surprise_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckRequirement()
            {
                ...
                if (auto creature = player->FindNearestCreature(NPC_ABYSSAL_FLAMEBRINGER, 10, false))
                    if (creature->isDead())
                        return SPELL_CAST_OK;
    ...
    public:
        ...
        class spell_item_loom_of_fate_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                ...
                if (auto caster = eventInfo.GetActionTarget())
            ...
            if (auto caster = eventInfo.GetActor())
            {
                if (auto target = eventInfo.GetActionTarget())
                {
                    ...
                    case 30455: //ice lance
                        if (Aura* fingersOfFrost = caster->GetAura(44544, caster->GetGUID()))
...
public:
    ...
    class spell_item_indomitable_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (auto actionTarget = eventInfo.GetActionTarget())
            {
                if (!eventInfo.GetDamageInfo())
                    return false;
...
public:
    ...
    class spell_item_planting_dynamite_bundle_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            //    if (auto go = GetCaster()->SummonGameObject(GOB_DYNAMITE_BUNDLE, dynamitePosition[i].GetPositionX(), dynamitePosition[i].GetPositionY(), dynamitePosition[i].GetPositionZ(), dynamitePosition[i].GetOrientation(), 0, 0, 0, 0, 1000))
            //        go->Delete(20000);
        }
...
class spell_item_ds_spine_stacking_trinkets : public SpellScriptLoader
{
public:
    spell_item_ds_spine_stacking_trinkets() : SpellScriptLoader("spell_item_ds_spine_stacking_trinkets") { }

    class spell_item_ds_spine_stacking_trinkets_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_ds_spine_stacking_trinkets_AuraScript);

        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if ((eventInfo.GetHitMask() & PROC_EX_INTERNAL_DOT) || (eventInfo.GetHitMask() & PROC_EX_INTERNAL_HOT))
            {               
                return false;
            }
            return true;
        ...
        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(spell_item_ds_spine_stacking_trinkets_AuraScript::CheckProc);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_item_ds_spine_stacking_trinkets_AuraScript();
    }
};

class spell_item_tabard_of_Old_Gods : public SpellScriptLoader
{
public:
    spell_item_tabard_of_Old_Gods() : SpellScriptLoader("spell_item_tabard_of_Old_Gods") { }
    class spell_item_tabard_of_Old_Gods_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_tabard_of_Old_Gods_AuraScript);
        ...
        void HandleApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            if (auto c = GetCaster())
                if (auto o = GetOwner())
                    if (c->GetGUID() == o->GetGUID())
                        if (auto oP = o->ToPlayer())
                               if (oP->HasItemOrGemWithIdEquipped(100006, 1) || oP->HasItemOrGemWithIdEquipped(100007, 1))
        ...
        void Register()
        {
            OnEffectApply += AuraEffectRemoveFn(spell_item_tabard_of_Old_Gods_AuraScript::HandleApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        }
    };
    ...
    AuraScript* GetAuraScript() const
    {
        return new spell_item_tabard_of_Old_Gods_AuraScript();
    }
};
...
{
    new spell_item_tabard_of_Old_Gods();
    new spell_item_ds_spine_stacking_trinkets();
    new spell_item_potion_of_illusion();
    new spell_item_illusion_player_morph();
    ...
    new spell_item_book_of_discovery("spell_item_book_of_glyph_mastery");
    new spell_item_book_of_discovery("spell_item_tome_of_discovery");
    ...
    new spell_item_food("Food_60_2", FOOD_AMOUNT_60, EFFECT_2);
    new spell_item_food("Food_90_1", FOOD_AMOUNT_90, EFFECT_1);
    new spell_item_food("Food_90_2", FOOD_AMOUNT_90, EFFECT_2);
    new spell_item_food_raid("Food_Raid_60", FOOD_AMOUNT_60);
    new spell_item_food_raid("Food_Raid_90", FOOD_AMOUNT_90);
    new spell_item_jarre_of_ancient_remede();
    new spell_item_bird_shot();
    new spell_item_tholo_thermometer();

    new spell_item_loom_of_fate();
    new spell_item_lightning_capacitor();
    new spell_item_stay_of_execution();
    new spell_item_matrix_restabilizer();
    new spell_wrath_of_tarecgosa();
    new spell_nick_of_time();
    new spell_item_indomitable();
    new spell_item_cleansing_flames();
    new spell_item_lightning_strike();
    new spell_item_faded_wizard_hat();
    // 57185 - Demoniac Vessel
    new spell_item_break_marls_trance();
    // 56018 - Dynamite Bundle
    new spell_item_planting_dynamite_bundle();
}
