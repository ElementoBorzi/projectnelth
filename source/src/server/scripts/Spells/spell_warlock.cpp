...
enum WarlockSpells
{
    ...
    SPELL_WARLOCK_MOUNT_NIGHTMARE_EFFECT            = 31725,
    ...
    public:
        ...
        class spell_warl_unstable_affliction_AuraScript : public AuraScript
        {
            ...
            void HandleDispel(DispelInfo* dispelInfo)
            {
                ...
                if (auto caster = GetCaster())
                    if (auto d = dispelInfo->GetDispeller())
                    {
                        CustomSpellValues values;
                        int32 damage{ aurEff->GetAmount() };
                        values.AddSpellMod(SPELLVALUE_BASE_POINT0, aurEff->GetTotalTicks());
                        values.AddSpellMod(SPELLVALUE_BASE_POINT1, damage);
                        caster->CastCustomSpell(SPELL_WARLOCK_UNSTABLE_AFFLICTION_DISPEL, values, d, TRIGGERED_FULL_MASK, NULL, NULL, GetCasterGUID());
                    }
            }
...
class spell_warlock_UA_Dispel_Damage : public SpellScriptLoader // 31117
{
public:
    spell_warlock_UA_Dispel_Damage() : SpellScriptLoader("spell_warlock_UA_Dispel_Damage") { }

    class spell_warlock_UA_Dispel_DamageSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warlock_UA_Dispel_DamageSpellScript);

        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            //float ticks         { float(GetSpellValue()->EffectBasePoints[EFFECT_0])    };
            //float damage        { float(GetSpellValue()->EffectBasePoints[EFFECT_1])    };
            //int32 final_damage  { int32(damage * ticks * 1.5f)                          };

            float damage{ float(GetSpellValue()->EffectBasePoints[EFFECT_1]) };
            int32 final_damage{ int32(damage * 9.f) };
            SetHitDamage(final_damage);
        }

        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_warlock_UA_Dispel_DamageSpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_warlock_UA_Dispel_DamageSpellScript();
    }
};
...
class spell_hand_of_guldan : public SpellScriptLoader
{
public:
    spell_hand_of_guldan() : SpellScriptLoader("spell_hand_of_guldan") { }

    class spell_hand_of_guldan_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_hand_of_guldan_SpellScript);
                ...
                if (auto o = c->GetOwner())
                    if (targets.size())
                        for (auto itr = targets.begin(); itr != targets.end();)
                            if (auto t = (*itr))
                                if (!c->IsValidAttackTarget(o->ToUnit()))
                                {
                                    targets.erase(itr++);
                                }
                                else
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_hand_of_guldan_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY); //TARGET_UNIT_DEST_AREA_ENEMY
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_hand_of_guldan_SpellScript();
    }
};

//
class spell_warl_mount : public SpellScriptLoader
{
public:
    spell_warl_mount() : SpellScriptLoader("spell_warl_mount") { }

    class spell_warl_mount_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_warl_mount_AuraScript);

        void OnMount(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            if (auto c = GetCaster())
                    c->CastSpell(c, SPELL_WARLOCK_MOUNT_NIGHTMARE_EFFECT, true);
        }

        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_warl_mount_AuraScript::OnMount, EFFECT_0, SPELL_AURA_MOUNTED, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_warl_mount_AuraScript();
    }
};
...
void AddSC_warlock_spell_scripts()
{
    ...
    new spell_warlock_UA_Dispel_Damage();
    ...
    new spell_hand_of_guldan();
    new spell_warl_mount();
}
