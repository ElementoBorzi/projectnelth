...
enum ShamanSpells
{
    ...
    SPELL_SHAMAN_SEARING_BOLT                   = 3606,
    ...
    SPELL_SHAMAN_ELEMENTAL_OVERLOAD             = 77222,
    SPELL_SHAMAN_ELEMENTAL_T13_4P               = 105816,
    SPELL_SHAMAN_ELEMENTAL_T13_4P_PROC          = 105821,
        ...
        class spell_sha_fire_nova_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (auto target = GetHitUnit())
                {
                    ...
                    {
                        caster->CastCustomSpell(SPELL_SHAMAN_FIRE_NOVA_TRIGGERED_R1, SPELLVALUE_RADIUS_MOD, 20000, target);
                    }
                    else if (caster->HasAura(28999))
                    {
                        caster->CastCustomSpell(SPELL_SHAMAN_FIRE_NOVA_TRIGGERED_R1, SPELLVALUE_RADIUS_MOD, 15000, target);
                    }
                    else
                    {
...
class spell_sha_flame_shock : public SpellScriptLoader
{
    public:
        ...
        class spell_sha_flame_shock_AuraScript : public AuraScript
        {
            ...
            {
                //TC_LOG_ERROR("sql.sql", "Aura Updating");
                Unit* caster = GetCaster();
                if (!caster)
                    return;
                std::list<Creature*> MinionList;
                caster->GetAllMinionsByEntry(MinionList, 2523);
                if (!MinionList.size())
                    return;
                        ...
                        if (float searing_distance = GetSearingDistance())
                            if (auto target = GetOwner()->ToUnit())
                                if (searing_totem->isAlive())
                                    ...
                                    {
                                        if (searing_totem->getVictim())
                                        {
                                            if (searing_totem->getVictim() == target)
                                                return;
                                            if (!searing_totem->getVictim()->HasAura(SPELL_SHAMAN_FLAME_SHOCK) && !searing_totem->getVictim()->HasAura(SPELL_SHAMAN_STORMSTRIKE))
                                            {
                                                searing_totem->AI()->DoCast(target, SPELL_SHAMAN_SEARING_BOLT, false);
                                                searing_totem->AI()->AttackStart(target);
                                            }
                                        }
                                        else
                                        {
                    ...
                    if (float searing_distance = GetSearingDistance())
                        if (auto target = GetOwner()->ToUnit())
                            if ((*itr)->isAlive())
                                ...
                                {
                                    (*itr)->AI()->DoCast(target, SPELL_SHAMAN_SEARING_BOLT, false);
                                    (*itr)->AI()->AttackStartCaster(target, searing_distance);
                                }
                }
            }
            ...
            void Register()
            {
                ...
                OnEffectUpdate += AuraEffectUpdateFn(spell_sha_flame_shock_AuraScript::OnUpdate, EFFECT_1, SPELL_AURA_PERIODIC_DAMAGE);
                OnEffectApply += AuraEffectApplyFn(spell_sha_flame_shock_AuraScript::Apply, EFFECT_1, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            }
        };
...
class spell_sha_storm_strike : public SpellScriptLoader
{
public:
    spell_sha_storm_strike() : SpellScriptLoader("spell_sha_storm_strike") { }

    class spell_sha_storm_strike_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_sha_storm_strike_AuraScript);

        bool Validate(SpellInfo const* /*spell*/)
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_SHAMAN_STORMSTRIKE))
                return false;
            return true;
        }
        ...
        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_sha_storm_strike_AuraScript::Apply, EFFECT_0, SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            OnEffectUpdate += AuraEffectUpdateFn(spell_sha_storm_strike_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_sha_storm_strike_AuraScript();
    }
};
                    ...
                    if (auto caster = GetCaster())
                    {
                        if (auto owner = caster->GetOwner())
                        {
                            ...
                            if (AuraEffect* purify = owner->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_SHAMAN, ICON_ID_PURIFICATION, 0))
                                AddPct(bonus, purify->GetAmount());

                            //Tidal surge
                            if (AuraEffect* tidal_surge = owner->GetAuraEffect(76155, EFFECT_1))
                                AddPct(bonus, tidal_surge->GetAmount());
    ...
    public:
        ...
        class spell_sha_spirit_link_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                ...
                if (auto link_totem = GetCaster())
                if (auto link_shaman = link_totem->GetOwner())
                    if (auto p = link_shaman->ToPlayer())
                        {
                            /*
                                Remove a target if they have a hostile smoke bomb which the casting totem does not.
                            */
                            if (unitList.size())
                                for (auto itr = unitList.begin(); itr != unitList.end(); itr++)
                                    if (auto this_itr = (*itr))
                                    {
                                        ...
                                        {
                                            auto auras_applied = target->GetAppliedAuras();
                                            if (auras_applied.size())
                                                for (auto applied_itr : auras_applied)
                                                    if (auto a = applied_itr.second)
                                                        if (auto aura = a->GetBase())
                                                            if (aura->GetId() == 88611)
                                                                if (auto c = aura->GetCaster())
                                                                    if (link_shaman->IsValidAttackTarget(c))
                                                                        valid_target = link_totem->HasAura(88611, c->GetGUID());
                            ...
                            /*
                                Remove a target if the totem has a smoke bomb that they do not.
                            */
                            auto auras_totem = link_totem->GetAppliedAuras();
                            if (auras_totem.size())
                                for (auto applied_itr : auras_totem)
                                    if (auto a = applied_itr.second)
                                        if (auto aura = a->GetBase())
                                            if (aura->GetId() == 88611)
                                                if (auto c = aura->GetCaster())
                                                    if (unitList.size())
                                                        for (auto itr = unitList.begin(); itr != unitList.end(); itr++)
                                                            if (auto this_itr = (*itr))
                                                            {
                                                                bool valid_target{ true };
                                                                if (auto target = this_itr->ToUnit())
                                                                    if (!target->HasAura(88611, c->GetGUID()))
                                                                        valid_target = false;
                            ...
                            //TC_LOG_ERROR("sql.sql", "spirit link processing onto final target collection.");
                            if (unitList.size())
                                for (auto itr = unitList.begin(); itr != unitList.end(); itr++)
                                    if (auto this_itr = (*itr))
                                        if (auto target = this_itr->ToUnit())
                                        {
                                            //TC_LOG_ERROR("sql.sql", "spirit link processing guid %u", target->GetGUID());
...
public:
    ...
    class spell_sha_lava_burst_SpellScript : public SpellScript
    {
        ...
        void HandleHit(SpellEffIndex /*effIndex*/)
        {
            if (unleashed)
            {
                if (!GetOriginalCaster()->HasAura(73683))
                {
                    int32 bonus = CalculatePct(GetHitDamage(), 16.6666667);
                    SetHitDamage(GetHitDamage() - bonus);
                }
            }
            else
            {
                if (GetOriginalCaster()->HasAura(73683))
                {
                    int32 bonus = CalculatePct(GetHitDamage(), 20.000);
                    SetHitDamage(GetHitDamage() + bonus);
                }
            }
        }
    ...
    private:
        ...
        void Register()
        {
            OnCast += SpellCastFn(spell_sha_lava_burst_SpellScript::CheckStacks);
            OnEffectHitTarget += SpellEffectFn(spell_sha_lava_burst_SpellScript::HandleHit, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
...
{
    /*
        INSERT INTO `spell_script_names` (`spell_id`, `ScriptName`) VALUES ('10444', 'spell_sha_flametongue_proc');
    */
public:
    spell_sha_flametongue_proc() : SpellScriptLoader("spell_sha_flametongue_proc") { }
    ...
    class spell_sha_flametongue_proc_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (auto caster = GetCaster())
                if (auto flametongue_player = caster->ToPlayer())
                {
                    ...
                    if (float numerator = float(caster->GetAttackTime(GetCastItem() == weapons[1] ? OFF_ATTACK : BASE_ATTACK)))
                        if (float denominator = 3600.00f)
                            if (int32 bonus = int32(float(GetHitDamage()) * ((denominator - numerator) / denominator)))
                                SetHitDamage(GetHitDamage() - bonus);
                }
        }
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_sha_flametongue_proc_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };
...
public:
    ...
    class spell_sha_elemental_overload_SpellScript : public SpellScript
    {
            ...
            if (auto caster = GetCaster())
                if (Aura* overload = caster->GetAura(SPELL_SHAMAN_ELEMENTAL_OVERLOAD, caster->GetGUID()))
                    if (Aura* overload_set = caster->GetAura(SPELL_SHAMAN_ELEMENTAL_T13_4P, caster->GetGUID()))
                        ...
                        {
                            //TC_LOG_ERROR("sql.sql", "setting elemental overload to 2");
                            overload->SetStackAmount(2);
                            caster->CastSpell(caster, SPELL_SHAMAN_ELEMENTAL_T13_4P_PROC, true);
                        }
        }
        ...
        SpellCastResult CheckCast()
        {
            if (auto caster = GetCaster())
                if (Aura* overload = caster->GetAura(SPELL_SHAMAN_ELEMENTAL_OVERLOAD, caster->GetGUID()))
                    if (overload->GetStackAmount() == 2)
                    {
                        //TC_LOG_ERROR("sql.sql", "cast aborted, overload is 2.");
                        return SPELL_FAILED_DONT_REPORT;
                    }
            return SPELL_CAST_OK;
        }
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_sha_elemental_overload_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
            OnCheckCast += SpellCheckCastFn(spell_sha_elemental_overload_SpellScript::CheckCast);
        }
    };
            ...
            if (GetSpellInfo()->Id == SPELL_SHAMAN_ELEMENTAL_OVERLOAD)
                if (auto caster = GetCaster())
                    if (Aura* overload = GetAura())
                        if (overload->GetStackAmount() == 2)
                        {
                            //TC_LOG_ERROR("sql.sql", "setting elemental overload to 1");
                            overload->SetStackAmount(1);
                        }
        }
        void Register()
        {
            OnEffectUpdate += AuraEffectUpdateFn(spell_sha_elemental_overload_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_DUMMY);           
        }   
    };
...
void AddSC_shaman_spell_scripts()
{
    ...
    new spell_sha_flametongue_proc();
    new spell_sha_storm_strike();
    ...
    new spell_sha_t12_taming_the_flames();
    new spell_sha_feedback();
    new spell_mealstrom_weapon_visual();
    new spell_sha_spiritwalkers_grace();
    new spell_sha_t13_2p_enhancement();
    new spell_sha_t13_2p_resto();
