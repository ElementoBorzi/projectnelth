...
enum DruidSpells
{
    ...
    SPELL_DRUID_SAVAGE_DEFENSE_PASSIVE      = 62600,
    SPELL_DRUID_SWIPE_EMOTE_ONLY            = 56705,
    SPELL_DRUID_GLYPH_OF_HURRICANE          = 54831,
    SPELL_DRUID_REND_AND_TEAR_1             = 48432,
    SPELL_DRUID_REND_AND_TEAR_2             = 48433,
    SPELL_DRUID_REND_AND_TEAR_3             = 48434,
        ...
        class spell_dru_eclipse_energize_SpellScript : public SpellScript
        {
            ...
            void HandleEnergize(SpellEffIndex /*effIndex*/)
            {
                ...
                switch (GetSpellInfo()->Id)
                {
                    ...
                    {
                        if (!caster->HasAura(SPELL_DRUID_LUNAR_ECLIPSE_MARKER) && !caster->HasAura(SPELL_DRUID_SOLAR_ECLIPSE_MARKER) && (GetSpellInfo()->Id == SPELL_DRUID_MOONFIRE))
                        {
                            caster->CastSpell(caster, SPELL_DRUID_LUNAR_ECLIPSE_MARKER, true);
                        ...
                        // trigger
                        if (GetSpellInfo()->Id == SPELL_DRUID_MOONFIRE)
                        {
                            if (
                                caster->GetPower(POWER_ECLIPSE) <= -92 && caster->HasAura(SPELL_DRUID_LUNAR_ECLIPSE_MARKER)
                                ||
                                caster->GetPower(POWER_ECLIPSE) >= 92 && caster->HasAura(SPELL_DRUID_SOLAR_ECLIPSE_MARKER)
                                )
                                caster->CastSpell(caster, 95746, true);
                        }
        ...
        class spell_dru_primal_madness_AuraScript : public AuraScript
        {
            ...
            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (auto c = GetCaster())
                {
                    ...
                    if (auto spell_info = sSpellMgr->GetSpellInfo(33878))
                    {
                        auto existing_cooldown = p->GetSpellCooldownDelay(33878) * 1000;
                        auto existing_gcd = p->GetGlobalCooldownMgr().GetGlobalCooldown(spell_info);
                        int32 mod = int32(existing_cooldown - existing_gcd);

                        p->ModifySpellCooldown(33878, -mod);
                    }

                    if (AuraEffect const* primalMadness = c->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_DRUID, 1181, EFFECT_1))
                    {
                        uint32 triggeredId = 0;
                        ...
                        if (auto trigger_info = sSpellMgr->GetSpellInfo(triggeredId))
                        {
                            c->CastSpell(c, triggeredId, true);
                            c->EnergizeBySpell(c, triggeredId, trigger_info->Effects[0].BasePoints, POWER_ENERGY);
                        }
                    }
                }
            }
            ...
            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (auto c = GetCaster())
                {
                    ...
                    {
                        if (auto trigger_info = madness->GetSpellInfo())
                        c->EnergizeBySpell(c, trigger_info->Id, -trigger_info->Effects[0].BasePoints, POWER_ENERGY);
                        madness->Remove(AURA_REMOVE_BY_DEFAULT);
                    }
                    else if (auto madness = c->GetAura(80886))
                    {
                        if (auto trigger_info = madness->GetSpellInfo())
                            c->EnergizeBySpell(c, trigger_info->Id, -trigger_info->Effects[0].BasePoints, POWER_ENERGY);
                        madness->Remove(AURA_REMOVE_BY_DEFAULT);
            ...
            void Register() override
            {
                ...
                {
                    AfterEffectApply += AuraEffectApplyFn(spell_dru_primal_madness_AuraScript::OnApply, EFFECT_0, SPELL_AURA_ADD_PCT_MODIFIER, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
                    AfterEffectRemove += AuraEffectRemoveFn(spell_dru_primal_madness_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_ADD_PCT_MODIFIER, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
                } else
                {
                    AfterEffectApply += AuraEffectApplyFn(spell_dru_primal_madness_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
                    AfterEffectRemove += AuraEffectRemoveFn(spell_dru_primal_madness_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
                }
            }
        };
    ...
    public:
        ...
        class spell_dru_survival_instincts_AuraScript : public AuraScript
        {
            ...
            void Register() override
            {
                ...
                OnEffectUpdate += AuraEffectUpdateFn(spell_dru_survival_instincts_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_DUMMY);
            }
        };
        ...
        class spell_dru_thorns_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
            {
                if (auto caster = GetCaster())
                {
                    ...
                    int32 amount2 = GetSpellInfo()->Effects[EFFECT_0].CalcValue(caster) + caster->SpellBaseDamageBonusDone(GetSpellInfo()->GetSchoolMask()) * coeff;
                    if (amount1 > amount2)
                        amount = amount1;
                    else
                        amount = amount2;
...
public:
    ...
    class spell_dru_mangle_SpellScript : public SpellScript
    {
                    ...
                    if (auto caster = GetCaster())
                        if (caster->HasAura(105725))
                            if (caster->HasAura(80951))
                                ...
                                {
                                    if (caster->HandleProcTriggerSpell(GetHitUnit(), 0, 0, 0, 0, 0, caster->GetAuraEffect(62600, EFFECT_0, caster->GetGUID()), sSpellMgr->GetSpellInfo(33878), MELEE_PROC_FLAG_MASK, PROC_EX_EX_TRIGGER_ALWAYS, 0))
                                    {
                                        //TC_LOG_ERROR("sql.sql", "proc success");
                                    }
                                }                          
        }
        ...
        void Register()
        {
            ...
            AfterHit += SpellHitFn(spell_dru_mangle_SpellScript::MangleHit);
        }
    };
...
class spell_dru_solar_eclipse : public SpellScriptLoader
{
public:
    spell_dru_solar_eclipse() : SpellScriptLoader("spell_dru_solar_eclipse") { }

    class spell_dru_solar_eclipse_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dru_solar_eclipse_AuraScript);
        ...
        void Register() override
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_dru_solar_eclipse_AuraScript::RemoveEffect, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_dru_solar_eclipse_AuraScript();
    }
};
...
class spell_dru_pulverize_t13 : public SpellScriptLoader
{
public:
    spell_dru_pulverize_t13() : SpellScriptLoader("spell_dru_t13_2p_bear") { }

    class spell_dru_pulverize_t13_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dru_pulverize_t13_AuraScript);
            ...
            if (eventInfo.GetSpellInfo()->Id == 33878)
                if (auto caster = GetCaster())
                    if (AuraEffect* druid_t13_feral_2p = caster->GetAuraEffect(105725, EFFECT_0, caster->GetGUID()))
                        ...
                        return true;
            /*
                This is just for cleanliness. We do not want a "double" proc to occur. it is being forced somewhere else.
            */
        }
        ...
        void Register()
        {
            DoCheckProc += AuraCheckProcFn(spell_dru_pulverize_t13_AuraScript::CheckProc);
            OnEffectProc += AuraEffectProcFn(spell_dru_pulverize_t13_AuraScript::HandleEffectProc, EFFECT_0, SPELL_AURA_DUMMY);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_dru_pulverize_t13_AuraScript();
    }
};


class spell_dru_fury_swipes : public SpellScriptLoader
{
public:
    spell_dru_fury_swipes() : SpellScriptLoader("spell_dru_fury_swipes") { }

    class spell_dru_fury_swipes_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dru_fury_swipes_SpellScript);

        void PlayEmote()
        {
            if (auto c = GetCaster())
                c->CastWithDelay(1000, c, SPELL_DRUID_SWIPE_EMOTE_ONLY, true);
        }

        void Register()
        {
            AfterCast += SpellCastFn(spell_dru_fury_swipes_SpellScript::PlayEmote);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_dru_fury_swipes_SpellScript();
    }
};
...
public:
    ...
    class spell_dru_hurricane_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
        {
            // do not set speed if not in cat form
            if (auto c = GetCaster())
                if (auto glyph = c->GetAura(SPELL_DRUID_GLYPH_OF_HURRICANE))
                if (auto eff = glyph->GetEffect(EFFECT_0))
...
public:
    ...
    class spell_dru_maul_SpellScript : public SpellScript
    {
        ...
        void HandleOnHit()
        {
            //TC_LOG_ERROR("sql.sql", "maul casted");
            /*            
                SPELL_DRUID_REND_AND_TEAR_1             = 48432,
                SPELL_DRUID_REND_AND_TEAR_2             = 48433,
                SPELL_DRUID_REND_AND_TEAR_3             = 48434,
            */
            ...
            if (auto c = GetCaster())
                if (auto hit = GetHitUnit())
                    if (hit->HasAuraWithMechanic(1 << MECHANIC_BLEED))
                    {
                        //TC_LOG_ERROR("sql.sql", "druid target has bleed.");

                        if (auto rend_and_tear = c->GetAura(SPELL_DRUID_REND_AND_TEAR_3))
                            if (auto eff = rend_and_tear->GetEffect(EFFECT_0))
                            {
                                auto dmg{ GetHitDamage() };
                                //TC_LOG_ERROR("sql.sql", "damage before buff: %u", dmg);

                                SetHitDamage(AddPct(dmg, eff->GetAmount()));
                                //TC_LOG_ERROR("sql.sql", "damage after buff: %u", GetHitDamage());
                                return;
                            }
                        if (auto rend_and_tear = c->GetAura(SPELL_DRUID_REND_AND_TEAR_2))
                            if (auto eff = rend_and_tear->GetEffect(EFFECT_0))
                            {
                                auto dmg{ GetHitDamage() };
                                //TC_LOG_ERROR("sql.sql", "damage before buff: %u", dmg);
                        ...
                        if (auto rend_and_tear = c->GetAura(SPELL_DRUID_REND_AND_TEAR_1))
                            if (auto eff = rend_and_tear->GetEffect(EFFECT_0))
                            {
                                auto dmg{ GetHitDamage() };
                                //TC_LOG_ERROR("sql.sql", "damage before buff: %u", dmg);
            ...
            /*
                    else TC_LOG_ERROR("sql.sql", "victim does not have bleed.");
                else TC_LOG_ERROR("sql.sql", "victim not found.");
            else TC_LOG_ERROR("sql.sql", "caster not found.");
            */
        }
...
void AddSC_druid_spell_scripts()
{
    ...
    new spell_dru_faerie_fire_damage();
    new spell_dru_thorns();
    new spell_dru_lacerate();
    new spell_dru_mangle();
    new spell_dru_glyph_healing_touch();
    new spell_dru_regrowth();
    new spell_dru_solar_eclipse();
    new spell_dru_t12_helper();
    new spell_dru_blood_in_the_water();
    new spell_dru_fury_swipes();
}
