...
enum PaladinSpells
{
    ...
    SPELL_PALADIN_HOLY_RADIANCE_DIRECT           = 82327,
    SPELL_PALADIN_HOLY_RADIANCE_OVERTIME         = 86452,
    ...
    class spell_pal_ancient_fury_SpellScript : public SpellScript
    {
        ...
        void ChangeDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            if (caster && target && _targetCount)
            {
                ...
                if (Aura* aura = caster->GetAura(86700, caster->GetGUID()))
                {
                    uint8 stacks = aura->GetStackAmount();

                    if (stacks >= 1)
                        damage = stacks * (damage + 0.1f * caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY));
                }
    ...
    public:
        ...
        class spell_pal_templar_s_verdict_SpellScript : public SpellScript
        {
            ...
            void ChangeDamage(SpellEffIndex /*effIndex*/)
            {
                ...
                SetHitDamage(damage);
                divine = false;//This will ensure that we can never accidentally add a second stack of divine purpose in case it procs on a dodge (if that's even possible).
            }
            ...
            void HandleBeforeCast()
            {
                if (auto caster = GetCaster())
                    ...
                    {
                        divine      = true;
                        duration    = DivinePurpose->GetDuration();
                        durationMAX = DivinePurpose->GetMaxDuration();
                    }
            }
            ...
            void HandleAfterCast()
            {
                if (divine)
                    if (auto target = GetExplTargetUnit())
                        ...
                        {
                            auto info = spell->GetTargetInfo();
                            if (info.size())
                                for (auto ihit = info.begin(); ihit != info.end(); ++ihit)
                ...
                /*
                * Cryptid:
                * https://youtu.be/gUOAVqjvEvQ?t=346
                * 
                * Divine purpose should not be removed on parries/dodges.
                * As an easy route, we're doing a slight workaround where we save and re-add the aura immediately after.
                * Preventing the consumption is doable, but on a first attempt I found that it was creating a situation where you had divine purpose active but was getting the "no holy power available" message.
                */

                if (replenish_needed)
                    if (auto caster = GetCaster())
                        if (!caster->HasAura(SPELL_PALADIN_DIVINE_PURPOSE_PROC))
                            if (auto DivinePurpose = caster->AddAura(SPELL_PALADIN_DIVINE_PURPOSE_PROC, caster))
                            {
                                DivinePurpose->SetMaxDuration(durationMAX);
                                DivinePurpose->SetDuration(duration);
                            }
            }
...
// 53600 - Shield of the Righteous
 ...
 class spell_spell_paladin_conviction : public SpellScriptLoader
 {
     /*
     REPLACE INTO `spell_script_names` VALUES (20057, 'spell_spell_paladin_conviction');
     */
 public:
     spell_spell_paladin_conviction() : SpellScriptLoader("spell_spell_paladin_conviction") { }

     class spell_spell_paladin_conviction_AuraScript : public AuraScript
     {
         PrepareAuraScript(spell_spell_paladin_conviction_AuraScript);

         bool CheckProc(ProcEventInfo& eventInfo)
         {
             if (eventInfo.GetSpellInfo())
             if (eventInfo.GetSpellInfo()->Id == SPELL_PALADIN_HOLY_RADIANCE_OVERTIME)//hot streak rank 3 talent
                 return false;
             return true;
         }
         ...
         void Register()
         {
             DoCheckProc += AuraCheckProcFn(spell_spell_paladin_conviction_AuraScript::CheckProc);
         }
     };

     AuraScript* GetAuraScript() const
     {
         return new spell_spell_paladin_conviction_AuraScript();
     }
 };

 // Hackfix to make judgement usable on levelup
 class levelupSeal : public PlayerScript
 {
 public:
     levelupSeal() : PlayerScript("levelupSeal") {}

     void OnLevelChanged(Player* player, uint8 /*newLevel*/)
     {
         if (player->getClass() == CLASS_PALADIN)
         {
             if (player->HasAura(20165)) // Seal of Insight
             {
                 player->RemoveAurasDueToSpell(20165);
                 player->CastSpell(player, 20165, true);
             }

             if (player->HasAura(20164)) // Seal of Justice
             {
                 player->RemoveAurasDueToSpell(20164);
                 player->CastSpell(player, 20164, true);
             }

             if (player->HasAura(20154)) // Seal of Righteousness
             {
                 player->RemoveAurasDueToSpell(20154);
                 player->CastSpell(player, 20154, true);
             }

             if (player->HasAura(31801)) // Seal of Truth
             {
                 player->RemoveAurasDueToSpell(31801);
                 player->CastSpell(player, 31801, true);
             }
         }
     }
 ...
 {
     /*
         REPLACE INTO `spell_script_names` VALUES (85696, 'spell_pal_zealotry');
     */
 public:
     ...
     class spell_pal_zealotry_SpellScript : public SpellScript
     {
         ...
         void Register()
         {
             BeforeCast += SpellCastFn(spell_pal_zealotry_SpellScript::HandleBeforeCast);
 ...
 class spell_spell_paladin_protector_of_the_innocent : public SpellScriptLoader
 {
     /*
     REPLACE INTO `spell_script_names` VALUES (20138, 'spell_spell_paladin_protector_of_the_innocent');
     REPLACE INTO `spell_script_names` VALUES (20139, 'spell_spell_paladin_protector_of_the_innocent');
     REPLACE INTO `spell_script_names` VALUES (20140, 'spell_spell_paladin_protector_of_the_innocent');
     */
 public:
     spell_spell_paladin_protector_of_the_innocent() : SpellScriptLoader("spell_spell_paladin_protector_of_the_innocent") { }

     class spell_spell_paladin_protector_of_the_innocent_AuraScript : public AuraScript
     {
         PrepareAuraScript(spell_spell_paladin_protector_of_the_innocent_AuraScript);

         bool CheckProc(ProcEventInfo& eventInfo)
         {
             if (eventInfo.GetSpellInfo())
                 if (eventInfo.GetSpellInfo()->Id == SPELL_PALADIN_BEACON_OF_LIGHT_HEAL)//beacon of light
                     return false;
             return true;
         }
         ...
         void Register()
         {
             DoCheckProc += AuraCheckProcFn(spell_spell_paladin_protector_of_the_innocent_AuraScript::CheckProc);
         }
     };

     AuraScript* GetAuraScript() const
     {
         return new spell_spell_paladin_protector_of_the_innocent_AuraScript();
     }
 };

 class spell_pal_censure_dot : public SpellScriptLoader
 {
 public:
     spell_pal_censure_dot() : SpellScriptLoader("spell_pal_censure_dot") { }

     class spell_pal_censure_dot_AuraScript : public AuraScript
     {
         PrepareAuraScript(spell_pal_censure_dot_AuraScript);
         ...
         void Register()
         {
             OnEffectPeriodic += AuraEffectPeriodicFn(spell_pal_censure_dot_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
         }
     };

     AuraScript* GetAuraScript() const
     {
         return new spell_pal_censure_dot_AuraScript();
     }
 };

void AddSC_paladin_spell_scripts()
{
    new spell_spell_paladin_protector_of_the_innocent();
    new spell_pal_zealotry();
    new spell_spell_paladin_conviction();
    ...
    new spell_pal_seals_duration();
    new spell_pal_aura_mastery();
    new spell_pal_divine_protection();
    new spell_pal_t13_2p_protection();
    new AuraScriptLoaderEx<spell_pal_ancient_healer_procAuraScript>("spell_pal_ancient_healer_proc");
    new levelupSeal();
    new spell_pal_censure_dot();
}
