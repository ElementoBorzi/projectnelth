...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (sWorld->getBoolConfig(CONFIG_SOLO_QUEUE_DPS_RESTRICTION))
        {
            ...
            infoQueue << "Queued dps: " << cache3v3Queue[TALENT_CAT_DPS] << "\n";
        ...
        if (sSoloQueueMgr->IsPlayerInSoloQueue(player))
            player->ADD_GOSSIP_ITEM_EXTENDED(-1, "|TInterface\\icons\\inv_misc_enggizmos_27:20:20:-10:0|tLeave Solo queue", GOSSIP_SENDER_MAIN, 3, "Are you sure you want to remove the solo queue?", 0, false);

        if (player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == NULL)
            player->ADD_GOSSIP_ITEM(-1, "|TInterface\\icons\\inv_scroll_16:20:20:-10:0|tCreate new Solo arena team", GOSSIP_SENDER_MAIN, 1);
        else
        {
            if (ArenaTeam* at = sArenaTeamMgr->GetArenaTeamById(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5))))
            {
                if (at->IsSoloQueueTeam())
                {
                    ...
                    {
                        player->ADD_GOSSIP_ITEM(-1, "|TInterface\\icons\\Achievement_bg_killxenemies_generalsroom:20:20:-10:0|tQueue up for 1vs1 Skirmish", GOSSIP_SENDER_MAIN, 20);
                        player->ADD_GOSSIP_ITEM(-1, "|TInterface\\icons\\achievement_guildperk_everybodysfriend:20:20:-10:0|tQueue up for 3vs3 Arena", GOSSIP_SENDER_MAIN, 2);
                    }
                }
                else
                    player->ADD_GOSSIP_ITEM(-1, "|TInterface\\icons\\Spell_shadow_sacrificialshield:20:20:-10:0|tYou are in a 5vs5 team!", GOSSIP_SENDER_MAIN, 21);
            }
        }

        player->ADD_GOSSIP_ITEM(-1, infoQueue.str().c_str(), GOSSIP_SENDER_MAIN, 0);
        player->SEND_GOSSIP_MENU(60015, creature->GetGUID());
        return true;
    }
            ...
            case 20:
            {
                if (sSoloQueueMgr->CheckRequirements(player))
                    if (player->HasHealSpec())
                        ChatHandler(player->GetSession()).SendSysMessage("Can't join 1vs1 Skirmish as a Healer. Please pick a Dps spec.");
                    else
                        Join1v1Queue(player);
                break;
            }
    ...
    void fetchQueueList()
    {
        ...
        if (sWorld->getBoolConfig(CONFIG_SOLO_QUEUE_DPS_RESTRICTION))
        {
            ...
            cache3v3Queue[TALENT_CAT_DPS] = sSoloQueueMgr->GetPlayerCountInQueue(TALENT_CAT_DPS);
