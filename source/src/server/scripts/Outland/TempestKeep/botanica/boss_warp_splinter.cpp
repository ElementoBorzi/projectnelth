        ...
        struct mob_warp_splinter_treantAI  : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if (!UpdateVictim())
                {
                    ...
                    {
                        if (auto Warp = Unit::GetUnit(*me, WarpGuid))
                        {
        ...
        struct boss_warp_splinterAI : public BossAI
        {
            ...
            void SummonTreants()
            {
                for (uint8 i = 0; i < 6; ++i)
                {
                    ...
                    if (auto pTreant = me->SummonCreature(CREATURE_TREANT, treant_pos[i][0], treant_pos[i][1], treant_pos[i][2], O, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 25000))
