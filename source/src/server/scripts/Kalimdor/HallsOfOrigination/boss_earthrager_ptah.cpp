...
public:
    ...
    struct boss_earthrager_ptahAI : public BossAI
    {
        ...
        void Cleanup()
        {
            ...
            GetCreatureListWithEntryInGrid(units, me, NPC_SAND_VORTEX, 100.0f);
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage)
        {
            if (me->HealthBelowPctDamaged(50, damage) && events.IsInPhase(PHASE_NORMAL) && !_hasDispersed && !me->HasUnitState(UNIT_STATE_CASTING))
            {
                ...
                events.ScheduleEvent(EVENT_PTAH_EXPLODE, 1, 0, PHASE_DISPERSE);
                ...
                std::list<Creature*> units;
                GetCreatureListWithEntryInGrid(units, me, NPC_DUSTBONE_HORROR, 200.0f);
                for (auto itr = units.begin(); itr != units.end(); ++itr)
                    if (auto npc = (*itr))
                    {
                        instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, npc);
                    }
            }
        }
        ...
        void SetData(uint32 index, uint32 value)
        {
            ...
            if (value == NPC_DUSTBONE_HORROR)
            {
                ++_summonDeaths;
                if (_summonDeaths == 2) // Both horrors dead
                {
                    ...
                    for (auto itr = units.begin(); itr != units.end(); ++itr)
                        ...
                        {
                            npc->DespawnOrUnsummon();
                            instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, npc);
                        }
                }
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SAND_VORTEX:
                        /*
                           FIXED: "earthragers tornado has a slightly different visual and is not moving, possibly cant even "catch" players" 
                        */
                        ...
                        if (auto c = me->SummonCreature(NPC_SAND_VORTEX, pos))
                        {
                            c->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
...
enum SandVortex
{
    EVENT_EJECT = 1,
    EVENT_RE_ENABLE,
    SPELL_SAND_VORTEX_DAMAGE = 25161,
};

class npc_ptah_sand_vortex : public CreatureScript
{
public:
    npc_ptah_sand_vortex() : CreatureScript("npc_ptah_sand_vortex") { }

    struct npc_ptah_sand_vortexAI : public VehicleAI
    {
        npc_ptah_sand_vortexAI(Creature* creature) : VehicleAI(creature)
        {
        }
        ...
        void InitializeAI()
        {
            ...
            me->SetSpeed(MOVE_RUN, 3.5, true);
            me->GetMotionMaster()->MoveRandom(35.f);
            me->SetHover(true);
        }
        void SpellHitTarget(Unit* target, SpellInfo const* spell) 
        {
            bool target_is_riding_me{ false };
            if (spell->Id == SPELL_SAND_VORTEX_DAMAGE)
            {
                if (target->ToPlayer() && enabled)
                {
                    if (auto v = target->GetVehicle())
                    {
                        if (auto b = v->GetBase())
                        {
                            if (b->GetGUID() != me->GetGUID())
                                target->ExitVehicle();
                            else
                                target_is_riding_me = true;
                        }

                        if (!target_is_riding_me)
                        {
        ...
        void PassengerBoarded(Unit* player, int8 seatId, bool apply)
        {
            ...
            {
                enabled = false;
                if (!events.HasEvent(EVENT_EJECT))
                    events.ScheduleEvent(EVENT_EJECT, 5000);
            }
            else
            {
                if (auto m = player->GetMotionMaster())
                {
                    m->Clear();
                    player->CastWithDelay(200, player, 65873, true);
                }
            }
        }
        ...
        void MassEject() 
        {
            ...
            events.ScheduleEvent(EVENT_RE_ENABLE, 5000);
        }
    ...
    {
        return new npc_ptah_sand_vortexAI(creature);
    }
};
