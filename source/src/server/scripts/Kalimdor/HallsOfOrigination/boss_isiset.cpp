        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SUPERNOVA:
                    me->ApplyAllSpellImmune(SPELL_SUPERNOVA, true);
                    Talk(SAY_SUPERNOVA);
...
class spell_spatial_arcane_energy_triggered : public SpellScriptLoader
{
public:
    spell_spatial_arcane_energy_triggered() : SpellScriptLoader("spell_spatial_arcane_energy_triggered") { }

    class spell_spatial_arcane_energy_triggered_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_spatial_arcane_energy_triggered_SpellScript);
        ...
        SpellCastResult CheckCast()
        {
            ...
            if (Aura* arcane_energy = caster->GetAura(GetSpellInfo()->Id))
            if (arcane_energy->GetStackAmount() > 9)
            {
                caster->CastSpell(caster, 74888, false);
                caster->RemoveAurasDueToSpell(74881);
                if (!caster->HasAura(74880))
                    caster->AddAura(74880, caster);
                return SPELL_FAILED_DONT_REPORT;
            }
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_spatial_arcane_energy_triggered_SpellScript::CheckCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_spatial_arcane_energy_triggered_SpellScript();
    }
};
...
{
public:
    npc_spatial_anomaly() : CreatureScript("npc_spatial_anomaly") { }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_spatial_anomalyAI(creature);
    }
};
...
void AddSC_boss_isiset()
{
    ...
    new spell_spatial_arcane_energy_triggered();
    new boss_isiset();
    new npc_energy_flux();
