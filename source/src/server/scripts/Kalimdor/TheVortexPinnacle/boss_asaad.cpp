...
public:
    ...
    struct script_impl : public BossAI
    {
        ...
        void Reset()
        {
            ...
            events.ScheduleEvent(EVENT_SUMMON_SKYFALL, urand(10000, 20000));
            if (IsHeroic())
            ...
            me->SetCanFly(false);
            me->DespawnCreaturesInArea(NPC_SKYFALL, 50.0f);
            me->DespawnCreaturesInArea(46387, 50.f);
            me->DespawnCreaturesInArea(46492, 50.f);
            stormTargetGUID = 0;
        }
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SUMMON_SKYFALL:
                    ...
                    events.ScheduleEvent(EVENT_SUMMON_SKYFALL, urand(15000, 20000));
                    break;
...
public:
    ...
    struct script_impl : public ScriptedAI
    {
        ...
        void GenerateTrianglePos()
        {
            ...
            _trianglePos[0].m_orientation = 0.0f;
            ...
            _trianglePos[1].m_orientation = 0.0f;
            ...
            _trianglePos[2].m_orientation = 0.0f;
                    ...
                    if (auto temp1 = me->SummonCreature(463871, _trianglePos[0], TEMPSUMMON_MANUAL_DESPAWN))
                        if (auto temp2 = me->SummonCreature(463871, _trianglePos[1], TEMPSUMMON_MANUAL_DESPAWN))
                            if (auto temp3 = me->SummonCreature(463871, _trianglePos[2], TEMPSUMMON_MANUAL_DESPAWN))
                            {
                                ...
                                temp3->CastSpell(c, SPELL_STORM_RUNE_BEAM, true);
                            }
                }
                break;
            default:
        ...
        {
            summons.DespawnAll();
            if(Creature* skyfallstar = me->FindNearestCreature(52019, 50.f))
                skyfallstar->DespawnOrUnsummon();
        }

        void UpdateAI(uint32 const diff)
        {
            if (pointId > 3 || isMoving)
                return;
            ...
            if (mui_timerMove <= diff)
            {
                ...
                {
                case 0:
                    //summon triangle triggers
                    triangle_1 = me->SummonCreature(46387, _trianglePos[0], TEMPSUMMON_MANUAL_DESPAWN);
                    triangle_2 = me->SummonCreature(46387, _trianglePos[1], TEMPSUMMON_MANUAL_DESPAWN);
                    triangle_3 = me->SummonCreature(46387, _trianglePos[2], TEMPSUMMON_MANUAL_DESPAWN);

                    
                    triangle_1->CastSpell(me, SPELL_STORM_RUNE_BEAM, true);
                    me->GetMotionMaster()->MovePoint(pointId + 2, _trianglePos[pointId + 1]);
                    break;
                    break;
                case 1:
                    triangle_1->CastSpell(triangle_2, SPELL_STORM_RUNE_BEAM, true);
                    triangle_2->CastSpell(me, SPELL_STORM_RUNE_BEAM, true);
                    me->GetMotionMaster()->MovePoint(pointId + 2, _trianglePos[pointId + 1]);
                    break;
                case 2:
                    triangle_2->CastSpell(triangle_3, SPELL_STORM_RUNE_BEAM, true);
                    triangle_3->CastSpell(me, SPELL_STORM_RUNE_BEAM, true);
                    me->GetMotionMaster()->MovePoint(1, _trianglePos[0]);
                    break;
                case 3:
                    triangle_3->CastSpell(triangle_1, SPELL_STORM_RUNE_BEAM, true);
                    break;
                default:
                    break;
                }
