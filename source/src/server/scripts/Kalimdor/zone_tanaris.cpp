...
public:
        ...
        if (quest->GetQuestId() == 29135)
        {
            if (player)
            {
                ...
                {                    
                    ...
                    creature->CastSpell(creature, 99627, false);
    ...
    bool OnQuestAccept(Player* player, Creature* creature, const Quest* quest)
    {
        if (quest->GetQuestId() == 29193)
            if (player)
                player->SummonGameObject(208430, -8467.65f, -4235.1f, -206.202f, 0.f, 0, 0, 0, 0, 0);

        return true;
    }
};

class npc_timeless_eye : public CreatureScript
{
public:
    npc_timeless_eye() : CreatureScript("npc_timeless_eye") {}

    struct npc_timeless_eyeAI : public ScriptedAI
    {
        npc_timeless_eyeAI(Creature* creature) : ScriptedAI(creature) { }
        ...
        void UpdateAI(const uint32 Diff)
        {
            ...
            else if (me->GetDisplayId() != 38699 && !me->HasAura(99632))
                me->SetDisplayId(38699);
        }
    };

    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_timeless_eyeAI(creature);
    }
