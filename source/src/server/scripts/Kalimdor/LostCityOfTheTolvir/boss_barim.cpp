...
enum Events
{
    ...
    EVENT_BURNING_SOULS,
    EVENT_UNEGG,
    ACTION_ACHIEVEMENT_KILL_IT_WITH_FIRE_PROGRESS,
};

enum Actions
{
    ACTION_BURNING_SOUL = 8,
    ACTION_UNEGG,
    ACTION_EGG_INTERMISSION,
};
    ...
    struct boss_high_prophet_barimAI : public BossAI
    {
        ...
        void JustSummoned(Creature* summon)
        {
            ...
            switch (summon->GetEntry())
            {
                ...
                {
                    summon->CastSpell(summon, SPELL_BLAZE_OF_THE_HEAVENS_EGG_VISUAL, true);
        ...
        void DoAction(int32 const action)
        {
            switch (action)
            {
                case PHASE_NORMAL:
                    ...
                    if (IsHeroic())
                        events.ScheduleEvent(EVENT_FENIX_SUMMON, 1000, 0, PHASE_NORMAL);
                    break;
                case ACTION_BURNING_SOUL:
                    break;
                case ACTION_ACHIEVEMENT_KILL_IT_WITH_FIRE_PROGRESS:
                    achievement_counter++;
        ...
        void DamageTaken(Unit* caster, uint32& damage)
        {
            if (!HealthAbovePct(50) && first)
            {
                ...
                if (auto blaze = me->FindNearestCreature(NPC_BLAZE_OF_HEAVENS, 100.0f, true))
                    blaze->DespawnOrUnsummon();
                if (auto blaze = me->FindNearestCreature(NPC_BLAZE_OF_HEAVENS, 100.0f, false))
                    blaze->DespawnOrUnsummon();
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_REPENTANCE_PULL:
                        DoCastAOE(SPELL_REPENTANCE_PULL);
                        events.ScheduleEvent(EVENT_REPENTANCE_KNEEL, 0, 0, PHASE_50_PRECENT);
                        break;
        ...
        struct npc_blaze_of_heavensAI : public ScriptedAI
        {
            ...
            void DoAction(const int32 act)
            {
                ...
                case PHASE_NORMAL:
                    ...
                    me->SetDisplayId(20245);
                    me->CastSpell(me, 99430, true);
                    events.ScheduleEvent(EVENT_UNEGG, 1000);
                    break;
                }
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                    switch (eventId)
                    {
                        ...
                        case EVENT_UNEGG:
                        {
                            if (Creature *c = me->FindNearestCreature(BOSS_HIGH_PROPHET_BARIM, 500.0f))
                            {
...
class npc_soul_fragment : public CreatureScript
{
    public:
        ...
        struct npc_soul_fragmentAI : public ScriptedAI
        {
            ...
            void MoveInLineOfSight(Unit* who)
            {
                if (who->GetEntry() == NPC_HARBINGER_OF_DARKNESS)
                if (!me->isMoving())
                if (auto harbinger = who->ToCreature())
                    {
                        me->CastStop();
                        DoCast(harbinger, SPELL_SOUL_FRAGMENT);
                        me->GetMotionMaster()->Clear();
...
class spell_blazing_soul_transform : public SpellScriptLoader
{
public:
    spell_blazing_soul_transform() : SpellScriptLoader("spell_blazing_soul_transform") { }

    class spell_blazing_soul_transform_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_blazing_soul_transform_AuraScript);

        void HandleOnEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes mode)
        {
            if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_DEATH)    //this aura was removed by a unit dying with it active
                if (auto victim = GetTarget())
                    if (auto victimCreature = victim->ToCreature())
                        if (victimCreature->GetEntry() == 43934)                        //this npc was entry 43934 (soul shard)
                        {
                            victimCreature->UpdateEntry(49219);
                            if (victimCreature->GetMap()->GetDifficulty() == DUNGEON_DIFFICULTY_HEROIC) //If this was in heroic
                                if (auto instance = victim->GetInstanceScript()) //If the instance is not a null
                                    if (auto barim = victimCreature->FindNearestCreature(BOSS_HIGH_PROPHET_BARIM, 100.f))
                                        barim->AI()->DoAction(ACTION_ACHIEVEMENT_KILL_IT_WITH_FIRE_PROGRESS);
                        }
        }
        void Register()
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_blazing_soul_transform_AuraScript::HandleOnEffectRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_blazing_soul_transform_AuraScript();
    }
};

void AddSC_boss_high_prophet_barim()
{
    new spell_blazing_soul_transform();
