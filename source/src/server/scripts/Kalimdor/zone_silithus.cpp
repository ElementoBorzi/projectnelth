...
#define GOSSIP_DEMITRIAN8 "Call Thunderaan the Windseeker for me! (Only once in 3 minutes.)"
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 7:
            if (time(NULL) - lastTimeSummoned > 180)
            {
                creature->SummonCreature(14435, -6237.22f, 1717.1f, 4.35301f, 0.616363f, TEMPSUMMON_TIMED_DESPAWN, 180000);
                lastTimeSummoned = time(NULL);
            }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->GetQuestStatus(7786) == QUEST_STATUS_REWARDED && player->GetQuestStatus(7787) != QUEST_STATUS_REWARDED)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_DEMITRIAN8, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 7);
