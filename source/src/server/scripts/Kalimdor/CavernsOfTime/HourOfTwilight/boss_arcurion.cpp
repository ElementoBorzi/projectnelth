...
enum Events
{
    ...
    EVENT_INTRO,
    EVENT_THRALL_RESPOND_TO_SPAWNS,
    EVENT_THRALL_ATTACK_FIRST_SPAWNS,
    EVENT_INTRO_1,
    EVENT_REACT_ARCURION_DEATH,
    EVENT_GHOST_WOLF,
    EVENT_INTRO_2,
    EVENT_INTRO_3,
    EVENT_STEP_1,
    ...
    struct boss_arcurionAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            if (auto thrall = me->FindNearestCreature(NPC_THRALL_EVENT_1, 100.0f, true))
            {
                ...
                thrall->AI()->EnterCombat(me);
                thrall->AddThreat(me, 100.f);
            }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SUMMON_FROZEN_SERVITOR:
                    {
                        ...
                        events.ScheduleEvent(EVENT_SUMMON_FROZEN_SERVITOR, urand(3000, 4000));
                        break;
                    }
                    default:
                       break;
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        ai->Talk(TALK_THRALL_INTRO, NULL, CHAT_MSG_MONSTER_SAY);
    ...
    struct npc_thrall_hot_0AI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 point) override
        {
            switch (point)
            {
                ...
                case 4:
                {
                    Talk(TALK_THRALL_MAGIC, NULL, CHAT_MSG_MONSTER_SAY);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SUMMON_WAVE_3:
                        ...
                        me->SetHomePosition(SecondaryHome_2);
                        events.ScheduleEvent(EVENT_LAVA_BURST, 2500);
                    ...
                    case EVENT_INTRO:
                        ...
                        break;
                    case EVENT_THRALL_RESPOND_TO_SPAWNS:
                        me->SetFacingTo(4.712389f);
                        me->TalkWithDelay(2500, 6, NULL);
                        events.ScheduleEvent(EVENT_THRALL_ATTACK_FIRST_SPAWNS, 4000);
                        break;
                    case EVENT_THRALL_ATTACK_FIRST_SPAWNS:
                        ...
                        me->SetHomePosition(SecondaryHome);
                        events.ScheduleEvent(EVENT_LAVA_BURST, 10000);
                        break;
                    case EVENT_INTRO_2:
                        Talk(TALK_THRALL_TAKE_A_MOMENT, NULL, CHAT_MSG_MONSTER_SAY);
    ...
    private:
        Position SecondaryHome = { 4930.907227f, 275.675812f, 96.949844f, 4.435370f };
        Position SecondaryHome_2 = { 4887.679688f, 198.055511f, 99.864990f, 4.197339f };
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (instance)
        {
            if (instance->IsDone(DATA_ARCURION))
            {
                creature->CastSpell(creature, SPELL_WOLF_GHOST);
                creature->AI()->DoAction(EVENT_GHOST_WOLF);
            }
            else
            {
        ...
        void WaypointReached(uint32 point)
        {
            switch (point)
            {
                case 3:
                    me->SetReactState(REACT_PASSIVE);
                    SetEscortPaused(true);
                    TalkWithDelay(500, TALK_THRALL_2_SHOW_YOURSELF, NULL, CHAT_MSG_MONSTER_SAY);
        ...
        void DoAction(const int32 action) override
        {
            switch (action)
            {
                ...
                case ACTION_STOP_COMBAT:
                    ...
                    events.ScheduleEvent(EVENT_REACT_ARCURION_DEATH, 7000);
                    break;
                case EVENT_GHOST_WOLF:
                    events.ScheduleEvent(EVENT_GHOST_WOLF, 4000);
                    break;
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_INTRO_1:
                        me->SetReactState(REACT_AGGRESSIVE);
                        break;
                    case EVENT_REACT_ARCURION_DEATH:
                        me->SetFacingTo(3.69f);
                        Talk(TALK_THRALL_2_DISCOVERED, NULL, CHAT_MSG_MONSTER_SAY);
                        break;
                    case EVENT_GHOST_WOLF:
                        Talk(TALK_THRALL_2_FOLLOW_ME, NULL, CHAT_MSG_MONSTER_SAY);
                        SetEscortPaused(false);
                        break;
                    case EVENT_INTRO_2:
                        SetEscortPaused(false);
