        ...
        void Reset() override
        {
            ...
            if (instance->GetData(DATA_RAID_MODE) == RAID_MODE_LFR)
            {
                if (!me->HasAura(SPELL_PRESENCE_OF_THE_DRAGON_SOUL))
                    me->CastSpell(me, instance->GetData(DATA_RAID_NERF_SPELL), true);
            ...
            GetCreatureListWithEntryInGrid(list_of_npcs, me, BLUE_GLOBULE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, RED_GLOBULE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, GREEN_GLOBULE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, YELLOW_GLOBULE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, VIOLETT_GLOBULE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, BLACK_GLOBULE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, NPC_MANA_VOID, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, NPC_FORGOTTEN_ONE, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, NPC_TIME_WARDEN, 200.0f);
            GetCreatureListWithEntryInGrid(list_of_npcs, me, NPC_HALO_JUMP_YORSAHJ, 200.0f);
            if (list_of_npcs.size())
                for (auto c : list_of_npcs)
                {
                    instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, c);
                    c->DeleteThreatList();
                    c->DespawnOrUnsummon();
                }
        }
                ...
                switch (eventid)
                {
                    ...
                    case EVENT_SUMMON_CORRUPTED:
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //TC_LOG_ERROR("sql.sql", "Event Summon Corrupted");
                                me->AddAura(SPELL_CORRUPTED_MINIONS, me);
                                //DoCast(SPELL_CORRUPTED_MINIONS_VIS);
...
public:
    ...
    class spell_ds_mana_diffusion_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& list_of_npcs)
        {
            if (list_of_npcs.size())
                for (auto itr = list_of_npcs.begin(); itr != list_of_npcs.end();)
                    if (auto t = (*itr))
                        if (auto unit = t->ToUnit())
                            if (unit->GetPrimaryStat() != STAT_INTELLECT)//filter condition
                                list_of_npcs.erase(itr++);
                            else
                                ++itr;

            _targets = list_of_npcs.size();
        }
...
class spell_ds_mana_void_Leech : public SpellScriptLoader
{
public:
    spell_ds_mana_void_Leech() : SpellScriptLoader("spell_ds_mana_void_Leech") { }

    class spell_ds_mana_void_Leech_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_ds_mana_void_Leech_SpellScript);
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ds_mana_void_Leech_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_ds_mana_void_Leech_SpellScript();
    }
};

class spell_yorsajh_summon_corrupted_aura : public SpellScriptLoader
{
public:
    spell_yorsajh_summon_corrupted_aura() : SpellScriptLoader("spell_yorsajh_summon_corrupted_aura") { }

    class spell_yorsajh_summon_corrupted_aura_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_yorsajh_summon_corrupted_aura_AuraScript);
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
                ...
                if (auto c = GetCaster())
                    {
                        Position pos(*c);
                        c->MoveBlink(pos, frand(0.f, 20.f), frand(0.f, M_PI * 2.f));
                        if (auto mob = c->SummonCreature(56265, pos))
                        {
                            //mob->CastSpell(mob, SPELL_CORRUPTED_MINIONS_VIS);
                        }
                    }
        }
        ...
        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_yorsajh_summon_corrupted_aura_AuraScript::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_yorsajh_summon_corrupted_aura_AuraScript();
    }
};
...
void AddSC_boss_yorsahj()
{
    ...
    new spell_ds_mana_void_Leech();
    new spell_yorsajh_summon_corrupted_aura();
