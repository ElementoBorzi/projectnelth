...
public:
    ...
    struct boss_ds_morchokAI : public BossAI
    {
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (auto rp_afro = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_LORD_AFRASASTRASZ)))
                rp_afro->AI()->DoAction(me->GetEntry());
        }
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            {
                if (me->isAlive())
                if (throw_timer >= diff) throw_timer -= diff;
                else
                {
                    if (urand(0, 2) == 0)
                        Talk(2, NULL, CHAT_MSG_MONSTER_YELL, TEXT_RANGE_MAP);
                    //TalkWithDelay(urand(1000, 5000), 2, NULL, CHAT_MSG_MONSTER_YELL);

                    if (auto c = me->SummonCreature(144951, siege_throw_targets[urand(0, 9)]))
                    {
                        me->SetFacingTo(me->GetAngle(c));
                        me->CastWithDelay(urand(1000, 5000), c, 110307, true);
    ...
    private:
        ...
        uint32 throw_timer{ 5000 };
        float oldOrientation;
        Position siege_throw_targets[10]
        {
            {-1858.370117f, -2344.017090f, 118.073402f, 6.220274f},
            {-1866.774048f, -2402.081787f, 141.192383f, 0.086312f},
            {-1843.749023f, -2453.737549f, 124.969406f, 0.274805f},
            {-1870.991943f, -2401.710449f, 196.151169f, 0.054893f},
            {-1836.736572f, -2459.161621f, 212.920212f, 6.220267f},
            {-1849.176025f, -2339.207764f, 217.371582f, 0.255168f},
            {-1822.277100f, -2307.974365f, 84.128593f, 5.858932f},
            {-1881.284180f, -2383.305664f, 58.351894f, 0.105872f},
            {-1877.384521f, -2421.682129f, 57.025200f, 0.023402f},
            {-1804.272583f, -2486.934326f, 69.577927f, 0.101943},
        };
    };
...
class npc_ds_siege_breaker_stalker : public CreatureScript
{
public:
    npc_ds_siege_breaker_stalker() : CreatureScript("npc_ds_siege_breaker_stalker") { }

    struct npc_ds_siege_breaker_stalkerAI : public PassiveAI
    {
        npc_ds_siege_breaker_stalkerAI(Creature* creature) : PassiveAI(creature), summons(creature)
        {
            instance = me->GetInstanceScript();
        }
        ...
        void Reset() override
        {
            events.ScheduleEvent(1, urand(3000, 12000));
        }
            ...
            while (uint32 eventid = events.ExecuteEvent())
                switch (eventid)
                {
                    case 1:
                    {
                        ...
                        {
                            if (urand(0, 3) == 3)
                                if (auto c = instance->GetCreature(NPC_TYRYGOSA))
                                {
                                    auto angle = c->GetAngle(me);
                                    me->SetFacingTo(angle);
                                    Position spawn_at(*me);
                                    me->MoveBlink(spawn_at, 100.f, 0.f);
                                    if (auto c2 = me->SummonCreature(RAND(57259, 57233), spawn_at))
                                        c2->CastSpell(c, 107996, true);
                                }
                            events.ScheduleEvent(1, urand(12000, 18000));
                        }
                        break;
                    }
                }
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            summon->GetMotionMaster()->MovePoint(500, *me);
        }
        ...
        void SummonedMovementInform(Creature* summon, uint32 type, uint32 id)
        {
            if (type == POINT_MOTION_TYPE)
                if (id == 500)
                {
                    summon->CastSpell(summon, 100626, true);
                    summon->Kill(summon, false, false);
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_ds_siege_breaker_stalkerAI(creature);
    }
};

void AddSC_boss_morchok()
{
    new npc_ds_siege_breaker_stalker();
    new boss_ds_morchok();
    new boss_ds_kohcrom();
