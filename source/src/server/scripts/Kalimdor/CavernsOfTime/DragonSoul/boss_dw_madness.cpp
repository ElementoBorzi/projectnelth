...
Position const DeathwingSpawnPos = { -11903.930f, 11989.140f, -95.204f, 2.1642f };
...
Position blood_spawn[4]
{
    { -12121.710f, 12071.200f, 2.392f, 0.00f },
    { -11956.290f, 12268.920f, 1.414f, 0.00f },
    { -12025.940f, 12218.560f, -6.081f, 0.00f },
    { -12094.860f, 12158.730f, -2.734f, 0.00f }
};
...
public:
    ...
    struct boss_deathwing_madnessAI : public BossAI
    {
        ...
        {
            instance->DoRemoveAurasDueToSpellOnPlayers(106527);
        ...
        void SummonedCreatureDies(Creature* summon, Unit* killer) override
        {
            ...
            if (!summons.HasEntry(NPC_WING_TENTACLE) 
                && !summons.HasEntry(NPC_ARM_TENTACLE_2) 
                && !summons.HasEntry(NPC_ARM_TENTACLE) 
                && !events.IsInPhase(PHASE_HEAD))
            {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
        {
            ...
            if (spellInfo && spellInfo->Id == 109971 /*Dragon soul fire impact w/ heart of the aspects visible*/)//sent manually from thrall
            {
                me->PlayDirectSound(26348);
                me->CastSpell(me, SPELL_DEATH);
                if (auto arm = me->FindNearestCreature(NPC_DEATHWING_ARM_LEFT, 100.f))
                    arm->CastSpell(arm, SPELL_DEATH);
                if (auto arm = me->FindNearestCreature(NPC_DEATHWING_ARM_RIGHT, 100.f))
                    arm->CastSpell(arm, SPELL_DEATH);

                events.ScheduleEvent(666, 9000);
            }
        }
        ...
        void DoAction(int32 const actionId) override
        {
            switch (actionId)
            {
                ...
                me->DespawnCreaturesInArea(NPC_DEATHWING_HEAD, 800.f);
                me->DespawnCreaturesInArea(NPC_ARM_TENTACLE, 800.f);
                me->DespawnCreaturesInArea(NPC_ARM_TENTACLE_2, 800.f);
                me->DespawnCreaturesInArea(NPC_WING_TENTACLE, 800.f);
                me->DespawnCreaturesInArea(NPC_BLISTERING_TENTACLE, 800.f);
                me->DespawnCreaturesInArea(NPC_ELEMENTIUM_FRAGMENT, 800.f);
                me->DespawnCreaturesInArea(NPC_MUTETED_CORRUPTION, 800.f);
                me->DespawnCreaturesInArea(NPC_CRUSH_TARGET, 800.f);
                me->DespawnCreaturesInArea(NPC_REGENERATIVE_BLOOD, 800.f);
                me->DespawnCreaturesInArea(NPC_SUMMON_ENABLER, 800.f);
                me->DespawnCreaturesInArea(NPC_ELEMENTIUM_BOLT_TARGET, 800.f);
                me->DespawnCreaturesInArea(NPC_TIME_ZONE, 800.f);
                me->DespawnCreaturesInArea(NPC_TIME_ZONE_TARGET, 800.f);
                me->DespawnCreaturesInArea(NPC_CORRUPTING_PARASITE, 800.f);
                me->DespawnCreaturesInArea(NPC_CORRUPTING_PARASITE_TENTACLE, 800.f);
                me->DespawnCreaturesInArea(NPC_CONGAELING_BLOOD_TARGET, 800.f);
                    ...
                    {
                        c->AI()->RemoveEncounterFrame();
                        c->DespawnOrUnsummon();
                    }

                    me->RemoveAurasDueToSpell(SPELL_SLUMP);
                    me->RemoveAurasDueToSpell(106818);
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);

                    me->DespawnCreaturesInArea(NPC_MUTETED_CORRUPTION, 800.0f);
                    me->DespawnCreaturesInArea(NPC_REGENERATIVE_BLOOD, 800.0f);
                    me->DespawnCreaturesInArea(NPC_ELEMENTIUM_BOLT_TARGET, 800.0f);
                    me->DespawnCreaturesInArea(NPC_ELEMENTIUM_FRAGMENT, 800.0f);
                    me->DespawnCreaturesInArea(NPC_ELEMENTIUM_TERROR, 800.0f);
                    me->DespawnCreaturesInArea(NPC_TIME_ZONE, 800.0f);
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (auto kale = instance->GetCreature(NPC_KALECGOS))
                kale->SetVisible(false);
        }
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            while (uint32 eventid = events.ExecuteEvent())
            {
                ...
                case 666:
                    ...
                    me->CastSpell(me, SPELL_PLAY_MOVIE);
                    if (auto p = me->FindNearestPlayer(1000.f))
                        JustDied(p);
                    break;
                    ...
                    case EVENT_START_PHASE_2:
                    {
                        ...
                        for (SummonList::iterator i = summons.begin(); i != summons.end(); ++i)
                        {
                            if (auto summon = ObjectAccessor::GetCreature(*me, *i))
                            {
                                ...
                                if (summon->GetEntry() != NPC_DEATHWING_ARM_LEFT
                                    && summon->GetEntry() != NPC_DEATHWING_ARM_RIGHT
                                    && summon->GetEntry() != NPC_DEATHWING_WING_RIGHT
                                    && summon->GetEntry() != NPC_DEATHWING_WING_LEFT
                                    && summon->GetEntry() != NPC_DEATHWING_TAIL
                                    )
                        ...
                        if (auto thrall = instance->GetCreature(NPC_DW_MADNESS_THRALL))
                        {
                            ...
                            thrall->AI()->TalkWithDelay(55000, SAY_PHASE_TWO_8);//5s
                            thrall->AI()->TalkWithDelay(61000, SAY_PHASE_TWO_9);
                        }
    ...
    private:
        ...
        bool firstParasite;
        bool finished_fight{ false };
        bool phase_two_rp{ false };
        uint64 tentacles[9];
    };
...
public:
    ...
    private:
        ...
        bool below16PctCheck{ false };
        bool below11PctCheck{ false };
        bool below6PctCheck{ false };
    };
...
public:
    ...
    struct npc_ds_thrall_madnessAI : public Scripted_NoMovementAI
    {
        ...
        void MoveInLineOfSight(Unit* mover) override
        {
            ...
            {
                intro = true;
                events.ScheduleEvent(NPC_DW_MADNESS, 8000);
            }
        }

        void OnSpellCastFinished(SpellInfo const* spell, SpellFinishReason reason) 
        {  
            //TC_LOG_ERROR("sql.sql", "thrall finished casting %u", spell->Id);
            if (auto madness = instance->GetCreature(DATA_MADNESS_OF_DEATHWING))
                madness->AI()->SpellHit(me, spell);
        }
        ...
        void sGossipSelect(Player* /*player*/, uint32 sender, uint32 action) override
        {
            if (instance->GetBossState(DATA_MADNESS_OF_DEATHWING) != IN_PROGRESS && !instance->IsDone(DATA_MADNESS_OF_DEATHWING))
            {
                if (!IsHeroic()
                    || (IsHeroic() 
                        && instance->IsDoneInHeroic(DATA_MORCHOK) 
                        && instance->IsDoneInHeroic(DATA_YORSAHJ) 
                        && instance->IsDoneInHeroic(DATA_WARLORD_ZONOZZ) 
                        && instance->IsDoneInHeroic(DATA_HAGARA) 
                        && instance->IsDoneInHeroic(DATA_ULTRAXION) 
                        && instance->IsDoneInHeroic(DATA_WARMASTER_BLACKHORN) 
                        && instance->IsDoneInHeroic(DATA_SPINE_OF_DEATHWING)))
                {
        ...
        void DoAction(int32 const action) override
        {
            switch (action)
            {
                case ACTION_OUTRO:
                    ...
                    if (auto deathwing = instance->GetCreature(DATA_MADNESS_OF_DEATHWING))
                    me->CastWithDelay(27000, deathwing, SPELL_FIRE_DRAGON_SOUL);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case 0:
                        break;
                    case NPC_DW_MADNESS:
                        Talk(INTRO_THRALL, NULL, CHAT_MSG_MONSTER_SAY, TEXT_RANGE_MAP);
                        ...
                        if (auto deathwing = instance->GetCreature(DATA_MADNESS_OF_DEATHWING))
                            if (auto timeZone = me->FindNearestCreature(NPC_TIME_ZONE, 200.f))
                                for (uint8 i = 0; i < 6; i++)
                                {
                                    ...
                                    //TC_LOG_ERROR("sql.sql", "casting summon for %u", i);
                                    Position spawn_at{ *timeZone };
                                    float angle = deathwing->GetAngle(timeZone);
                                    float angle_diff{ M_PI / 12 };
                                    float angle_changed{ 0.f };

                                    switch (i)
                                    {
                                    case 0:
                                        angle_changed = -angle_diff * 3.f;
                                        break;
                                    case 1:
                                        angle_changed = -angle_diff * 2.f;
                                        break;
                                    case 2:
                                        angle_changed = -angle_diff * 1.f;
                                        break;
                                    case 3:
                                        angle_changed = angle_diff * 1.f;
                                        break;
                                    ...
                                    timeZone->MovePosition(spawn_at, frand(9.f, 13.f), angle + angle_changed);
                                    
                                    me->CastSpell(
                                        spawn_at.GetPositionX(),
                                        spawn_at.GetPositionY(),
                                        spawn_at.GetPositionZ(), SPELL_HEMORRHAGE, true, NULL, NULL, NULL);
                                }
                            //else TC_LOG_ERROR("sql.sql", "could not find timezone.");
                        //else TC_LOG_ERROR("sql.sql", "could not find deathwing for event hemo");
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_IMPALE:
                    {
                        auto playerList = me->GetPlayersInRange(100.f, true);

                        if (playerList.size())
                            for (auto itr = playerList.begin(); itr != playerList.end(); ++itr)
                                if (auto t = (*itr))
                                    if (!me->IsWithinMeleeRange(t))//filter condition
...
public:
    ...
    struct npc_ds_elementium_fragmentAI : public Scripted_NoMovementAI
    {
        ...
        void MoveInLineOfSight(Unit* mover) override
        {
            if (me->GetDistance(mover) <= 60.f)
                if (!me->isInCombat())
                    if (auto p = mover->ToPlayer())
                        if (!p->isGameMaster())
                        {

                            auto list_of_players = me->GetPlayersInRange(60.f, true);
    ...
    class spell_ds_spellweave_SpellScript : public SpellScript
    {
        ...
        void FilterTarget(std::list<WorldObject*>& targets)
        {
            if (auto caster = GetCaster())
                if (auto procTarget = GetExplTargetUnit())
                    if (targets.size())
                        for (auto itr = targets.begin(); itr != targets.end(); ++itr)
                            if (auto t = (*itr))
                                if (t->GetGUID() == procTarget->GetGUID())//filter condition
                                {
                                    targets.erase(itr);
                                    //TC_LOG_ERROR("sql.sql", "removing target %u", t->GetGUID());
                                }
                                //else TC_LOG_ERROR("sql.sql", "allowing target %u", t->GetGUID());
        }
...
class spell_ds_madness_impale_aspects : public SpellScriptLoader // 82746
{
public:
    spell_ds_madness_impale_aspects() : SpellScriptLoader("spell_ds_madness_impale_aspects") { }

    class spell_ds_madness_impale_aspectsSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_ds_madness_impale_aspectsSpellScript);


        void CalculateDamage(SpellEffIndex /*effIndex*/)
        {
            if (auto aspect = GetHitUnit())
            {
                SetHitDamage(aspect->GetMaxHealth() * 0.30);
            }
        }

        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_ds_madness_impale_aspectsSpellScript::CalculateDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_ds_madness_impale_aspectsSpellScript();
    }
};
...
/*
    ...
    Heroic 25: 110042
*/
class spell_ds_madness_cataclysm_wipe : public SpellScriptLoader
{
public:
    spell_ds_madness_cataclysm_wipe() : SpellScriptLoader("spell_ds_madness_cataclysm_wipe") { }

    class spell_ds_madness_cataclysm_wipe_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_ds_madness_cataclysm_wipe_SpellScript);

        void HandleHit(SpellEffIndex /*effIndex*/)
        {
            if (auto target = GetHitUnit())
            {
                target->CastSpell(target, 68707, true);

                std::list<Creature*> list_of_npcs;
                GetCreatureListWithEntryInGrid(list_of_npcs, target, NPC_PLATFORM_TRIGGER, 500.0f);
                GetCreatureListWithEntryInGrid(list_of_npcs, target, NPC_JUMP_PAD, 500.0f);
                GetCreatureListWithEntryInGrid(list_of_npcs, target, NPC_TIME_ZONE, 500.0f);
                GetCreatureListWithEntryInGrid(list_of_npcs, target, NPC_SUMMON_ENABLER, 500.0f);
                GetCreatureListWithEntryInGrid(list_of_npcs, target, NPC_SUMMON_ENABLER_STALKER, 500.0f);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_ds_madness_cataclysm_wipe_SpellScript::HandleHit, EFFECT_2, SPELL_EFFECT_FORCE_CAST);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_ds_madness_cataclysm_wipe_SpellScript();
    }
};

class go_ds_loot_chest : public GameObjectScript
{
public:
    go_ds_loot_chest() : GameObjectScript("go_ds_loot_chest") { }

    bool OnGossipHello(Player* player, GameObject* go)
    {
        //TC_LOG_ERROR("sql.sql", "OnGossipHello from player %u", player->GetGUID());
        if (auto instance = go->GetInstanceScript())
        {
            if (auto g = player->GetGroup())
                if (g->GetLootMethod() == MASTER_LOOT)
                if (g->GetLooterGuid() == player->GetGUID())
                {
                    auto members = go->GetPlayersInRange(50.f, true, true);

                    for (auto p : members)
                        if (player->IsInSameGroupWith(p))
                        if (p->GetGUID() != player->GetGUID())
                        {
                            //TC_LOG_ERROR("sql.sql", "removing %u from looting.", p->GetGUID());
                            p->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_LOOTING);
                            if (auto s = p->GetSession())
                                s->DoLootRelease(go->GetGUID());
                        }
                }
        }
        return false;
    ...
    struct go_ds_loot_chestAI : public GameObjectAI
    {

        go_ds_loot_chestAI(GameObject* go) : GameObjectAI(go)
        {

            //TC_LOG_ERROR("sql.sql", "initializing gameobject script %u %u", go->GetGUID(), go->GetEntry());
        }

        void UpdateAI(uint32 diff)
        {
            if (go->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE))
                if (!go->FindNearestPlayer(20.f))
                {
                    //TC_LOG_ERROR("sql.sql", "go %u %U removing GO_FLAG_IN_USE", go->GetGUID(), go->GetEntry());
    ...
    {
        return new go_ds_loot_chestAI(go);
    }
};
...
void AddSC_boss_deathwing_madness()
{
    ...
    new at_isle_jump();
    new spell_ds_madness_impale_aspects();
    new spell_ds_madness_cataclysm_wipe();
    new go_ds_loot_chest();
};
