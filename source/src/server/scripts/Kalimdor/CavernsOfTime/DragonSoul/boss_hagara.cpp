...
enum Spells
{
    ...
    SPELL_ICICLE_FALL_DAMAGE_PLAYER     = 92203,
    SPELL_ICICLE_FALL_DAMAGE_NOPLAYER   = 69425,
...
public:
    ...
    struct boss_ds_hagaraAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            GetCreatureListWithEntryInGrid(list_of_npcs, me, 55695, 200.0f);
        ...
        void DoAction(int32 const actionId) override
        {
            switch (actionId)
            {
                ...
                {
                    auto lock_angle{ me->GetOrientation() };
...
public:
    ...
    struct npc_stormbinder_adeptAI : public ScriptedAI
    {
        ...
        npc_stormbinder_adeptAI(Creature* creature) : ScriptedAI(creature), summons(me)
        {
        }
        ...
        void JustSummoned(Creature* summon) override
        {
            if (summon->GetEntry() == 57921)
            {
                summon->DeleteThreatList();
                summon->SetReactState(REACT_PASSIVE);
                summon->AddAura(109440, summon);
        ...
        { {EVENT_NEVER_USE,   0,                  9999999,        9999999,        false,      false},//DO NOT TOUCH
            ...
            {EVENT_TORNADO,     SPELL_TORNADO,      8000,          15000,          true,       true},
        };
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
                switch (eventId)
                {
                case EVENT_TORNADO:     Spell_Rotation(eventId);  break;
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_stormbinder_adeptAI(creature);
    }
};

class npc_stormborn_mymidon : public CreatureScript
{
public:
    npc_stormborn_mymidon() : CreatureScript("npc_stormborn_mymidon") { }


    struct npc_stormborn_mymidonAI : public ScriptedAI
    {

        npc_stormborn_mymidonAI(Creature* creature) : ScriptedAI(creature), summons(me)
        {
        }
        ...
        { {EVENT_NEVER_USE,   0,                  9999999,        9999999,        false,      false},//DO NOT TOUCH
            ...
            {EVENT_CHAIN_LIGHTNING,     SPELL_CHAIN_LIGHTNING,      13000,          25000,          true,       true},
            {EVENT_SPARK,               SPELL_SPARK,                3000,          5000,          true,       true},
        };
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
                ...
                case EVENT_SPARK:               Spell_Rotation(eventId);  break;
                case EVENT_WIPE_CHECK:
                    if (checkWipe())
                    {
                        me->DeleteThreatList();
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_stormborn_mymidonAI(creature);
    }
};

class npc_ds_hagara_frozen_grasp_stalker : public CreatureScript
{
public:
    npc_ds_hagara_frozen_grasp_stalker() : CreatureScript("npc_ds_hagara_frozen_grasp_stalker") {}

    struct npc_ds_hagara_frozen_grasp_stalkerAI : public Scripted_NoMovementAI
    {
        npc_ds_hagara_frozen_grasp_stalkerAI(Creature* creature) : Scripted_NoMovementAI(creature)
        {
            me->DeleteThreatList();
            me->SetReactState(REACT_PASSIVE);
            me->AddUnitState(UNIT_STATE_ROOT);
            me->AddAura(109318, me);
            me->AddAura(109295, me);
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_DISABLE_MOVE);
        }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_ds_hagara_frozen_grasp_stalkerAI(creature);
    }
};

class spell_ds_hagara_frozen_grasp : public SpellScriptLoader // 109317
{
public:
    spell_ds_hagara_frozen_grasp() : SpellScriptLoader("spell_ds_hagara_frozen_grasp") {}

    class spell_ds_hagara_frozen_grasp_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_ds_hagara_frozen_grasp_SpellScript);
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (targets.size())
                for (auto itr = targets.begin(); itr != targets.end();)
                    if (auto t = (*itr))
                        ...
                        if (!target->ToPlayer() || target->HasAura(109307))
                        {
                            targets.erase(itr++);
                        }
                        else
            ...
            if (targets.size())
            if (auto c = GetCaster())
            if (auto me = c->ToCreature())
            if (auto random_target = Trinity::Containers::SelectRandomContainerElement(targets))
                if (auto target = random_target->ToUnit())
                {
                    me->CastSpell(target, 109307);
                }
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ds_hagara_frozen_grasp_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_ds_hagara_frozen_grasp_SpellScript();
    }
};

class spell_ds_frost_corruption_periodic : public SpellScriptLoader
{
public:
    spell_ds_frost_corruption_periodic() : SpellScriptLoader("spell_ds_frost_corruption_periodic") { }

    class spell_ds_frost_corruption_periodic_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_ds_frost_corruption_periodic_AuraScript);

        void HandleEffectPeriodicUpdate(AuraEffect* aurEff)
        {
            int32 bp{ aurEff->GetBaseAmount() };	//getamount causes compounding increases
            int32 bp_log = bp;
            float coef{ float(aurEff->GetTickNumber()) / float(aurEff->GetTotalTicks()) };
            aurEff->SetAmount(int32(float(bp) * coef));
        }


        void Register()
        {
            OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_ds_frost_corruption_periodic_AuraScript::HandleEffectPeriodicUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
        }
    ...
    AuraScript* GetAuraScript() const override
    {
        return new spell_ds_frost_corruption_periodic_AuraScript();
    }
};

class spell_ds_hagara_icicle_periodic : public SpellScriptLoader
{
public:
    spell_ds_hagara_icicle_periodic() : SpellScriptLoader("spell_ds_hagara_icicle_periodic") { }

    class spell_ds_hagara_icicle_periodic_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_ds_hagara_icicle_periodic_AuraScript);
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (auto c = GetCaster())
            {
                c->CastSpell(c, SPELL_ICICLE_FALL_DAMAGE_PLAYER);
                c->CastSpell(c, SPELL_ICICLE_FALL_DAMAGE_NOPLAYER);
            }
        }

        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_ds_hagara_icicle_periodic_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_ds_hagara_icicle_periodic_AuraScript();
    }
};

class spell_ds_hagara_icicle_triggered_nonplayer : public SpellScriptLoader // 69425
{
public:
    spell_ds_hagara_icicle_triggered_nonplayer() : SpellScriptLoader("spell_ds_hagara_icicle_triggered_nonplayer") {}

    class spell_ds_hagara_icicle_triggered_nonplayer_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_ds_hagara_icicle_triggered_nonplayer_SpellScript);
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ds_hagara_icicle_triggered_nonplayer_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ds_hagara_icicle_triggered_nonplayer_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ds_hagara_icicle_triggered_nonplayer_SpellScript::FilterTargets, EFFECT_2, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_ds_hagara_icicle_triggered_nonplayer_SpellScript();
    }
};
...
void AddSC_boss_hagara()
{
    ...
    new npc_stormborn_mymidon();
    new npc_ds_hagara_frozen_grasp_stalker();
    new spell_ds_hagara_frozen_grasp();
    new spell_ds_frost_corruption_periodic();
    new spell_ds_hagara_icicle_periodic();
    new spell_ds_hagara_icicle_triggered_nonplayer();
};