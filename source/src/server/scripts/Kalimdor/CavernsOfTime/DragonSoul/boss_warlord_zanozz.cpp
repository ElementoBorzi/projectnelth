...
{
	{ -1802.477f, -1850.386f, -221.339f, 5.345f }, //8
	{ -1745.312f, -1846.073f, -221.357f, 4.405f }, //1
	{ -1701.801f, -1883.280f, -221.397f, 3.651f }, //2
	{ -1697.114f, -1940.587f, -221.384f, 2.855f }, //3
	{ -1734.128f, -1984.644f, -221.333f, 2.116f }, //4
	{ -1791.751f, -1989.054f, -221.355f, 1.511f }, //5
	{ -1835.634f, -1951.853f, -221.308f, 0.709f }, //6
	{ -1840.202f, -1894.214f, -221.307f, 0.022f }  //7
};
...
public:
	...
	struct boss_ds_warlord_zonozzAI : public BossAI
	{
		...
		{
			if (me->isAlive() && who->GetTypeId() == TYPEID_PLAYER)
			{
					if (!me->isInCombat())
					if (me->GetDistance(who) <= 25.0f)
					{
						me->AddThreat(who, 100.f);
						me->CombatStart(who);
					}
			}
		}
		...
		void UpdateAI(uint32 const diff) override
		{
			...
			while (uint32 eventid = events.ExecuteEvent())
			{
				...
				case EVENT_BLACK_BLOOD_OF_GORATH:
				{
					...
					// Ping pong achieve
					/*if (Aura* voidDiffusion = me->GetAura(SPELL_VOID_DIFFUSION_ZONOZZ))
				...
				{
					//me->RemoveAurasDueToSpell(SPELL_VOID_DIFFUSION_ZONOZZ);
			...
			if (Aura* voidDiffusion = me->GetAura(104543)) // Focused Anger buff ID
				if (voidDiffusion->GetStackAmount() >= 10)
					bAchieve = true;
		}
	private:
		bool bAchieve; // Ping pong achieve
	};
	...
	struct npc_ds_void_of_the_unmakingAI : public ScriptedAI
	{
		...
		void IsSummonedBy(Unit* /*summoner*/)
		{
			...
			if (instance->GetData(DATA_RAID_MODE) != RAID_MODE_LFR)
			{
				...
				events.ScheduleEvent(EVENT_START_MOVEMENT, 3000);
		...
		void UpdateAI(uint32 const diff) override
		{
			...
			while (uint32 eventid = events.ExecuteEvent())
			{
					...
					if (me->HasAura(SPELL_VOID_OF_THE_UNMAKING_ACTIVE))
					{
						if (auto target = SelectTarget(SELECT_TARGET_NEAREST, 0, 3.0f, true))
						{
							if (target->isAlive())
							{
								...
								events.CancelEvent(EVENT_START_MOVEMENT);
								events.ScheduleEvent(EVENT_START_MOVEMENT, 200);
								events.ScheduleEvent(EVENT_CHECK_NEAR_TARGET, 5000);
								events.ScheduleEvent(EVENT_CHANGE_DIRECTION, 100);
								events.ScheduleEvent(EVENT_ENABLE_EXPLOSON, 5000);
							}
						}
						...
						{
							events.CancelEvent(EVENT_CHECK_NEAR_TARGET);
							DoCast(me, SPELL_BLACK_BLOOD_ERUPTION);
							me->DespawnOrUnsummon(5000);
							Creature* zonozz = instance->GetCreature(DATA_WARLORD_ZONOZZ);
							zonozz->AI()->DoAction(ACTION_SUMMON_NEW_VOID);
						}
						else
							events.ScheduleEvent(EVENT_CHECK_NEAR_TARGET, 800);
					}
					else
						events.ScheduleEvent(EVENT_CHECK_NEAR_TARGET, 800);
					break;
				...
				case EVENT_ENABLE_EXPLOSON:
					...
					//events.ScheduleEvent(EVENT_CHECK_NEAR_TARGET, 1000);
					break;
				...
				case EVENT_START_MOVEMENT:
				{
					...
					events.ScheduleEvent(EVENT_START_MOVEMENT, 500);
					break;
				}
...
class npc_ds_claw_of_gorath_zonozz : public CreatureScript
{
public:
	npc_ds_claw_of_gorath_zonozz() : CreatureScript("npc_ds_claw_of_gorath_zonozz") {}

	struct npc_ds_claw_of_gorath_zonozzAI : public Scripted_NoMovementAI
	{
		npc_ds_claw_of_gorath_zonozzAI(Creature* creature) : Scripted_NoMovementAI(creature)
		{
			me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
			me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
		}
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_ds_claw_of_gorath_zonozzAI(creature);
	}
};
				...
				case EVENT_OOZE_SPIT:
					if (auto currentTarget = me->getVictim())
						if (!me->IsWithinMeleeRange(currentTarget))
							...
							{
								DoCastRandom(SPELL_OOZE_SPIT, 200.0f);

								if (!events.HasEvent(EVENT_OOZE_SPIT))
	...
	struct npc_ds_warden_transportAI : public PassiveAI
	{
		...
		void PassengerBoarded(Unit* player, int8 seatId, bool apply)
		{
			if (!apply)
			{
				...
				/*
					We need to prevent the boss from spamming intros to the entire raid when one of them enters the intro area, so this will all be private packets
				*/

				if (!boss_sent_whisper)
				if (auto p_rider = player->ToPlayer())
				{
					boss_sent_whisper = true;
					LocaleConstant loc_idx = p_rider->GetSession()->GetSessionDbLocaleIndex();
					bool at_yors{ me->GetEntry() == NPC_TIME_WARDEN };

					if (auto boss = instance->GetCreature(at_yors ? DATA_YORSAHJ : DATA_WARLORD_ZONOZZ))
						if (boss->isAlive())
							if (!boss->isInCombat())
							{
								char const* yors_whisper	{ "Our numbers are endless, our power beyond reckoning! All who oppose the Destroyer will DIE A THOUSAND DEATHS!" };
								char const* yors_yell		{ "Ak'agthshi ma uhnish, ak'uq shg'cul vwahuhn! H'iwn iggksh Phquathi gag OOU KAAXTH SHUUL!" };

								char const* zon_whisper		{ "Once more shall the twisted flesh-banners of N'Zoth chitter and howl above the fly-blown corpse of this world. After millennia, we have returned." };
								char const* zon_yell		{ "Vwyq agth sshoq'meg N'Zoth vra zz shfk qwor ga'halahs agthu. Uulg'ma, ag qam." };

								WorldPacket data1(SMSG_MESSAGECHAT, 200);
								boss->BuildMonsterChat(&data1, CHAT_MSG_MONSTER_YELL, at_yors ? yors_yell : zon_yell, LANG_UNIVERSAL, boss->GetNameForLocaleIdx(loc_idx), p_rider->GetGUID());
								p_rider->GetSession()->SendPacket(&data1);

								WorldPacket data2(SMSG_MESSAGECHAT, 200);
								boss->BuildMonsterChat(&data2, CHAT_MSG_MONSTER_WHISPER, at_yors ? yors_whisper : zon_whisper, LANG_UNIVERSAL, boss->GetNameForLocaleIdx(loc_idx), p_rider->GetGUID());
								p_rider->GetSession()->SendPacket(&data2);

								p_rider->PlayDirectSound(at_yors ? 26328 : 26337, p_rider);
							}
				}
			}
			else
...
void AddSC_boss_warlord_zonozz()
{
	...
	new npc_ds_claw_of_gorath_zonozz();
