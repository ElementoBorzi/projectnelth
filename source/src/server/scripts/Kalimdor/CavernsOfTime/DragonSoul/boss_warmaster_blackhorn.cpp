...
enum Spells
{
    ...
    // Sky captain swayze
    SPELL_TELEPORT_ALL_TO_GUNSHIP   = 106051, // 13456.6 Y: -12133.8 Z: 151.168
...
Position const shipFireStalkerPos[19] =
{
    { 13488.26f, -12130.09f, 170.5464f, 0.00f },
    { 13366.94f, -12129.88f, 156.1623f, 0.00f },//bow
    { 13493.64f, -12160.52f, 162.1842f, 0.00f },
    { 13494.39f, -12142.64f, 170.5455f, 0.00f },
    { 13487.15f, -12174.49f, 155.4944f, 0.00f },
    { 13395.83f, -12144.88f, 156.4096f, 0.00f },//left gun
    { 13496.22f, -12111.84f, 163.4314f, 0.00f },
    { 13492.84f, -12096.21f, 155.4947f, 0.00f },
    { 13476.037109f, -12134.240234f, 160.177856f, 3.355354f},//engine room fire
    { 13503.00f, -12132.30f, 170.5578f, 0.00f },
    { 13494.36f, -12156.25f, 155.4958f, 0.00f },


    //i = 12
    { 13501.614258f, -12177.804688f, 127.589920f, 2.624942f},
    { 13372.102539f, -12170.499023f, 119.991898f, 0.893139f},
    { 13374.997070f, -12092.648438f, 120.889915f, 0.245186f},
    { 13505.955078f, -12094.215820f, 126.142403f, 4.246801f},
    { 13393.420898f, -12161.927734f, 140.036179f, 0.131305f},
    { 13476.237305f, -12174.803711f, 139.650360f, 1.631416f},
    { 13479.076172f, -12094.803711f, 139.084488f, 4.757293f},
    { 13395.042969f, -12099.879883f, 140.581726f, 4.741584f},
};
...
public:
                ...
                case 5922:
                    ...
                    if (instance->IsDone(DATA_ULTRAXION) && instance->IsDone(DATA_WARMASTER_BLACKHORN) && !instance->IsDone(DATA_SPINE_OF_DEATHWING))
                        if (!creature->GetMap()->IsHeroic()
                            ...
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (player->GetTeam() == HORDE ? "FOR THE HORDE!" : "FOR THE ALLIANCE!"), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
                    break;
                case 5923:
                    if (instance->IsDone(DATA_ULTRAXION))
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (player->GetTeam() == HORDE ? "We are the Horde. We are always ready." : "We are the Alliance. We are always ready."), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
                    break;
                default:
                    break;
            }
    ...
    struct npc_ds_sky_captain_swayzeAI : public Scripted_NoMovementAI
    {
        ...
        void JustSummoned(Creature* summon)
        {
            ...
            switch (summon->GetEntry())
            {
                case NPC_THE_SKYFIRE:
                    ...
                    summon->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_DISPEL, true);
        ...
        void SummonedCreatureDamageTaken(Unit* attacker, Creature* summon, uint32& damage, SpellInfo const* spell)
        {
            switch (summon->GetEntry())
            {
                case NPC_THE_SKYFIRE:
                    ...
                    if (summon->HealthBelowPctDamaged(75, damage) && phaseCount == 0)
                    {
                        for (uint8 i = 0; i < 14; i++)
                            if (auto fire = me->SummonCreature(NPC_FIRE_STALKER, shipFireStalkerPos[i]))
                            {
                                fire->AddAura(SPELL_ENGINE_FIRE, fire);
                            }
        ...
        void DoAction(int32 const action)
        {
            switch (action)
            {
                ...
                case ACTION_FIGHT_FINISHED:
                    ...
                    Talk(3, NULL, CHAT_MSG_MONSTER_SAY, TEXT_RANGE_MAP);
                    TalkWithDelay(12000, 4, NULL, CHAT_MSG_MONSTER_SAY);
                    break;
                ...
                case 55:
                {
                    //TC_LOG_ERROR("sql.sql", "swayze action 55.");
                    me->TalkWithDelay(5000, 0, NULL, CHAT_MSG_MONSTER_YELL);
                    me->TalkWithDelay(12000, 1, NULL, CHAT_MSG_MONSTER_YELL);
                    events.ScheduleEvent(55, 13000);
                    break;
                }
                default:
                    break;
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case 55:
                {
                    ...
                    if (auto c = me->FindNearestCreature(NPC_KEANU_ORC, 50.f))
                    {
                        ...
                        c->AddUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR);
                        c->AddUnitState(UNIT_STATE_IN_FLIGHT);
                        c->AddUnitState(UNIT_STATE_CONTROLLED);
                        c->SetHover(true);
                        c->SetCanFly(true);
                        c->ClearUnitState(UNIT_STATE_CANNOT_TURN);
                        c->SetDisableGravity(true);
                        c->GetMotionMaster()->ForceMoveJump(Jump_PORT.GetPositionX(), Jump_PORT.GetPositionY(), Jump_PORT.GetPositionZ(), 35.f, 30.f);
                        c->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_FLYFALL);
                    }
                    ...
                    me->AddUnitState(UNIT_STATE_CONTROLLED);
                    me->SetHover(true);
                    me->SetCanFly(true);
                    me->ClearUnitState(UNIT_STATE_CANNOT_TURN);
                    me->SetDisableGravity(true);
                    me->GetMotionMaster()->ForceMoveJump(Jump_STBD.GetPositionX(), Jump_STBD.GetPositionY(), Jump_STBD.GetPositionZ(), 35.f, 30.f);
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_FLYFALL);
                    events.ScheduleEvent(56, 100);
                    break;
                }
                case 56:

                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_FLYFALL);
                    if (auto c = me->FindNearestCreature(NPC_KEANU_ORC, 250.f))
                        c->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_FLYFALL);
                    events.ScheduleEvent(56, 100);
                    break;
                    ...
                    case EVENT_INTRO_TALK:
                        ...
                        for (uint8 i = 12; i < 19; i++)
                            if (auto fire = me->SummonCreature(NPC_FIRE_STALKER, shipFireStalkerPos[i]))
                            {
                                auto delay = uint32(i * urand(1, 400));
                                fire->CastWithDelay(delay, fire, 110261, true);
                                fire->CastWithDelay(delay, fire, SPELL_ENGINE_FIRE, true);
                            }
...
public:
    ...
    struct npc_ds_gorionaAI : public ScriptedAI
    {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            if (spell->Id == SPELL_TWILIGHT_ONSLAUGHT)
                if (target->GetEntry() == NPC_ONSLAUGHT_TARGET)
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_MOVE_TO_MACHINERY:
                        if (me->HasAuraWithMechanic(1 << MECHANIC_STUN)) events.ScheduleEvent(EVENT_MOVE_TO_MACHINERY, 500);
                        else
                        {
