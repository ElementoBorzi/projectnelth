...
enum Spells
{
    ...
    //molten spewer/erupter
    SPELL_MOLTEN_SPEWER_VOLLEY              = 99611,
    SPELL_MOLTEN_ERUPTER_BOLT               = 99579,
    SPELL_MOLTEN_BLAST_PUNISH               = 99613,
...
enum Events
{
    ...
    EVENT_SPEWER_SUBMERGE,
    EVENT_SPEWER_VANISH,
    EVENT_SPEWER_MOVE,
    EVENT_SPEWER_APPEAR,
    EVENT_SPEWER_CAST_ATTACK,

    EVENT_ERUPTER_SUBMERGE,
    EVENT_ERUPTER_VANISH,
    EVENT_ERUPTER_MOVE,
    EVENT_ERUPTER_APPEAR,
    EVENT_ERUPTER_CAST_ATTACK,
};
...
const Position LavaWormLocations[] =
{
    {946.042f, -41.7915f, 40.000f, 3.70422f},  //NW
    {946.042f, -74.0f, 40.000f, 2.64394f},   //NE
    {909.57f, -41.7915f, 40.000f, 3.70422f},   //SW
    {909.57f, -74.0f, 40.000f, 2.64394f},    //SE
};

enum LavaWormSpots
{
    WORM_LOCATION_NORTH_WEST,
    WORM_LOCATION_NORTH_EAST,
    WORM_LOCATION_SOUTH_WEST,
    WORM_LOCATION_SOUTH_EAST,
    WORM_LOCATION_MAX
};
    ...
    struct boss_ragnaros_firelandsAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            for (std::list<Creature*>::iterator itr = units.begin(); itr != units.end(); ++itr)
                (*itr)->AI()->DoAction(ACTION_ASSIST_WORM);
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            _JustDied();
            me->DespawnCreaturesInArea(NPC_DREADFLAME_SPAWN, 200.f);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_LIVING_METEOR:
                        _meteorCount++;
                        DoCastAOE(SPELL_LIVING_METEOR_SUMMON_METEOR, true);
                        events.ScheduleEvent(EVENT_LIVING_METEOR, 45000, 0, PHASE_3);
                        break;
...
class npc_fl_molten_erupter : public CreatureScript
{
    public:
        ...
        struct npc_fl_molten_erupterAI : public ScriptedAI
        {
            ...
            int8 newWormLocation()
            {
                if (me->GetDistance(LavaWormLocations[WORM_LOCATION_NORTH_EAST]) >= 5.0f)
                    return WORM_LOCATION_NORTH_EAST;
                return WORM_LOCATION_SOUTH_WEST;
            }
            ...
            void DoAction(const int32 action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_ASSIST_WORM:
                        ...
                        events.ScheduleEvent(EVENT_ERUPTER_CAST_ATTACK, urand(6000, 10000));
                        break;
                }
            }
            ...
            void UpdateAI(const uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                    case EVENT_ERUPTER_SUBMERGE:
                        events.CancelEvent(EVENT_ERUPTER_CAST_ATTACK);
                        me->SetStandState(UNIT_STAND_STATE_SUBMERGED);
                        me->PlayOneShotAnimKit(ANIM_KIT_SUBMERGE);
                        events.ScheduleEvent(EVENT_ERUPTER_VANISH, 2000);
                        break;
                    case EVENT_ERUPTER_VANISH:
                        me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        events.ScheduleEvent(EVENT_ERUPTER_MOVE, 2000);
                        break;
                    case EVENT_ERUPTER_MOVE:
                        me->NearTeleportTo(LavaWormLocations[newWormLocation()]);
                        events.ScheduleEvent(EVENT_ERUPTER_APPEAR, 2000);
                        break;
                    case EVENT_ERUPTER_APPEAR:
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        me->SetStandState(UNIT_STAND_STATE_STAND);
                        me->PlayOneShotAnimKit(ANIM_KIT_EMERGE);
                        events.ScheduleEvent(EVENT_ERUPTER_CAST_ATTACK, urand(6000, 10000));
...
class npc_fl_molten_spewer : public CreatureScript
{
    public:
        ...
        struct npc_fl_molten_spewerAI : public ScriptedAI
        {
            ...
            int8 newWormLocation()
            {
                if (me->GetDistance(LavaWormLocations[WORM_LOCATION_NORTH_WEST]) >= 5.0f)
                    return WORM_LOCATION_NORTH_WEST;
                return WORM_LOCATION_SOUTH_EAST;
            }
            ...
            void DoAction(const int32 action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_ASSIST_WORM:
                        ...
                        events.ScheduleEvent(EVENT_SPEWER_CAST_ATTACK, urand(6000, 10000));
                        break;
                }
            }
            ...
            void UpdateAI(const uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                    case EVENT_SPEWER_SUBMERGE:
                        events.CancelEvent(EVENT_SPEWER_CAST_ATTACK);
                        me->SetStandState(UNIT_STAND_STATE_SUBMERGED);
                        me->PlayOneShotAnimKit(ANIM_KIT_SUBMERGE);
                        events.ScheduleEvent(EVENT_SPEWER_VANISH, 2000);
                        break;
                    case EVENT_SPEWER_VANISH:
                        me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        events.ScheduleEvent(EVENT_SPEWER_MOVE, 2000);
                        break;
                    case EVENT_SPEWER_MOVE:
                        me->NearTeleportTo(LavaWormLocations[newWormLocation()]);
                        events.ScheduleEvent(EVENT_SPEWER_APPEAR, 2000);
                        break;
                    case EVENT_SPEWER_APPEAR:
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        me->SetStandState(UNIT_STAND_STATE_STAND);
                        me->PlayOneShotAnimKit(ANIM_KIT_EMERGE);
                        events.ScheduleEvent(EVENT_SPEWER_CAST_ATTACK, urand(6000, 10000));
...
public:
    ...
    class spell_fl_summon_living_meteor_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            GetCaster()->CastSpell(GetHitUnit(), GetSpellInfo()->Effects[EFFECT_0].BasePoints, TRIGGERED_CAST_DIRECTLY);
        }
