...
public:
    ...
    struct instance_zulfarrak_InstanceMapScript : public InstanceScript
    {
        ...
        uint32 GahzRillaEncounter = NOT_STARTED;
        uint64 ZumrahGUID;
        uint64 BlyGUID;
        uint64 WeegliGUID;
        uint64 OroGUID;
        ...
        virtual void Update(uint32 diff)
        {
            switch (PyramidPhase)
            {
                ...
                case PYRAMID_ARRIVED_AT_STAIR:
                    SetData(EVENT_PYRAMID, PYRAMID_WAVE_0);
                    minor_wave_Timer = 5000;
                    break;
                case PYRAMID_WAVE_0:
        ...
        void MoveNPCIfAlive(uint32 entry, float x, float y, float z, float o)
        {
           if (auto npc = instance->GetCreature(GetData64(entry)))
           {
               if (npc->isAlive())
               {
                    ...
                    if (GetData(EVENT_PYRAMID) == PYRAMID_KILLED_ALL_TROLLS) {
                        npc->GetMotionMaster()->MovePoint(66, x, y, z);
                    }
                    else {
        ...
        void SpawnPyramidWave(uint32 wave)
        {
            for (int i = 0; i < pyramidSpawnTotal; i++)
            {
                ...
                {
                    Position pos = { pyramidSpawns[i][2], pyramidSpawns[i][3], 9.87f, 0 };
            ...
            for (uint32 addCount = 0; addCount<count && !addsAtBase.empty(); addCount++)
            {
                if (auto add = instance->GetCreature(*addsAtBase.begin()))
                {
                    if (add->isAlive()) {
                        ...
                        add->setDeathState(JUST_RESPAWNED);
                        add->UpdateObjectVisibility();
                        add->GetMotionMaster()->MovePath(PATH_ADDS, false);
                        movedadds.push_back(add->GetGUID());
                    }
                }
