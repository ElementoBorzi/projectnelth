...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* areaTrigger)
    {
        ...
        if (player->HasAura(82724) || player->HasAura(88858) || player->HasAura(93286) || player->HasAura(93287) || player->HasAura(93288))
            return false;
        ...
        {
        case 6274:
            if (auto c = player->SummonCreature(144951, { -80.842f, 1020.651f, 199.492f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            player->SetUnderACKmountAdvanced();
            player->SetSkipOnePacketForASH(true);
            player->JumpTo(55.f, 40.f, true, true);
            player->AddUnitMovementFlag(MOVEMENTFLAG_FALLING);
            break;
        case 6275:
            if (auto c = player->SummonCreature(144951, { -255.214f, 849.186f, 199.491f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            ...
            break;
        case 6276:
            if (auto c = player->SummonCreature(144951, { 155.783f, 846.531f, 199.491f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            ...
            break;
        case 6277:
            if (auto c = player->SummonCreature(144951, { -15.109f, 1020.446f, 199.492f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            ...
            break;
        case 6278:
            if (auto c = player->SummonCreature(144951, { -18.136f, 609.507f, 199.492f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            ...
            break;
        case 6279:
            if (auto c = player->SummonCreature(144951, { 155.898f, 780.698f, 199.492f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            ...
            break;
        case 6280:
            if (auto c = player->SummonCreature(144951, { -255.230f, 783.763f, 199.492f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
            ...
            break;
        case 6290:
            if (auto c = player->SummonCreature(144951, { -83.918f, 609.413f, 199.491f, 0.f }))
            {
                player->SetOrientation(player->GetAngle(c));
                c->DespawnOrUnsummon();
            }
...
public:
    ...
    struct npc_slipstream_alakirAI : public PassiveAI
    {
        ...
        void PassengerBoarded(Unit* who, int8 seatId, bool apply)
        {
            if (!(who->HasAura(82724) || who->HasAura(88858) || who->HasAura(93286) || who->HasAura(93287) || who->HasAura(93288)))
            {
                if (apply)
