...
public:
    ...
    struct instance_throne_of_the_four_winds_InstanceMapScript: public InstanceScript
    {
        ...
        void OnGameObjectCreate(GameObject* go)
        {
            ...
            case GOB_ALAKIR_WINDWALL:
            {
                go->SetGoState(GO_STATE_ACTIVE_ALTERNATIVE);
                if (IsDone(DATA_CONCLAVE_OF_WIND_EVENT))
                {
                    TC_LOG_ERROR("sql.sql", "wind event complete, setting wall.");
                }
                else
                    TC_LOG_ERROR("sql.sql", "wind event not complete, state: %u", GetBossState(DATA_CONCLAVE_OF_WIND_EVENT));

                windWall_guid = go->GetGUID();
                break;
            }
                default:
                    break;
        ...
        bool SetBossState(uint32 data, EncounterState state)
        {
            ...
            {
                //TC_LOG_ERROR("sql.sql", "failed setting boss state %u to %u", data, uint32(state));
                return false;
            }
