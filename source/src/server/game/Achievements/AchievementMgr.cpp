    ...
    class AchievementChatBuilder
    {
        public:
            ...
            void operator()(WorldPacket& data, LocaleConstant loc_idx)
            {
                ...
                std::string text = bct ? bct->GetText(loc_idx, _player->getGender()) : "";
                data << uint8(_msgType);
                data << uint32(bct ? Language(bct->Language) : LANG_UNIVERSAL);
                data << uint64(_player->GetGUID());
                data << uint32(5);
                data << uint64(_player->GetGUID());
                data << uint32(text.size()+1);
                data << text.c_str();
                data << uint8(0);
                data << uint32(_achievementId);
            }
...
bool AchievementCriteriaData::IsValid(AchievementCriteriaEntry const* criteria)
{
    ...
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_ACRHAEOLOGY_COMPLETE_PROJECT:
            return true;
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT:
            ...
            return true;
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM:
            if (team.team != ALLIANCE && team.team != HORDE)
            {
                TC_LOG_ERROR("sql.sql", "Table `achievement_criteria_data` (Entry: %u Type: %u) for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM (%u) has unknown team in value1 (%u), ignored.",
...
bool AchievementCriteriaData::Meets(uint32 criteria_id, Player const* source, Unit const* target, uint32 miscValue1 /*= 0*/) const
{
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM:
            return source->GetOTeam() == team.team;
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_DRUNK:
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_ACRHAEOLOGY_COMPLETE_PROJECT:
        {
            return (miscValue1 == archaeology_project.itemQuality);
        }
        default:
            break;
    }
...
template<class T>
void AchievementMgr<T>::ResetAchievementCriteriaByEntry(uint32 entry, bool evenIfCriteriaComplete)
{
    if (AchievementCriteriaEntry const* achievementCriteria = sAchievementMgr->GetAchievementCriteria(entry))
    {
        AchievementEntry const* achievement = sAchievementMgr->GetAchievement(achievementCriteria->achievement);
        if ((IsCompletedCriteria(achievementCriteria, achievement) && !evenIfCriteriaComplete) || HasAchieved(achievement->ID))
            return;
...
template<class T>
void AchievementMgr<T>::CompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, Player* referecePlayer)
{

}

template<>
void AchievementMgr<Player>::CompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, Player* referencePlayer)
{
    // disable for gamemasters with GM-mode enabled
    if (GetOwner()->isGameMaster())
        return;

    AchievementEntry const* achievement = sAchievementStore.LookupEntry(achievementCriteria->achievement);
    if (!achievement)
        return;

    if (!IsCompletedCriteria(achievementCriteria, achievement))
    {
        SetCriteriaProgress(achievementCriteria, 1, GetOwner(), PROGRESS_ACCUMULATE);
        CompletedCriteriaFor(achievement, GetOwner());
    }
}

template<>
void AchievementMgr<Guild>::CompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, Player* referecePlayer)
{
    AchievementEntry const* achievement = sAchievementStore.LookupEntry(achievementCriteria->achievement);
    if (!achievement || !referecePlayer)
        return;

    if (!IsCompletedCriteria(achievementCriteria, achievement))
    {
        SetCriteriaProgress(achievementCriteria, 1, NULL, PROGRESS_ACCUMULATE);
        CompletedCriteriaFor(achievement, referecePlayer);
    }
}
    ...
    if (achievementResult)
    {
        do
        {
            ...
            if (AchievementReward const* reward = sAchievementMgr->GetAchievementReward(achievement))
                if (uint32 titleId = reward->titleId[achievement->ID == 1793 ? GetOwner()->getGender() : (GetOwner()->GetOTeam() == ALLIANCE ? 0 : 1)])
...
void AchievementMgr<Guild>::LoadFromDB(PreparedQueryResult achievementResult, PreparedQueryResult criteriaResult)
{
    if (achievementResult)
    {
        do
        {
            ...
            CompletedAchievementData& ca = m_completedAchievements[achievementid];
            ca.date = time_t(fields[1].GetUInt32());
            /// this is weird, guids are saved with ',' as seperator
            /// i'll keep it as it is, don't touch working things
            Tokenizer guids(fields[2].GetString(), ' ');
            for (uint32 i = 0; i < guids.size(); ++i)
                ca.guids.insert(MAKE_NEW_GUID(strtoul(guids[i], nullptr, 10), 0, HIGHGUID_PLAYER));
...
static const uint32 achievIdByArenaSlot[MAX_NORMAL_ARENA_SLOT] = { 1057, 1107, 1108 };
 ...
 */
template<class T>
void AchievementMgr<T>::UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1 /*= 0*/, uint64 miscValue2 /*= 0*/, uint64 miscValue3 /*= 0*/, Unit const* unit /*= NULL*/, Player* referencePlayer /*= NULL*/, GameObject* go /*= NULL*/)
{
    ...
    for (auto i = achievementCriteriaList.begin(); i != achievementCriteriaList.end(); ++i)
    {
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_TEAM_RATING:
            {
                ...
                {
                    for (uint32 arena_slot = 0; arena_slot < MAX_NORMAL_ARENA_SLOT; ++arena_slot)
                    {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_REACH_BG_RATING:
            {
                if (miscValue1)
                    ...
                    SetCriteriaProgress(achievementCriteria, referencePlayer->GetRBGPersonalRating(), referencePlayer, PROGRESS_HIGHEST);
                break;
            }
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ARCHAEOLOGY_PROJECTS:
            {
                SetCriteriaProgress(achievementCriteria, miscValue3, referencePlayer, PROGRESS_ACCUMULATE);
                break;
            }
            case ACHIEVEMENT_CRITERIA_TYPE_CATCH_FROM_POOL:
            {
                // http://www.wowhead.com/achievement=5036/thats-a-lot-of-bait#comments
                // catching 2 or more "items" (suckerfish, quest item, etc) in a single "catch" still only counts as 1 tick
                SetCriteriaProgress(achievementCriteria, 1, referencePlayer, PROGRESS_ACCUMULATE);
                break;
            }
...
bool AchievementMgr<T>::IsCompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, AchievementEntry const* achievement)
{
    ...
    switch (AchievementCriteriaTypes(achievementCriteria->type))
    {
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ARCHAEOLOGY_PROJECTS:
            return progress->counter >= achievementCriteria->archaeology_project.itemQuantity;
        case ACHIEVEMENT_CRITERIA_TYPE_EARNED_PVP_TITLE:
             return progress->counter >= 1;
...
void AchievementMgr<T>::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint64 changeValue, Player* referencePlayer, ProgressType ptype)
{
    ...
    if (referencePlayer && criteriaComplete && achievement->flags & ACHIEVEMENT_FLAG_SHOW_CRITERIA_MEMBERS && !progress->CompletedGUID)
...
void AchievementMgr<Player>::CompletedAchievement(AchievementEntry const* achievement, Player* referencePlayer)
{
    ...
    if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_REACH | ACHIEVEMENT_FLAG_REALM_FIRST_KILL | ACHIEVEMENT_FLAG_REALM_FIRST_GUILD))
        if (sWorld->getBoolConfig(CONFIG_REALMFIRST_BLOCK_FOR_STAFF))
            if (auto s = referencePlayer->GetSession())
                if (s->GetSecurity() >= SEC_MODERATOR)
                {
                    TC_LOG_DEBUG("achievement", "AchievementMgr<Player>::CompletedAchievement: [Player %s blocked due to staff level account] Achievement %u", referencePlayer->GetGUID(), achievement);
                    return;
                }
...
void AchievementMgr<Guild>::SendAchievementInfo(Player* receiver, uint32 achievementId /*= 0*/) const
{
    ...
    for (auto itr = criteria->begin(); itr != criteria->end(); ++itr)
    {
...
template<class T>
bool AchievementMgr<T>::CanUpdateCriteria(AchievementCriteriaEntry const* criteria, AchievementEntry const* achievement, uint64 miscValue1, uint64 miscValue2, uint64 miscValue3, Unit const* unit, Player* referencePlayer, GameObject* go /*= NULL*/)
{
    ...
    if ((achievement->requiredFaction == ACHIEVEMENT_FACTION_HORDE    && referencePlayer->GetOTeam() != HORDE) ||
        (achievement->requiredFaction == ACHIEVEMENT_FACTION_ALLIANCE && referencePlayer->GetOTeam() != ALLIANCE))
    {
...
template<class T>
bool AchievementMgr<T>::RequirementsSatisfied(AchievementCriteriaEntry const* achievementCriteria, uint64 miscValue1, uint64 miscValue2, uint64 miscValue3, Unit const* unit, Player* referencePlayer, GameObject* go /*= NULL*/) const
{
    switch (AchievementCriteriaTypes(achievementCriteria->type))
    {
        case ACHIEVEMENT_CRITERIA_TYPE_ACCEPTED_SUMMONINGS:
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_DEATH:
        {
            ...
            bool notfit = false;
            for (int j = 0; j < MAX_NORMAL_ARENA_SLOT; ++j)
            {
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_EARNED_PVP_TITLE:
            ...
            {
                case 6789:        // Champion  of  the  Naaru  432
                case 9058:        // WoW's  4th  Anniversary  2398
                case 11238:        // Jade  Tiger  3636
                case 12562:        // WoW's  5th  Anniversary  4400
                case 16082:        // WoW's  6th  Anniversary  5512
                case 17804:        // WoW's  7th  Anniversary  5863
                case 18500:        // WoW's  8th  Anniversary  6131
                    return false;
                default:
                    break;
            }
...
bool AchievementMgr<T>::AdditionalRequirementsSatisfied(AchievementCriteriaEntry const* criteria, uint64 miscValue1, uint64 miscValue2, Unit const* unit, Player* referencePlayer) const
{
    for (uint8 i = 0; i < MAX_ADDITIONAL_CRITERIA_CONDITIONS; ++i)
    {
        ...
        {
            case ACHIEVEMENT_CRITERIA_ADDITIONAL_CONDITION_UNK2:
                // Guild Gladiators
                if (reqValue == 12676)
                    if (!referencePlayer || !referencePlayer->HasAchieved(2091) || referencePlayer->GetReputation(PLAYER_GUILD_REPUTATION) < 21000) // Gladiator
                        return false;
                // Guild Duelists
                if (reqValue == 12673)
                    if (!referencePlayer || !referencePlayer->HasAchieved(2092) || referencePlayer->GetReputation(PLAYER_GUILD_REPUTATION) < 21000) // Duelist
                        return false;
                // Guild Rivals
                if (reqValue == 12674)
                    if (!referencePlayer || !referencePlayer->HasAchieved(2093) || referencePlayer->GetReputation(PLAYER_GUILD_REPUTATION) < 21000) // Rival
                        return false;
                // Guild Challengers
                if (reqValue == 12675)
                    if (!referencePlayer || !referencePlayer->HasAchieved(2090) || referencePlayer->GetReputation(PLAYER_GUILD_REPUTATION) < 21000) // Challenger
                        return false;
                // Champion of the Naaru
                // TODO: implement this, achievementcriteria: 6789
                if (reqValue == 6689)
                    return false;
                break;
            ...
            case ACHIEVEMENT_CRITERIA_ADDITIONAL_CONDITION_TARGET_ZONE: // 41
                if ((!unit || unit->GetZoneId() != reqValue) && (!referencePlayer || referencePlayer->GetZoneId() != reqValue))
                    return false;
                break;
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u achievement criteria and %u guild achievement criteria in %u ms", criterias, guildCriterias, GetMSTimeDiffToNow(oldMSTime));
}
