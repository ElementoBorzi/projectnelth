...
Map* MapInstanced::CreateInstanceForPlayer(const uint32 mapId, Player* player)
{
    ...
    uint8 groupHardMode = 0;
    uint8 playerHardMode = 0;
    uint32 playerCompletedEncounterMask = 0;
    uint32 groupCompletedEncounterMask = 0;
    ...
    else
    {
        ...
        {
            pBind = player->GetBoundInstance(GetId(), player->GetDifficulty(IsRaid()), false);
            if (!pBind)
                pBind = player->GetBoundInstance(GetId(), player->GetDifficulty(IsRaid()), true);
        }
        else
            pBind = player->GetBoundInstance(GetId(), player->GetDifficulty(IsRaid()), player->GetGroup() ? player->GetGroup()->IsNonPermanentLFGGroup() : false);
        ...
        if (group)
        {
            groupBind = group->GetBoundInstance(this, group->IsNonPermanentLFGGroup());
        }
        ...
        if (pBind && !playerHardMode)
            if ((playerCompletedEncounterMask & (playerCompletedEncounterMask ^ groupCompletedEncounterMask)) == 0)
            {
                if (groupBind)
                    pSave = groupBind->save;
            }
