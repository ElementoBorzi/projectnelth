    ...
    i_scriptLock(false)
{
    ...
    if (auto cap = sWorld->getIntConfig(CONFIG_LAYER_CAP))
        for (uint8 i = 1; i <= cap; ++i)
            layerCountContainer[i] = 0;
}
...
void Map::DeleteFromWorld(Player* player)
{
    ...
    if (sWorld->getIntConfig(CONFIG_LAYER_CAP))
        ModifyLayerCount(player->GetLayerMask(), false);
...
template<>
void Map::InitializeObject(DynamicObject* obj)
{
    obj->_moveState = MAP_OBJECT_CELL_MOVE_NONE;
}
...
bool Map::AddToMap(T *obj)
{
    ...
    obj->UpdateObjectVisibility(true);
    // need to call relocationnotifier for creature, so creature is also notified
...
{
    for (auto i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
...
{
    obj->ToGameObject()->RemoveFromWorld();
    obj->UpdateObjectVisibility(true);
...
void Map::UnloadAll()
{
    ...
    for (auto i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end();)
    {
        NGridType &grid(*i->getSource());
        ++i;
...
void Map::AddWorldObject(WorldObject* obj) 
{
    ...
    obj->SetLayerMask(obj->GetProperLayerMask(), true, this);
}
...
uint32 Map::GetLayerCount(uint8 layer)
{
    return layerCountContainer[layer];
}

uint8 Map::GetLeastPopulatedLayer(uint8 layer_to_discount)
{
    uint8 found_min_layer{ 1 };
    uint32 found_min_layer_count{ 0 };
    auto cap = uint8(sWorld->getIntConfig(CONFIG_LAYER_CAP)+1);
    ...
    for (uint8 i = 1; i <= cap; ++i)
    {
        auto count = GetLayerCount(i);
        //TC_LOG_ERROR("sql.sql", "layer %u has count %u, comparing to %u on layer %u", i, count, found_min_layer_count, found_min_layer);
        if ((count + (i == layer_to_discount) ? 1 : 0) <= found_min_layer_count)
        {
            found_min_layer_count = count;
            found_min_layer = i;
        }
    }

    //TC_LOG_ERROR("sql.sql", "settled on layer %u", found_min_layer);
    return found_min_layer;
}
...
uint32 Map::GetLevelForArea(uint32 area_id, bool max_recommended)
{
    WorldMapAreaEntry const* entry = GetMapAreaEntryForArea(area_id);

    if (entry)
        return (max_recommended ? entry->maxRecommendedLevel : entry->minRecommendedLevel);

    return 0;
    //TC_LOG_FATAL("sql.sql", "Map::GetLevelForArea could not find dbc data for area %u", area_id);
...
{
    if (GetId() == 720 && GetAreaId(x1, y1, z1) == 5766)
...
float Map::GetStaticHeight(float x, float y, float z, bool checkVMap /*= true*/, float maxSearchDist /*= DEFAULT_HEIGHT_SEARCH*/)
{
    // find raw .map surface under Z coordinates
    uint32 mapId = GetId();
    float mapHeight = VMAP_INVALID_HEIGHT_VALUE;
    ...
    if (GridMap* gmap = GetGrid(x, y))
        gridHeight = gmap->getHeight(x, y);
    else gridHeight = VMAP_INVALID_HEIGHT_VALUE;

    if (G3D::fuzzyGe(z, gridHeight - 0.05f))
        mapHeight = gridHeight;
    ...
    if (checkVMap)
    {
        VMAP::IVMapManager* vmgr = VMAP::VMapFactory::createOrGetVMapManager();
        if (vmgr->isHeightCalcEnabled())
            vmapHeight = vmgr->getHeight(mapId, x, y, z, maxSearchDist);
    }
...
bool Map::IsInWater(float x, float y, float pZ, LiquidData* data) const
{
    ...
    if (const_cast<Map*>(this)->GetGrid(x, y))
    {
        ...
        if (getLiquidStatus(x, y, pZ, MAP_ALL_LIQUIDS, liquid_ptr) & (LIQUID_MAP_IN_WATER | LIQUID_MAP_UNDER_WATER))
            return true;
    }
    return false;
}

bool Map::IsAboveWater(float x, float y, float pZ, LiquidData* data) const
{
    // Check surface in x, y point for liquid
    if (const_cast<Map*>(this)->GetGrid(x, y))
    {
        LiquidData liquid_status;
        LiquidData* liquid_ptr = data ? data : &liquid_status;
        if (getLiquidStatus(x, y, pZ, MAP_ALL_LIQUIDS, liquid_ptr) & LIQUID_MAP_ABOVE_WATER)    
            return true;
    }
    return false;
}
...
void Map::SendInitSelf(Player* player)
{
    ...
    if (Transport* transport = player->GetTransport())
        transport->BuildPassengersBlockForPlayer(player, &data);
        ...
        if (!obj->IsPermanentWorldObject())
        {
            switch (obj->GetTypeId())
            {
                ...
                case TYPEID_DYNAMICOBJECT:
                    SwitchGridContainers<DynamicObject>(obj->ToDynObject(), on);
                    break;
                default:
                    break;
            }
...
void Map::ModifyLayerCount(uint32 layer, bool add)
{
    //if (layer >= 32) TC_LOG_ERROR("sql.sql", "layer: %u", layer);

    ASSERT(layer < 32);

    if (layerCountContainer.find(layer) == layerCountContainer.end())
        layerCountContainer[layer] = 0;

    if (add)
    {
        layerCountContainer[layer] += 1;
    }
    else if (layerCountContainer[layer] > 0)
            layerCountContainer[layer] -= 1;
};

uint32 Map::GetAlivePlayerCountExceptGMs() const
{
    uint32 count = 0;
    for (MapRefManager::const_iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
...
template <>
void Map::AddToActive(Corpse* obj)
{
    AddToActiveHelper(obj);
}
...
void InstanceMap::InitVisibilityDistance()
{
    ...
    case 754: // Throne of the Four Winds
        ...
        break;
    case 720: // Firelands
        m_VisibleDistance = 430.0f;
        break;
    case 967: // Dragon Soul
        m_VisibleDistance = 280.0f;
        break;
    default:
...
bool InstanceMap::CanEnter(Player* player)
{
    ...
    if (!player->IsLoading() && IsDungeon() && GetInstanceScript() && GetInstanceScript()->IsEncounterInProgress())
    {
...

    {
        ...
        if (IsDungeon())
        {
            ...
            if (!mapSave)
            {
                ...
                mapSave = sInstanceSaveMgr->AddInstanceSave(GetId(), GetInstanceId(), Difficulty(GetSpawnMode()), 0, true, false, (group && group->IsNonPermanentLFGGroup()));
            }
            // check for existing instance binds
            InstancePlayerBind* playerBind = player->GetBoundInstance(GetId(), Difficulty(GetSpawnMode()), (group && group->IsNonPermanentLFGGroup()));

            if (group)
            {
                // solo saves should be reset when entering a group
                InstanceGroupBind* groupBind = group->GetBoundInstance(this, group->IsNonPermanentLFGGroup());
...
void InstanceMap::Update(const uint32 t_diff)
{
    ...
    if (i_data)
    {
        ...
        i_data->GetScheduler().Update(t_diff);
...
void InstanceMap::CreateInstanceData(bool load)
{
    ...
    if (load)
    {
        ...
        if (result)
        {
            ...
            i_data->SetBindToHardModeDifficulty(fields[2].GetUInt8());
            if (data != "")
            {
...
void InstanceMap::PermBindAllPlayers(Player* source)
{
    ...
    if (auto i = GetInstanceScript())
    if (i->IsChallengeModeStarted() || i->IsChallengeModeFinished())
        return;
    ...
    if (save->IsLfg())
    {
        TC_LOG_ERROR("maps", "Cannot bind player (GUID: %u, Name: %s), because instance save is marked as LFG for instance map (Name: %s, Entry: %u, InstanceId: %u)!", source->GetGUIDLow(), source->GetName().c_str(), source->GetMap()->GetMapName(), source->GetMapId(), GetInstanceId());
        return;
    }
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
        ...
        InstancePlayerBind* bind = player->GetBoundInstance(save->GetMapId(), save->GetDifficulty(), save->IsLfg());
        if (!bind || !bind->perm)
        {
