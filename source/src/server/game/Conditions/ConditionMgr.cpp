...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
    ...
    auto condition_target_index = ConditionTarget;
    auto condition_anchor{ sourceInfo.mConditionTargets[1] };


    WorldObject* object_owner{ nullptr };
    // object not present, return false

    if (condition_target_index == CONDITION_TARGET_OWNER)
    {
        switch (condition_anchor->GetTypeId())
        {
            case TYPEID_ITEM:
                if (auto base = condition_anchor->ToItem())
                    object_owner = base->GetOwner();
                        break;
            case TYPEID_UNIT:
                if (auto base = condition_anchor->ToUnit())
                    object_owner = base->GetOwner();
                        break;
            case TYPEID_PLAYER:
                if (auto base = condition_anchor->ToPlayer())
                    object_owner = base->GetOwner();
                        break;
            case TYPEID_GAMEOBJECT:
                if (auto base = condition_anchor->ToGameObject())
                    object_owner = base->GetOwner();
                        break;
            default:
                break;
        ...
        if (!object_owner)
            object_owner = condition_anchor;
        else
        {
            if (condition_target_index == CONDITION_TARGET_OWNER)
                sourceInfo.mConditionTargets[CONDITION_TARGET_OWNER] = object_owner ? object_owner : condition_anchor;
        }
    }
    WorldObject* object = sourceInfo.mConditionTargets[condition_target_index];
    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_HAS_MOUNT:
        {
                switch (object->GetTypeId())
                {
                case TYPEID_PLAYER:
                    condMeets = false;
                    if (auto base = object->ToPlayer())
                    {
                        auto mount_status_required{ ConditionValue1 };

                        if (!mount_status_required)
                        {
                            condMeets = true;
                            if (base->HasAuraType(SPELL_AURA_MOUNTED))
                                condMeets = false;
                            if (base->HasAuraType(SPELL_AURA_FLY))
                                condMeets = false;
                        }
                        else
                        {
                            if (mount_status_required == 2)//flight
                            {
                                condMeets = false;
                                if (base->HasAuraType(SPELL_AURA_FLY))
                                    condMeets = true;
                ...
                default:
                    condMeets = false;
                    TC_LOG_ERROR("sql.sql", "ConditionMgr.cpp: CONDITION_HAS_MOUNT target condition not set to a player.");
                    break;
                }
            break;
        }
...
void ConditionMgr::LoadConditions(bool isReload)
{
    ...
    do
    {
        ...
        if (iConditionTypeOrReference < 0)//it has a reference
        {
            ...
            //     TC_LOG_ERROR("sql.sql", "Condition %s %i has wrong data in value2 (%u)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue2);
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_ACHIEVEMENT:
        {
            ...
            if (cond->ConditionValue2 && cond->ConditionValue2 != 1)
        ...
        case CONDITION_OBJECT_ENTRY_GUID:
        {
            switch (cond->ConditionValue1)
            {
                ...
                case TYPEID_GAMEOBJECT:
                    ...
                    {
                        TC_LOG_ERROR("sql.sql", "ObjectEntryGuid condition has non existing game object template entry  (%u), skipped", cond->ConditionValue2);
                        return false;
                    }
                    if (cond->ConditionValue3)
                    {
                        if (GameObjectData const* goData = sObjectMgr->GetGOData(cond->ConditionValue3))
                        {
                            ...
                            {
                                TC_LOG_ERROR("sql.sql", "ObjectEntryGuid has guid %u set but does not match gameobject entry (%u), skipped.", cond->ConditionValue3, cond->ConditionValue2);
                                return false;
                            }
                        }
                        else
                        {
                            TC_LOG_ERROR("sql.sql", "ObjectEntryGuid has non existing gameobject guid (%u), skipped.", cond->ConditionValue3);
                            return false;
                        }
                    }
                    break;
