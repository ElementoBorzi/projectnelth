...
SpellImplicitTargetInfo::StaticData  SpellImplicitTargetInfo::_data[TOTAL_SPELL_TARGETS] =
{
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_CONE,    TARGET_CHECK_ENTRY,    TARGET_DIR_FRONT},       // 110 TARGET_UNIT_CONE_AREA_ENTRY_110
    ...
    {TARGET_OBJECT_TYPE_CORPSE, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_AREA,  TARGET_CHECK_RAID,     TARGET_DIR_NONE},        // 119 TARGET_UNK_119
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_TARGET, TARGET_SELECT_CATEGORY_AREA,    TARGET_CHECK_ENEMY,    TARGET_DIR_NONE},        // 126 only used by ID - 98399 Clump Check which is a enemy area effect
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* target) const
{
            ...
            if (GtSpellScalingEntry const* gtScaling = sGtSpellScalingStore.LookupEntry((_spellInfo->ScalingClass != -1 ? _spellInfo->ScalingClass - 1 : MAX_CLASSES - 1) * 100 + level - 1))
            {
                ...
                if (DeltaScalingMultiplier)
                {
                    ...
                    if (delta > 0)
                        ...
                        TC_LOG_ERROR("spells", "SpellEffectInfo::CalcValue: wrong DeltaScalingMultiplier '%f' for spell %d", delta, _spellInfo->Id);
                }
    ...
    else
    {
        if (caster)
        {
            ...
            if (_spellInfo->Id != 1804) // pick lock exception ? or wrong formula ?
    ...
    if (caster)
    {
        ...
        if ((!caster->IsControlledByPlayer() || caster->GetEntry() == 15438) && !_spellInfo->GetSpellScaling() && !basePointsPerLevel && _spellInfo->Attributes & SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION && _spellInfo->SpellLevel != caster->getLevel())
        {
...
float SpellEffectInfo::CalcRadius(Unit* caster, Spell* spell) const
{
    if (!HasRadius())
    {
        if (HasMaxRadius())
        {
            float radius = MaxRadiusEntry->RadiusMin;
            if (caster)
            {
    ...
    if (HasRadius() && HasMaxRadius())
        if (RadiusEntry->ID != 36 && MaxRadiusEntry->ID != 36) // dont calculate radius with radius 0.0f
...
SpellEffectInfo::StaticData SpellEffectInfo::_data[TOTAL_SPELL_EFFECTS] =
{
    ...
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_DEST}, // 106 SPELL_EFFECT_RAID_MARKER
...
bool SpellInfo::IsEffectAffectingOrTargetingArea(uint8 effectMask /*= MAX_EFFECT_MASK*/) const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (effectMask & (1 << i))
...
bool SpellInfo::IsAffectingArea(uint8 effectMask /*= MAX_EFFECT_MASK*/) const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (effectMask & (1 << i) && Effects[i].IsEffect() && (Effects[i].IsTargetingArea() || Effects[i].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA) || Effects[i].IsAreaAuraEffect()))
            return true;
    return false;
}

// checks if spell targets are selected from area, doesn't include spell effects in check (like area wide auras for example)
bool SpellInfo::IsTargetingArea(uint8 effectMask /*= MAX_EFFECT_MASK*/) const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (effectMask & (1 << i) && Effects[i].IsEffect() && Effects[i].IsTargetingArea())
            return true;
    return false;
}
...
bool SpellInfo::NeedsToBeTriggeredByCaster(uint8 effectMask /*= MAX_EFFECT_MASK*/) const
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (effectMask & (1 << i) && Effects[i].IsEffect())
        {
...
{
    return IsPassive() && !HasEffect(SPELL_EFFECT_APPLY_AURA) && !HasEffect(SPELL_EFFECT_APPLY_AREA_AURA_RAID);
}
...
{
    return AttributesEx8 & SPELL_ATTR8_BATTLE_RESURRECTION;
}
...
bool SpellInfo::IsMoveAllowedChannel() const
{
    if (!IsChanneled())
        ...
        if ((ChannelInterruptFlags & AURA_INTERRUPT_FLAG_MOVE) || (ChannelInterruptFlags & AURA_INTERRUPT_FLAG_TURNING))
            return HasAttribute(SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING);
...
bool SpellInfo::CanPierceImmuneAura(SpellInfo const* aura, const uint8 effectMask /*= MAX_EFFECT_MASK*/) const
{
    // these spells pierce all avalible spells (Resurrection Sickness for example)
    if (Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY)
        return true;

    if (!IsPositive(effectMask) && HasAura(SPELL_AURA_INTERFERE_TARGETTING))
        return true;
...
bool SpellInfo::CanDispelAura(SpellInfo const* aura) const
{
    ...
    // passive auras cant be dispelled
    if (aura->Attributes & SPELL_ATTR0_PASSIVE)
        return false;
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player) const
{
    ...
    if (saBounds.first != saBounds.second)
    {
        for (auto itr = saBounds.first; itr != saBounds.second; ++itr)
        {
            ...
            {
                return SPELL_CAST_OK;
                TC_LOG_INFO("sql.dev", "spell_%u_area: spell cast allowed by player %u", itr->second.spellId, player ? player->GetGUID() : 0);
            }
            else TC_LOG_INFO("sql.dev", "spell_%u_area: spell cast failed due to spell area requirements", itr->second.spellId);
        }
        return SPELL_FAILED_INCORRECT_AREA;
    }
    ...
    switch (Id)
    {
        ...
        case 34976:                                         // Netherstorm Flag
            return (map_id == 566 || map_id == 968) && player && player->InBattleground() ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        ...
        case 81748:                                         // Alliance Buff
        case 81744:                                         // Horde Buff
        {
            MapEntry const* mapEntry = sMapStore.LookupEntry(map_id);
            if (!mapEntry)
                return SPELL_FAILED_INCORRECT_AREA;

            return mapEntry->IsBattleground() && player && player->IsInRatedBattleground() ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        }
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    if (unitTarget)
    {
        ...
        // only spells with SPELL_ATTR3_ONLY_TARGET_GHOSTS can target ghosts except battle resurrection spells
        if (((AttributesEx3 & SPELL_ATTR3_ONLY_TARGET_GHOSTS) != 0) != unitTarget->HasAuraType(SPELL_AURA_GHOST))
        {
            ...
            else if (!HasAttribute(SPELL_ATTR8_BATTLE_RESURRECTION))
                return SPELL_FAILED_BAD_TARGETS;
        }
        ...
        if (caster != unitTarget)
        {
            if (caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (unitTarget->isSummon() && !unitTarget->isHunterPet())
                ...
                if (Mechanic == MECHANIC_DISARM)
                {
                    ...
                    else if (!(unitTarget->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID)    //mainhand
                        || unitTarget->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1)      //offhand
                        || unitTarget->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2)     //ranged
                        || unitTarget->GetEntry() == 40177)                             //Throngus never actually equips an item
                        )
                    {
                        //TC_LOG_ERROR("sql.sql", "Weapon check failure at spellinfo 1800");
                        return SPELL_FAILED_TARGET_NO_WEAPONS;
                    }
                }
            }
...
bool SpellInfo::CheckTargetCreatureType(Unit const* target) const
{
    ...
    if (target->GetEntry() == 75979)//Incorporeal affix npc is vulnerable to all types of crowd control, as much as possible
        return true;
...
uint32 SpellInfo::GetMaxTicks() const
{
	...
	for (uint8 x = 0; x < MAX_SPELL_EFFECTS; x++)
	{
			...
			switch (Effects[x].ApplyAuraName)
		{
			...
			case SPELL_AURA_OBS_MOD_HEALTH:
			case SPELL_AURA_OBS_MOD_POWER:
			case SPELL_AURA_PERIODIC_TRIGGER_SPELL_BY_CLIENT:
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell, bool baseCost) const
{
    ...
    // skip all modifiers for base cost calculation
    if (baseCost && ManaCostPercentage)
    {
        if (powerCost < 0)
            powerCost = 0;
    ...
    for (auto i = aurasPct.begin(); i != aurasPct.end(); ++i)
    {
        if (!((*i)->GetMiscValue() & schoolMask))
            continue;
        if (!((*i)->GetMiscValueB() & (1 << PowerType)))
    ...
    switch (SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch (Id)
            {
                ...
                case 99844: // blazing claw (alysrazor voracious hatchling
                case 101729:
                case 101730:
                case 101731:
                    return false;
