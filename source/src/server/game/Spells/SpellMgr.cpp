...
std::list<DiminishingGroup> GetDiminishingReturnsGroupForSpell(SpellInfo const* spellproto, bool triggered)
{
    ...
    switch (spellproto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            //root should never affect DRs
            /*
            if (spellproto->Id == 20511)
                temp.push_back(DIMINISHING_NONE);
            */
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            // Unstable affliction
                ...
                temp.push_back(DIMINISHING_NONE); // DIMINISHING_SILENCE
...
bool SpellArea::IsFitToRequirements(Player const* player, uint32 newZone, uint32 newArea) const
{
    if (!player)
    {
        TC_LOG_INFO("sql.dev", "spell_%u_area: returning false due to player not existing.", spellId);
        return false;
    }
    ...
    if (gender != GENDER_NONE)                   // not in expected gender
        ...
        {
            TC_LOG_INFO("sql.dev", "spell_%u_area: returning false due to player gender: %u not matching required gender %u", spellId, player->getGender(), gender);
            return false;
        }
    ...
    if (raceMask)                                // not in expected race
        ...
        {
            TC_LOG_INFO("sql.dev", "spell_%u_area: returning false due to player race %u not matching required race mask %u", spellId, player->getRaceMask(), raceMask);
            return false;
        }
    ...
    if (areaId)                                  // not in expected zone
        ...
        {
            TC_LOG_INFO("sql.dev", "spell_%u_area: returning false due to zone/area: newZone %u must match areaID %u, newArea %u must match areaID %u", spellId, newZone, areaId, newArea, areaId);
            return false;
        }
    ...
    if (player)
    {
        bool meets_start_condition  { bool(questStart)  };
        bool meets_end_condition    { bool(questEnd)    };

        if (questStart)                              // not in expected required quest state
        {
            uint8 status{ uint8(player->GetQuestStatus(questStart)) };
            uint8 status_flag{ uint8((1 << status)) };
            bool meets{ bool(questStartStatus & status_flag) };
            TC_LOG_INFO("sql.dev", "spell_%u_area::quest_start quest id: %u, status to end: %u, actual status:%u(%u core enum), match found: %u ", spellId, questStart, questStartStatus, status_flag, status, meets);
            meets_start_condition = meets;

        }
        if (questEnd)                                // not in expected forbidden quest state
        {
            uint8 status       { uint8(player->GetQuestStatus(questEnd))      };
            uint8 status_flag  { uint8((1 << status))                         };
            bool meets        { !(bool(questEndStatus & status_flag))         };
            TC_LOG_INFO("sql.dev", "spell_%u_area::quest_end quest id: %u, status to end: %u, actual status:%u(%u core enum), match found: %u ", spellId, questEnd, questEndStatus, status_flag, status, meets);
            meets_end_condition = meets;
        }

        if ((questStart && !meets_start_condition) || (questEnd && meets_end_condition))
        {
            TC_LOG_INFO("sql.dev", "spell_%u_area: meets_start:%u, meets_end:%u = returning false", spellId, meets_start_condition, meets_end_condition);
            return false;
        }
        else
            TC_LOG_INFO("sql.dev", "spell_%u_area: meets_start:%u, meets_end:%u = continuing to other checks.", spellId, meets_start_condition, meets_end_condition);
    ...
    if (auto db_auraspell_id = abs(auraSpell))
    {
        bool needs_aura = (auraSpell > 0);
        bool has_aura = player->HasAura(db_auraspell_id);
            if (needs_aura != has_aura)
            {
                TC_LOG_INFO("sql.dev", "spell_%u_area: false due to aura_spell %u presence requirement = %u not matching presence %u", spellId, db_auraspell_id, needs_aura, has_aura);
                return false;
            }
    }
    ...
    switch (spellId)
    {
        case 91604: // No fly Zone - Wintergrasp
        {
            ...
            {
                TC_LOG_INFO("sql.dev", "spell_%u_area: false due to no-flying in wintergrasp", spellId);
                return false;
            }
            break;
        }
    ...
    TC_LOG_INFO("sql.dev", "spell_%u_area true after clearing all checks.", spellId);
    return true;
}
...
void SpellMgr::LoadSpellAreas()
{
    ...
    do
    {
        ...
        uint32 spell = fields[0].GetUInt32();
        SpellArea spellArea;
        spellArea.spellId             = spell;                              //[0] in db
        spellArea.areaId              = fields[1].GetUInt32();              //[1] in db
        spellArea.questStart          = fields[2].GetUInt32();              //[2] in db
        //"quest_start_active" Unused in core, unmentioned by TC.           //[3] in db
        spellArea.questEnd            = fields[5].GetUInt32();              //[4] in db
        spellArea.auraSpell           = fields[6].GetInt32();               //[5] in db
        spellArea.raceMask            = fields[7].GetUInt32();              //[6] in db
        spellArea.gender              = Gender(fields[8].GetUInt8());       //[7] in db
        spellArea.autocast            = fields[9].GetBool();                //[8] in db
        spellArea.questStartStatus    = fields[3].GetUInt32();              //[9] in db
        spellArea.questEndStatus      = fields[4].GetUInt32();              //[10] in db
...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    for (uint32 i = 0; i < mSpellInfoMap.size(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
        case 58984: // Shadowmeld trigger temp vanish effect
            spellInfo->Effects[EFFECT_1].Effect = SPELL_EFFECT_TRIGGER_SPELL;
            spellInfo->Effects[EFFECT_1].TriggerSpell = 62199;
            break;
        case 62199: // Shadowmeld Temp improved stealth
            spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(574); // 200 ms duration
            spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_APPLY_AURA;
            spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_STEALTH;
            spellInfo->Effects[EFFECT_0].BasePoints = 1000; // From Vanish spell
            break;
        case 76523: // Windwalk
        case 90691:
            spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
            break;
        case 76036: // Water Infused Blades
            spellInfo->ProcChance = 50;
            break;
        case 110310: // Dampening
            spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_HEALING_DONE_PERCENT;
            spellInfo->Effects[EFFECT_0].BasePoints = -1;
            spellInfo->Effects[EFFECT_1].ApplyAuraName = SPELL_AURA_MOD_HEALING_PCT;
            spellInfo->Effects[EFFECT_1].BasePoints = -1;
            spellInfo->StackAmount = 100;
            break;
        case 1852://gm silence(.freeze)
            spellInfo->Attributes |= (SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY | SPELL_ATTR0_HIDE_IN_COMBAT_LOG);
            spellInfo->AttributesEx2 |= (SPELL_ATTR2_CAN_TARGET_DEAD | SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE);
            spellInfo->AttributesEx3 |= (SPELL_ATTR3_DEATH_PERSISTENT | SPELL_ATTR3_IGNORE_HIT_RESULT | SPELL_ATTR3_UNINTERRUPTIBLE | SPELL_ATTR3_ONLY_TARGET_PLAYERS | SPELL_ATTR3_STACK_FOR_DIFF_CASTERS);
            spellInfo->AttributesEx6 |= (SPELL_ATTR6_CAN_TARGET_UNTARGETABLE);
            spellInfo->AttributesEx7 |= SPELL_ATTR7_IGNORE_DURATION_MODS;
            break;
            case 698://warlock ritual of summoning
            case 10059:
            case 11416:
            case 11417:
            case 11418:
            ...
            case 64321: // Potent Pheromones (Freya)
                ...
                break;
            case 48391: // Druid Owlkin Frenzy dont remove on shapeshift
                spellInfo->Stances = 0;
                break;
            ...
            case 49345: // Call Emerald Drake
                ...
                break;
            /*
                [DUNGEON] SHADOWFANG KEEP - SPELLS
            */
            case 93722:
            case 93737:
                spellInfo->AttributesEx &= ~SPELL_ATTR1_CHANNEL_TRACK_TARGET;
                break;
            case 91220://cowering roar of undead in courtyard
                spellInfo->PreventionType = SPELL_PREVENTION_TYPE_SILENCE;
                spellInfo->ProcFlags = 0x000A22A8;
                ...
                spellInfo->Category = 43;
                break;
            case 96345://Pistol barrrage with category recovery time
            case 93707://godfrey - summon ghouls
            ...
            /*
                Ashbury Asphyxiate:
                93422 - Stuns players, scripted to lift and move players.
                93423/93710 - spell casted by boss, scripted to periodically cast 93424 as damage.
                93424 - periodic damage triggered by main spell. damage is calculated in main spell's scripts.
            */
            case 93423:
            case 93710:
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_6_SECONDS);
            case 93424://Damage dealt each tick, Description: You are Asphyxiated, inflicting Physical damage every second!
                spellInfo->ChannelInterruptFlags = 0;
                spellInfo->InterruptFlags = 0;
                spellInfo->AuraInterruptFlags = 0;
                spellInfo->AttributesEx4 |= SPELL_ATTR4_IGNORE_RESISTANCES;
            ...
            */
            ...
            case 86881:
                ...
                //spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_CHANNEL_TARGET;
                //spellInfo->Effects[EFFECT_1].TargetB = NULL;
                //spellInfo->AttributesEx = (SPELL_ATTR1_CHANNELED_2 | SPELL_ATTR1_CHANNEL_TRACK_TARGET);
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                break;

            case 81638://periodic triggers 81637
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                break;
            case 81637://triggered by 81637 every 100 ms
            case 92012://summon a shardling
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_DEST;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                break;
            case 86838: //IMPENDING DOOOOOOOOOOOOOOOOOOOOOM
                spellInfo->AttributesEx |= (SPELL_ATTR1_CHANNELED_1 | SPELL_ATTR1_CHANNELED_2 | SPELL_ATTR1_NO_THREAT | SPELL_ATTR1_DONT_DISPLAY_IN_AURA_BAR | SPELL_ATTR1_CHANNEL_DISPLAY_SPELL_NAME | SPELL_ATTR1_CHANNEL_TRACK_TARGET);
                spellInfo->AttributesEx6 = (SPELL_ATTR6_UNK26);
                spellInfo->AttributesEx8 = (SPELL_ATTR8_UNK17);
                spellInfo->Category = 46;
                spellInfo->DmgClass = SPELL_DAMAGE_CLASS_MAGIC;
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_APPLY_AREA_AURA_PARTY;
                break;
            case 79922://flayer
                spellInfo->AuraInterruptFlags |= (AURA_INTERRUPT_FLAG_CAST | AURA_INTERRUPT_FLAG_MOVE | AURA_INTERRUPT_FLAG_JUMP | AURA_INTERRUPT_FLAG_TURNING);
                spellInfo->InterruptFlags |= SPELL_INTERRUPT_FLAG_MOVEMENT;
                spellInfo->ChannelInterruptFlags = 0x00007C0C;
                break;
            /*
                THE STONECORE - END SPELLS
            */
            ...
            case 86292: //ertan cyclone's area damage aura
            case 93991:
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_2].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                break;
            case 86911: // Unstable Grounding Field
            case 86930: // Supremacy of the storm            
            case 87622:
            case 93993:
                spellInfo->AttributesEx3 = SPELL_ATTR3_UNINTERRUPTIBLE;
                break;
            case 88194: // Chilling Blast
            case 92759: // Chilling Blast
                spellInfo->AttributesEx4 |= SPELL_ATTR4_IGNORE_RESISTANCES;
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                spellInfo->AttributesEx3 = SPELL_ATTR3_UNINTERRUPTIBLE;
                break;
            ...
            case 88850:
                ...
                break;
            case 73564:
                spellInfo->AttributesCu |= SPELL_ATTR0_CAST_TRACK_TARGET;
                break;
            case 81642:
                //spellInfo->AttributesEx5 |= SPELL_ATTR5_DONT_TURN_DURING_CAST;
                break;
            case 84768://augh smoke bomb
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_3_SECONDS);
            ...
            case 83131: //summon shockwave N base research on http://www.youtube.com/watch?v=CtTepsVPpRQ
                spellInfo->Effects[EFFECT_0].RadiusEntry = NULL;
                spellInfo->AttributesEx2 = NULL;
            ...
            case 89874:
                ...
                // Rajh
                /*
                    Aura placed on rajh to trigger the aoe sun strike
                */
            case 73872:
            case 89887:
                ...
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_150_YARDS);
                spellInfo->Effects[EFFECT_1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_150_YARDS);
                spellInfo->CategoryRecoveryTime = 0;
                spellInfo->RecoveryTime = 0;
                spellInfo->InterruptFlags = 0;
                spellInfo->ChannelInterruptFlags = 0;
                ...
                break;
                /*
                    Aura placed on players due to the sun strike cast
                */
            case 73874:
            case 90009:
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_150_YARDS);
            ...
            case 87653:
                ...
                spellInfo->CategoryRecoveryTime = 0;
                /*
                    Rework leap from a vehicle riding (caused targeting issues) to a target jump.
                    Effect 1: Id 42 (SPELL_EFFECT_JUMP_DEST)
                    BasePoints = 5  Multiple = 4.00
                    Targets (87, 0) (TARGET_DEST_DEST, NO_TARGET)
                    EffectMiscValueA = 25
                    EffectMiscValueB = 100
                */
                spellInfo->Effects[EFFECT_0].ApplyAuraName = NULL;
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_JUMP;
                spellInfo->Effects[EFFECT_0].BasePoints = 5;
                spellInfo->Effects[EFFECT_0].ValueMultiplier = 4;
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
            ...
            case 89131:
                ...
                break;
            case 89133: //Rajh firewall damage
            case 89878:
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                break;
                // Halls Of Origination spells
            case 87755://temple fireshaper eruption
                spellInfo->Effects[EFFECT_0].BasePoints *= 10;
                break;
                // Spatial anomaly - arcane energy
            case 74880:
                spellInfo->Effects[EFFECT_1].TriggerSpell = spellInfo->Effects[EFFECT_0].TriggerSpell;
                break;
            ...
            case 76608: // Disable Beacon Beams R
                ...
                //spellInfo->SpellVisual[0] = 180;
                //spellInfo->SpellVisual[1] = 180;
                ...
                spellInfo->InterruptFlags = (SPELL_INTERRUPT_FLAG_MOVEMENT | SPELL_INTERRUPT_FLAG_ABORT_ON_DMG);

                spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(CAST_TIME_10000_MS);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_45_YARDS);
                break;
                //Rajh Summoning vortex
            case 74104://aura that triggers 74105
                spellInfo->AuraInterruptFlags = 0;
                spellInfo->ChannelInterruptFlags = 0;
                spellInfo->InterruptFlags = 0;
                break;
            ...
            case 88100:
                ...
                spellInfo->AuraInterruptFlags = 0;
                break;
                //Setesh void lord void burst
            case 77475:
            case 89847:
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                break;
                //Anraphet Alpha beams
                //case 76904:
                //case 76956:
            case 76184:
                spellInfo->InterruptFlags = 0;
                ...
                spellInfo->AuraInterruptFlags = 0;
                break;
            case 91177://Alpha beams
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                break;
            case 77347:
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_11_SECONDS);
                break;
            case 77349:
            ...
            case 77335:
                ...
                break;
            /*
                END HALLS OF ORIGINATION
            */
                /*
                    corla dragon beam visual possibilities
                */
            case 75677:spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 75610:
                spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(CAST_TIME_0_MS);
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
            ...
            */
            ...
            case 21009:
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_ADD_FLAT_MODIFIER;
                spellInfo->Effects[EFFECT_0].MiscValue = 23;
                break;
                */
            case 3286:
                spellInfo->Effects[EFFECT_0].TargetA =  SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
            ...
            case 97547: // Solar Beam
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                break;
            case 80964: // Skull Bash actual spell, not effects
            case 80965: // Skull Bash actual spell, not effects
            ...
            case 87023: // Cauterize
                spellInfo->AttributesEx10 |= SPELL_ATTR10_IGNORE_RESILIENCE;
                break;
            ...
            /////////////////////
            case 92815:
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_9_SECONDS);
                break;
            case 79025://magmaw chained visual, left
            case 79026://magmaw chained visual, right
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
            ...
            //////////////////////////
            case 88226:
                spellInfo->Effects[0].TargetA = TARGET_UNIT_NEARBY_ALLY;
                spellInfo->Effects[0].TargetB = NULL;
                break;
            //////////////////////////////
            // END BASTION OF TWILIGHT  //
            //////////////////////////////
            ...
            case 99335:
                ...
                spellInfo->Effects[EFFECT_1].TargetB = TARGET_UNIT_CASTER;
                break;
                /*Alysrazor fire tornado*/
            case 99817: //aura casted
            case 99816: //Normal 10:
            case 100733: //Normal 25: 
            case 100734: //Heroic 10: 
            case 100735: //Heroic 25: 
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                spellInfo->Effects[EFFECT_1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS);
                break;
            case 100445:/*Unstable Magma "Superheated"*/
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CHANNEL_TARGET;
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_CHANNEL_TARGET;
                spellInfo->Effects[EFFECT_2].TargetA = TARGET_UNIT_CHANNEL_TARGET;
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_CHANNEL_TARGET;
                spellInfo->Effects[EFFECT_1].TargetB = TARGET_UNIT_CHANNEL_TARGET;
                spellInfo->Effects[EFFECT_2].TargetB = TARGET_UNIT_CHANNEL_TARGET;
                break;
            ////////////////////////////
            ...
            // End Time
            // /////////////////////////
            case 109289://baine quest credit, muscle spell, no visual
                spellInfo->AuraInterruptFlags = 0;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
                break;
            case 109291://baine quest credit, visual dummy
                spellInfo->AuraInterruptFlags = 0;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_10_SECONDS);
                break;

            case 102149:    //Echo of tyrande - moonlance trigger periodic damage/stun
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_NEGATIVE_EFF1;
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                break;
            case 102534://Hourglass suppression
                //spellInfo->Effects[EFFECT_0].TargetB = NULL;
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_TARGET_ANY;
                break;
            case 102414:
                ...
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_DEST_TARGET_ENEMY;
                break;
            case 101842:
            ...
            case 101841:
                spellInfo->AuraInterruptFlags = 0;
                break;
            case 101337://jaina frostblade periodic aura
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_2_YARDS);
                break;
            case 102156:
            ...
            case 109307:
                ...
                break;
            case 105285://hagara ice lance targeting
                spellInfo->Attributes |= SPELL_ATTR0_CANT_CANCEL;
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_NEGATIVE;
                break;
                /*
                    END HAGARA ENCOUNTER SPELLS
                */
                /*
                    ULTRAXION ENCOUNTER SPELLS
                */

            case 105579:
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_8_YARDS);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_8_YARDS);

                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                spellInfo->AttributesEx3 &= ~SPELL_ATTR3_IGNORE_HIT_RESULT;
                break;
                /*
                    END ULTRAXION ENCOUTNER SPELLS
                */
            ...
            case 109364:
                spellInfo->MaxAffectedTargets = 1;
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_NEGATIVE;
                //spellInfo->AreaGroupId = 5892;
                break;
            case 105244://Absorb blood, tick intervals made faster so that runaway blobs don't outrun the buff collection.
                spellInfo->Effects[EFFECT_0].Amplitude = 350;
                break;
            ...
            case 106040: // Spellweaving
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
                spellInfo->ProcFlags |= (PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG | PROC_FLAG_DONE_OFFHAND_ATTACK | PROC_FLAG_DONE_MAINHAND_ATTACK);
                break;
            case 107835:
            case 109847:
            case 109849:
            ...
            case 106200:
                ...
                break;
            case 108096:
                spellInfo->Effects[0].TargetA = NULL;
            ...
            case 106108:
                ...
                break;
            case 105636:
            case 109558: //yorsajh minion summon attack
                spellInfo->InterruptFlags = 0;
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
                break;
            case 105637://yorsajh minion summon giving low dmg
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                break;
            case 109527://kalecgos portal opening
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 105863:
            ...
            case 110307:
                ...
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                break;
            case 107770://DS Pure water - dispel dust storm
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->Effects[EFFECT_1].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                break;
                /*
                    DS madness aspect beams
                */
            case 106588:
            case 106600:
            case 106613:
            ...
            case 91104:
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                /*
                    spellInfo->Effects[EFFECT_0].RadiusEntry       = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_55_YARDS);
                    spellInfo->Effects[EFFECT_0].MaxRadiusEntry    = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_55_YARDS);
                */
                break;
            case 87472:
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_1_SECONDS);
                break;
            case 88239:
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                ...
                */
                ...
                case 93247:
                    spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
                    break;
                case 88214://lightning strike damage
                case 95764:
                    spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                    break;
                case 88230://lightning area visuals
                    spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_TARGET_ANY;
                    spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                    /*
                    spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
            ...
            case 100865:
                spellInfo->AttributesEx3 |= SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50_YARDS);
                break;
            case 101867:
                spellInfo->AttributesEx4 |= SPELL_ATTR4_FIXED_DAMAGE;
                break;
            ...
            case 96434:
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_15_SECONDS);
                break;
            case 97001://tiki torch AOE
            case 97352:
                break;
            case 96684:
                spellInfo->Effects[EFFECT_1].MiscValue = 40;
                spellInfo->Effects[EFFECT_1].MiscValueB = 40;
                break;
            case 99921:
            case 99922:
           ...
           case 68827://IOC Glaive Thrower
                ...
                break;
           case 67448://IOC Glaive Thrower
               spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
               //spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_30_YARDS);
               spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_30_YARDS);
                break;
           case 69515://IOC Glaive thrower Barrage
                spellInfo->Effects[EFFECT_0].TriggerSpell = 68829;
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_5_YARDS);
                break;
           case 68829://IOC Glaive thrower Barrage effect
            ...
            case 68169:
                ...
                break;
            case 69495: //IOC Gunship Cannon Fire
                spellInfo->Effects[EFFECT_0].TriggerSpell = 67436;
                spellInfo->Effects[EFFECT_0].BasePoints = 20000;
                spellInfo->Effects[EFFECT_1].BasePoints = 8000;
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_30_YARDS);
                break;
            case 67436: //IOC Gunship Cannon Fire
                spellInfo->Effects[EFFECT_0].BasePoints = 20000;
                spellInfo->Effects[EFFECT_1].BasePoints = 8000;
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_30_YARDS);
            ...
            case 94528: // Flare - Missile trigger
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                break;
            ...
            case 102173:
                spellInfo->MaxAffectedTargets = 1;
                break;
            case 101626://echo of baine leap visual
                spellInfo->Effects[EFFECT_0].MiscValue  = 45;
                spellInfo->Effects[EFFECT_0].MiscValueB = 45;
                break;
            case 101815://echo of baine actual destruction of platforms
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                break;
            case 101810:
            ...
            case 97779: // Lashtail Hatchling
                ...
                break;
                // [Dungeon] well of eternity start
            case 102465: // Coldflame WoE Frost Mage
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(35); // 4 seconds
                spellInfo->Effects[EFFECT_1].ApplyAuraName = SPELL_AURA_PERIODIC_TRIGGER_SPELL;
                spellInfo->Effects[EFFECT_1].Amplitude = 1000;
                spellInfo->Effects[EFFECT_1].TriggerSpell = 102464;
                break;
            ...
            case 102919:
                ...
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 108759://deathwing ds beam (woe)
            case 108809://old god ds beam (woe)
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_TARGET_ANY;
                break;
                //Portal Pull
            case 105335:
                ...
                spellInfo->Effects[EFFECT_2].Effect = SPELL_EFFECT_DUMMY;
                spellInfo->Effects[EFFECT_2].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_2].TargetB = NULL;
                break;
            case 102291://Enchanted Reflection
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 105496://Perotharn charge
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 104905://perotharn drain essence.
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                spellInfo->InterruptFlags           = 0x00000000;
                spellInfo->AuraInterruptFlags       = 0x00000000;
                spellInfo->ChannelInterruptFlags    = 0x00000000;
                break;
            case 102561://demon grip
                spellInfo->Effects[EFFECT_1].ApplyAuraName = SPELL_AURA_DUMMY;
                break;
            ...
            case 102860:
                ...
                break;
            case 107980://thrall vs asira target type
                spellInfo->Effects[0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 103565://light balls - benedictus
            case 103767://shadow balls - benedictus
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_DUMMY;
                break;
            case 103776:
            case 103648:
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_DEST;
                break;
            case 102230://Taunt on faceless brute
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_SRC_AREA_ENEMY;
                break;
            case 102215://launching ooze summon
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            ...
            case 102925:
                ...
                break;
                /*
                // [DUNGEON] HOUR OF TWILIGHT END
                */
            //Unbanish Azaloth should remove aura through immunity
            case 37834:
                spellInfo->Attributes |= SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY;
            case 36980: //AV: Archer shoot
                spellInfo->Effects[EFFECT_0].BasePoints *= 4.0;
                break;
            // Allow mount on bg teleporters
            case 66548:
            case 66549:
            case 66638:
            case 66629:
            case 54640:
                ...
                break;
            //POLL: QIRAJI BATTLE TANKS OUTSIDE AQ
            //case 25863: Black Qiraji Battle Tank is an alternate to the shop item
            //case 26655: Black Qiraji Battle Tank is the shop item
            //case 25953: Blue Qiraji Battle Tank is an archaeology grind
            case 26054:
            case 26055:
            case 26056:
                spellInfo->AreaGroupId = 0;
                break;
            case 68508://IOC: Boss Crushing Leap
                spellInfo->Effects[EFFECT_0].BasePoints *= 5.0;
                break;
            case 67280://IOC: Boss Throw Dagger
                spellInfo->Effects[EFFECT_0].BasePoints *= 5.0;
                spellInfo->Effects[EFFECT_1].BasePoints *= 5.0;
                break;
            case 21667://AV: Ivus Wrath
                spellInfo->Effects[EFFECT_0].BasePoints *= 50.0;
                break;
            case 75332://AV: Ivus Starfire
                spellInfo->Effects[EFFECT_0].BasePoints *= 12.0;
                break;
            case 96211://SOTA: Turret damage
                //spellInfo->Effects[EFFECT_0].BasePoints *= 10.0;
                break;
            case 52339://SOTA: Demolisher damage
                //spellInfo->Effects[EFFECT_0].BasePoints *= 10.0;
                break;
            case 67454://IOC: Keep cannon fire
                spellInfo->Effects[EFFECT_0].BasePoints *= 2.6666667;
                break;
            case 67439://IOC: Demolisher boulder damage
                spellInfo->Effects[EFFECT_0].BasePoints *= 10;
                break;
            case 67816://IOC: SIEGE: Ram Damage
                spellInfo->Effects[EFFECT_1].BasePoints *= 5;
                break;
            case 67798://IOC: SIEGE: Steam Rush Damage
                spellInfo->Effects[EFFECT_2].BasePoints *= 5;
                break;
            case 67459://IOC: SIEGE: Main cannon fire
                spellInfo->Effects[EFFECT_0].BasePoints *= 5;
                break;
            case 53420://IOC: SIEGE: Napalm fire replacement
                spellInfo->CategoryRecoveryTime = 8000;
                spellInfo->RecoveryTime = 8000;
                break;
            case 49716://IOC: SIEGE: Napalm fire replacement (triggered)
                spellInfo->Effects[EFFECT_0].BasePoints *= 10;
                break;
            case 66184://IOC: SIEGE: fire stream ticks
                spellInfo->Effects[EFFECT_0].BasePoints *= 33;
                break;
            case 67462://IOC: SIEGE: steam blast
            ...
            case 5246://Intimidating shout
                ...
                break;
            case 20511://Intimidating shout root
                /*
                    THIS CAUSES TRINKET ITEMS TO BE UNUSABLE DURING THE CC
                */
                /*
                spellInfo->Mechanic = MECHANIC_ROOT;
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_ROOT;
                */
                break;
            case 81261://solar beam periodic
                spellInfo->AttributesEx2 = (SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS);
                break;
            case 1515://tame pet channel
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_DUMMY;
                break;
            case 13481://Tame pet actual taming
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 80325:
            ...
            case 80886:
                spellInfo->Attributes |= SPELL_ATTR0_CANT_CANCEL;
                    break;
            case 23588: //fury warrior offhand damage
                spellInfo->Effects[EFFECT_1].BasePoints *= 2.0;
                break;
            case 66://mage invis
            case 32612:
                spellInfo->InterruptFlags |= (SPELL_INTERRUPT_FLAG_ABORT_ON_DMG);
                spellInfo->AuraInterruptFlags |= (AURA_INTERRUPT_FLAG_CAST | AURA_INTERRUPT_FLAG_MELEE_ATTACK | AURA_INTERRUPT_FLAG_DIRECT_DAMAGE | AURA_INTERRUPT_FLAG_TAKE_DAMAGE | AURA_INTERRUPT_FLAG_TAKE_DAMAGE2);
                break;
            case 6615:
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_6_SECONDS);
                break;
            ...
            case 58683://Savage Combat debuff from combat rogues
                spellInfo->AuraInterruptFlags = AURA_INTERRUPT_FLAG_LEAVE_COMBAT | AURA_INTERRUPT_FLAG_MOUNT/*Backup scenario in case leaving combat fails to remove it.*/;
                spellInfo->AttributesEx5 |= SPELL_ATTR5_HIDE_DURATION;
            ...
            case 12654:
            case 49016://
            case 50536: //Unholy Blight, temporary until the AttributesEx10 unknown 1 check in SpellauraEffects.cpp for applying resil is confirmed safe to change
                spellInfo->AttributesEx10 |= SPELL_ATTR10_IGNORE_RESILIENCE;//Ignore Resilence (Only For Auras Currently)
                break;              
            case 61442://Unused tailor carpet mounts
            case 61444:
            case 61446:
                spellInfo->SpellVisual[0] = 12844;
                spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(16); // 1.5 second
                spellInfo->InterruptFlags = 0x0000001F;
                spellInfo->AuraInterruptFlags = 0x00000000;
                spellInfo->ChannelInterruptFlags = 0x00000000;
                spellInfo->Mechanic = MECHANIC_MOUNT;
                spellInfo->SpellIconID = 3833;
                spellInfo->Effects[EFFECT_0].MiscValueB = 247;
                spellInfo->Effects[EFFECT_0].TriggerSpell = 3908;
                spellInfo->Effects[EFFECT_1].Effect = NULL;
                spellInfo->Effects[EFFECT_1].BasePoints = NULL;
                spellInfo->Effects[EFFECT_1].TargetA = NULL;
                spellInfo->Effects[EFFECT_1].TargetB = NULL;
                spellInfo->Effects[EFFECT_1].MiscValue = NULL;
                spellInfo->Effects[EFFECT_1].MiscValueB = NULL;
                break;
            case 3714://Path of Frost
                spellInfo->AuraInterruptFlags = AURA_INTERRUPT_FLAG_TAKE_DAMAGE;
                break;
            case 6544://heroic leap
                spellInfo->Speed *= 1.6f;
                break;
            case 43680:
            case 43681:
                /*
                    Battleground Idle auras
                    Previously removed by leaving combat and taking damage.
                    This meant that a completely idle player who gets dragged into combat by nearby teammates and then maybe takes damage from an enemy could have their idle auras removed without doing anything.
                    With this change, dealing damage or clicking a flag will remove the idle aura.
                */
                spellInfo->AuraInterruptFlags = AURA_INTERRUPT_FLAG_MELEE_ATTACK | AURA_INTERRUPT_FLAG_SPELL_ATTACK | AURA_INTERRUPT_FLAG_USE;
                break;
                /*
                    [DUNGEON] Zul Aman
                */
            case 42376:
            case 42478:
                //spellInfo->Effects[0].MiscValueB = 63;
                break;
            case 42625:
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_DEST;
                break;
            case 97628://Blood worms
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_TARGET_ENEMY;//only cast on one target
                break;
            case 43648:
                spellInfo->InterruptFlags = 0x00000000;
                spellInfo->AuraInterruptFlags = 0x00000000;
            ...
            case 42630:
                ...
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_2_YARDS);
                break;
            case 43501://Siphon soul
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_NEGATIVE;//appear as debuff

                /*
                spellInfo->AttributesEx = (SPELL_ATTR1_CHANNELED_1 | SPELL_ATTR1_CANT_BE_REDIRECTED);
                spellInfo->AttributesEx2 = (SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS);
                spellInfo->AttributesEx3 = (SPELL_ATTR3_IGNORE_HIT_RESULT);
                spellInfo->AttributesEx6 = (SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS);
                */

                spellInfo->AttributesEx5 = (SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING | SPELL_ATTR5_CAN_NOT_TARGET_PETS | SPELL_ATTR5_START_PERIODIC_AT_APPLY);
                spellInfo->Effects[0].ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
                spellInfo->Effects[0].Amplitude = 200;
                break;
                /*
                     End [DUNGEON] Zul Aman
                */
                /*
                    [DUNGEON] Zul Gurub
                */
            case 15588://"Classic DMG" - Boulder roller thunderclap
            case 24050://"Classic DMG" - voodoo spirit spirit burst
                spellInfo->Effects[EFFECT_0].BasePoints *= 10;
                break;
            case 96457:
                spellInfo->AttributesEx5 |= SPELL_ATTR5_START_PERIODIC_AT_APPLY;
                break;
            case 97161://jindo shadow spike radius edited to match visual
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
            ...
            case 96559:
                spellInfo->Effects[EFFECT_1].ApplyAuraName = SPELL_AURA_DUMMY;
                break;
            case 97355://pride of bethekk minimum cast distance
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
                spellInfo->Effects[EFFECT_0].RadiusEntry = NULL;//sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
            case 97357:
            case 97358:
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ENEMY;//only cast on one target
                spellInfo->Effects[EFFECT_0].TargetB = NULL;//only cast on one target
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_TARGET_ENEMY;//only cast on one target
                spellInfo->Effects[EFFECT_1].TargetB = NULL;//only cast on one target
                break;

                case 96730://flower and venoxis stage gas
                case 97368:
                    spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
                    spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
                    break;
            //berserker stomp on chains
            case 97198:
                spellInfo->InterruptFlags = 0;
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                ...
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                break;
            //Berserker Knock Away Damage
            case 96341:
                spellInfo->Effects[EFFECT_0].BasePoints = 120;
                break;
            //Bloodlord devastating slam
            case 96739:
            case 96740:
            case 96742:
            case 97385:
            ...
            case 96761:
                ...
                break;
            //Jindo the godbreaker phasemask
            case 96689:
                spellInfo->Effects[EFFECT_1].MiscValueB = 174;
                break;
            //zanzili zombie - disease touch
            case 3234:
                /*
                    Fuck try
                */
                //spellInfo->Effects[0].BasePoints *= 800;
                break;
            //Zanzil shadow bolt:
            case 96346:
                spellInfo->InterruptFlags |= (SPELL_INTERRUPT_FLAG_MOVEMENT | SPELL_INTERRUPT_FLAG_INTERRUPT);
                break;
            //Tiki bone toss
            case 96874: //aura casting
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                break;
            case 96875:
            ...
            //Full of Meat
            case 81252:
                spellInfo->Effects[0].BasePoints *= 400;
                break;
            ...
            //case 97368:
                ...
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry     = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_0_5_YARDS);
                spellInfo->Effects[EFFECT_0].TargetA            = TARGET_UNIT_SRC_AREA_ENEMY;
                spellInfo->Effects[EFFECT_0].TargetB            = NULL;
                */
                break;
            case 96822://tiki channel
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_NEGATIVE;//appear as debuff
                break;
            case 96328://cauldron buff
                spellInfo->Attributes = SPELL_ATTR0_CASTABLE_WHILE_MOUNTED;
                break;
            case 6277://bind sight
                ...
                break;
            case 96834://boulder periodic spam
                spellInfo->Attributes |= SPELL_ATTR0_HIDE_IN_COMBAT_LOG;
                spellInfo->Attributes |= SPELL_ATTR0_HIDDEN_CLIENTSIDE;
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
                spellInfo->Effects[EFFECT_1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
                spellInfo->Effects[EFFECT_2].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
                break;
            case 97974://sanctify
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_DEST_AREA_ALLY;
                break;
            case 17228: //shadowbolt volley
            ...
            case 97978:
                ...
                spellInfo->AuraInterruptFlags = 0x00000000;
                break;
            case 97597://Spirit warrior gaze
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ENEMY;//only cast on one target
                spellInfo->Effects[EFFECT_0].TargetB = NULL;//only cast on one target
                break;
            case 97320://Spirit warrior sunder rift debuff
                //spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_DEST_AREA_ALLY;
                break;
            case 97170://Dead zone
            case 97600:
            case 101360:
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_7_YARDS);
                ...
                spellInfo->AuraInterruptFlags = 0x00000000;
                break;
            case 97173://Shadows of hakkar
                spellInfo->Effects[EFFECT_0].DamageMultiplier = 0.15f;
                break;
                /*
                     End [DUNGEON] Zul Gurub
                */

                /*IOC huge bomb (dev purposes, DO NOT APPLY)
            case 66672:
            ...
            case 61999:
                spellInfo->Effects[EFFECT_1].Effect = 0;
                break;
                // revive pet should break autoattack
            case 982:
                spellInfo->AttributesEx6 |= SPELL_ATTR6_AUTOATTACK_STOP;
                break;
                // War Academy
            case 84570:
            case 84571:
            ...
            case 1535:
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_12_YARDS);
                break;
            case 84230: // Feral Spirit Link "lost city"
            case 84231:
            case 84236:
            case 84237:
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_5_YARDS);
                break;
                // Horn of Winter, Battle Shout, Commanding Shout fix
            case 469:
            case 6673:
            case 57330:
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;           
            ...
            */
            ...
            case 88337:
                ...
                */
            case 92201:
                spellInfo->Effects[1].ApplyAuraName = SPELL_AURA_PERIODIC_TRIGGER_SPELL;
                break;
            case 92438:
                spellInfo->Effects[EFFECT_0].MiscValue = 50;
                break;
            case 92559://nightmare slow
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_30_SECONDS);
                break;
            case 91735:
            ...
            case 92618:
                ...
                break;
            case 92567://nightmare elixir
            case 92565://nightmare elixir
            //case 92113:
            case 95650://cookie challenge aura
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
                spellInfo->AreaGroupId = 1581;
                break;
            case 92277://nightmare gauntlet lightning
            case 92169://Nightmare guauntlet fire
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
                break;
            case 88177://Frost Blossom has an effect 2
            case 91274:
                spellInfo->Effects[2].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS);
                spellInfo->Effects[2].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS);
            case 88173://Fire Blossom, does not have an effect 2
            case 91275:
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS);
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS);
                spellInfo->Effects[1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS);
                spellInfo->Effects[1].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS);
                break;
            case 95495://cannonball explosion
                spellInfo->AttributesEx3 &= ~SPELL_ATTR3_IGNORE_HIT_RESULT;
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                break;
            case 89734://cookie's rotten food aura getting maze treatment
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
                break;
            case 92171: // Glubtok nightmare flames
                //spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
                break;
            case 88268:
                //helix failing to toss a bomb while on the oaf
                spellInfo->ExcludeCasterAuraSpell = NULL;
                break;
            case 91397://Glubtok firewall bunny - reworked to prevent multi-ticking
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD);
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_EFFECT_DUMMY;
                break;
            case 89652://Ogrish Motivation
                spellInfo->Effects[0].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                break;
            case 91043://Ogrish Motivation
                spellInfo->Effects[0].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                break;
            case 92747://Ogrish Motivation
                spellInfo->Effects[0].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                break;
            case 92748://Ogrish Motivation
                spellInfo->Effects[0].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS);
                break;
            case 89769:
            ...
            case 90932:
                ...
                spellInfo->Effects[1].TargetA = TARGET_UNIT_DEST_AREA_ALLY;
                spellInfo->Effects[1].TargetB = TARGET_DEST_DYNOBJ_ALLY;
                break;
            case 90933:
                spellInfo->Effects[0].ApplyAuraName = SPELL_AURA_MOD_DAMAGE_PERCENT_DONE;
            ...
            case 76097:
                ...
                break;
                /*
                    Enslaved brute and RP
                */
            case 36005:
                spellInfo->Category = 35;
                ...
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_5_YARDS);

                spellInfo->Attributes = 0;
                spellInfo->AttributesEx |= (SPELL_ATTR1_CANT_BE_REDIRECTED | SPELL_ATTR1_CANT_BE_REFLECTED);
                break;
            case 76138: //enslave gronn RP 1
                spellInfo->Effects[0].Effect = SPELL_EFFECT_KNOCK_BACK;
            case 76151: //enslave gronn RP 2
            ...
            case 76703:
                ...
                spellInfo->PreventionType = SPELL_PREVENTION_TYPE_NONE;
                break;
            case 74884: //enslave gronn cosmetic 2
            case 74867: //enslave gronn cosmetic 1
                spellInfo->Effects[0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[0].TargetB = TARGET_UNIT_TARGET_ANY;
                /*
                spellInfo->ChannelInterruptFlags = 0;
                spellInfo->AttributesEx = (SPELL_ATTR1_NO_THREAT | SPELL_ATTR1_DONT_DISPLAY_IN_AURA_BAR);
                */
                break;
            ...
            case 74039:
                ...
                spellInfo->CategoryRecoveryTime = 2000;
                break;
            case 91058://summon welps
                spellInfo->Effects[2].Effect = SPELL_EFFECT_DUMMY;
                break;
            case 76626://Azureborne warlord - conjure twisted visages
            case 90695:
            ...
            case 90858:
                break;
            case 76093://burning wounds
                spellInfo->RecoveryTime = 1500;
                spellInfo->CategoryRecoveryTime = 1500;
                break;
                /*
                    [DUNGEON] Grim Batol End

                */
                /*
                    [DUNGEON] Blackrock Caverns
                */
            case 63677: //"Heat wave should not miss"
                spellInfo->AttributesEx8 |= SPELL_ATTR8_CANT_MISS;
                break;
            case 14100://"https://youtu.be/ycSBxDyNJLc?t=1133 - beauty fear can be outranged"
                //spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_20_YARDS);
                break;
                /*
                    End Blackrock Caverns
                */
            case 93766:
            ...
            case 85085:
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_20_YARDS);
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_20_YARDS);
                break;
                // Storm Rune Beam AA
            case 86981:
            case 86921:
            ...
            case 88201://young drake healing well
                spellInfo->InterruptFlags = 0;
                spellInfo->Effects[0].TargetB = TARGET_UNIT_DEST_AREA_ALLY;
                spellInfo->AttributesEx3 = SPELL_ATTR3_UNINTERRUPTIBLE;
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS);
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS);
                break;
                // Grounding Field
            case 87725: //periodic invis aura that triggers aura-placing spell
                spellInfo->Effects[0].Amplitude = 250;
                break;
            case 87726: //spell that places aura on allies
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_1_SECONDS);
                spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_30_YARDS);
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_30_YARDS);
                break;
                // Grounding Field Visual Beam
            case 87517:
                spellInfo->Effects[0].TargetA = TARGET_DEST_CASTER;
                spellInfo->Effects[1].TargetA = TARGET_DEST_CASTER;
                spellInfo->Effects[2].TargetA = TARGET_DEST_CASTER;
                spellInfo->Effects[0].TargetB = TARGET_UNIT_DEST_AREA_ENTRY;
                spellInfo->Effects[1].TargetB = TARGET_UNIT_DEST_AREA_ENTRY;
                spellInfo->Effects[2].TargetB = TARGET_UNIT_DEST_AREA_ENTRY;
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50_YARDS);
                spellInfo->Effects[1].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50_YARDS);
                spellInfo->Effects[2].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50_YARDS);
                break;
                // Lighning Lash
            case 87762:
            ...
            case 96260:
                spellInfo->Effects[0].TargetA = TARGET_DEST_CASTER;
                break;
                // The Vortex Pinnacle End
                /*
                    Tolvir
                */
            case 83455:
                //spellInfo->AttributesEx3 = 0;
                spellInfo->Effects[EFFECT_0].DamageMultiplier = 0.3f;
                break;
            case 84956:
            case 84546:
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_DEST;
            ...
            case 83089:
                ...
                */
            case 94170: //ozumat grab neptulon
                spellInfo->Effects[EFFECT_0].TargetA = NULL;
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_TARGET_ANY;
                break;
            case 94171:
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                break;
            case 84037://neptulon casting pure water at ozumat
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 83525: // Blight of Ozumat
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
                break;
            case 76133: // vague des maree (throne of tides)
                ...
                */
                spellInfo->AttributesEx3 |= SPELL_ATTR3_ONLY_TARGET_PLAYERS;
                break;
            case 75690: //waterspout
                break;

            case 83931://unyielding behemoth hero landing
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                break;
            case 76604:
                //spellInfo->Effects[EFFECT_0].Amplitude = 500;
                break;
            case 76593://ground pound
            case 91468:
                //spellInfo->Effects[0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_5_YARDS);
                spellInfo->Effects[0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_8_YARDS);
                break;
                // Nazjar Geyser
            ...
            case 96311:
				...
				case 76307:
				case 91492:
				spellInfo->Effects[EFFECT_0].BasePoints = 80000000;
				break;
				// Void Rip
				case 76609:
				spellInfo->Effects[EFFECT_0].TargetB = 0;
                //spellInfo->Effects[EFFECT_2].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_25_YARDS);
				break;
                // Throne of the Tides End
				// Challenge mode spells
            case 101166:
                spellInfo->Effects[0].Amplitude = 1000;
                spellInfo->StackAmount = 999;
                break;
            case 97463:
            case 64217:
                spellInfo->StackAmount = 999;
            ...
            case 54657://Incorporeal spell
                spellInfo->AttributesEx3 |= SPELL_ATTR3_ONLY_TARGET_PLAYERS;

                spellInfo->InterruptFlags = 0x00000009;
                spellInfo->ChannelInterruptFlags = 0x00007C0C;

                spellInfo->AttributesEx |= SPELL_ATTR1_CHANNELED_2;
                spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(CAST_TIME_1500_MS);
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_1200_SECONDS);
                spellInfo->Effects[EFFECT_0].Amplitude = 500;
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_PERIODIC_TRIGGER_SPELL;
                spellInfo->Effects[EFFECT_0].TriggerSpell = 72391;
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
                spellInfo->Effects[EFFECT_0].TargetB = NULL;
                break;
            case 107801://Overflowing affix (new)
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->Attributes |= SPELL_ATTR0_CANT_CANCEL;
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_NEGATIVE_EFF1;
                spellInfo->AttributesEx8 |= SPELL_ATTR8_AURA_SEND_AMOUNT;
                break;
                /*
                23153, 23154, 23155, 23169 are afflictions used for the afflicted affix. they must be any target to prevent targeting issues.
                */
            case 23153:
                spellInfo->Effects[EFFECT_2].TargetA = TARGET_UNIT_TARGET_ANY;
            case 23169:
            ...
            case 98064:
                ...
                break;
            case 34947: // warlock pet mastery hackfix
                spellInfo->Effects[EFFECT_2].ApplyAuraName = SPELL_AURA_DUMMY;
                break;
            case 31117:
                //spellInfo->AttributesEx4 &= ~SPELL_ATTR4_FIXED_DAMAGE;
            ...
            case 755:
                ...
                break;
            case 51862: // rogue savage combat poison proc fix
            case 58413:
                spellInfo->ProcFlags |= PROC_FLAG_ON_CAST;
                break;
                // Spell "yes"
            case 34655:
                spellInfo->AttributesEx4 |= SPELL_ATTR4_FIXED_DAMAGE;
            ...
            case 73983:
                ...
                spellInfo->Effects[EFFECT_1].ApplyAuraName = SPELL_AURA_NONE;
                break;
            case 86657://Protection guardian channel visual
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ALLY;
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
                break;
            case 66968: // Boulder
                spellInfo->Effects[EFFECT_0].BasePoints *= (irand(9, 10) / 0.9);
                break;
            case 69725: // Land Mine Knockback
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_DEST_AREA_ENTRY;
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_DEST_CASTER;
                spellInfo->Effects[EFFECT_1].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->Effects[EFFECT_1].BasePoints *= 0.6;
                break;
            case 69280: // Mortar Round
            ...
            case 68380: // Overheat
                ...
                spellInfo->Effects[EFFECT_1].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                break;
            case 68710: // Shrunkified
                spellInfo->Effects[EFFECT_0].BasePoints = -85;
                break;
            case 68422: // Summon Scalding Water Lord 
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(6); // 100 yards
                break;
            case 66860: // Energy Jump 1
                spellInfo->Effects[EFFECT_0].MiscValue = 40;
                break;
            case 68583: // Energy Jump 2
                spellInfo->Effects[EFFECT_0].MiscValue = 40;
                break;
            case 151307: // Search Random
                spellInfo->MaxAffectedTargets = 4;
                break;
            case 66731: // FROSTBURN 
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CONE_ALLY;
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(50);
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_CONE_ALLY;
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(50);
                break;
            case 66757: // CHILLED GROUND 
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_DEST_DYNOBJ_ALLY;
                break;
            case 69881: // SUMMON_WEEPING SOUL_MISSILE
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_CASTER;
                spellInfo->Effects[EFFECT_0].TargetB = TARGET_UNIT_SRC_AREA_ENTRY;
                spellInfo->MaxAffectedTargets = 1;
                spellInfo->Effects[EFFECT_0].TriggerSpell = 0;
                break;
            case 68242: // Soul Consumption
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(112);
                break;
            case 69812: // Deaths
                spellInfo->Effects[EFFECT_0].MiscValue = 2048;
                spellInfo->Effects[EFFECT_0].MiscValueB = 180;
                break;
            case 80794: // Speed Boost
                spellInfo->Effects[EFFECT_1].ApplyAuraName = SPELL_AURA_MOD_INCREASE_VEHICLE_FLIGHT_SPEED;
                spellInfo->Effects[EFFECT_1].BasePoints = 380;
                break;  
            case 68266: // empowered_naga_health green
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_INCREASE_HEALTH_PERCENT;
                spellInfo->Effects[EFFECT_0].BasePoints = 150;
                break;
            case 68255: // empowered_naga_health blue
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_INCREASE_HEALTH_PERCENT;
                spellInfo->Effects[EFFECT_0].BasePoints = 150;
                break;
            case 69481: // Impaling Spine Impact
                spellInfo->Effects[EFFECT_2].Effect = SPELL_EFFECT_SUMMON;
                spellInfo->Effects[EFFECT_2].MiscValue = 74955;
                spellInfo->Effects[EFFECT_2].MiscValueB = 64;
                break;
            case 69230: // Cryomatic 16 Freeze Lens
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 83927: // Free Webbed Victim (ORC)
                spellInfo->Effects[EFFECT_0].MiscValueB = 2541;
                break;
            case 83982: // Summon Agatha1
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_CASTER_RANDOM;
                spellInfo->Effects[EFFECT_0].MiscValueB = 2541;
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(8);
                break;
            case 84128: // Summon Sylvanas + Horse 
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_CASTER;
                spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(0);
                spellInfo->Effects[EFFECT_1].Effect = SPELL_EFFECT_DUMMY;
            ...
            case 84180: // Kill me
                spellInfo->MaxAffectedTargets = 1;
                break;
            case 84574: // Rocket Blast aoe
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(91);
                spellInfo->Effects[EFFECT_0].MaxRadiusEntry = sSpellRadiusStore.LookupEntry(20);
                break;
            case 84468: // Ping Bomb
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13);
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_NEARBY_ENTRY;
                break;
            case 84467: // Toss Bomb
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13);
                break;
            case 84474: // Explosion Forteski too high knockback value
                spellInfo->Effects[EFFECT_0].MiscValue = 60;
                spellInfo->Effects[EFFECT_0].BasePoints = 60;
                break;
            case 85073: // Arthura flight 2 no need Belmont summon
                spellInfo->Effects[EFFECT_1].Effect = 0;
                spellInfo->Effects[EFFECT_1].TriggerSpell = 0;
                spellInfo->Effects[EFFECT_1].MiscValue = 0;
                spellInfo->Effects[EFFECT_1].BasePoints = 0;
                break;
            case 85517: // SUMMON GODFREY
            case 85518: // SUMMON WALDEN
            case 85519: // SUMMON ASHBURY
            case 80893: // SUMMON UNGEARED MESSNER
            case 80940: // SUMMON UNGEARED JORGENSEN
            case 80941: // SUMMON UNGEARED KRAKAUER
            case 80943: // SUMMON UNGEARED DANFORTH
            case 82002: // Summon GEARED (Keeshan) 	
            case 82004: // Summon GEARED (Messner) 	
            case 82005: // Summon GEARED (Jorgensen)
            case 82007: // Summon GEARED (Danforth) 
            case 82008: // Summon GEARED (Krakauer) 
                spellInfo->Attributes &= (~SPELL_ATTR0_CANT_USED_IN_COMBAT);
                break;
            case 73066: // BAZZOOKA
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(34);
                break;
            case 97933: // Warrior Intimidating Shout
                spellInfo->AuraInterruptFlags &= ~AURA_INTERRUPT_FLAG_TAKE_DAMAGE;
                spellInfo->AuraInterruptFlags |= 0x00000000;
                break;
            case 100874:
                //spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_8_SECONDS);
                break;
            case 62092: // Blessed Herb Bundle
                spellInfo->Effects[EFFECT_1].Effect = 0;
                spellInfo->Effects[EFFECT_1].TargetA = 0;
                spellInfo->Effects[EFFECT_1].MiscValue = 0;
                spellInfo->Effects[EFFECT_1].RadiusEntry = 0;
                break;
            case 62291: //  Call Down the Thunder: Spirit Channel Effect
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_NEARBY_ENTRY;
                break;
            case 65449: // Master's Glaive Intelligence Credit 1
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(21);
                break;
            case 65463: // Master's Glaive Intelligence Credit 2
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(21);
                break;
            case 80746: // Orgus' Belly Bomb
                spellInfo->Effects[EFFECT_1].MiscValue = 50;
                spellInfo->Effects[EFFECT_1].BasePoints = 50;
                break;
            case 81537: // Bloodsail Attack Boat
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 81986: // Bloodsail's End: Spacer Aura
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(DURATION_18_SECONDS);
                break;
            case 80830: // Through the Troll Hole: Portal to ZG
            case 80914: // Plunging Into Zul'Gurub: Portal to Venoxis
                spellInfo->Effects[EFFECT_2].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 81878: // Kill Credit Blackrock Invader
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
                break;
            case 69993: // Throw Footbomb
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(8);
                break;
            case 61178: // Grab Passenger
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 68059: // Miner Troubles : Summon Frightened Miner
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(3);
                break;
            case 56576: // Wild Clucker Eggs
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 66726: // Trading Up: Summon Spiny Raptor Egg
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
                break;
            case 72971: // Rockin' Powder Infused Rocket Boots // without this correction - no visual effect applied on player
                spellInfo->Effects[EFFECT_0].TriggerSpell = 151162;
                spellInfo->Effects[EFFECT_0].MiscValue = 0;
                spellInfo->Effects[EFFECT_0].MiscValueB = 0;
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_FORCE_CAST;
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(305);
                break;
            case 74028: // Rockin' Powder Infused Rocket Boots (to platform) // without this correction - no visual effect applied on player
                spellInfo->Effects[EFFECT_0].TriggerSpell = 151163;
                spellInfo->Effects[EFFECT_0].MiscValue = 0;
                spellInfo->Effects[EFFECT_0].MiscValueB = 0;
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_FORCE_CAST;
                break;
            case 42365: //Murloc Costume
                ...
                spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_DIRECT_DAMAGE;
                break;
            case 67411: // Spirit Siphon
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 67501: // Summon Raging Vortex
                spellInfo->Effects[EFFECT_0].MiscValue = 1;
                spellInfo->Effects[EFFECT_0].MiscValueB = 169;
                break;
            case 67605: // Raging Vortex Knockback
                spellInfo->Effects[EFFECT_0].MiscValue = 50;
                spellInfo->Effects[EFFECT_1].TargetA = TARGET_UNIT_CASTER;
                spellInfo->Effects[EFFECT_1].TargetB = 0;
                break;
            case 68086: // Naga Disguise Male - set aura interrupt same as female version
                spellInfo->AuraInterruptFlags = 0x005A8000;
                break;
            case 68677: // Summon Centaur Commander
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 19588: // Place Ghost Magnet
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_SUMMON;
                spellInfo->Effects[EFFECT_0].MiscValue = 74919;
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 68525: // Charm Lord Azrethoc
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 80789: // Wings of Hir'eek
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 68246: // Hydronis Frostbite
                spellInfo->RecoveryTime = 15000;
                spellInfo->CategoryRecoveryTime = 0;
                spellInfo->StartRecoveryCategory = 133;
                spellInfo->StartRecoveryTime = 1500;
                break;
            case 68249: // Hydronis Tidal Wave
                spellInfo->RecoveryTime = 8000;
                spellInfo->CategoryRecoveryTime = 0;
                spellInfo->StartRecoveryCategory = 133;
                spellInfo->StartRecoveryTime = 1500;
                break;
            case 68250: // Hydronis Water Missile
                spellInfo->RecoveryTime = 3000;
                spellInfo->CategoryRecoveryTime = 0;
                spellInfo->StartRecoveryCategory = 133;
                spellInfo->StartRecoveryTime = 1500;
                break;
            case 68252: // Hydronis Ice Block
                spellInfo->RecoveryTime = 30000;
                spellInfo->CategoryRecoveryTime = 0;
                spellInfo->StartRecoveryCategory = 133;
                spellInfo->StartRecoveryTime = 1500;
                break;
            case 79929: // Search Vilebranch
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_CREATE_RANDOM_ITEM;
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
                spellInfo->Effects[EFFECT_0].MiscValue = 79929;
                break;
            case 80839: // Zanzil's Secret Aura
                spellInfo->Effects[EFFECT_0].Amplitude = 5000;
                break;
            case 83980: // Summon Skystriderv
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 68221: // Summon Greymane's Horse
                spellInfo->Effects[EFFECT_0].BasePoints = 0;
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 68228: // Rescue Krennan
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_VEHICLE;
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY;
                spellInfo->Effects[EFFECT_0].MiscValue = 0;
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 68908: // Rope in Horse
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_CASTER;
                spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_APPLY_AURA;
                spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_DUMMY;
                break;
            case 76212: // Summon Emerald Flameweaver
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_DB;
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(5);
                break;
            case 83014: // Rapid Shot Aura
                spellInfo->Effects[EFFECT_0].Amplitude = 500;
                break;
            case 83013: // Rapid Shot Trigger
                spellInfo->MaxAffectedTargets = 1;
                break;
            case 77574: // Summon Float Vehicle
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            case 75397: // Summon Agility Trainer
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_DB;
                break;
            case 68121: // Naga Breath
                spellInfo->Effects[EFFECT_2].TriggerSpell = 0;
            case 98183: // Summon Firekin
                spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(4);
                break;
            case 97775: // Molten Splash
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_DEST_TRAJ;
                break;
            case 100992: // Flamewave
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                spellInfo->SpellAuraRestrictionsId = 0;
                break;
            case 70315: // Recapturing
                spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(2);
                break;
            case 70312: // Recapturing AOE
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(13);
                break;
            case 70888: // Hook Shot Chains
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_TARGET_ANY;
                break;
            case 72596: // Summon Northwatch Gryphon
            case 72609: // Summon Northwatch Gryphon
                spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_NEARBY_ENTRY;
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(13);
                break;
            case 73577: // Summon Twinbraid's Flying Machine 
                spellInfo->Effects[EFFECT_0].MiscValue = 39179;
                break;
            case 73670: // ID - Bael'dun Inferno
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(15); // from 7 to 3 yards
                break;
            case 73592: // ID - Quest Phase - Baeldun is Burning
                spellInfo->Effects[EFFECT_0].MiscValue = 8;
                spellInfo->Effects[EFFECT_0].MiscValueB = 172;
                break;
            case 74077: // Summon Spirit of Lo'Gosh
            case 74078: // Summon Spirit of Lo'Gosh
                spellInfo->Effects[EFFECT_0].MiscValueB = 64;
                break;
            default:
                break;
