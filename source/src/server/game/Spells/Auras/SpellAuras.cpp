...
void AuraApplication::BuildUpdatePacket(ByteBuffer& data, bool remove) const
{
    ...
    uint32 flags = _flags;
    if (((aura->GetMaxDuration() > 0) || (aura->GetDuration() > 0)) && !(aura->GetSpellInfo()->AttributesEx5 & SPELL_ATTR5_HIDE_DURATION))      
...
m_owner(owner), m_ownerObjectType(UNIT_AURA_TYPE), m_timeCla(0), m_updateTargetMapInterval(0),
...
{
    if (auto o = GetOwner())
        return (o->GetTypeId() == TYPEID_DYNAMICOBJECT) ? DYNOBJ_AURA_TYPE : UNIT_AURA_TYPE;

    return (AuraObjectType const)m_ownerObjectType;
}
...
void Aura::RefreshTimers()
{
    ...
    // HACKFIX - CalcMaxDuration overwrite the PvP duration on refresh thats why maxduration is not set on spellscript
    ...
    // HACKFIX - Mythic Affix bursting stacking
    if (GetId() == 101166 && GetOwner()->GetInstanceScript()->IsChallengeModeStarted())
        m_maxDuration = 4000;
...
bool Aura::CanBeSaved() const
{
    ...
    switch (GetId())
    {
        case 94338: //Solar eclipse - Force sunfire to overlay moonfire
    ...
    if (saBounds.first != saBounds.second)
    {
        ...
        for (SpellAreaForAreaMap::const_iterator itr = saBounds.first; itr != saBounds.second; ++itr)
        {
            ...
            if (!itr->second->IsFitToRequirements((Player*)target, zone, area))
            {
                ...
                TC_LOG_INFO("sql.dev", "spell_%u_area: aura being removed from player %u", itr->second->spellId, target->GetGUID());
            }
            ...
            else
            {
                if (itr->second->autocast)
                    ...
                    {
                        TC_LOG_INFO("sql.dev", "spell_%u_area application success: aura being added to player %u", itr->second->spellId, target->GetGUID());
                        target->CastSpell(target, itr->second->spellId, true);
                    }
                    else TC_LOG_INFO("sql.dev", "spell_%u_area: aura would be applied to player %u, but is already present.", itr->second->spellId, target->GetGUID());
                else TC_LOG_INFO("sql.dev", "spell_%u_area: aura would be applied to player %u, but is not set to autocast", itr->second->spellId, target->GetGUID());
            }
        }
    }
            ...
            case SPELLFAMILY_GENERIC:
                switch (GetId())
                {
                    ...
                    case 50836: //Petrifying Grip, becomes stoned
                        ...
                        break;
                    case 8326: //Quick and the dead
                        if (target->HasAura(83950))
                            target->CastSpell(target, 84559, true);
                        break;
                    case 20707: // soulstone
                        if (caster->HasAura(56231))
                            if (caster->getClass() != CLASS_WARLOCK)
                                caster->AddAura(56231, target);
                        break;
                        //Battleground Flags drop while mounted ( For High Ping Players )
                    case 23333:                      
                    case 23335:
                    case 34976:
                    case 100196:
                        if (caster->ToPlayer()->IsMounted())
                            caster->RemoveAurasByType(SPELL_AURA_MOUNTED);
                        break;     
                    case 102129: // priest 4p t12 bonus visual
                        SetDuration(5000);
                        break;
                    case 98067: // Shaman 2p t13 healing rain bonus
                        SetDuration(10000);
                        break;
                }
                break;
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (GetId())
            {
                ...
                case 79683:
                    if (caster && caster->GetTypeId() == TYPEID_PLAYER)
                    {
                        if (caster == target && caster->ToPlayer()->HasSpell(5143))
                        {
                            if (apply)
				...
				{
					target->RemoveAurasDueToSpell(79808);
					target->RemoveAurasDueToSpell(79683);
				}
...
void UnitAura::Remove(AuraRemoveMode removeMode)
{
    ...
    if (!o->IsDuringRemoveFromWorld())
        o->RemoveOwnedAura(this, removeMode);
}
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        else
        {
            ...
            switch (GetSpellInfo()->Effects[effIndex].Effect)
            {
                ...
                {
                    Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius);
                    Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> checker(GetUnitOwner(), targetList, u_check);

                    GetUnitOwner()->VisitNearbyObject(radius, checker);
                    break;
                }
