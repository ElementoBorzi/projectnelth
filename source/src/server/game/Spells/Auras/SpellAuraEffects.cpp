...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         // 48 SPELL_AURA_PERIODIC_TRIGGER_SPELL_BY_CLIENT triggered by client
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //343 SPELL_AURA_MOD_MELEE_DAMAGE_FROM_CASTER
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //359 SPELL_AURA_MOD_HEALING_DONE_PERCENT_WITH_SPECIFIC_AURA implemented in Unit::SpellHealingBonusDone
    &AuraEffect::HandleNULL,                                      //360 SPELL_AURA_PROC_TRIGGER_SPELL_COPY
    &AuraEffect::HandleNULL,                                      //361 SPELL_AURA_PROC_TRIGGER_SPELL_OVERRIDE implemented in Unit::ProcDamageAndSpellFor
...
AuraEffect::AuraEffect(Aura* base, uint8 effIndex, int32 *baseAmount, Unit* caster):
    ...
    m_canBeRecalculated(true), m_isPeriodic(false), m_extraTick(false)
{
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_MOD_FEAR:
            ...
            if (caster)
            {
                ...
                for (auto itr = overrideClassScripts.begin(); itr != overrideClassScripts.end(); ++itr)
                {
            ...
            sSpellMgr->IsSpellMemberOfSpellGroup(GetId(), SPELL_GROUP_ELIXIR_GUARDIAN)))
        {
            if (caster->HasAura(53042) && caster->HasSpell(GetSpellInfo()->Effects[0].TriggerSpell))
            {
                switch (GetId())
                {
                    ...
                    case 79481: // Impossible Accuracy
                    case 79632: // Mighty Speed
                        amount += 40;
                        break;
        ...
        if (caster)
        {
            ...
            if (m_spellInfo->IsChanneled())
            {
                if (m_spellInfo->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION)
                {
                    ...
                    // @TODO: some channeled spells should change channel time AND tick time if they have SPELL_ATTR5_HASTE_AFFECT_DURATION
                    // maybe it have something todo with SPELL_ATTR8_DONT_RESET_PERIODIC_TIMER ?
                    if (m_spellInfo->Id == 64843 || m_spellInfo->Id == 64901)
...
void AuraEffect::ChangeAmount(int32 newAmount, bool mark, bool onStackOrReapply)
{
    ...
    for (auto apptItr = effectApplications.begin(); apptItr != effectApplications.end(); ++apptItr)
...
void AuraEffect::Update(uint32 diff, Unit* caster)
{
    ...
    GetBase()->CallScriptEffectUpdateHandlers(this, diff);
}
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (HotWSpellId)
            {
                ...
                for (auto i = mModTotalStatPct.begin(); i != mModTotalStatPct.end(); ++i)
                {
    ...
    else
    {
        ...
        for (auto itr = shapeshifts.begin(); itr != shapeshifts.end(); ++itr)
        {
            if ((*itr) != this)
            {
                newAura = *itr;
...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        // apply glow vision
        if (target->GetTypeId() == TYPEID_PLAYER
            && m_spellInfo->Id != 32727
            && m_spellInfo->Id != 100643 //wondi's request
            && m_spellInfo->Id != 98677 //wondi's request
            && m_spellInfo->Id != 68631)
    ...
    else
    {
        ...
        else
        {
            ...
            for (auto i = invisAuras.begin(); i != invisAuras.end(); ++i)
            {
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        if (form == FORM_BEAR)
            if (target->HasAura(105735)) // druid t13 4p bonus
            {
                uint32 now = time(NULL);
                target->ToPlayer()->AddSpellCooldown(105737, 0, now + 15);
            }
        if (form == FORM_BEAR) // tier 13 bear form
            if (target->HasAura(105735))
                target->AddAura(108095, target);
                        ...
                        if (auto caster = GetCaster())
                        {
                            if (caster->HasAura(52648))         // Glyph of the Penguin
                            {
                                if (caster->HasAura(57927))         // Glyph of the Monkey
                                    model_id = ((urand(0,1) == 1) ? 21362 : 26452);
                                else
                                model_id = 26452;
                            }
                            else
                            ...
                            switch (model_id)
                            {
                            case 21362:// Glyph of the Monkey
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        target->VisitNearbyObject(target->GetMap()->GetVisibilityRange(), searcher);
            ...
            for (auto nearby_unit : targets)
            {
                if (nearby_unit->HasUnitState(UNIT_STATE_CASTING))
                    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
                        if (nearby_unit->GetCurrentSpell(i) && nearby_unit->GetCurrentSpell(i)->m_targets.GetUnitTargetGUID() == target->GetGUID())
                            nearby_unit->InterruptSpell(CurrentSpellTypes(i), false);
            
                if (auto pet = nearby_unit->ToPet())
                    if (auto owner = pet->GetOwner())
                        if (nearby_unit->getVictim() == target)
                            if (auto owner_player = owner->ToPlayer())
                                if (auto ai = nearby_unit->GetAI())
                                    ai->HandleReturnMovement();
            }
        target->CombatStop();
        target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION);
...
void AuraEffect::HandleAuraModDeterrence(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // drop flag at deterrence - http://www.wowhead.com/forums&topic=189317/deterrence-on-4-1a-patch
    if (apply)
        target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION);
}
...
void AuraEffect::HandleAuraTrackCreatures(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    uint32 spellmask = ((uint32)1) << (GetMiscValue() - 1);
    uint32 trackingmask = target->GetUInt32Value(PLAYER_TRACK_CREATURES);
    target->SetUInt32Value(PLAYER_TRACK_CREATURES, (apply) ? (trackingmask |= spellmask) : (trackingmask &= ~spellmask));
}
...
void AuraEffect::HandleAuraTrackResources(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    uint32 spellmask = ((uint32)1) << (GetMiscValue() - 1);
    uint32 trackingmask = target->GetUInt32Value(PLAYER_TRACK_RESOURCES);
    target->SetUInt32Value(PLAYER_TRACK_RESOURCES, (apply) ? (trackingmask |= spellmask) : (trackingmask &= ~spellmask));
}
...
void AuraEffect::HandleAuraAllowFlight(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
   ...
   if (!apply)
   {
       ...
       uint32 speed = target->GetSpeed(MOVE_FLIGHT);
       target->SetSpeed(MOVE_FLIGHT, speed / 1.5);
   }
...
void AuraEffect::HandleAuraModRoot(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    /*
        Recount of where and what went wrong:
        - AV archers were given UNIT_STATE_ROOT to prevent them from ever being moved or knocked back.
        - Fearing an archer and then the fear fading would remove that UNIT_STATE_ROOT.
        - To Prevent this, I (wrongly) prevented fear effects from executing SetControlled mentioned below.
        - I Should have applied a root immune as well as UNIT_STATE_ROOT to prevent the fear from affecting UNIT_STATE_ROOT in the first place.
    */
    ...
    {
        auto spell = aura->GetSpellInfo();

        if (spell->Id == 5782)//Fear
            if (!caster->HasAura(56244))
                return;

        if (spell->Id == 8122)//Scream
...
void AuraEffect::HandleModStateImmunityMask(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (aura_immunity_list.empty() && applyAuraImmunities)
    {
        ...
        // HACK FIX: SPELL_AURA_MOD_DECREASE_SPEED block stealth auras on battleground Preparation
        if ((miscVal & (1<<6)) && GetId() != 44521)
            aura_immunity_list.push_back(SPELL_AURA_MOD_DECREASE_SPEED);
        if (miscVal & (1<<0))
            aura_immunity_list.push_back(SPELL_AURA_MOD_ROOT);
        if (miscVal & (1<<2))
...
void AuraEffect::HandleAuraModSchoolImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetSpellInfo()->Mechanic == MECHANIC_BANISH)
    {
        ...
        else
        {
            ...
            for (auto i = banishAuras.begin(); i != banishAuras.end(); ++i)
...
void AuraEffect::HandleModTargetResistance(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER && (GetMiscValue() & SPELL_SCHOOL_MASK_SPELL))
...
void AuraEffect::HandleAuraModStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_STAT);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
        if (sSpellMgr->CheckSpellGroupStackRules(aurApp->GetBase()->GetSpellInfo(), (*i)->GetSpellInfo(), true) == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT)
            if (abs((*i)->GetAmount()) >= abs(MaxAmount))
                MaxAmount = (*i)->GetAmount();
    }

    if (abs(GetAmount()) <= abs(MaxAmount))
        return;

    //Calc Amount to add ,if no stackrule 3 maxamount = 0
    int32 amount = GetAmount() - MaxAmount;
    ...
    for (int32 i = STAT_STRENGTH; i < MAX_STATS; i++)
    {
        ...
        if (GetMiscValue() < 0 || GetMiscValue() == i)
        {
            ...
            target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_VALUE, float(amount), apply);
            if (target->GetTypeId() == TYPEID_PLAYER || target->ToCreature()->isPet())
                target->ApplyStatBuffMod(Stats(i), (float)amount, apply);
        }
    }
}
...
void AuraEffect::HandleModTotalPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
    ...
    {
        //calc amount
        float amount = ( ( ( ( float(GetAmount()) + 100 ) / 100 ) / ( ( float(MaxAmount) + 100 ) / 100 ) ) * 100 ) - 100;
        //apply amount
        for (int32 ii = STAT_STRENGTH; ii < MAX_STATS; ii++)
        {
            if (GetMiscValueB() & 1 << ii || !GetMiscValueB()) // 0 is also used for all stats
            {
                target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + ii), TOTAL_PCT, amount, apply);
                if (target->GetTypeId() == TYPEID_PLAYER || target->ToCreature()->isPet())
                    target->ApplyStatPercentBuffMod(Stats(ii), amount, apply);
            }
        }
    }
...
void AuraEffect::HandleAuraModIncreaseHealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (aurApp->GetBase()->GetId())
    {
        ...
        case 105737:// druid t13 4p bonus
            if (apply)
            {
                uint32 addHealth = 0;
    ...
    int64 addBaseHalth = GetAmount() * target->GetModifierValue(UNIT_MOD_HEALTH, TOTAL_PCT);

    if (apply)
    {
        target->HandleStatModifier(UNIT_MOD_HEALTH, TOTAL_VALUE, float(GetAmount()), apply);
        target->ModifyHealth(addBaseHalth);
    }
    else
    {
        if (int64(target->GetHealth()) > addBaseHalth)
...
void AuraEffect::HandleAuraModIncreaseEnergy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_INCREASE_ENERGY);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
    ...
    //calc amount
    float amount = (float(GetAmount()) - float(MaxAmount));
    //apply amount
...
void AuraEffect::HandleAuraModIncreaseHealthPercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->isAlive())
        target->SetHealth((target->GetMaxHealth() / 100) * percent);
}
...
void AuraEffect::HandleAuraModCritPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_CRIT_PCT);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
    ...
    //deapply or re-apply old
    if (MaxAmount)
    {
        target->ToPlayer()->HandleBaseModValue(CRIT_PERCENTAGE,         FLAT_MOD, float (abs(MaxAmount)), !apply);
        target->ToPlayer()->HandleBaseModValue(OFFHAND_CRIT_PERCENTAGE, FLAT_MOD, float (abs(MaxAmount)), !apply);
        target->ToPlayer()->HandleBaseModValue(RANGED_CRIT_PERCENTAGE,  FLAT_MOD, float (abs(MaxAmount)), !apply);
    }
      //apply new amount
    target->ToPlayer()->HandleBaseModValue(CRIT_PERCENTAGE,         FLAT_MOD, float (abs(GetAmount())), apply);
    target->ToPlayer()->HandleBaseModValue(OFFHAND_CRIT_PERCENTAGE, FLAT_MOD, float (abs(GetAmount())), apply);
    target->ToPlayer()->HandleBaseModValue(RANGED_CRIT_PERCENTAGE,  FLAT_MOD, float (abs(GetAmount())), apply);
...
void AuraEffect::HandleSpellPowerPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        bool softRemove = false;
        std::queue<AuraEffect*> removeList;
        AuraEffect* highest = NULL;
        Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_SPELL_POWER_PCT);
        for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
        {
            if (this == (*i) && !apply)
                continue;
            ...
            removeList.push((*i));
            highest = highest ? highest->GetAmount() > (*i)->GetAmount() ? highest : (*i) : (*i);
        }
        ...
        if (highest)
        {
            ...
            {
                AuraEffect* eff = removeList.front();
                if (eff != highest)
                    eff->SetAmount(0);
                else
...
void AuraEffect::HandleModCastingSpeed(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
    ...
    Unit::AuraEffectList const& mTotalAuraList2 = target->GetAuraEffectsByType(SPELL_AURA_HASTE_SPELLS);
    for (auto i = mTotalAuraList2.begin(); i != mTotalAuraList2.end(); ++i)
    {
        if (this == (*i))
            continue;
        if (sSpellMgr->CheckSpellGroupStackRules(aurApp->GetBase()->GetSpellInfo(), (*i)->GetSpellInfo(), true) == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT)
            if (abs((*i)->GetAmount()) >= abs(MaxAmount))
                if (!target->ToCreature() || (target->ToCreature() && !target->ToCreature()->isWorldBoss())) // bosses should be immune against this, should they?
                    MaxAmount = (*i)->GetAmount();
    }
    ...
    if (MaxAmount)
        target->ApplyCastTimePercentMod((float)MaxAmount, !apply);
...
void AuraEffect::HandleModMeleeRangedSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetAuraType() == SPELL_AURA_MOD_MELEE_RANGED_HASTE_2 || GetAuraType() == SPELL_AURA_MOD_MELEE_RANGED_HASTE)
    {
        int32 MaxAmount = 0;
        //  check spell group stack rules
        Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_MELEE_RANGED_HASTE_2);
        for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
        {
            if (this == (*i))
                continue;
        ...
        Unit::AuraEffectList const& mTotalAuraList2 = target->GetAuraEffectsByType(SPELL_AURA_MOD_MELEE_RANGED_HASTE);
        for (auto i = mTotalAuraList2.begin(); i != mTotalAuraList2.end(); ++i)
        {
            if (this == (*i))
                continue;
        ...
        if (MaxAmount)
        {
            target->ApplyAttackTimePercentMod(BASE_ATTACK, (float)MaxAmount, !apply);
            target->ApplyAttackTimePercentMod(OFF_ATTACK, (float)MaxAmount, !apply);
            target->ApplyAttackTimePercentMod(RANGED_ATTACK, (float)MaxAmount, !apply);
        }
    }
...
void AuraEffect::HandleAuraModAttackPowerPercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_ATTACK_POWER_PCT);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
    ...
    float amount = ( ( ( ( float(GetAmount()) + 100 ) / 100 ) / ( ( float(MaxAmount) + 100 ) / 100 ) ) * 100 ) - 100;
    //apply amount
    target->HandleStatModifier(UNIT_MOD_ATTACK_POWER, TOTAL_PCT, amount, apply);
}
...
void AuraEffect::HandleAuraModRangedAttackPowerPercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_RANGED_ATTACK_POWER_PCT);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
    ...
    float amount = ( ( ( ( float(GetAmount()) + 100 ) / 100 ) / ( ( float(MaxAmount) + 100 ) / 100 ) ) * 100 ) - 100;
    //apply amount
    target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED, TOTAL_PCT, amount, apply);
}
...
void AuraEffect::HandleModDamageDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetSpellInfo()->AttributesEx3 & SPELL_ATTR3_NO_DONE_BONUS)
        return;
...
void AuraEffect::HandleModDamagePercentDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 MaxAmount = 0;
    // check spell group stack rules
    Unit::AuraEffectList const& mTotalAuraList = target->GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (this == (*i))
            continue;
     ...
     //TODO: no recalculation for amount; there should be only -10% debuffs, for some magic edit soome spellgroupcheck in _ApplyWeaponDependentAuraDamageMod
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        for (int i = 0; i < MAX_ATTACK; ++i)
    ...
    if ((GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) && (GetSpellInfo()->EquippedItemClass == -1 || target->GetTypeId() != TYPEID_PLAYER))
    {
        if (MaxAmount)
        {
            target->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND,         TOTAL_PCT, float (MaxAmount), !apply);
            target->HandleStatModifier(UNIT_MOD_DAMAGE_OFFHAND,          TOTAL_PCT, float (MaxAmount), !apply);
            target->HandleStatModifier(UNIT_MOD_DAMAGE_RANGED,           TOTAL_PCT, float (MaxAmount), !apply);

            if (target->GetTypeId() == TYPEID_PLAYER)
                target->ToPlayer()->ApplyPercentModFloatValue(PLAYER_FIELD_MOD_DAMAGE_DONE_PCT, float (MaxAmount), !apply);
...
void AuraEffect::HandleShieldBlockValue(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            int32 blockPct = apply ? target->GetBlockPercent() + GetAmount() : target->GetBlockPercent() - GetAmount();
            if (blockPct > 100)
                blockPct = 100;
            else if (blockPct < 30) // cataclysm min/base block is 30 %
                blockPct = 30;

            target->SetUInt32Value(PLAYER_SHIELD_BLOCK, blockPct);
        }
        else
...
void AuraEffect::HandleNoReagentUseAura(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    for (auto i = noReagent.begin(); i != noReagent.end(); ++i)
            ...
            case 93795: // Sturmwind Champion
                {
                    ...
                    if (auto p = caster->ToPlayer())
                    {
                        //TC_LOG_ERROR("sql.sql", "setting champion faction to %u", FactionID);
                        p->SetChampioningFaction(FactionID);
                    }
                    break;
                }
                // LK Intro VO (1)
    ...
    case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            case 58686:
                ...
                break;
            case 43265:
                if (caster->HasAura(48707) && target->HasAura(43265))
                    if (target->isPet() || target->isSummon())
                        return;
                else                                      
                {
                    caster->CastWithDelay(1000, target, 52212, true);
                    caster->CastWithDelay(2000, target, 52212, true);
                    caster->CastWithDelay(3000, target, 52212, true);
                    caster->CastWithDelay(4000, target, 52212, true);
                    caster->CastWithDelay(5000, target, 52212, true);
                    caster->CastWithDelay(7000, target, 52212, true);
                    caster->CastWithDelay(8000, target, 52212, true);
                    caster->CastWithDelay(9000, target, 52212, true);
                    caster->CastWithDelay(10000, target, 52212, true);
                }
                break;
            }
            break;
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit* target, Unit* caster) const
{
    ...
    case SPELLFAMILY_GENERIC:
        ...
        case 64821: // Fuse Armor (Razorscale)
            ...
            break;
        case 98971: // dk t12 dd 2p Smoldering Rune
            ...
            case 105737:
                {
                    if (auto caster = GetCaster())
                    {
                        ...
                        if (rage == 0)
                            break;
                        int32 mod = (rage < 100) ? rage : 100;
                        int32 points = caster->CalculateSpellDamage(caster, GetSpellInfo(), 1);
                        int32 regen = (target->GetMaxHealth() * (mod * points / 100) / 1000) / 2;
                        caster->CastCustomSpell(target, 105739, &regen, 0, 0, true, 0, this);
                    }
                }
                break;
            }
            ...
            case 51690:
                {
                    ...
                    int32 killingSpreeTeleport = 57840;

                    switch (spellTarget->GetEntry())
                    {
                        case 55294: // X: -1739.17 Y: -2388.63 Z: 340.843 || Ultraxion sniff verified
                            killingSpreeTeleport = 109833;
                            break;
                        case 56846: // Deathwing: Left Arm Tentacle
                            killingSpreeTeleport = 110661;
                            break;
                        case 56167: // Deathwing: Right Arm Tentacle
                            killingSpreeTeleport = 110662;
                            break;
                        case 56168: // Deathwing: Wing Tentacle
                            if (spellTarget->GetPositionY() > 12185.f) // Left Wing Tentacle
                                killingSpreeTeleport = 101882;
                            else                                      // Right Wing Tentacle
                                killingSpreeTeleport = 101881;
                            break;
                        case 56173: // Deathwing: Head
                            killingSpreeTeleport = 101880;
                            break;

                    }

                    target->CastSpell(spellTarget, killingSpreeTeleport, true);
        ...
        if (GetSpellInfo()->SpellFamilyFlags[0] & 0x20)
        {
            ...
            if (caster)
                if (DynamicObject* dyn = caster->GetDynObject(GetId()))
                {
                    ...
                    targets.SetDst(dyn->GetPositionX(), dyn->GetPositionY(), dyn->GetPositionZ(), 0);

                    SpellInfo const* decay = sSpellMgr->GetSpellInfo(52212);

                    caster->CastSpell(targets, decay, NULL, triggerMask);
                }
            break;
        }
        // Blood of the North
    ...
    else
    {
        ...
        switch (auraId)
        {
            ...
            case 79333:
            {
                int stack = 0;
                float ray = 0.0f;
                if (Aura* grow = target->GetAura(92475))
                    stack = grow->GetStackAmount();
                for (int i = 0; i < stack; i++)
                    ray += 0.2f;
                target->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD, (1.0f - ray) * 10000.0f, NULL, true, NULL, this);
                return;
            }
            case 91857:
            {
                float mxS = m_tickNumber / 2;
                if (mxS > 5)
                    mxS = 5;
                float baseRadius = 1.0 + mxS / 5.0f;
                target->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD, (int32)(baseRadius * 10000.0f), NULL, true, NULL, this);
                return;
            }
            case 96520: // acerbes
            {
                ...
                if (newRadiusMod > 200000) //old max radius
                    newRadiusMod = 200000;
                caster->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD, newRadiusMod, NULL, true, NULL, this);
                return;
            }
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        if (GetBase()->GetType() == DYNOBJ_AURA_TYPE || GetBase()->GetSpellInfo()->StackAmount)
    ...
    int32 dmg = damage;
    if (!(GetSpellInfo()->AttributesEx10 & SPELL_ATTR10_IGNORE_RESILIENCE))
    ...
    damage = uint32(sChallengeModeMgr->ApplyDamageMultiplier(caster, damage, GetSpellInfo()->Id));
    ...
    if (damage > 0)
    {
        if (GetSpellInfo()->IsAffectingArea(1 << m_effIndex))
        {
            resist += damage;
    ...
    caster->ProcDamageAndSpellForAttacker(target, procAttacker, procVictim, procEx, damage, absorb, resist, 0, cleanDamage.mitigated_damage, BASE_ATTACK, GetSpellInfo(), NULL, NULL, false, false, (1 << m_effIndex));

    caster->DealDamage(target, damage, &cleanDamage, DOT, GetSpellInfo()->GetSchoolMask(), GetSpellInfo(), true);

    caster->ProcDamageAndSpellForVictim(target, procAttacker, procVictim, procEx, damage, absorb, resist, 0, cleanDamage.mitigated_damage, BASE_ATTACK, GetSpellInfo(), NULL, NULL, false, false, (1 << m_effIndex));
}
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (!haveCastItem && GetAuraType() != SPELL_AURA_OBS_MOD_HEALTH)
        caster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx, damage, absorb, 0, 0, 0, BASE_ATTACK, GetSpellInfo(), NULL, NULL, true);
}
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    // Yorsahj mana void "hack"
    if (m_spellInfo->Id == 105530 && caster->ToCreature())
        caster->ToCreature()->AI()->SetData(17, drainedAmount);
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit* target, Unit* caster) const
{
    ...
    caster->ProcDamageAndSpellForAttacker(damageInfo.target, procAttacker, procVictim, procEx, damageInfo.damage, damageInfo.absorb, damageInfo.resist, damageInfo.blocked, 0, BASE_ATTACK, spellProto, NULL, NULL, false, false, (1 << m_effIndex));

    caster->DealSpellDamage(&damageInfo, true);

    caster->ProcDamageAndSpellForVictim(damageInfo.target, procAttacker, procVictim, procEx, damageInfo.damage, damageInfo.absorb, damageInfo.resist, damageInfo.blocked, 0, BASE_ATTACK, spellProto, NULL, NULL, false, false, (1 << m_effIndex));
}
