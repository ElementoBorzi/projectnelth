...
SmartAI::SmartAI(Creature* c) : CreatureAI(c)
{
    ...
    mCanAutoAttack = true;
    mCombatMove = SMART_COMBAT_MOVE_FULL;
    ...
    if (mDespawnTime < diff)
    {
        ...
        else
        {
            ...
            if (me->GetDBTableGUIDLow())
            {
                ...
                Position spawn = { x, y, z, o };
                me->NearTeleportTo(spawn);
                me->SetHomePosition(spawn);
            }
        }
...
void SmartAI::UpdateAI(const uint32 diff)
{
    ...
    if (mFollowGuid)
    {
        ...
        {
            if (mFollowArrivedEntry && me->FindNearestCreature(mFollowArrivedEntry, INTERACTION_DISTANCE, true))
            {
                StopFollow();
                return;
            }
...
void SmartAI::MoveInLineOfSight(Unit* who)
{
    ...
    if (me->HasReactState(REACT_PASSIVE) || AssistPlayerInCombat(who) || me->HasAuraType(SPELL_AURA_CONTROL_VEHICLE))
        return;
...
{
    if (who && me->Attack(who, mCombatMove != SMART_COMBAT_MOVE_CAST) && IsAIControlled())
    {
        SetRun(mRun);
        if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) == POINT_MOTION_TYPE)
            me->GetMotionMaster()->MovementExpired();

        if (mCombatMove != SMART_COMBAT_MOVE_NONE)
            me->GetMotionMaster()->MoveChase(who, mCombatMove == SMART_COMBAT_MOVE_CAST ? 15.f : 0.f);
...
{
    if (spellInfo->HasAura(SPELL_AURA_MOD_SILENCE))
    {
        if (mCombatMove == SMART_COMBAT_MOVE_CAST && me->getVictim() && IsAIControlled())
        {
            me->GetMotionMaster()->Clear();
            me->GetMotionMaster()->MoveChase(me->getVictim());

            me->GetScheduler().Schedule(Milliseconds(spellInfo->GetDuration()), [this](TaskContext /*context*/)
            {
                if (me->getVictim())
                {
                    me->GetMotionMaster()->Clear();
                    me->GetMotionMaster()->MoveChase(me->getVictim(), 15.f);
                }
            });
        }
    }
...
void SmartAI::SetCombatMove(SmartCombatMove combatMove)
{
    if (mCombatMove == combatMove)
        return;

    mCombatMove = combatMove;
    ...
    if (!HasEscortState(SMART_ESCORT_ESCORTING))
    {
        if (combatMove == SMART_COMBAT_MOVE_FULL && me->getVictim())
        {
...
void SmartGameObjectAI::SpellHit(Unit* caster, SpellInfo const* spellInfo)
{
