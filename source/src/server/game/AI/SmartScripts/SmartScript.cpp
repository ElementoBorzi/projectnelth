...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    if (Quest const* q = sObjectMgr->GetQuestTemplate(e.action.quest.quest))
                    {
                        ...
                        if ((*itr)->ToPlayer()->CanCompleteQuest(e.action.quest.quest))
                            (*itr)->ToPlayer()->CompleteQuest(e.action.quest.quest);
        ...
        case SMART_ACTION_CAST:
        {
            if (!me)
                break;
            auto guid = me->GetDBTableGUIDLow();
            auto entry = me->GetEntry();
            auto type_id = me->GetTypeId();
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
            if (auto target = (*itr))
            if (auto u_target = target->ToUnit())
            {
                    if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(u_target)->HasAura(e.action.cast.spell))
                    {
                        if (me)
                        {
                            ...
                            if (e.action.cast.flags & SMARTCAST_COMBAT_MOVE)
                            {
                                ...
                                if (me->GetDistance(u_target) > spellInfo->GetMaxRange(true) ||
                                    me->GetDistance(u_target) < spellInfo->GetMinRange(true) ||
                                    !me->IsWithinLOSInMap(u_target) ||
                                    mana < spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask()))
                                    _allowMove = true;

                                CAST_AI(SmartAI, me->AI())->SetCombatMove(_allowMove ? SMART_COMBAT_MOVE_CAST : SMART_COMBAT_MOVE_NONE);
                            }
                            if (e.action.cast.flags & CAST_FORCE_TARGET_SELF)
                                u_target->CastSpell(u_target, e.action.cast.spell, (e.action.cast.flags & SMARTCAST_TRIGGERED));
                            else
                                me->CastSpell(u_target, e.action.cast.spell, (e.action.cast.flags & SMARTCAST_TRIGGERED));
                        }
                        else if (go)
                        {
                            if (e.action.cast.flags & CAST_FORCE_TARGET_SELF)
                                ...
                                go->CastSpell(u_target, e.action.cast.spell);
                        }
                        TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST:: %s: %u casts spell %u on target %u with castflags %u",
                                       GetLogNameForGuid(me ? me->GetGUID() : go->GetGUID()), me ? me->GetGUIDLow() : go->GetGUIDLow(), e.action.cast.spell, (u_target)->GetGUIDLow(), e.action.cast.flags);
                    }
            }
        ...
        case SMART_ACTION_ALLOW_COMBAT_MOVEMENT:
        {
            ...
            CAST_AI(SmartAI, me->AI())->SetCombatMove(SmartCombatMove(e.action.combatMove.move));
        ...
        case SMART_ACTION_CALL_KILLEDMONSTER:
        {
            ...
            else if (GetBaseObject())
            {
                ...
                if (targets->size())
                for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (auto non_unit_itr = (*itr))
                if (auto unit = non_unit_itr->ToUnit())
                {
                    // Special handling for vehicles
                    ...
                    p->RewardPlayerAndGroupAtEvent(e.action.killedMonster.creature, (*itr)->ToPlayer());
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsGameObject(*itr))
                {
                    if (GameObject *go = (*itr)->ToGameObject())
                        go->Delete(e.action.forceDespawn.delay + 1);
                    continue;
                }
        ...
        case SMART_ACTION_SET_DATA:
        {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (auto obj = (*itr))
                {
                    if (auto c = obj->ToCreature())
                    {
                        if (auto ai = c->AI())
                            ai->SetData(e.action.setData.field, e.action.setData.data);
                    }
                    else if (auto g = obj->ToGameObject())
                    {
                        if (auto ai = g->AI())
                            ai->SetData(e.action.setData.field, e.action.setData.data);
                    }
        ...
        case SMART_ACTION_TELEPORT:
        {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                    ...
                    (*itr)->ToUnit()->NearTeleportTo(e.target.x, e.target.y, e.target.z, e.target.o, false);
            }
        ...
        case SMART_ACTION_WP_PAUSE:
        {
            ...
            if (CAST_AI(SmartAI, me->AI())->GetNextWayPoint())
            {
        ...
        case SMART_ACTION_MOVE_TO_POS:
        {
            ...
            if (!target)
            {
                Position pos{ e.target.x, e.target.y, e.target.z, 0.f };
                float o{ me->GetAngle(&pos) };

                G3D::Vector3 dest(e.target.x, e.target.y, e.target.z);
                if (e.action.MoveToPos.isTransport)
                    if (auto trans = me->GetTransport())
                        trans->CalculatePassengerPosition(dest.x, dest.y, dest.z, o);
            ...
            else
            {
                ...
                me->GetMotionMaster()->MovePoint(e.action.MoveToPos.pointId, x, y, z);
            }
            break;
        }
        ...
        case SMART_ACTION_CROSS_CAST:
        {
            ...
            for (auto itr = casters->begin(); itr != casters->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    if (e.action.cast.flags & SMARTCAST_INTERRUPT_PREVIOUS)
        ...
        case SMART_ACTION_ACTIVATE_TAXI:
        {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsPlayer(*itr))
                    (*itr)->ToPlayer()->ActivateTaxiPathTo(e.action.taxi.id, 0, true);
        ...
        case SMART_ACTION_JUMP_TO_POS:
        {
            ...
            me->GetMotionMaster()->MoveJump(e.target.x, e.target.y, e.target.z, (float)e.action.jump.speedxy, (float)e.action.jump.speedz, e.action.jump.id);
        ...
        case SMART_ACTION_SEND_QUEST_EVENT:
        {
            ...
            bool isCreature = me->GetTypeId() == TYPEID_UNIT;
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
        ...
        case SMART_ACTION_COMBAT_STOP:
        {
            ...
            me->CombatStop(true);
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_COMBAT_STOP: %u CombatStop", me->GetGUID());
            break;
        }
        ...
        case SMART_ACTION_SET_GO_FLAG:
        {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsGameObject(*itr))
                    (*itr)->SetUInt32Value(GAMEOBJECT_FLAGS, e.action.GO_FLAGS.flag_value);
        ...
        case SMART_ACTION_ADD_GO_FLAG:
        {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsGameObject(*itr))
                    (*itr)->ToGameObject()->SetFlag(GAMEOBJECT_FLAGS, e.action.GO_FLAGS.flag_value);
        ...
        case SMART_ACTION_REMOVE_GO_FLAG:
        {
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsGameObject(*itr))
                    (*itr)->ToGameObject()->RemoveFlag(GAMEOBJECT_FLAGS, e.action.GO_FLAGS.flag_value);
            ...
            if (targets)
                if (targets->size())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto itrObj = (*itr))
                        {
                            std::list<TempSummon*>* group   { nullptr };
                            itrObj->SummonCreatureGroup(e.action.SMART_ACTION_SUMMON_CREATURE_GROUP.GroupId, group);
                            if (e.action.SMART_ACTION_SUMMON_CREATURE_GROUP.GroupId)
                                if (group->size())
                                    for (auto itr2 = group->begin(); itr2 != group->end(); ++itr2)
                                        if (auto tempSummon = (*itr))
                                            if (auto c = tempSummon->ToCreature())
                                                c->AI()->AttackStart(itrObj->ToUnit());
                        }
        ...
        case SMART_ACTION_MOVE_OFFSET:
        {
            ...
            if (targets)
                ...
                for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                    ...
                    {
                        float x, y, z, o;
                        itrObj->GetPosition(x, y, z, o);
                        x += e.target.x;
                        y += e.target.y;
                        z += e.target.z;
                        o += e.target.o;
                        Position destination{ x, y, z, o };

                        if (auto target = GetBaseObject())
                            if (auto tUnit = target->ToUnit())
                                tUnit->GetMotionMaster()->MovePoint(e.action.moveOffset.pointId, destination, true);
                    }
                    ...
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto itrObj = (*itr))
                        {
                            if (auto target = GetBaseObject())
                                if (auto tUnit = target->ToUnit())
                                {
                                    if (e.action.disableEvade.DoDisable)
                                        tUnit->ClearUnitState(UNIT_STATE_EVADE);
                                    else
                                        tUnit->AddUnitState(UNIT_STATE_EVADE);
                                }
                        }
        ...
        case SMART_ACTION_PLAY_ANIMKIT:
        {
            ...
            if (targets)
                if (targets->size())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto itrObj = (*itr))
                            if (auto unit = itrObj->ToUnit())
                                switch (e.action.playAnimKit.animType)
                                {
                                    case 0://oneshot
                                        unit->PlayOneShotAnimKit(uint16(e.action.playAnimKit.id));
                                        break;
                                    case 1://aiAnim
                                        if (e.action.playAnimKit.tempOnly)
                                            unit->PlayAIAnimKit(uint16(e.action.playAnimKit.id));
                                        else
                                            unit->SetAIAnimKitId(uint16(e.action.playAnimKit.id));
                                        break;
                                    case 2://meleeAnim
                                        if (e.action.playAnimKit.tempOnly)
                                            unit->PlayMeleeAnimKit(uint16(e.action.playAnimKit.id));
                                        else
                                            unit->SetMeleeAnimKitId(uint16(e.action.playAnimKit.id));
                                        break;
                                    case 3://movementAnim
                                        if (e.action.playAnimKit.tempOnly)
                                            unit->PlayMovementAnimKit(uint16(e.action.playAnimKit.id));
                                        else
                                            unit->SetMovementAnimKitId(uint16(e.action.playAnimKit.id));
                                        break;
                                    default:
                ...
                if (targets->size())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto itrObj = (*itr))
                            if (auto unit = itrObj->ToUnit())
                            {
                                float max_health {float(unit->GetMaxHealth())};
                                float pct    {float(e.action.SET_HEALTH_PCT.pct) * .01f };

                                if (pct > 100.f)//check for over 100% helath
                                {
                                    if (e.action.SET_HEALTH_PCT.surpass_100_pct)
                                    {
                                        max_health = (uint32 (float(unit->GetMaxHealth()) * pct));
                                        unit->SetMaxHealth(uint32(max_health));
                                    }
                                    else
                                        pct = 100.f;
                                }
                                    if (pct < 0.f)//check for under 0 percent
                                        pct = 0.f;


                                    float health_to_set { max_health * pct };

                                    //execute set health
                                    unit->SetHealth(uint32(health_to_set));
                            }
            delete targets;
            break;
        }
            ...
            if (targets)
                if (targets->size())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto itrObj = (*itr))
                            if (auto unit = itrObj->ToUnit())
                                if (auto baseObject = GetBaseObject())
                                if (auto base = baseObject->ToUnit())
                                {
                                    base->CastSpell(unit, SPELL_CLONE_SUMMONER, true);
                                    base->CastSpell(unit, SPELL_COPY_WEAPON, true);
                                    base->CastSpell(unit, SPELL_COPY_OFFHAND_WEAPON, true);
                                }
            delete targets;
            break;
        }
        case SMART_ACTION_SET_OWNER:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                if (auto target = (*itr))
                {
                    ...
                    {
                        case TYPEID_ITEM:
                            object->ToItem()->SetOwnerGUID(target->GetGUID());
                            break;
                        case TYPEID_UNIT:
                            object->ToUnit()->SetOwnerGUID(target->GetGUID());
                            break;
                        case TYPEID_PLAYER:
                            object->ToPlayer()->SetOwnerGUID(target->GetGUID());
                            break;
                        case TYPEID_GAMEOBJECT:
                            object->ToGameObject()->SetOwnerGUID(target->GetGUID());
                            break;
                        default:
                            TC_LOG_ERROR("sql.sql", "SMART_ACTION_SET_OWNER called on object entry %u with invalid object Type ID %u", object->GetEntry(), object->GetTypeId());
                            break;
                    }
                }
                ...
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            ...
                            {
                                if (auto m = unit->GetMotionMaster())
                                {
                                    if (e.action.movefall.id)
                                        m->MoveFall(e.action.movefall.id);
                                    else
                                        m->MoveFall();
                                }
                            }
        ...
        case SMART_ACTION_REMOVE_QUEST:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            ...
                            {
                                uint32 entry{ e.action.REMOVE_QUEST.quest_id };
        ...
        case SMART_ACTION_ADD_UNIT_STATE:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            if (auto unit = target->ToUnit())
                            {
                                unit->AddUnitState(e.action.UnitStates.value);
                            }
                delete targets;
            }
            break;
        }
        case SMART_ACTION_CLEAR_UNIT_STATE:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            if (auto unit = target->ToUnit())
                            {
                                unit->ClearUnitState(e.action.UnitStates.value);
                            }
                delete targets;
            }
            break;
        }
        case SMART_ACTION_CHANGE_VEHICLE_SEAT:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            if (auto unit = target->ToUnit())
                            {
                                unit->ChangeSeat(e.action.change_seat.seat_id);
                            }
                delete targets;
            }
            break;
        }
        case SMART_ACTION_CAST_RANDOM_SPELL:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (auto itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                // Select non-zero random spell
                uint32 spells[4] = { e.action.castRandom.spell1, e.action.castRandom.spell2, e.action.castRandom.spell3, e.action.castRandom.spell4 };
                uint32 randomSpellId;
                do {
                    randomSpellId = spells[urand(0, 3)];
                } while (randomSpellId == 0);

                if (!(e.action.castRandom.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(randomSpellId))
                {
                    TriggerCastFlags triggerFlag = TRIGGERED_NONE;
                    if (e.action.castRandom.castFlags & SMARTCAST_TRIGGERED)
                    {
                        if (e.action.castRandom.triggerFlags)
                            triggerFlag = TriggerCastFlags(e.action.castRandom.triggerFlags);
                        else
                    ...
                    if (me)
                    {
                        if (e.action.castRandom.castFlags & SMARTCAST_INTERRUPT_PREVIOUS)
                            me->InterruptNonMeleeSpells(false);

                        if (e.action.castRandom.castFlags & SMARTCAST_FORCE_DISMOUNT)
                        {
                            me->Dismount();
                            me->RemoveAurasByType(SPELL_AURA_MOD_SHAPESHIFT);
                            me->RemoveAura(783); // Travel Form
                        }

                        if (e.action.castRandom.castFlags & SMARTCAST_COMBAT_MOVE)
                        {
                            // If cast flag SMARTCAST_COMBAT_MOVE is set combat movement will not be allowed
                            // unless target is outside spell range, out of mana, or LOS.
                            ...
                            if (spellInfo)
                                if (me->GetPower(Powers(spellInfo->PowerType)) < cost)
                                    hasPower = false;
                            ...
                            CAST_AI(SmartAI, me->AI())->SetCombatMove(SMART_COMBAT_MOVE_FULL);
                        }

                        me->CastSpell((*itr)->ToUnit(), randomSpellId, triggerFlag);
                    }
                    else if (go)
                        go->CastSpell((*itr)->ToUnit(), randomSpellId);

                    if (me || go)
                    {
                        TC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction:: SMART_ACTION_CAST_RANDOM_SPELL:: %s casts spell %u on target %s with castflags %u",
                            (me ? me->GetGUID() : go->GetGUID()), randomSpellId, (*itr)->GetGUID(), e.action.castRandom.castFlags);
                    }
                    else
                        TC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction:: SMART_ACTION_CAST_RANDOM_SPELL:: smart_script " SI64FMTD " triggered without me or go", e.entryOrGuid);
                }
                else
                    TC_LOG_ERROR("sql.sql","SmartScript::ProcessAction:: SMART_ACTION_CAST_RANDOM_SPELL:: Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (%s) already has the aura", randomSpellId, (*itr)->GetGUID());
            }

            delete targets;
            break;
        }
        case SMART_ACTION_CONNECT_TO_TRANSPORT:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            if (auto object = target->ToGameObject())
                                if (auto t = object->ToTransport())
                                {
                                    Position reached(*me);
                                    t->AddPassenger(me);
                                    float pass_x{ reached.m_positionX };
                                    float pass_y{ reached.m_positionY };
                                    float pass_z{ reached.m_positionZ };
                                    float pass_o{ reached.m_orientation };
                                    t->CalculatePassengerOffset(pass_x, pass_y, pass_z, pass_o);
                                    me->DisableSpline();
                                    me->m_movementInfo.t_guid = t->GetGUID();
                                    me->m_movementInfo.t_pos.Relocate(pass_x, pass_y, pass_z, pass_o);
                                    me->SendTeleportPacket(me->m_movementInfo.t_pos);
                                }
                                else
                                    TC_LOG_ERROR("sql.sql", "SMART_ACTION_CONNECT_TO_TRANSPORT called for object entry %u but object is not a transport", object->GetEntry());
                            else
                                TC_LOG_ERROR("sql.sql", "SMART_ACTION_CONNECT_TO_TRANSPORT called for object entry %u but target is not a gameobject", target->GetEntry());
                delete targets;
            }
            break;
        }
        case SMART_ACTION_SEND_TRANSPORT_INIT:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            if (auto tarObj = object->ToGameObject())
                                if (auto t = tarObj->ToTransport())
                                    if (auto player = target->ToPlayer())
                                    {
                                        UpdateData transData(player->GetMapId());
                                        tarObj->BuildCreateUpdateBlockForPlayer(&transData, player);
                                        WorldPacket packet;
                                        ...
                                        TC_LOG_ERROR("sql.sql", "SMART_ACTION_SEND_TRANSPORT_INIT called for object ENTRY %u, OBJECT_TYPE %u but target is not a player", object->GetEntry(), int32(object->GetTypeId()));
                                else
                                    TC_LOG_ERROR("sql.sql", "SMART_ACTION_SEND_TRANSPORT_INIT called for object entry %u but object is not a transport", object->GetEntry());
                            else
                                TC_LOG_ERROR("sql.sql", "SMART_ACTION_SEND_TRANSPORT_INIT called for object entry %u but target is not a gameobject", target->GetEntry());
                delete targets;
            }
            break;
        }
        case SMART_ACTION_SEND_TRANSPORT_UPDATE:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                        ...
                        if (auto target = (*itr))
                            if (auto tarObj = object->ToGameObject())
                                if (auto t = tarObj->ToTransport())
                                    ...
                                    {
                                        UpdateData transData(player->GetMapId());
                                        tarObj->BuildValuesUpdateBlockForPlayer(&transData, player);
                                        ...
                                        TC_LOG_ERROR("sql.sql", "SMART_ACTION_SEND_TRANSPORT_UPDATE called for object entry %u type %u but object is not a transport", object->GetEntry(), int32(object->GetTypeId()));
                                else
                                    TC_LOG_ERROR("sql.sql", "SMART_ACTION_SEND_TRANSPORT_UPDATE called for object entry %u but object is not a transport", object->GetEntry());
                            else
                                TC_LOG_ERROR("sql.sql", "SMART_ACTION_SEND_TRANSPORT_UPDATE called for object entry %u but target is not a gameobject", target->GetEntry());
                delete targets;
            }
            break;
        }
        case SMART_ACTION_SET_SPEED:
        {
            ...
            if (targets)
            {
                if (e.action.SMART_ACTION_SET_SPEED.speedValue <= 0)
                {
                    TC_LOG_ERROR("sql.sql", "SMART_ACTION_SET_SPEED called to adjust object entry %u but speed is not a valid value.", unit->GetEntry());
                }
                else
                if (auto object = GetBaseObject())
                    for (auto itr = targets->begin(); itr != targets->end(); ++itr)
                        if (auto target = (*itr))
                            if (auto tarUnit = object->ToUnit())
                                tarUnit->SetSpeed(UnitMoveType(e.action.SMART_ACTION_SET_SPEED.speedType), (float(e.action.SMART_ACTION_SET_SPEED.speedValue) / 1000.f), true);
            }
            break;
        }
        default:
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_CREATURE_RANGE:
        {
            ...
            for (auto itr = units->begin(); itr != units->end(); ++itr)
            {
                ...
                if (
                        (
                            (
                                e.target.unitRange.creature 
                            && 
                                (*itr)->ToCreature()->GetEntry() == e.target.unitRange.creature
                            ) 
                        || 
                            !e.target.unitRange.creature
                        )
                    ...
                    && 
                        baseObject->IsInRange(*itr, (float)e.target.unitRange.minDist, (float)e.target.unitRange.maxDist)
                    )
                    if ((uint32)e.target.unitRange.maxTargets > 0)
                    {
                        if (l->size() < (uint32)e.target.unitRange.maxTargets)
                            l->push_back(*itr);
                    }
                    else
                        l->push_back(*itr);
        ...
        case SMART_TARGET_CREATURE_DISTANCE:
        {
            ...
            for (auto itr = units->begin(); itr != units->end(); ++itr)
            {
                ...
                if ((e.target.unitDistance.creature && (*itr)->ToCreature()->GetEntry() == e.target.unitDistance.creature) || !e.target.unitDistance.creature)
                    if ((uint32)e.target.unitDistance.maxTargets > 0)
                    {
                        if (l->size() < (uint32)e.target.unitDistance.maxTargets)
                            l->push_back(*itr);
                    }
                    else
                        l->push_back(*itr);
        ...
        case SMART_TARGET_PLAYER_RANGE:
        {
            ...
            ObjectList* units = GetWorldObjectsInDist((float)e.target.playerRange.maxDist);
                ...
                for (auto itr = units->begin(); itr != units->end(); ++itr)
                    if (auto it = (*itr))
                        if (auto player_itr = it->ToPlayer())
                            if ((!player_itr->isGameMaster() && player_itr->isGMVisible()))
                            {
                                    if (baseObject->IsInRange(*itr, (float)e.target.playerRange.minDist, (float)e.target.playerRange.maxDist))
                                        if ((uint32)e.target.playerRange.maxTargets > 0)
                                        {
                                            if (l->size() < (uint32)e.target.playerRange.maxTargets)
                                                l->push_back(*itr);
                                        }
                                        else
                                            l->push_back(*itr);
                ...
                if (units->size())
                    for (auto itr = units->begin(); itr != units->end(); ++itr)
                        if (auto it = (*itr))
                            ...
                            if ((!player_itr->isGameMaster() && player_itr->isGMVisible()))
                            {
                                    if (baseObject->IsInRange(*itr, 0.f, (float)e.target.playerDistance.dist))
                                    if ((uint32)e.target.playerDistance.maxTargets > 0)
                                    {
                                        if (l->size() < (uint32)e.target.playerDistance.maxTargets)
                                            l->push_back(*itr);
                                    }
                                    else
                                        l->push_back(*itr);
        ...
        case SMART_TARGET_POSITION:
            break;
        case SMART_TARGET_SUMMONS:
        {
            Unit* owner;
            switch (e.target.summons.target)
            {
            case 0:
                owner = me;
                break;
            case 1:
                owner = scriptTrigger->GetCharmerOrOwnerOrSelf();
                break;
            case 2:
                owner = me->GetOwner();
                break;
            case 3:
                ...
                break;
            default:
                TC_LOG_ERROR("sql.sql", "SMART_TARGET_SUMMONS has unused value for e.target.summons.target");
                break;
            }
            ...
            if (owner)
            {
                ...
                auto controlled = owner->getControlList();
                if (controlled.size())
                    for (auto itr = controlled.begin(); itr != controlled.end(); ++itr)
                        if (auto cont = (*itr))
                            if (auto tempsum = cont->ToTempSummon())
                                if (e.target.summons.entry)
                                {
                                    if (cont->GetEntry() == e.target.summons.entry)
                                        summons.push_back(tempsum);
                                }
                                else
                                {
                                    summons.push_back(tempsum);
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_RANGE:
        {
            ...
            if (v->IsInWorld())
            if (me->IsInRange(v, (float)e.event.minMaxRepeat.min, (float)e.event.minMaxRepeat.max))
                ProcessTimedAction(e, e.event.minMaxRepeat.repeatMin, e.event.minMaxRepeat.repeatMax, v);
            break;
        }
...
void SmartScript::InitTimer(SmartScriptHolder& e)
{
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_DISTANCE_GAMEOBJECT:
            ...
            break;
        case SMART_EVENT_DISTANCE_PLAYER:
            RecalcTimer(e, e.event.playerDistance.repeat, e.event.playerDistance.repeat);
            break;
        default:
            e.active = true;
            break;
...
Unit* SmartScript::DoFindClosestFriendlyInRange(float range, bool playerOnly)
{
    ...
    Unit* unit = NULL;
    Trinity::AnyFriendlyUnitInObjectRangeCheck u_check(me, me, range, NULL, playerOnly);
