...
void CombatAI::EnterCombat(Unit* who)
{
    ...
    for (SpellVct::iterator i = spells.begin(); i != spells.end(); ++i)
    {
        ...
        if (AISpellInfo[*i].condition == AICOND_AGGRO)
            ...
            events.ScheduleEvent(eventId, urand(AISpellInfo[*i].cooldown, 2 * AISpellInfo[*i].cooldown));
    }
}
...
void CombatAI::UpdateAI(const uint32 diff)
{
    ...
    if (uint32 eventId = events.ExecuteEvent())
    {
        ...
        DoCast(spellId);
        events.ScheduleEvent(eventId, urand(AISpellInfo[spellId].cooldown, 2 * AISpellInfo[spellId].cooldown));
    }
    else
        DoMeleeAttackIfReady();
}
...
void CasterAI::EnterCombat(Unit* who)
{
    ...
    for (SpellVct::iterator itr = spells.begin(); itr != spells.end(); ++itr, ++count)
    {
        ...
        else if (AISpellInfo[*itr].condition == AICOND_COMBAT)
        {
            ...
            uint32 eventId = count + 1;
            events.ScheduleEvent(eventId, cooldown);
        }
    }
}
