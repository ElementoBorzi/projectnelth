...
bool UnitAI::DoSpellAttackIfReady(uint32 spell)
{
    ...
    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell))
    {
        ...
        if (me->IsWithinCombatRange(v, spellInfo->GetMaxRange(false)))
        {
            me->CastSpell(v, spell, false);
            me->resetAttackTimer();
            return true;
        }
    }
...
void UnitAI::DoCastVictim(uint32 spellId, bool triggered)
{
    ...
    if (auto v = me->getVictim()) me->CastSpell(v, spellId, triggered);
}
...
bool NonSpecificTargetSelector::operator()(Unit const* target) const
{
    if (!target)
        return false;
    ...
    return target->GetGUID() != _excludeGUID;
}
