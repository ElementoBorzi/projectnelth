...
void SummonList::DespawnEntry(uint32 entry, uint32 timer)
{
    for (iterator i = begin(); i != end();)
    {
        Creature* summon = Unit::GetCreature(*me, *i);
        ...
        {
            erase(i++);
            summon->DespawnOrUnsummon(timer);
        }
        else
            ++i;
    }
...
void SummonList::DespawnAll(uint32 msTimeToDespawn /*= 0*/)
{
    while (!empty())
    {
    ...
    switch (me->GetEntry())
    {
        ...
        case NPC_SARTHARION:                                         // sartharion (calculate box)
            if (x > 3218.86f && x < 3275.69f && y < 572.40f && y > 484.68f)
                return false;
            break;
        case 47120:                                                // argaloth (calculate from center of room)
            if (me->GetDistance2d(269.737f, 1147.640f) < 43.0f)
                return false;
            break;
        case 43296:
            if (me->GetDistance2d(-106.353f, 24.903f) < 50.00f)     // chimaeron
                return false;
            break;
        case 41378:
            if (me->GetDistance2d(-106.063f, -464.795f) < 58.12f)   // maloriak
                return false;
            break;
        case 41442:
            if (me->GetDistance2d(216.925f, -224.771f) < 102.93f)   // atramedes
                return false;
            break;
        case NPC_ERUDAX:
            if (me->GetPositionX() < -640.0f)
                return false;
            break;
        case 42188://Ozruk:
            if (me->GetDistance2d(me->GetHomePosition().GetPositionX(), me->GetHomePosition().GetPositionY()) < 130.f)   //Ozruk:
                return false;
            break;
...
void GetPositionWithDistInOrientation(Position* unit, float dist, float orientation, float& x, float& y)
{
