...
bool Player::UpdateStats(Stats stat)
{
    ...
    // Required for Nerves of Cold Steel
    // @TODO: find a better place for this update...
...
void Player::UpdateBlockPercentage()
{
    ...
    // Warrior Mastery
    float ctcCap = 102.4f;
    float critBlock = GetMasteryAmount(76857, EFFECT_0);
    float ctc = 0.0f;
    ...
    if (CanBlock())
    {
        ...
        ctc = value + GetUnitParryChance() + GetUnitDodgeChance() + 5;
        if (ctc > ctcCap)
            critBlock += ctc - ctcCap;

        if (AuraEffect* mastery = GetAuraEffect(76857, EFFECT_0))
        {
            mastery->SetAmount(critBlock);
            mastery->GetBase()->SetNeedClientUpdateForTargets();
        }
    }
...
void Player::UpdateMastery()
{
    ...
    // warlock pet mastery hackfix
    if (getClass() == CLASS_WARLOCK && (GetPrimaryTalentTree(GetActiveSpec()) == TALENT_TREE_WARLOCK_DEMONOLOGY))
    {
        if (Pet* pet = GetPet())
        {
            switch (pet->GetEntry())
            ...
            case 50675:
            {
                pet->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, ((pet->GetModifierValue(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT) - 1.f) * 100.f), false);
                float masteryBonusPct = (GetRatingBonusValue(CR_MASTERY) + 8) * 2.3f;
                pet->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, masteryBonusPct, true);
                break;
            }
            default:
                break;
...
float Player::GetMasteryAmount(uint32 spellId, SpellEffIndex effIndex)
{
    if (!IsMasteryLearned())
        return 0.0f;
    ...
    Aura* mastery = GetAura(spellId);

    if (!mastery || !mastery->HasEffect(effIndex))
        return 0.0f;

    float value = GetTotalAuraModifier(SPELL_AURA_MASTERY);
    value += GetRatingBonusValue(CR_MASTERY);

    return value * mastery->GetSpellInfo()->Effects[effIndex].BonusMultiplier;
}
...
void Creature::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    float att_power_dps{ 0.f };
    float base_value{ 0.f };
    /*
        Huge PN fuckup missing this here.
        - We've spent years correcting npc damage ignoring attack power because attack power wasn't being implemented into npc damage properly.
        Fast forward, now we've increased the npcs damage correctly mostly in dungeons and raids, but the attack power now sends "fixed" npcs damage through the fucking roof.
        We need to partition the two groups and try to keep the best of both worlds.
    */
    bool use_old_calculation_to_ignore_npc_attack_power{ false };
    
    if (auto o = GetOwner()) //Don't buff pets
    if (o->GetTypeId() == TYPEID_PLAYER)
    {
        use_old_calculation_to_ignore_npc_attack_power = true;
        //TC_LOG_ERROR("sql.sql", "true 1");
    }
    /*
    if (GetGUIDLow() != GetDBTableGUIDLow()) //This NPC is instanced
        {
            use_old_calculation_to_ignore_npc_attack_power = true;
            //TC_LOG_ERROR("sql.sql", "true 2");
        }

    if (GetCreatureTemplate()->maxlevel >= 81)//This NPC could be related to cata
        {
            use_old_calculation_to_ignore_npc_attack_power = true;
            //TC_LOG_ERROR("sql.sql", "true 3");
        }

    */

    att_power_dps = GetTotalAttackPowerValue(attType) - GetCreatureTemplate()->attackpower;
    base_value = GetModifierValue(unitMod, BASE_VALUE) + (att_power_dps * GetAPMultiplier(attType, false) / 14.0f);
    /*
    if (use_old_calculation_to_ignore_npc_attack_power)
    {
        //TC_LOG_ERROR("sql.sql", "use_old_calculation_to_ignore_npc_attack_power");
    ...
    {
        //TC_LOG_ERROR("sql.sql", "use new calcs");
        att_power_dps = (GetTotalAttackPowerValue(attType) / 14.f);
        base_value = GetModifierValue(unitMod, BASE_VALUE) + (att_power_dps * GetAPMultiplier(attType, false));
    }
    */
...
{
    // Fire elementary and earth elementary has fixed max health don't modify it here...
    if (GetEntry() == 15438 || GetEntry() == 15352)
        return;
    ...
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        switch (GetEntry())
        {
            ...
            case ENTRY_TREANT:
            {
                ...
                if (spellDmg > 0)
                    bonusDamage = spellDmg * 0.235f * 0.9f * 0.91f; // 23.5% of Spell power) * 0.9 * 0.91
                break;
            }
            //greater fire elemental
            case ENTRY_FIRE_ELEMENTAL:
            {
                bonusDamage = m_owner->GetUInt32Value(PLAYER_PET_SPELL_POWER) * 0.55f;
                break;
            }
            default :
                break;
