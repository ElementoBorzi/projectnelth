...
void Unit::Update(uint32 p_time)
{
    ...
    {
        bool InCombatWithOnlyVehicle = true;
        HostileReference* ref = m_HostileRefManager.getFirst();
        while (ref)
        {
            HostileReference* nextRef = ref->next();
            if (auto UnitRef = ref->getSource()->getOwner())
            {
                if (!UnitRef->IsVehicle())
                    InCombatWithOnlyVehicle = false;

                if (auto c = UnitRef->ToCreature())
                    if (auto ai = c->AI())
                        if (ai->CanAIAttack(this))
                            InCombatWithOnlyVehicle = false;
            }
            ref = nextRef;
        }
        ...
        if (m_HostileRefManager.isEmpty() || InCombatWithOnlyVehicle)
        {
            ...
            if (m_CombatTimer <= p_time)
            {
                ...
                if (InCombatWithOnlyVehicle)
                {
                    ref = m_HostileRefManager.getFirst();
                    while (ref)
                    {
                        HostileReference* nextRef = ref->next();
                        if (auto UnitRef = ref->getSource()->getOwner())
                            UnitRef->DeleteThreatList();
                        ref = nextRef;
                    }
                }
            }
    ...
    /*if (GetTypeId() == TYPEID_PLAYER)
        InterruptMovementBasedAuras();          // moved directly to Unit::UpdatePosition cause it delays interrupt, thus interrupting casts that happened right after movement stop
...
{
    return (GetMeleeReach() + obj->GetMeleeReach());
}

float Unit::GetMeleeReEnagementDistance(const Unit* obj) const
{
    return (GetMeleeRange(obj) * 0.7f);
}

bool Unit::IsWithinCombatRange(const Unit* obj, float dist2compare) const
{
    bool result{ IsWithinMeleeRange(obj, dist2compare) };
...
bool Unit::IsWithinMeleeRange(const Unit* obj, float dist) const
{
    ...
    float their_Z       { obj->GetPositionZ() };
    float my_Hover      { HasUnitMovementFlag(MOVEMENTFLAG_HOVER) ? GetFloatValue(UNIT_FIELD_HOVERHEIGHT) + GetMeleeReach() : 0.0f };
    float their_Hover   { obj->HasUnitMovementFlag(MOVEMENTFLAG_HOVER) ? obj->GetFloatValue(UNIT_FIELD_HOVERHEIGHT) + obj->GetMeleeReach() : 0.0f };
    float dZ_ABS        { fabs(my_Z - their_Z) };
    bool I_AM_HIGHER    { my_Z > their_Z };

    float hover_correction                  //We don't want a lower npc with a higher hover to be registered as further away than they should be.
    { 
        I_AM_HIGHER 
            ? 
                fabs(my_Hover > dZ_ABS ? (dZ_ABS - my_Hover) : (0.f - my_Hover))
            :
                fabs(my_Hover > dZ_ABS ? (dZ_ABS - their_Hover) : (0.f - their_Hover))
    };

    if (hover_correction > dZ_ABS)
        hover_correction = dZ_ABS;

    float dz            { fabs(dZ_ABS  - hover_correction) };
    float distsq        { (GetExactDist2d(obj)*GetExactDist2d(obj)) + (dz * dz) };
    ...
    if (GetEntry() == 44658)
    {
        ...
        debug_report << "my_Hover: " << my_Hover << ", their hover: " << their_Hover << ", I_AM_HIGHER: " << I_AM_HIGHER << ", hover_correction: " << hover_correction << ", dZ_ABS: " << dZ_ABS << ", final dZ: " << dz << ".\n";
        debug_report << "maxdist: " << maxdist << ", maxdist_SQ: " << maxdist_SQ << ", distsq" << distsq <<", 2d dist sq:"<< GetExactDist2d(obj)*GetExactDist2d(obj) << "\n-----\n";
        sWorld->SendGMText(LANG_GM_BROADCAST, debug_report.str().c_str());
    }
    */
    return (distsq <= maxdist_SQ);
}
...
void Unit::GetRandomContactPoint(const Unit* obj, float &x, float &y, float &z, float distance2dMin, float distance2dMax) const
{
    ...
    GetNearPoint(obj, x, y, z, obj->GetCombatReach(), distance2dMin+(distance2dMax-distance2dMin) * (float)rand_norm()
        , GetAngle(obj) + (attacker_number ? (static_cast<float>(M_PI / 2) - static_cast<float>(M_PI) * (float)rand_norm()) * float(attacker_number) / combat_reach * 0.3f : 0), 0.f);
}
...
bool Unit::HasBreakableByDamageAuraType(AuraType type, uint32 excludeAura) const
{
    ...
    for (auto itr = auras.begin(); itr != auras.end(); ++itr)
        if (auto aura = (*itr))
        if (auto info = aura->GetSpellInfo())
        if ((!excludeAura || excludeAura != info->Id) && //Avoid self interrupt of channeled Crowd Control spells like Seduction
            (info->Attributes & SPELL_ATTR0_BREAKABLE_BY_DAMAGE
            || info->AuraInterruptFlags & AURA_INTERRUPT_FLAG_TAKE_DAMAGE2
...
bool Unit::HasBreakableByDamageCrowdControlAura(Unit* excludeCasterChannel) const
{
    ...
    if (auto currentChanneledSpell = excludeCasterChannel->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
        if (auto i = currentChanneledSpell->GetSpellInfo())
            excludeAura = i->Id; //Avoid self interrupt of channeled Crowd Control spells like Seduction
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool splitted)
{
    if (victim->IsAIEnabled)
        if (auto vai = victim->GetAI())
            vai->DamageTaken(this, damage);

    if (spellProto)
    if (spellProto->Id)
    if (victim->IsAIEnabled)
        if (auto vai = victim->GetAI())
            vai->SpellDamageTaken(this, damage, cleanDamage, damagetype, damageSchoolMask, spellProto, durabilityLoss, splitted);

    if (auto script = victim->GetInstanceScript())
        script->OnDamageTaken(this, victim, damage, damagetype);

    if (TempSummon* summon = victim->ToTempSummon())
        if (auto summoner = summon->GetSummoner())
            if (auto summ_cr = summoner->ToCreature())
            if (auto ai = summ_cr->AI())
                ai->SummonedCreatureDamageTaken(this, summon, damage, spellProto);
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && this != victim)
    {
        ...
        if (auto ancient = victim->ToPlayer()->GetPetSummoned(46506))
        {
            if (!ancient->getVictim() && !victim->getVictim())
            {
                ancient->ToCreature()->SetReactState(REACT_ASSIST);
                ancient->ToCreature()->AI()->AttackStart(this);
            }
        }
        if(Unit* runeweapon = victim->ToPlayer()->GetPetSummoned(27893))
        {
            if (!runeweapon->getVictim() && !victim->getVictim())
            {
                runeweapon->ToCreature()->SetReactState(REACT_ASSIST);
                runeweapon->ToCreature()->AI()->AttackStart(this);
            }
        }
    ...
    if (GetTypeId() == TYPEID_UNIT && this != victim && isPet())
        if (auto p = ToPet())
            if (auto ai = p->AI())
                ai->JustAttacked(victim);
    ...
    if (damagetype != NODAMAGE)
    {
        ...
        if (auto o = GetOwner())
        if (!o->IsFriendlyTo(victim))
        {
            SetInCombatWith(victim);
            if (victim->GetTypeId() == TYPEID_PLAYER)
                victim->SetInCombatWith(this);
            AddThreat(victim, 0.2f);

            o->SetInCombatWith(victim);
            o->AddThreat(victim, 0.1f);
        }
    ...
    if (GetTypeId() == TYPEID_PLAYER && this != victim)
    {
        ...
        if (victim->GetTypeId() == TYPEID_PLAYER)
        {
            if (auto bg = killer->GetBattleground())
                bg->UpdatePlayerScore(killer, SCORE_DAMAGE_DONE, damage);
        }

        if (auto vPlayer = victim->ToPlayer())
            if (auto v_bg = vPlayer->GetBattleground())
                v_bg->UpdatePlayerScore(vPlayer, SCORE_DAMAGE_TAKEN, damage);
    ...
    else if (GetTypeId() == TYPEID_UNIT && this != victim)
    {
        if (GetOwner())
        {
            if (auto owner = GetOwner()->ToPlayer())
            {
                if (victim->GetTypeId() == TYPEID_PLAYER)
                {
                    if (auto bg = owner->GetBattleground())
...
{
    GetScheduler().Schedule(Milliseconds(delay), [this, x, y, z, speedXY, speedZ, id](TaskContext /*context*/)
    {
        ExitVehicle();
        GetMotionMaster()->MoveJump(x, y, z, speedXY, speedZ, id);
    });
}
...
{
    if (victim->IsPlayer() && victim->ToPlayer()->isTotalImmunity())
        return 0;

    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellID);
    if (SpellHitResult(victim, spellInfo, false) != SPELL_MISS_NONE)
        return 0;
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (!(spellInfo->AttributesEx10 & SPELL_ATTR10_IGNORE_RESILIENCE))
                ApplyResilience(victim, &damage);
            break;
        }
        default:
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    damage = uint32(sChallengeModeMgr->ApplyDamageMultiplier(this, damage));
    ...
    for (auto i = autoAttack.begin(); i != autoAttack.end(); ++i)
    {
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->damage)
    {
        ...
        for (auto dmgShieldItr = vDamageShieldsCopy.begin(); dmgShieldItr != vDamageShieldsCopy.end(); ++dmgShieldItr)
        {
            ...
            if (auto caster = (*dmgShieldItr)->GetCaster())
            {
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    for (auto i = reductionAuras.begin(); i != reductionAuras.end(); ++i)
    ...
    for (auto j = ResIgnoreAuras.begin(); j != ResIgnoreAuras.end(); ++j)
    {
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    for (auto itr = AbsIgnoreAurasA.begin(); itr != AbsIgnoreAurasA.end(); ++itr)
    {
    ...
    for (auto itr = AbsIgnoreAurasB.begin(); itr != AbsIgnoreAurasB.end(); ++itr)
    {
        if (!((*itr)->GetMiscValue() & schoolMask))
            continue;
    ...
    for (auto itr = vSchoolAbsorbCopy.begin(); (itr != vSchoolAbsorbCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
    ...
    for (auto itr = vManaShieldCopy.begin(); (itr != vManaShieldCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
        AuraEffect* absorbAurEff = *itr;
        // Check if aura was removed during iteration - we don't need to work on such auras
        AuraApplication const* aurApp = absorbAurEff->GetBase()->GetApplicationOfTarget(victim->GetGUID());
...
void Unit::CalcHealAbsorb(Unit* victim, const SpellInfo* healSpell, uint32 &healAmount, uint32 &absorb)
{
    ...
    for (auto i = vHealAbsorb.begin(); i != vHealAbsorb.end() && RemainingHeal > 0; ++i)
    {
        ...
        // HACKFIX - Mythic Affix overflowing
        if (auto player = victim->ToPlayer())
            if (auto effect = (*i))
                if (effect->GetId() == 107801)
                    if (auto aura = (*i)->GetBase())
                        if (victim->GetHealthPct() >= 100.f)
                        {
                            auto remaining_absorb   { uint32(effect->GetAmount())                           };
                            auto temp_absorb        { uint32(absorb) };
                            uint32 new_shield_amount{ temp_absorb + remaining_absorb };
                                if (temp_absorb)
                                {
                                    effect->SetAmount(new_shield_amount + remaining_absorb);
                                    aura->SetMaxDuration(60000);
                                    aura->SetDuration(60000);
                                }
                                absorb = int32(new_shield_amount);
                                //TC_LOG_ERROR("sql.sql", "calchealabsorb setting absorb amount to %u", temp_absorb);
                                continue;
                        }
    ...
    if (existExpired)
    {
        for (auto i = vHealAbsorb.begin(); i != vHealAbsorb.end();)
        {
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    {
        uint32 leveldiffr = (victim->getLevel() - getLevel());
        if (leveldiffr > 3)
        {
            for (leveldiffr; leveldiffr != 0; --leveldiffr)
            {
                tmp += 100;
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER || (GetOwner() && GetOwner()->ToPlayer()))
            dodge_chance -= int32((ToPlayer() ? ToPlayer() : GetOwner()->ToPlayer())->GetExpertiseDodgeOrParryReduction(attType) * 100);
        else
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER || (GetOwner() && GetOwner()->ToPlayer()))
            parry_chance -= int32((ToPlayer() ? ToPlayer() : GetOwner()->ToPlayer())->GetExpertiseDodgeOrParryReduction(attType) * 100);
        else
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    if (victim)
        ...
        else data.append((const ByteBuffer)0);
    else data.append((const ByteBuffer)0);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    if (canDodge)
    {
        ...
        // Reduce dodge chance by attacker expertise rating
        if (GetTypeId() == TYPEID_PLAYER || (GetOwner() && GetOwner()->ToPlayer()))
            dodgeChance -= int32((ToPlayer() ? ToPlayer() : GetOwner()->ToPlayer())->GetExpertiseDodgeOrParryReduction(attType) * 100.0f);
        else
    ...
    if (canParry)
    {
        ...
        // Reduce parry chance by attacker expertise rating
        if (GetTypeId() == TYPEID_PLAYER || (GetOwner() && GetOwner()->ToPlayer()))
            parryChance -= int32((ToPlayer() ? ToPlayer() : GetOwner()->ToPlayer())->GetExpertiseDodgeOrParryReduction(attType) * 100.0f);
        else
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell, const uint8 effectMask)
{
    ...
    // Spells with SPELL_ATTR3_IGNORE_HIT_RESULT will additionally fully ignore
    // resist and deflect chances
    if (spell->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT && !victim->HasAura(19263)) // not for Deterrence
        return SPELL_MISS_NONE;
    ...
    if (spell->Id != 50796)//chaos bolt is the one spell that penetrates deflect
    if (victim->HasInArc(M_PI, this) || victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
        int32 deflect_chance = victim->GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS) * 100;
        tmp += deflect_chance;
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    if (CanReflect && !spell->IsAffectingArea(effectMask))
    {
        ...
        for (auto i = mReflectSpellsSchool.begin(); i != mReflectSpellsSchool.end(); ++i)
...
float Unit::GetUnitCriticalChance(WeaponAttackType attackType, const Unit* victim) const
{
    ...
    for (auto i = dummyAuras.begin(); i != dummyAuras.end(); ++i)
    {
        switch ((*i)->GetId())
        {
    ...
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
        if (spell->getState() != SPELL_STATE_FINISHED && spell->IsChannelActive())
            if (auto info = spell->GetSpellInfo())
                if (info->IsMoveAllowedChannel())
                return false;

    if (Spell* spell2 = m_currentSpells[CURRENT_GENERIC_SPELL])
        if (spell2->getState() != SPELL_STATE_FINISHED && spell2->IsChannelActive())
            if (auto info2 = spell2->GetSpellInfo())
            if (info2->IsMoveAllowedChannel())
                return false;
...
void Unit::_UnapplyAura(AuraApplication * aurApp, AuraRemoveMode removeMode)
{
    ...
    if (isActiveObject())
    if (aurApp)
        if (auto b = aurApp->GetBase())
            ASSERT(b->GetApplicationOfTarget(GetGUID()) == aurApp);
    ...
    TC_LOG_ERROR("entities.unit", "Unit::_UnapplyAura: unit guid %u (entry %u) attempting to removed non apply or already removed aura %u", GetGUID(), GetEntry(), spellId);
}
...
void Unit::_RemoveNoStackAurasDueToAura(Aura* aura)
{
    ...
    bool remove = false;
    for (auto i = m_appliedAuras.begin(); i != m_appliedAuras.end(); ++i)
    {
        if (remove)
        {
            remove = false;
...
{
    for (auto iter = m_appliedAuras.lower_bound(spellId); iter != m_appliedAuras.upper_bound(spellId);)
    {
        Aura const* aura = iter->second->GetBase();
        if (((aura->GetEffectMask() & reqEffMask) == reqEffMask)
...
bool Unit::ConsumeAuraAtZeroCharges(SpellInfo const* auraInfo)
{
    switch (auraInfo->Id)
    {
    case 79440:
...
void Unit::RemoveAuraFromStack(uint32 spellId, uint64 casterGUID, AuraRemoveMode removeMode)
{
    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;)
    {
        ...
        if ((aura->GetType() == UNIT_AURA_TYPE)
                && (!casterGUID || aura->GetCasterGUID() == casterGUID))
        {
            if (ConsumeAuraAtZeroCharges(aura->GetSpellInfo()))
...
void Unit::RemoveTriggeredAurasDueToSpell(uint32 spellId, uint64 castItemGuid, uint64 casterGUID, uint8 reqEffMask, AuraRemoveMode removeMode)
{
    ...
    for (auto iter = m_appliedAuras.lower_bound(spellId); iter != m_appliedAuras.upper_bound(spellId);)
    {
        ...
        if (castItemGuid)
        {
            if (aura->GetCastItemGUID() == castItemGuid)
                if (removed == false)
                    doRemove = true;
        }
        else
        {
            if ((aura->GetEffectMask() & reqEffMask) == reqEffMask && (!casterGUID || aura->GetCasterGUID() == casterGUID))
                doRemove = true;
        }
...
void Unit::RemoveNotOwnSingleTargetAuras(uint32 newPhase)
{
    ...
    if (m_appliedAuras.size())
    for (auto iter : m_appliedAuras)
        if (auto aurApp = iter.second)
            if (auto aura = aurApp->GetBase())
            if (auto info = aura->GetSpellInfo())
            {
                ...
                if (auto info = aura->GetSpellInfo())
                if(info->IsSingleTarget() || info->NeedsSpecialTreatment())
                    if (!newPhase) aura->Remove();
                    else
                    {
                        if (auto caster = aura->GetCaster())
                            if (!caster->InSamePhase(newPhase))
                                if (GetAreaId() == caster->GetAreaId())
                                    switch (GetAreaId())
                                    {
                                    case 5928://Ultraxion Platform, needs a special exception
                                        break;
                                    default:
                                        aura->Remove();
                                        break;
    ...
    auto scAuras{ GetSingleCastAuras() };
    if (scAuras.size())
        for (auto aura : scAuras)
            if (auto info = aura->GetSpellInfo())
            {
                if (auto o = aura->GetUnitOwner())
                    if (o->GetGUID() != GetGUID())
                        if (!o->InSamePhase(newPhase))
                            if (GetAreaId() == o->GetAreaId())
                                switch (GetAreaId())
                                {
                                case 5928://Ultraxion Platform, needs a special exception
                                    break;
...
AuraApplication * Unit::GetAuraApplication(uint32 spellId, uint64 casterGUID, uint64 itemCasterGUID, uint8 reqEffMask, AuraApplication * except) const
{
    AuraApplicationMapBounds range = m_appliedAuras.equal_range(spellId);
    for (; range.first != range.second; ++range.first)
    if (auto app = range.first->second)
    if (auto aura = app->GetBase())
    {
...
{
    for (auto iter = m_appliedAuras.begin(); iter != m_appliedAuras.end(); ++iter)
    {
...
bool Unit::HasAuraTypeWithCaster(AuraType auratype, uint64 caster) const
{
    ...
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
...
uint32 Unit::GetDiseasesByCaster(uint64 casterGUID, bool remove)
{
    ...
    {
        for (auto i = m_modAuras[*itr].begin(); i != m_modAuras[*itr].end();)
        {
    ...
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (((*i)->GetMiscValue() & misc_mask))
        {
            ...
            if (!sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup))
            {
                ...
                float a = (1.f - (1.f * (((float)(*i)->GetAmount() / 100.f) * -1.f)));
                tempMod = 100.f - (((a - (a * ((tempMod / 100.f) * -1.f))) / 1.f) * 100.f);
                tempMod *= -1;           
            }              
        }
    }

    float tempPositiveMod = 0.0f;
    for (auto i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (((*i)->GetMiscValue() & misc_mask))
        {
            ...
            if (!sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup))
            {
                ...
                tempPositiveMod += (*i)->GetAmount();
            }
        }
    }

    if (tempPositiveMod != 0)
    {
        if (tempMod != 0)
            tempMod = ((100.f + tempMod) * ((100.f + tempPositiveMod) / 100.f)) - 100.f;
        else tempMod += tempPositiveMod;
    }     
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 12846:
                {
                    ...
                    /*if (victim->IsPlayer() || victim->GetOwner())
                    {
                        uint32 dmgReduce = victim->GetDamageReduction(basepoints0);

                        float precentage = 1.0f;
                        float resilience = (float)dmgReduce / (float)basepoints0;
                        precentage -= resilience;
                        
                        float dmgBoost = (float)dmgReduce;
                        dmgBoost /= precentage;

                        basepoints0 += (uint32)dmgBoost;
                    }*/
                    break;
                }
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummySpell->SpellFamilyFlags[1] & 0x00000010)
            {
                ...
                if (triggeredByAura->GetAmount() <= int32(damage) || GetHealth() <= damage)
                {
                    ...
                    if (auto caster = GetUnit(*this, casterGuid))
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 81749:
                {
                    ...
                    // dragon soul hackfix
                    if (victim->GetTypeId() == TYPEID_UNIT && (victim->GetEntry() == 55294 || victim->GetEntry() == 57694 || victim->GetEntry() == 57686))
                        radius = ((victim->GetCombatReach() + 15.f) * 10000.f) / 15.f;
        ...
        { 
            ...
            switch (dummySpell->Id)
            {
                ...
                case 85804:
                {
                    ...
                    basepoints0 = triggerAmount * (HasAura(90174) ? 3 : GetPower(POWER_HOLY_POWER));
                    break;
                }
                ...
                case 53601:
                {
                    ...
                    if (auto caster = triggeredByAura->GetCaster())
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (dummySpell->SpellFamilyFlags[0] & 0x200000)
            {
                ...
                float fire_onhit = float(CalculatePct(dummySpell->Effects[EFFECT_0].CalcValue(), 1.0f)); //Misan changed from 25.0f

				float add_spellpower = (float)(SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE)
					+ victim->SpellBaseDamageBonusTaken(SPELL_SCHOOL_MASK_FIRE)); //Misan added this too

				ApplyPct(add_spellpower, 3.84f); //Misan added this too
                ...
                if (castItem->GetSlot() == EQUIPMENT_SLOT_OFFHAND && procFlag & PROC_FLAG_DONE_OFFHAND_ATTACK)
                {
					...
					basepoints0 = int32((fire_onhit * BaseWeaponSpeed) + (add_spellpower * BaseWeaponSpeed)); //Misan added this too
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49028)
            {
                ...
                Unit* pPet = NULL;
                for (auto itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr) // Find Rune Weapon
            ...
            switch (dummySpell->Id)
            {
                case 85768: // Dark intent on warlock
                {
                    *handled = true;
                    ...
                    for (auto itr = scAuras.begin(); itr != scAuras.end(); itr++)
                    {
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_HUNTER:
            {
                if (auraSpellInfo->SpellIconID == 3247)     // Piercing Shots
                {
                    ...
                    for (auto i = mDamageDoneMechanic.begin(); i != mDamageDoneMechanic.end(); ++i)
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 62600:
        {
            ...
            // Item - Druid T13  2P Bonus Savage Defense
        ...
        case 84588:
        {
            ...
            if (victim)
                ...
                {
                    aur->SetDuration(aur->GetSpellInfo()->GetMaxDuration(), true);
                    //" [Lambs to the Slaughter] should refresh  Rend duration without  initial tick damage."
                    //https://forum.projectnelth.com/bugtracker/lambs-to-the-slaughter-rend-refresh-tick-timer-r217/
                    /*
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
...
{
    if (FactionTemplateEntry const* entry = sFactionTemplateStore.LookupEntry(getFaction()))
        return entry;
    else
    {
...
ReputationRank Unit::GetReactionTo(Unit const* target) const
{
    if (!target)
    {
        TC_LOG_ERROR("entities.unit", "Unit::GetReactionTo target does not exist for unit type %u guid %u", GetTypeId(), GetGUID());
        return REP_NEUTRAL;
    }
...
bool Unit::AttackStop(Unit* specified_stopattack_target)
{
    if (!this)
        return false;

    if (specified_stopattack_target)
    {
        if (auto t = getVictim())
        {
            if (t->GetGUID() != specified_stopattack_target->GetGUID())
                return false;
            if (auto c = ToCreature())
                c->SetTarget(0);
        }
    ...
    if (auto t = getVictim())
    {
        ...
        {
            if (auto p = ToPlayer())
                p->SendAutoRepeatCancel(t);
            SendMeleeAttackStop(t);
        }
    }
    else return false;
...
void Unit::CombatStop(bool includingCast)
{
    ...
    if (auto p = ToPlayer())
    {
        p->SendAttackSwingCancelAttack();     // melee and ranged forced attack cancel
        p->m_holyPowerRegenTimerCount = 0;

        if (p->pvpInfo.pvpFlagStatus != PVP_STATE_IN_PVP_NORMAL)
            if (!p->IsPvP())
            p->UpdatePvP(false, true);
    }
    ClearInCombat();
}
...
void Unit::CombatStopWithPets(bool includingCast)
{
    ...
    for (auto itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
        (*itr)->CombatStop(includingCast);
}

bool Unit::CheckCombat()
{
    if (m_attackers.size())
        return true;
    if (getThreatManager().getThreatList().size())
        return true;
...
bool Unit::isAttackingPlayer() const
{
    ...
    for (uint8 i = 0; i < MAX_SUMMON_SLOT; ++i)
        if (m_SummonSlot[i])
            if (auto summon = GetMap()->GetCreature(m_SummonSlot[i]))
...
{
    if (m_attackers.size())
        for (auto atk : m_attackers)
        {
            if (atk->AttackStop(this))
            {
...
Player* Unit::GetAffectingPlayer() const
{
    ...
    if (auto owner = GetCharmerOrOwner())
...
Minion *Unit::GetFirstMinion() const
{
    ...
    {
        if (auto pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*this, pet_guid))
...
Unit* Unit::GetCharm() const
{
    ...
    {
        if (auto pet = ObjectAccessor::GetUnit(*this, charm_guid))
            return pet;
...
void Unit::RemoveAllMinionsForDuel()
{
    for (auto itr = m_Controlled.begin(); itr != m_Controlled.end(); )
    {
        Unit* unit = *itr;
        ...
        if (TempSummon* thisTempSummon = unit->ToTempSummon()) // minion, actually
            if (thisTempSummon->GetTimer() != 0) // has a despawn timer
                thisTempSummon->UnSummon();
    }
}
...
void Unit::SetCharm(Unit* charm, bool apply)
{
    if (apply)
    {
        ...
        else
        {
            ...
            charm->SetByteValue(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_PVP_ENABLING);
            charm->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE);
        }
    ...
    else
    {
        ...
        else if (auto player = charm->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            charm->m_ControlledByPlayer = true;
            charm->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE);
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if (auto script = victim->GetInstanceScript())
        script->OnHealReceived(this, victim, addhealth);
    ...
    if (auto player = unit->ToPlayer())
    {
        ...
        if (auto p = victim->ToPlayer())
            if (auto bg2 = p->GetBattleground())
                bg2->UpdatePlayerScore(p, SCORE_HEALING_TAKEN, gain);
...
Unit* Unit::GetMagicHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    for (auto itr = magnetAuras.begin(); itr != magnetAuras.end(); ++itr)
        if (auto magnet = (*itr)->GetBase()->GetCaster())
            return magnet;
...
Unit* Unit::GetMeleeHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    for (auto i = hitTriggerAuras.begin(); i != hitTriggerAuras.end(); ++i)
    {
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    for (auto itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        ...
        if (auto pet = Target->GetGuardianPet())
...
void Unit::UnsummonAllTotems()
{
    for (uint8 i = 0; i < MAX_SUMMON_SLOT; ++i)
    {
        ...
        if (auto OldTotem = GetMap()->GetCreature(m_SummonSlot[i]))
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
    {
        ...
        {
            if (auto owner = GetOwner())
                return owner->SpellDamageBonusDone(victim, spellProto, pdamage, damagetype, stack) / 2;
        }
    }
    ...
    for (auto i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
    ...
    if (!(spellProto->AttributesEx10 & SPELL_ATTR10_IGNORE_RESILIENCE))
        if (auto modOwner = GetSpellModOwner())
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype)
{
    ...
    if(GetTypeId() == TYPEID_UNIT && (ToCreature()->isPet() || ToCreature()->isGuardian())) // warlock pet mastery hackfix
    {
        if (auto owner = GetOwner())
        {
            if (auto Powner = owner->ToPlayer())
            {
                if ((Powner->getClass() == CLASS_WARLOCK) && Powner->IsMasteryLearned() && (Powner->GetPrimaryTalentTree(Powner->GetActiveSpec()) == TALENT_TREE_WARLOCK_DEMONOLOGY))
                {
                    float masteryBonusPct = ((Powner->GetRatingBonusValue(CR_MASTERY) + 8) * 2.3f) / 100.f + 1.f;
                    DoneTotalMod *= masteryBonusPct;
                }
            }
        }
    }
    ...
    for (auto i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
    {
        ...
        for (auto itr = mTotalAuraList.begin(); itr != mTotalAuraList.end(); ++itr)
    ...
    for (auto i = mDamageDoneVersus.begin(); i != mDamageDoneVersus.end(); ++i)
    ...
    for (auto i = mDamageDoneVersusAurastate.begin(); i != mDamageDoneVersusAurastate.end(); ++i)
    ...
    for (auto i = mDamageDoneByPower.begin(); i != mDamageDoneByPower.end(); ++i)
        if (( *i )->GetMiscValue() & spellProto->GetSchoolMask())
        {
    ...
    for (auto i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch (( *i )->GetMiscValue())
        {
            ...
            case 4993:
            {
                ...
                for (auto itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                {
        ...
        case SPELLFAMILY_MAGE:
            ...
            if (spellProto->GetSchoolMask() & SPELL_SCHOOL_MASK_ARCANE)
            {
                if (victim->HasAuraWithMechanic(( 1 << MECHANIC_SNARE ) | ( 1 << MECHANIC_SLOW_ATTACK )))
                {
                    ...
                    for (auto i = mDumyAuras.begin(); i != mDumyAuras.end(); ++i)
                    {
...
uint32 Unit::SpellDamageBonusTaken(SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, uint64 casterGUID)
{
    ...
    for (auto i = mOwnerTaken.begin(); i != mOwnerTaken.end(); ++i)
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask)
{
    ...
    for (auto i = mDamageDone.begin(); i != mDamageDone.end(); ++i)
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (auto i = mDamageDoneOfStatPercent.begin(); i != mDamageDoneOfStatPercent.end(); ++i)
        {
        ...
        for (auto i =mDamageDonebyAP.begin(); i != mDamageDonebyAP.end(); ++i)
            if ((*i)->GetMiscValue() & schoolMask)
        ...
        for (auto i = mSpellPowerPct.begin(); i != mSpellPowerPct.end(); ++i)
            tmpMod += (*i)->GetAmount();
...
int32 Unit::SpellBaseDamageBonusTaken(SpellSchoolMask schoolMask)
{
    ...
    for (auto i = mDamageTaken.begin(); i != mDamageTaken.end(); ++i)
...
bool Unit::isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType, Spell* spell) const
{
    ...
    for (auto i = critAuras.begin(); i != critAuras.end(); ++i)
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    for (auto i = mHealingDonePct.begin(); i != mHealingDonePct.end(); ++i)
        tmpMod += (*i)->GetAmount();
    ...
    for (auto i = mHealingDonePctWithAura.begin(); i != mHealingDonePctWithAura.end(); ++i)
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    for (auto i = mHealingGet.begin(); i != mHealingGet.end(); ++i)
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask)
{
    ...
    for (auto i = mHealingDone.begin(); i != mHealingDone.end(); ++i)
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (auto i = mHealingDoneOfStatPercent.begin(); i != mHealingDoneOfStatPercent.end(); ++i)
        {
        ...
        for (auto i = mHealingDonebyAP.begin(); i != mHealingDonebyAP.end(); ++i)
            if ((*i)->GetMiscValue() & schoolMask)
                spellPower += int32(CalculatePct(GetTotalAttackPowerValue(BASE_ATTACK), (*i)->GetAmount()));
...
bool Unit::IsImmunedToDamage(SpellInfo const* spellInfo, bool checkSchoolMask, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    if (checkSchoolMask && spellInfo->Id != 42292 && spellInfo->Id != 59752 && spellInfo->Id != 76780 && spellInfo->Id != 19975)
    {
        // If m_immuneToSchool type contain this school type, IMMUNE damage.
        SpellImmuneList const& schoolList = m_spellImmune[IMMUNITY_SCHOOL];
        for (auto itr = schoolList.begin(); itr != schoolList.end(); ++itr)
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo, Unit* caster, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    if (spellInfo->Id != 42292 && spellInfo->Id != 59752 && spellInfo->Id != 53490 && spellInfo->Id != 70029 && spellInfo->Id != 76780 && spellInfo->Id != 19975)
    {
        SpellImmuneList const& schoolList = m_spellImmune[IMMUNITY_SCHOOL];
        for (auto itr = schoolList.begin(); itr != schoolList.end(); ++itr)
        {
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    if (spellInfo->Id == 19386)
        return false;

    if (spellInfo->Id == 49560 && HasUnitState(UNIT_STATE_CHARGING))
        return true;
    ...
    if (uint32 aura = spellInfo->Effects[index].ApplyAuraName)
    {
        ...
        for (auto iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    for (auto i = mDamageDoneCreature.begin(); i != mDamageDoneCreature.end(); ++i)
        if (creatureTypeMask & uint32((*i)->GetMiscValue()))
    ...
    if (attType == RANGED_ATTACK)
    {
        ...
        for (auto i = mCreatureAttackPower.begin(); i != mCreatureAttackPower.end(); ++i)
            if (creatureTypeMask & uint32((*i)->GetMiscValue()))
                APbonus += (*i)->GetAmount();
    }
    else
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    for (auto i = IgnoreResistAuras.begin(); i != IgnoreResistAuras.end(); ++i)
    {
    ...
    //for (auto i = mclassScritAuras.begin(); i != mclassScritAuras.end(); ++i)
    //{
    //    switch ((*i)->GetMiscValue())
    //    {
    //    }
    ...
    if (attType != RANGED_ATTACK)
    {
        ...
        for (auto i = mModMeleeDamageTakenPercent.begin(); i != mModMeleeDamageTakenPercent.end(); ++i)
            tmpMod += (*i)->GetAmount();
    }
    else
    {
        AuraEffectList const& mModRangedDamageTakenPercent = GetAuraEffectsByType(SPELL_AURA_MOD_RANGED_DAMAGE_TAKEN_PCT);
        for (auto i = mModRangedDamageTakenPercent.begin(); i != mModRangedDamageTakenPercent.end(); ++i)
            tmpMod += (*i)->GetAmount();
    }
...
void Unit::ApplyAllSpellImmune(uint32 spellId, bool apply)
{
    ...
    for (Mechanics mechanic : mechanics)
        ApplySpellImmune(spellId, IMMUNITY_MECHANIC, mechanic, apply);
}
...
class RemoveMountCastEmote : public BasicEvent
{
public:
    RemoveMountCastEmote(Player* player) : _player(player) { }

    bool Execute(uint64 /*execTime*/, uint32 /*diff*/)
    {
        if (_player->IsInWorld())
        {
            _player->HandleEmoteCommand(0);
        }
        return true;
    }
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    if (auto player = ToPlayer())
    {
        ...
        {
            player->m_Events.AddEvent(new RemoveMountCastEmote(player), player->m_Events.CalculateTime(500));
            player->m_Events.AddEvent(new RemoveMountCastEmote(player), player->m_Events.CalculateTime(2000));
        }
...
void Unit::Dismount()
{
    if (!IsMounted() || this == NULL || this == nullptr || !this)
        return;

    SetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID, 0);
    RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT);

    if (auto thisPlayer = ToPlayer())
    ...
    if (ToPlayer())
    {
        ToPlayer()->SetUnderACKmountAdvanced(); //for momentum
        ToPlayer()->SetIsAllowedToFall(true); //when falling, actually falling - don't consider it as fall hack (angle)
    }
...
void Unit::CombatStart(Unit* target, bool initialAggro)
{
    if (initialAggro && !target->HasUnitState(UNIT_STATE_EVADE))
    {
        ...
        if (target->GetTypeId() == TYPEID_UNIT && !target->ToCreature()->isPet())
            if (!target->getThreatManager().getThreat(this, false))
                target->AddThreat(this, 1.0f);
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (PvP || (enemy && (enemy->ToCreature() && enemy->ToCreature()->GetScriptName() == "npc_training_dummy") || enemy->IsVehicle()))
        m_CombatTimer = 5500;
    ...
    if (auto creature = ToCreature())
    {
        ...
        if (enemy)
        {
            if (!getThreatManager().getThreat(enemy, true))
                AddThreat(enemy, 1.0f);
            ...
            if (IsAIEnabled)
            {
                ...
                if (auto o = enemy->GetOwner())
                    if (o->ToPlayer())
                        RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC); // unit has engaged in combat, remove immunity so players can fight back

                if (auto script = GetInstanceScript())
                    script->OnCreatureEnterCombat(creature);
            }
...
void Unit::ClearInCombat()
{
    ...
    if (auto creature = ToCreature())
    if (auto t = creature->GetCreatureTemplate())
    {
        if (t->unit_flags & UNIT_FLAG_IMMUNE_TO_PC)
...
bool Unit::isTargetableForAttack(bool checkFakeDeath, bool attackedbyPlayer) const
{
    if (!isAlive())
        return false;
    if (!IsInWorld())
    ...
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC) && attackedbyPlayer)
        return false;
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    {
        if (playerAttacker->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_COMMENTATOR))
            return false;
    }
    ...
    if (playerAffectingAttacker && playerAffectingTarget)
    {
        if (target->GetByteValue(UNIT_FIELD_BYTES_2, 1) & UNIT_BYTE2_FLAG_PVP_ENABLING)
            return true;
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    // !target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) &&
        ...
        && !((target->GetByteValue(UNIT_FIELD_BYTES_2, 1) & UNIT_BYTE2_FLAG_PVP_ENABLING)))
    {
...
bool Unit::IsAlwaysVisibleFor(WorldObject const* seer) const
{
    ...
    if (Player const* seerPlayer = seer->ToPlayer())
        if (auto owner =  GetOwner())
            if (auto ownerPlayer = owner->ToPlayer())
    ...
    switch (mtype)
    {
        ...
        case MOVE_RUN:
        {
            ...
            else
            {
                ...
                if (auto p = ToPlayer())
                    if (p->HasSpell(21009))
                        if (p->HasAuraType(SPELL_AURA_MOD_STEALTH))
                            stack_bonus += 0.05f;
            }
            break;
        }
            ...
            if (GetTypeId() == TYPEID_UNIT)
            {
                ...
                if (auto owner = GetCharmerOrOwner())
                {
                        ...
                        ToCreature()->HasUnitState(UNIT_STATE_FOLLOW) && !ToCreature()->isInCombat())
                    {
                        ...
                        speed *= std::min(0.6f + (GetDistance(owner) / 10.0f), 1.2f);
                    }
                }
            }
            break;
...
void Unit::setDeathState(DeathState s)
{
    ...
    else if (s == JUST_RESPAWNED || s == ALIVE)
...
bool Unit::CanHaveThreatList() const
{
    ...
    if (HasUnitState(UNIT_STATE_EVADE))//https://forum.projectnelth.com/bugtracker/combat-bug-reflecting-damage-to-evading-npcs-r68/
        return false;
    ...
    //    return false;
    if (auto c = ToCreature())
        if (auto cinfo = c->GetCreatureTemplate())
...
void Unit::DeleteThreatList()
{
    ...
    {
        for (auto i : m_ThreatManager.getOnlineContainer().getThreatList())
        {
            if (auto target = i->getSource())
                if (auto this_tar = target->getOwner())
                {
                    this_tar->_removeAttacker(this);
                    _removeAttacker(this_tar);
                }
        }
...
void Unit::TauntApply(Unit* taunter)
{
    ...
    {
        float myThreat = getThreatManager().getThreat(taunter);
        float itsThreat = getThreatManager().getCurrentVictim()->getThreat();
        if (itsThreat > myThreat)
            getThreatManager().addThreat(taunter, itsThreat - myThreat);
    }

    //Set aggro victim to caster
    if (!getThreatManager().getOnlineContainer().empty())
        if (HostileReference* forcedVictim = getThreatManager().getOnlineContainer().getReferenceByTarget(taunter))
            getThreatManager().setCurrentVictim(forcedVictim);

    if (ToCreature()->IsAIEnabled && !ToCreature()->HasReactState(REACT_PASSIVE))
...
Unit* Creature::SelectVictim()
{
    ...
    */
    ...
    if (tauntAuras.size())
    for (auto aura : tauntAuras)
        if (auto caster = aura->GetCaster())
        {
            if (!caster->isAlive())
                continue;

            if (!canSeeOrDetect(caster, true))
                continue;

            if (!IsValidAttackTarget(caster))
                continue;

            if (auto c = ToCreature())
                if (caster->isInAccessiblePlaceFor(c))
                    continue;

            if (_IsTargetAcceptable(caster) && canCreatureAttack(caster))
            target = caster;
        }
    ...
    */
    ...
    if (!target)
        if (getThreatManager().getThreatList().size())
            if (auto threat_target = getThreatManager().getHostilTarget())
    ...
    if (!target)
    if (m_attackers.size())                                         //check that lists have a size before iterating through them.
        for (auto attacker : m_attackers)
        {
            if (attacker->GetTypeId() != TYPEID_PLAYER)                 //attacker is not a player
                if (!canCreatureAttack(attacker))                           // attacker can be attacked
                    continue;

            if (auto c_att = attacker->ToCreature())                    // attacker is a creature
                if (c_att->HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN)) // attacker is a guardian
                    continue;

            if (_IsTargetAcceptable(attacker) && canCreatureAttack(attacker))
            target = attacker;
        }
    ...
    else if (!target)
    {
        ...
        if (auto help_t = getAttackerForHelper())
            if (_IsTargetAcceptable(help_t) && canCreatureAttack(help_t))
            target = help_t;

        if (isSummon())
            if (auto t = ToTempSummon())
                if (auto owner = t->GetOwner())
                {
                    if (owner->isInCombat())
                        if (auto owner_target = owner->getAttackerForHelper())
                            if (_IsTargetAcceptable(owner_target) && canCreatureAttack(owner_target))
                            target = owner_target;
                    ...
                    */
                    for (auto itr : owner->m_Controlled)
                        if (itr->isInCombat())
                            if (auto other_minion_target = itr->getAttackerForHelper())
                                if (_IsTargetAcceptable(other_minion_target) && canCreatureAttack(other_minion_target))
                                target = other_minion_target;
                }
    }
    ...
    if (auto found_target = SelectNearestTargetInAttackDistance(m_CombatDistance ? m_CombatDistance : ATTACK_DISTANCE))
        if (_IsTargetAcceptable(found_target) && canCreatureAttack(found_target))
            target = found_target;
    ...
    if (m_attackers.size())                                         //check that lists have a size before iterating through them.
        for (auto itr = m_attackers.begin(); itr != m_attackers.end(); ++itr)
            if (auto attacker = *itr)
            {
                if (attacker->GetTypeId() != TYPEID_PLAYER)                 //attacker is not a player
                    if (!canCreatureAttack(attacker))                           // attacker can be attacked
                        if (auto c_att = attacker->ToCreature())                    // attacker is a creature
    ...
    if (target)
        if (auto m = GetMap())
        if (_IsTargetAcceptable(target) && canCreatureAttack(target))
        {

            if (auto v = GetVehicle())
                if (auto b = v->GetBase())
                    if (auto bc = b->ToCreature())
                    {
                        if (!bc->_IsTargetAcceptable(target) || !bc->canCreatureAttack(target))
                            return NULL;
                    }

            if (isWorldBoss())//world bosses can have infinite aggro range
            {
                return target;
            }
            else if ((GetGUID() != GetDBTableGUIDLow()) && m->Instanceable())  //instanced creatures can aggro throughout the dungeon
            {
                return target;
            }
            else if (GetDistance(GetHomePosition()) < (MAX_AGGRO_RADIUS * 1.33f))
            {
                return target;
            }
            else if (time(NULL) - GetLastDamagedTime() <= MAX_AGGRO_RESET_TIME)
            {
                return target;
            }
        }
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group)
{
    for (Diminishing::iterator i = m_Diminishing.begin(); i != m_Diminishing.end(); ++i)
    {
        ...
        if (i->stack == 0 && getMSTimeDiff(i->hitTime, getMSTime()) > (15000 + i->hitTimeRandomComponent))
        {
...
void Unit::ApplyDiminishingAura(DiminishingGroup group, bool apply)
{
    ...
    for (Diminishing::iterator i = m_Diminishing.begin(); i != m_Diminishing.end(); ++i)
    {
        ...
        else if (i->stack)
        {
            ...
            if (i->stack == 0)
            {
                i->hitTimeRandomComponent = (rand() % 5000);
                //TC_LOG_ERROR("entities.unit", "random component set to %u mS", i->hitTimeRandomComponent);
                i->hitTime = getMSTime();
            }
        }
        break;
...
void Unit::CleanupsBeforeDelete(bool finalCleanup)
{
    ...
    if (auto thisCreature = ToCreature())
        if (auto t = GetTransport())
            t->RemovePassenger(thisCreature);
}
  ...
  _stayX(0.0f), _stayY(0.0f), _stayZ(0.0f)
{
    for (uint8 i = 0; i < MAX_SPELL_CHARM; ++i)
        _charmspells[i].SetActionAndType(0, ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO);
...
{
    if (_unit->GetTypeId() == TYPEID_UNIT)
        if (auto creature = _unit->ToCreature())
            creature->SetReactState(_oldReactState);
}

void CharmInfo::InitPetActionBar()
{
    SetActionBar(ACTION_BAR_INDEX_START, COMMAND_ATTACK, ACTIONBAR_PET_MOTIONCOMMAND);
    SetActionBar(ACTION_BAR_INDEX_START + 1, COMMAND_FOLLOW, ACTIONBAR_PET_MOTIONCOMMAND);
    SetActionBar(ACTION_BAR_INDEX_START + 2, COMMAND_MOVE_TO, ACTIONBAR_PET_MOTIONCOMMAND);

    // middle 4 SpellOrActions are spells/special attacks/abilities
    for (uint32 i = 0; i < ACTION_BAR_INDEX_PET_SPELL_END-ACTION_BAR_INDEX_PET_SPELL_START; ++i)
        SetActionBar(ACTION_BAR_INDEX_PET_SPELL_START + i, 0, ACT_PASSIVE);

    SetActionBar(ACTION_BAR_INDEX_PET_SPELL_END, REACT_ASSIST, ACTIONBAR_PET_REACTSTATE);
    SetActionBar(ACTION_BAR_INDEX_PET_SPELL_END + 1, REACT_DEFENSIVE, ACTIONBAR_PET_REACTSTATE);
    SetActionBar(ACTION_BAR_INDEX_PET_SPELL_END + 2, REACT_PASSIVE, ACTIONBAR_PET_REACTSTATE);
}
...
void CharmInfo::InitCharmCreateSpells(bool petSpells /* = true*/)
{
    ...
    for (uint32 x = 0; x < MAX_SPELL_CHARM; ++x)
    {
        ...
        if (!spellInfo || spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD)
        {
            _charmspells[x].SetActionAndType(spellId, ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO);
            continue;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    newstate = ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST;
                    ToggleCreatureAutocast(spellInfo, true);
                }
                else
                    newstate = ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO;
            }
...
bool CharmInfo::AddSpellToActionBar(SpellInfo const* spellInfo, ActiveStates newstate)
{
    ...
    for (uint8 i = 0; i < MAX_UNIT_ACTION_BAR_INDEX; ++i)
    {
        ...
        {
            SetActionBar(i, spell_id, newstate == ACT_DECIDE ? spellInfo->IsAutocastable() ? ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO : ACT_PASSIVE : newstate);
            return true;
        }
    }
    return false;
...
void CharmInfo::ToggleCreatureAutocast(SpellInfo const* spellInfo, bool apply)
{
    ...
    for (uint32 x = 0; x < MAX_SPELL_CHARM; ++x)
        if (spellInfo->Id == _charmspells[x].GetAction())
            _charmspells[x].SetType(apply ? ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST : ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO);
}
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    for (; index < ACTION_BAR_INDEX_END; ++iter, ++index)
    {
        ...
        //TC_LOG_ERROR("sql.sql", "pet guid %u setting action slot %u to %u (type %u)", _unit->GetGUID(), index, action, type);
        SetActionBar(index, action, type);
...
void CharmInfo::SetSpellAutocast(SpellInfo const* spellInfo, bool state)
{
    for (uint8 i = 0; i < MAX_UNIT_ACTION_BAR_INDEX; ++i)
    {
        ...
        {
            PetActionBar[i].SetType(state ? ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST : ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO);
            break;
        }
    }
}
...
void CharmInfo::SetActionBar(uint8 index, uint32 spellOrAction, ActiveStates type)
{
        ...
        //TC_LOG_ERROR("sql.sql", "%u set to action %u and type %u", index, PetActionBar[index].GetAction(), PetActionBar[index].GetType());
}
...
void Unit::ProcJumpAuras()
{

    Unit* actor = this;
    Unit* actionTarget = this;
    Unit* target = this;
    auto procSpell = sSpellMgr->GetSpellInfo(NULL);
    uint32 procFlag = PROC_FLAG_JUMP;
    uint32 procExtra = 0;
    uint32 absorbed = 0;
    uint32 damage = 0;
    bool isVictim = true;
    uint32 resisted = 0;
    uint32 blocked = 0;
    uint32 armorMitigated = 0;
    auto interruptedSpell = sSpellMgr->GetSpellInfo(NULL);
    bool procSpellIsHeal = false;
    WeaponAttackType attType = MAX_ATTACK;
    uint32 effectMask = 0;

    DamageInfo damageInfo = DamageInfo(actor, actionTarget, 0, sSpellMgr->GetSpellInfo(NULL), SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE);
    damageInfo.SetAbsorb(0);
    damageInfo.SetResist(0);
    damageInfo.SetArmorMitigated(0);
    HealInfo healInfo = HealInfo(actor, actionTarget, 0, procSpell, SPELL_SCHOOL_MASK_NORMAL);
    healInfo.SetAbsorb(0);
    ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, 0, procExtra, NULL, &damageInfo, &healInfo);

    for (auto itr = GetAppliedAuras().begin(); itr != GetAppliedAuras().end(); ++itr)
        if (auto aurApp = itr->second)
        if (auto aura = aurApp->GetBase())
            if (auto aura_info = sSpellMgr->GetSpellInfo(aura->GetId()))
            if (aura_info->ProcFlags & PROC_FLAG_JUMP)
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (aurApp->HasEffect(i))
                if (auto aurEff = aura->GetEffect(i))
                    if (auto trigger_spell = aura_info->Effects[i].TriggerSpell)
                        if (!HasSpellCooldown(trigger_spell))
                        {

                            if (!aura->CallScriptCheckProcHandlers(aurApp, eventInfo))
                                continue;
                           
                            CastSpell(this, trigger_spell, true);
                            aura->CallScriptAfterProcHandlers(aurApp, eventInfo);
                        }
}
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_SPELL_MAGNET:
                    {
                        if (!onCast && procSpell && procSpell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC)
                        {
                            ...
                            if (!cantTrigger)
                                if (auto magnet = triggeredByAura->GetBase()->GetUnitOwner())
...
void Unit::GetProcAurasTriggeredOnEvent(std::list<AuraApplication*>& aurasTriggeringProc, std::list<AuraApplication*>* procAuras, ProcEventInfo eventInfo)
{
    ...
    if (procAuras)
    {
        for (auto itr = procAuras->begin(); itr!= procAuras->end(); ++itr)
        {
...
{
    for (auto itr = aurasTriggeringProc.begin(); itr != aurasTriggeringProc.end(); ++itr)
    {
        if (!(*itr)->GetRemoveMode())
...
Pet* Unit::CreateTamedPetFrom(Creature* creatureTarget, uint32 spell_id)
{
    ...
    uint8 level = (getLevel() > uint8(85) ? uint8(85) : getLevel());
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
    ...
    if (jumps > 0)
    {
        if (auto caster = triggeredByAura->GetCaster())
        {
            ...
            if (auto target = GetRaidMemberOrPetByHealth(radius, true))
            {
...
bool Unit::HandleAuraRaidProcFromCharge(AuraEffect* triggeredByAura)
{
    ...
    if (jumps > 0)
    {
        ...
        {
            float radius = triggeredByAura->GetSpellInfo()->Effects[triggeredByAura->GetEffIndex()].CalcRadius(caster);
            if (auto target= GetNextRandomRaidMemberOrPet(radius))
            {
...
void Unit::PlayAIAnimKit(uint16 animKitId)
{
...
void Unit::PlayMeleeAnimKit(uint16 animKitId)
{
    WorldPacket data(SMSG_SET_MELEE_ANIM_KIT, 7 + 2);
    data.append(GetPackGUID());
    data << uint16(animKitId);
    SendMessageToSet(&data, true);
}

void Unit::PlayMovementAnimKit(uint16 animKitId) 
{
    WorldPacket data(SMSG_SET_MOVEMENT_ANIM_KIT, 7 + 2);
    data.append(GetPackGUID());
    data << uint16(animKitId);
    SendMessageToSet(&data, true);
}
...
void Unit::Kill(Unit* victim, bool durabilityLoss, bool isRewardAllowed)
{
    ...
    if (creature)
    {
        ...
        if (auto instance = creature->GetInstanceScript())
            if (instance->IsChallengeModeStarted() || instance->IsChallengeModeFinished())
    ...
    {
        // Exploit fix
        if (creature && creature->GetOwnerGUID() == player->GetGUID())
        ...
        if (isLootRewardAllowed)
        {
            ...
            if (creature)
            {
                ...
                loot->generateMoneyLoot(creature->GetCreatureTemplate()->mingold, creature->GetCreatureTemplate()->maxgold, this);
    ...
    {
        AuraEffectList const& dummyAuras = victim->GetAuraEffectsByType(SPELL_AURA_DUMMY);
        for (auto itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
        {
    ...
    if (auto killerPlr = ToPlayer())
    {
        if (auto killedPlr = victim->ToPlayer())
            sScriptMgr->OnPVPKill(killerPlr, killedPlr);
        else if (auto killedCre = victim->ToCreature())
            sScriptMgr->OnCreatureKill(killerPlr, killedCre);
    }
    else if (auto killerCre = ToCreature())
    {
        if (auto killed = victim->ToPlayer())
...
void Unit::SetStunned(bool apply)
{
    if (apply)
    {
        ...
        if (!movespline->isParabolic() && !HasUnitState(UNIT_STATE_CHARGING))
            StopMoving();
        ...
        // if there is a possibility for charge vs charge then call this on update to let charge spline happen first
        if (!this->HasAura(7922) && !this->HasAura(20253))
            SendMoveRoot();
        else m_needSendMoveRoot = true;
...
{
    if (auto veh = GetVehicleBase())
        if (auto c = veh->ToCreature())
            return c;
...
{
    SetLayerMask(GetProperLayerMask(), true, GetMap());
...
void Unit::KnockbackFrom(float x, float y, float speedXY, float speedZ)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        player = ToPlayer();
    else if (auto charmer = GetCharmer())
    {
    ...
    else
    {
        ...
        if (auto player_found = ObjectAccessor::FindPlayer(GetGUID()))
        {
            player_found->SetSkipOnePacketForASH(true);
            player_found->SetUnderACKmount();
        }
    }
...
{
    uint8 currentRace;
    if (const Player* caster = ToPlayer())
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        switch (form)
        {
            case FORM_CAT:
                ...
                else if (currentRace == RACE_TROLL)
                {
                    uint8 hairColor = GetByteValue(PLAYER_BYTES, 3);
                    switch (hairColor)
                    {
                ...
                else if (currentRace == RACE_WORGEN)
                {
                    // Based on Skin color
                ...
                // Based on Skin color
                else if (currentRace == RACE_TAUREN)
                {
                    uint8 skinColor = GetByteValue(PLAYER_BYTES, 0);
                    // Male
                    if (getGender() == GENDER_MALE)
                ...
                else if (Player::TeamForRace(currentRace) == ALLIANCE)
                    return 892;
                else
                    return 8571;
            case FORM_BEAR:
            ...
            case FORM_FLIGHT:
                if (Player::TeamForRace(currentRace) == ALLIANCE)
                    return 20857;
                else if (currentRace == RACE_TROLL)
                    return 38252;
                return 20872;
    ...
    if (formEntry && formEntry->modelID_A)
    {
        ...
        else
        {
            ...
            if (!modelid && Player::TeamForRace(currentRace) == HORDE)
                modelid = formEntry->modelID_A;
        }
    }
...
void Unit::JumpTo(float speedXY, float speedZ, bool forward, bool withCollision)
{
    ...
    else
    {
        ...
        ObjectAccessor::FindPlayer(GetGUID())->SetUnderACKmount();
        ObjectAccessor::FindPlayer(GetGUID())->SetSkipOnePacketForASH(true);
    }
}
...
void Unit::ExitVehicle(Position const* exitPosition /*= NULL*/)
{
    ...
    /*
        Prevent camera from bugging under the water.
    */
    if (!HasAuraType(SPELL_AURA_WATER_WALK))
        RemoveUnitMovementFlag(MOVEMENTFLAG_WATERWALKING);

        if (auto p = ToPlayer())
            p->SendMovementWaterWalking();
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    Position dest{ x, y, z, orientation };
    if (relocated)
    {
        SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);
...
void Unit::RewardRage(uint32 baseRage, bool attacker, bool isMagic)
{
    ...
    else
    {
        ...
        //if (isMagic)
            addRage = floor(0.5f + (45.50f * baseRage / GetMaxHealth()));//You can see here that 100% health generates roughly 45% rage in every case. https://www.youtube.com/watch?v=vFVKI8F0gYk&ab_channel=Swifty
        //else
...
void Unit::OutDebugInfo() const
{
    ...
    if (IsVehicle())
    {
        ...
        for (SeatMap::iterator itr = GetVehicleKit()->Seats.begin(); itr != GetVehicleKit()->Seats.end(); ++itr)
            if (auto passenger = ObjectAccessor::GetUnit(*GetVehicleBase(), itr->second.Passenger.Guid))
                o << passenger->GetGUID() << ", ";
        TC_LOG_INFO("entities.unit", "%s", o.str().c_str());
    }
...
uint32 Unit::GetRemainingPeriodicAmount(uint64 caster, uint32 spellId, AuraType auraType, uint8 effectIndex) const
{
    ...
    for (auto i = periodicAuras.begin(); i != periodicAuras.end(); ++i)
    {
...
Stats Unit::GetPrimaryStat()
{
    Stats highest{ STAT_INTELLECT };

    for (int8 i = STAT_STRENGTH; i < MAX_STATS; ++i)
        if (i != STAT_STAMINA && i != STAT_SPIRIT)
            if (GetTotalStatValue(Stats(i)) > GetTotalStatValue(Stats(highest)))
                highest = Stats(i);
...
{
    if (auto target = getThreatManager().getHostilTarget())
        m_attacking = target;
...
bool Unit::IsVisionObscured(Unit* target, bool ranged)
{
    ...
    for (auto i = vAuras.begin(); i != vAuras.end(); ++i)
    {
    ...
    for (auto i = myAuras.begin(); i != myAuras.end(); ++i)
    {
        myAura = (*i)->GetBase();
...
bool Unit::CanExecutePendingSpellCastRequest(SpellInfo const* spellInfo, bool without_queue/* = false*/) const
{
    if (!ToPlayer())
        return false;

    auto request{ GetCastRequest(spellInfo->StartRecoveryCategory) };

    //check already queued spell
    if (!without_queue)
    if (request)
    {
        if (!request->active)
    ...
    if (auto caster = this)
    {
        if (auto cast_player = ObjectAccessor::FindPlayer(caster->GetGUID()))
        {
            auto cd_mgr = cast_player->GetGlobalCooldownMgr();


            if (cd_mgr.GetGlobalCooldown(spellInfo) > uint32(0))
            {
                if (request && !without_queue)
                if (cd_mgr.GetGlobalCooldown(spellInfo) > uint32(GetSpellQueueWindow()))
                    cast_player->CancelPendingCastRequest(spellInfo->StartRecoveryCategory);

                //TC_LOG_ERROR("sql.sql", "global cooldown for %u is %u", spellInfo->Id, cd_mgr.GetGlobalCooldown(spellInfo));
                return false;
            }
    ...
    if (GetSpellCooldownDelay(spellInfo->Id) > (without_queue ? 0 : GetSpellQueueWindow()))
        return false;

    //check spell in progress
    for (auto spellSlot : { CURRENT_MELEE_SPELL, CURRENT_GENERIC_SPELL })
        if (auto spell = GetCurrentSpell(spellSlot))
        {
            ...
            if (request)
                if (request->spell_id)
                    if (auto info = sSpellMgr->GetSpellInfo(request->spell_id))
                        if (info->IsAutoshot())
                            autoshot = true;

                    if (IsNonMeleeSpellCasted(false, true, true, autoshot))
                    {
                        //TC_LOG_ERROR("sql.sql", "CanExecutePendingSpellCastRequest false 3");
                        return false;
                    }
        }

    //TC_LOG_ERROR("sql.sql", "CanExecutePendingSpellCastRequest (%u) is true", !without_queue);
    return true;
}

bool Unit::IsSpellQueueEnabled() const
{
    /*
        DEFAULT RETURN
    */
...
void Unit::SetPendingCastRequest(PendingSpellCastRequest new_request)
{
    auto info = sSpellMgr->GetSpellInfo(new_request.spell_id);
...
void Unit::CancelPendingCastRequest(uint32 category)
{
    auto request = GetCastRequest(category);
    if (!request)
    {
        //TC_LOG_ERROR("sql.sql", "CancelPendingCastRequest category %u", category);
        return;
    }
    else
    {
        auto packet = m_pendingCasts[category].request_packet;
        if (auto p = ToPlayer())
            if (auto s = p->GetSession())
            {
                m_pendingCasts[category].cancel_in_progress = true;
                s->HandleCastSpellOpcode(packet);
            }
...
uint16 Unit::GetSpellQueueWindow(bool gcd) const
{
    /*
...
PendingSpellCastRequest* Unit::GetCastRequest(SpellInfo const* spellInfo) const
{
    if (spellInfo)
...
PendingSpellCastRequest* Unit::GetCastRequest(uint32 gcd_category) const
{
...
void Unit::ClearCastRequest(SpellInfo const* info)
{
    if (info)
        ClearCastRequest(info->StartRecoveryCategory);
}

void Unit::ClearCastRequest(uint32 category)
{
...
bool Unit::CanRequestSpellCast(SpellInfo const* spellInfo) const
{
    if (IsSpellQueueEnabled())
    {
        ...
        auto castRequest = GetCastRequest(spellInfo);
        if (castRequest)
        ...
        if (auto caster = this)
            ...
            {
                auto cd_mgr = cast_player->GetGlobalCooldownMgr();
                if (cd_mgr.GetGlobalCooldown(spellInfo) > uint32(GetSpellQueueWindow(true)))
                    return false;
            }
        ...
        for (CurrentSpellTypes spellSlot : { CURRENT_MELEE_SPELL, CURRENT_GENERIC_SPELL })
            if (auto spell = GetCurrentSpell(spellSlot))
                if (uint16(spell->GetCastTimeRemaining(true)) > GetSpellQueueWindow())
                    return false;
...
void Unit::ProcessPendingSpellCastRequest(uint32 category)
{
    auto request = GetCastRequest(category);
    if (!request)
        return;
    ...
    if (!CanExecutePendingSpellCastRequest(sSpellMgr->GetSpellInfo(request->spell_id))) return;

    auto packet = request->request_packet;
    if (packet.empty())
    {
    ...
    if (auto p = ToPlayer())
        if (auto s = p->GetSession())
        {
            ...
            /*for (auto spellSlot : { CURRENT_MELEE_SPELL, CURRENT_GENERIC_SPELL })
                if (auto spell = GetCurrentSpell(spellSlot))
                    if (auto id = m_pendingCasts[category].spell_id)
                        if (auto info = spell->GetSpellInfo())
                            if (id == info->Id)
                                spell->update(1);*/
...
void Unit::RemoveSameTickQueueBlock(uint32 category)
{
    if (m_SameTickBlockList.size())
        if (m_SameTickBlockList.find(category) != m_SameTickBlockList.end())
        {
            ...
            //TC_LOG_ERROR("sql.sql", "removing same tick block from category %u at %u", category, getMSTime());
        }
}

void Unit::AddSameTickQueueBlock(uint32 category)
{
    //TC_LOG_ERROR("sql.sql", "adding same tick block to category %u at %u", category, getMSTime());
    m_SameTickBlockList[category] = getMSTime();
}


bool Unit::HasSameTickQueueBlock(uint32 category, bool ignore_time) const
{
    if (m_SameTickBlockList.size())
        if (m_SameTickBlockList.find(category) != m_SameTickBlockList.end())
        {
            auto block = m_SameTickBlockList.find(category);
            auto time = block->second;
...
void Unit::ExecuteSortedCastRequests()
{
    ...
    if (organized_list.size())
        ...
        {
            //auto request = GetCastRequest(i->second);
            //if (request)TC_LOG_ERROR("sql.sql", "time: %u, spell: %u", i->first, request->spell_id);
