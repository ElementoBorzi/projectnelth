...
enum SpellValueMod
{
    ...
    SPELLVALUE_SPEED,
    SPELLVALUE_DURATION,        // final values, no mods anymore
    SPELLVALUE_MAXDURATION,     // final values, no mods anymore
    SPELLVALUE_CAST_TIME,
};
...
enum UnitBytes1_Flags
{
    ...
    UNIT_BYTE1_FLAG_ALWAYS_FLYING   = (UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER), //NPC will not appear to "stand" in the air when it pauses for a waypoint.
...
{
    UNIT_BYTE2_FLAG_PVP_ENABLING         = 0x01,
...
enum UnitFlags2
{
    ...
    UNIT_FLAG2_HIDE_NAMEPLATE                       = 0x04000000,   // hides unit name and hp bar, doesn't disable spellclick
    UNIT_FLAG2_IGNORE_SPELL_MIN_RANGE_RESTRICTIONS  = 0x08000000,   // Allows Player to ignore Spell Min Range Restrictions on Creatures
};
...
enum NPCFlags
{
    ...
    UNIT_NPC_FLAG_MAILBOX               = 0x04000000,       // Mailbox for npc
...
enum MovementFlags
{
    ...
    MOVEMENTFLAG_TRACKING_TARGET       = 0x40000000,
...
struct DiminishingReturn
{
    DiminishingReturn(DiminishingGroup group, uint32 t, uint32 count)
        : DRGroup(group), stack(0), hitTime(t), hitTimeRandomComponent(t), hitCount(count)
    {}

    DiminishingGroup        DRGroup:16;
    uint16                  stack:16;
    uint32                  hitTime;
    uint32                  hitTimeRandomComponent;
    uint32                  hitCount;
};
...
public:
    ...
    uint32 GetArmorMitigated() const { return m_armorMitigated; };
    void SetAbsorb(uint32 val) { m_absorb = val; }
    void SetResist(uint32 val) { m_resist = val; }
    void SetBlock(uint32 val) { m_block = val; }
    void SetArmorMitigated(uint32 val) { m_armorMitigated = val; }
...
enum ActiveStates
{
    ...
    //spell states
    ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO     = 0x81,                                    // 0x80 - castable
    ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST    = 0xC1,                                    // 0x40 | 0x80 - auto cast + castable

    // move,stay,follow
    ACTIONBAR_PET_MOTIONCOMMAND                      = 0x07,                                    // 0x01 | 0x02 | 0x04
    // passive, assist, defensive
    ACTIONBAR_PET_REACTSTATE                         = 0x06,                                    // 0x02 | 0x04
...
{
    UnitActionBarEntry() : packedData(uint32(ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO) << 24) {}
    ...
    bool IsActionBarForSpell() const
    {
        ...
        return Type == ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO || Type == ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST || Type == ACT_PASSIVE;
    }
...
struct CharmInfo
{
    public:
        ...
        void InitCharmCreateSpells(bool petSpellBar = true);
...
class Unit : public WorldObject
{
    public:
        ...
        float GetMeleeRange(const Unit* obj) const;
        bool IsWithinCombatRange(const Unit* obj, float dist2compare) const;
        bool IsWithinMeleeRange(const Unit* obj, float dist = MELEE_RANGE) const;
        float GetMeleeReEnagementDistance(const Unit* obj) const;
        ...
        bool AttackStop(Unit* specified_stopattack_target = nullptr);
        ...
        std::list<Unit*> SelectNearbyUnits(uint32 entry = 0, float dist = NOMINAL_MELEE_RANGE, bool alive = true) const;
        ...
        {
            return !HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_FLEEING | UNIT_STATE_FLEEING_MOVE | UNIT_STATE_IN_FLIGHT |
        ...
        float GetPowerPct(Powers power) const { return GetPower(power) ? 100.f * GetPower(power) / GetMaxPower(power) : 0.0f; }
        ...
        void setPowerType(Powers power, uint32 maxPowerValue = 0);
        ...
        void ApplyRegenMod(WeaponAttackType att, float val, bool apply);
        ...
        { 
            ...
            getHostileRefManager().deleteNewlyInvalidReferences();
        }
        ...
        bool IsPvP() const { return HasByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_PVP_ENABLING); }
        void SetPvP(bool state)
        {
            if (state)
                SetByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_PVP_ENABLING);
            else
                RemoveByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_PVP_ENABLING);
        }
        ...
        void PlayAIAnimKit(uint16 animKitId);
        void PlayMeleeAnimKit(uint16 animKitId);
        void PlayMovementAnimKit(uint16 animKitId);
        ...
        void Kill(Unit* victim, bool durabilityLoss = true, bool isRewardAllowed = true);
        int32 DealHeal(Unit* victim, uint32 addhealth);

        void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false, bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
        void ProcDamageAndSpellForAttacker(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorb, uint32 resisted, uint32 blocked, uint32 armorMitigated, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false, bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
        void ProcDamageAndSpellForVictim(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorb, uint32 resisted, uint32 blocked, uint32 armorMitigated, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false, bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
        void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false, bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
        void ProcJumpAuras();
        ...
        SpellMissInfo MeleeSpellHitResult(Unit* victim, SpellInfo const* spell, const uint8 effectMask = MAX_EFFECT_MASK);
        SpellMissInfo MagicSpellHitResult(Unit* victim, SpellInfo const* spell, const uint8 effectMask = MAX_EFFECT_MASK);
        SpellMissInfo SpellHitResult(Unit* victim, SpellInfo const* spell, bool canReflect = false, const uint8 effectMask = MAX_EFFECT_MASK);
        ...
        int32 GetBinaryResistance(Unit*, const SpellInfo*);
        ...
        bool isTargetableForAttack(bool checkFakeDeath = true, bool attackedbyPlayer = false) const;
        ...
        void CastCustomWithDelay(uint32 delay, Unit* victim, uint32 spellid, int32 bp0, int32 bp1, int32 bp2, bool triggered);
        ...
        void SendPlaySpellVisual(uint32 id, Unit* target, float x, float y, float z, float o);
        ...
        void NearTeleportTo(Position pos, bool /*casting*/ = false)  { NearTeleportTo(pos.m_positionX, pos.m_positionY, pos.m_positionZ, pos.m_orientation); }
        ...
        void JumpTo(float speedXY, float speedZ, bool forward = true, bool withCollision = false);
        ...
        */
        ...
        uint64 GetOwnerGUID() const { return  GetUInt64Value(UNIT_FIELD_SUMMONEDBY) ? GetUInt64Value(UNIT_FIELD_SUMMONEDBY) : NULL; }
        ...
        void RemoveAllMinionsForDuel();
        void SetCharm(Unit* target, bool apply);
        Unit* GetNextRandomRaidMemberOrPet(float radius);
        Unit* GetRaidMemberOrPetByHealth(float radius, bool ascending = true);
        bool SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp = NULL);
        void RemoveCharmedBy(Unit* charmer);
        void RestoreFaction();

        ControlList m_Controlled;
        ControlList getControlList() { return m_Controlled; };
        ...
        bool ConsumeAuraAtZeroCharges(SpellInfo const* auraInfo);
        ...
        void RemoveTriggeredAurasDueToSpell(uint32 spellId, uint64 castItemGuid = 0, uint64 casterGUID = 0, uint8 reqEffMask = 0, AuraRemoveMode removeMode = AURA_REMOVE_BY_DEFAULT);
        ...
        void InterruptSpell(CurrentSpellTypes spellType, bool withDelayed = true, bool withInstant = true, bool onNewCastedSpell = false, Spell* interruptSpell = NULL);
        ...
        void InterruptNegativeNonMeleeSpells(bool withDelayed, uint32 spellid = 0, bool withInstant = true);
        ...
        inline bool IsInBearForm() const
        {
            return GetShapeshiftForm() == FORM_BEAR;
        }
        ...
        DynamicObject* GetLastDynObject();
        ...
        void ApplyAllSpellImmune(uint32 spellId, bool apply);
        void ApplySpellDispelImmunity(const SpellInfo* spellProto, DispelType type, bool apply);
        virtual bool IsImmunedToSpell(SpellInfo const* spellInfo, Unit* caster = NULL, const uint8 effectMask = MAX_EFFECT_MASK);
                                                            // redefined in Creature
        bool IsImmunedToDamage(SpellSchoolMask meleeSchoolMask);
        bool IsImmunedToDamage(SpellInfo const* spellInfo, bool checkSchoolMask = true, const uint8 = MAX_EFFECT_MASK);
        ...
        void  UpdateSpeed(UnitMoveType mtype, bool forced, bool hardForced = false);
        float GetSpeed(UnitMoveType mtype) const;
        float GetSpeedRate(UnitMoveType mtype) const { return m_speed_rate[mtype]; }
        void SetSpeed(UnitMoveType mtype, float rate, bool forced = false, bool hardForce = false);
        float m_TempSpeed;
        ...
        ComboPointHolderSet& GetComboPointHolder() { return m_ComboPointHolders; }
        ...
        void RewardRage(uint32 baseRage, bool attacker, bool isMagic = false);
        ...
        bool CanExecutePendingSpellCastRequest(SpellInfo const* spellInfo, bool without_queue = false) const;
    ...
    private:
        bool IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent, const uint8 effectMask = MAX_EFFECT_MASK);
        bool HandleAuraProcOnPowerAmount(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, const uint8 effectMask = MAX_EFFECT_MASK);
        bool HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, SpellInfo const* interruptedSpell, bool procSpellIsHeal, bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
        bool HandleAuraProc(Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, bool * handled, const uint8 effectMask = MAX_EFFECT_MASK);
        bool HandleOverrideClassScriptAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 cooldown, const uint8 effectMask = MAX_EFFECT_MASK);
