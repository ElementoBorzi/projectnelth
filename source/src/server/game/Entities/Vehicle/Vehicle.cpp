...
void Vehicle::ApplyAllImmunities()
{
    ...
    if (_me->ToCreature() && _me->ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_MECHANICAL && !_me->ToCreature()->isWorldBoss())
    {
        ...
        // Removed IMMUNITY_STATE, SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN to allow Rogue Savage Combat 58413 to effect vehicle Defias Reaper/Watcher.
    ...
    switch (GetVehicleInfo()->m_ID)
    {
        ...
        case 452: // Strand of the Ancients
        case 453: // Strand of the Ancients
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    /*
        This is an experimental fix intended for isle of conquest where players ride on gunships (transports)
        ...
        if (player->GetZoneId() == 4710/*Isle of conquest*/)
            if (auto vehicleUnit = GetBase())
                if (Transport* VehicleBase_Transport = vehicleUnit->GetTransport())
                {
                    ...
                    player->SetTransport(VehicleBase_Transport);
                    VehicleBase_Transport->AddPassenger(player);
                    player->TeleportTo(vehicleUnit->GetMapId(), VehicleBase_Transport->GetPositionX(), VehicleBase_Transport->GetPositionY(), VehicleBase_Transport->GetPositionZ(), player->GetOrientation(), TELE_TO_NOT_LEAVE_TRANSPORT);
                }
...
public:
    ...
    bool Execute(uint64 /*execTime*/, uint32 /*diff*/) override
    {
        if (auto passenger = ObjectAccessor::GetUnit(*_base, _passengerGUID))
        {
            ...
            /// @hack For some reason the vehicle looses its model...
...
/*
*   Not all vehicle seats need to be facing front.
*/
float VehicleJoinEvent::GetSeatOrientation(uint32 seat_entry, uint32 base_entry, int8 seat_id)
{
    //TC_LOG_ERROR("sql.sql", "Seat entry: %u, base entry: %u, seat ID: %u");
    /*
        Specify by seat DBC entry
    */
    switch (seat_entry)
    {
    case 8962://Helix Gearbreaker
    case 8216://Redridge Boat - Messner
    case 8217://Redridge Boat - Jorgensen
    case 8218://Redridge Boat - Krakauer
    case 9569: // Stagecoach Carriage - Gwen Armstead
    case 9570: // Stagecoach Carriage - Krennan Aranas
    case 9571: // Stagecoach Carriage - Duskhaven Watchman
    case 7898: // Aessina Miracle Scene Vehicle
    case 7186: // Bloodhilts Carry Bunny (Southern Barrens)
        return M_PI;//Backwards
        break;
    default:
        break;
    }
    ...
    /*
        Specify by vehicle npc ID and possibly include seat ID.
        - Disabled until there's a case that uses this.
    */
    /*
    switch (base_entry)
    {
    //case IMAGINARY_NPC:
    //if (seat_id == IMAGINARY_SEAT)
    //  return M_PI*0.5f;// (left)
    default:
        break;
    }
    */
...
bool VehicleJoinEvent::Execute(uint64, uint32)
{
    ...
    */
    if (auto b = Target->GetBase())
    if (auto p = Passenger->ToPlayer())
    if (!b->IsUnderWater())
        if (!Passenger->HasUnitMovementFlag(MOVEMENTFLAG_WATERWALKING))
        {
            Passenger->AddUnitMovementFlag(MOVEMENTFLAG_WATERWALKING);
            p->SendMovementWaterWalking();
            Passenger->RemoveUnitMovementFlag(MOVEMENTFLAG_WATERWALKING);
        }
    /*
        Don't specify zero here, could mess up seats not set by our function.
    */

    if (GetSeatOrientation(veSeat->m_ID, Target->GetCreatureEntry(), Seat->first) != 0.f)
        Passenger->m_movementInfo.t_pos.Relocate(RelocatePosX, RelocatePosY, RelocatePosZ, GetSeatOrientation(veSeat->m_ID, Target->GetCreatureEntry(), Seat->first));
    else
    ...
    /*
        Some Boss NPCs need to remain targetable despite entering a vehicle.
        Examples:
        - Rajh uses an "enter vehicle" spell to leap
        - Helix gearbreaker enters players as a vehicle
    */
    switch (Passenger->GetEntry())
    {
    case 47296://helix Gearbreaker, enters players as vehicle and needs to remain targetable
    case 49674: //helix nightmare
    case 44648://unyielding behemothk, enters a vehicle to "float" before a jump
    case 54123: //Echo of sylvanas jumps to the center of the fight
    case 54968: //Asira dawnslayer jumps onto the drake during her intro
    case 52577://Lord Rhyolith Left & Right Foot
    case 53087://Lord Rhyolith Left & Right Foot
        break;
    default:
    ...
    init.SetFacing(GetSeatOrientation(veSeat->m_ID, Target->GetCreatureEntry(), Seat->first));
