...
bool Pet::hasCrowdControlPreventingActions(bool act_without_movement)
{

    if (HasAuraType(SPELL_AURA_MOD_STUN))
        return true;
    if (HasAuraType(SPELL_AURA_MOD_PACIFY))
        return true;

    if (HasAuraType(SPELL_AURA_MOD_FEAR))
        return true;

    if (HasAuraWithMechanic(MECHANIC_BANISH))
        return true;

    if (HasBreakableByDamageCrowdControlAura())
        return true;

    if (!act_without_movement)//the requested action can be done from a distance without moving to a target.
    {
...
bool Pet::LoadPet(Player* owner, PetData* t_pet, bool current)
{
    ...
    if (cinfo->type == CREATURE_TYPE_CRITTER)
    {
        ...
        auto ltransport = owner->GetLinearTransport();
        if (transport)
        {
        ...
        if (ltransport)
        {
            ltransport->CalculatePassengerOffset(px, py, pz, po);
            SetLinearTransport(ltransport);
            m_movementInfo.t_guid = ltransport->GetGUID();
            m_movementInfo.t_pos.Relocate(px, py, pz, po);
            ltransport->AddPassenger(this);
        }
    ...
    if (auto charmInfo = GetCharmInfo())
    {
        GetMotionMaster()->MoveFollow(owner, PET_FOLLOW_DIST, GetFollowAngle());
...
void Pet::Update(uint32 diff)
{
    ...
    {
        if (GetOwner()->HasTalent(47236, GetOwner()->GetActiveSpec())) // Demonic Pact
        {
        ...
        {
            if (Aura* aura = ToUnit()->GetAura(53646, ToCreature()->GetGUID()))
                aura->Remove();
            if (Aura* aura = GetOwner()->ToUnit()->GetAura(53646, ToCreature()->GetGUID()))
                aura->Remove();
        }
    }
                ...
                case 17252: // warlock mastery hackfix
                {
                    if (auto owner = GetOwner())
                    {
                        if (auto Powner = owner->ToPlayer())
                        {
                            ...
                            {
                                float masteryBonusPct = (Powner->GetRatingBonusValue(CR_MASTERY) + 8) * 2.3f;

                                HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, masteryBonusPct, true);
                                HandleStatModifier(UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT, masteryBonusPct, true);
                                HandleStatModifier(UNIT_MOD_DAMAGE_RANGED, TOTAL_PCT, masteryBonusPct, true);
                            }
                        }
                    }
                    break;
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case 27893:             // Dancing Rune Weapon
                {
                    ...
                    m_modMeleeHitChance += m_owner->GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE);
                    break;
                }
                case 89:                // Infernal
                {
                ...
                case 11859:                 // Doomguard
                {
                    ...
                    SetSpellBonusDamage(int32(m_owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SPELL))); // doomguard get 100% of owners spell, doombolt has his own coef.
                    break;
                }
...
void Pet::_LoadAuras(PetData* t_pet, uint32 timediff)
{
    ...
    TC_LOG_DEBUG("entities.pet", "Loading auras for pet %u", GetGUIDLow());
    {
        ...
        for (auto itr = t_pet->m_auras.auraList.begin(); itr != t_pet->m_auras.auraList.end(); ++itr)
        {
...
bool Pet::addSpell(uint32 spellId,PetSpellData& m_spell)
{
    ...
    if (itr != m_spells.end())
    {
        ...
        else if (m_spell._state == DATA_UNCHANGED && itr->second._state != DATA_UNCHANGED)
        {
            ...
            if (m_spell.active == ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST)
                ToggleAutocast(spellInfo, true);
            else if (m_spell.active == ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO)
    ...
    if (m_spell.active == ACT_DECIDE)                               // active was not used before, so we save it's autocast/passive state here
    {
        if (spellInfo->IsAutocastable())
            m_spell.active = ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO;
        else
...
void Pet::InitLevelupSpellsForLevel()
{
    ...
    if (PetDefaultSpellsEntry const* defSpells = sSpellMgr->GetPetDefaultSpellsEntry(petSpellsId))
    {
        for (uint8 i = 0; i < MAX_CREATURE_SPELL_DATA_SLOT; ++i)
        {
            ...
            // Owl hackfix
            if ((GetCreatureTemplate()->family == 26) && spellEntry->Id == 50541)
                continue;
    ...
    for (uint8 i = 0; i < MAX_UNIT_ACTION_BAR_INDEX; ++i)
        if (UnitActionBarEntry const* ab = m_charmInfo->GetActionBarEntry(i))
            if (ab->GetAction() && ab->IsActionBarForSpell())
            {
                ...
                else if (ab->GetType() == ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST)
                {
...
void Pet::ToggleAutocast(SpellInfo const* spellInfo, bool apply)
{
    ...
    if (apply)
    {
        ...
        if (i == m_autospells.size())
        {
            ...
            if (itr->second.active != ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST)
            {
                itr->second.active = ACTIONBAR_STATE_AURA_ACTIVE_OR_SPELL_AUTOCAST;
                if (itr->second._state != DATA_NEW)
    ...
    else
    {
        ...
        if (i < m_autospells.size())
        {
            ...
            if (itr->second.active != ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO)
            {
                itr->second.active = ACTIONBAR_STATE_AURA_NOTACTIVE_OR_SPELL_NOTAUTO;
                if (itr->second._state != DATA_NEW)
                    itr->second._state = DATA_CHANGED;
            }
        }
