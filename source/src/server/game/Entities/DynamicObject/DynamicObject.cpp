...
DynamicObject::DynamicObject(bool isWorldObject) : WorldObject(isWorldObject), MapObject(),
...
bool DynamicObject::CreateDynamicObject(uint32 guidlow, Unit* caster, SpellInfo const* spell, Position const& pos, float radius, DynamicObjectType type)
{
    ...
    m_type = type;
    if (radius > caster->GetObjectSize())
        radius -= caster->GetObjectSize();
    ...
    auto ltransport = caster->GetLinearTransport();
    ...
    if (transport)
    {
        ...
        m_movementInfo.t_pos.Relocate(x, y, z, o);
    ...
    if (!GetMap()->AddToMap(this))
    {
        ...
        if (transport)
            transport->RemovePassenger(this);
        if (ltransport)
            ltransport->RemovePassenger(this);
        return false;
    }
