...
m_respawnDelay(300), m_coldrespawnDelay(300), m_corpseDelay(60), m_respawnradius(0.0f), m_reactState(REACT_AGGRESSIVE),
m_defaultMovementType(IDLE_MOTION_TYPE), m_DBTableGuid(0), m_equipmentId(0), m_AlreadyCallAssistance(false),
m_AlreadySearchedAssistance(false), m_regenHealth(true), m_AI_locked(false), m_meleeDamageSchoolMask(SPELL_SCHOOL_MASK_NORMAL),
m_creatureInfo(NULL), m_creatureData(NULL), m_flags_affixes(0), m_path_id(0), m_formation(NULL), tmpEnergyReg(0.0f)
{
...
uint32 Creature::GenerateDynamicRespawn(uint32 base_respawn)
{
    uint32 null_1{ 0 };
    uint32 null_2{ 0 };
    uint32 null_3{ 0 };

    sObjectMgr->GenerateFloodResponseValue(FLOOD_RESPONSE_RESPAWN, this, base_respawn, null_1, null_2, null_3);
...
void Creature::RemoveCorpse(bool setSpawnTime)
{
 ...
 //   UpdateObjectVisibility(true);
    ...
    uint32 respawnDelay = GenerateDynamicRespawn(m_respawnDelay);
...
bool Creature::InitEntry(uint32 Entry, uint32 /*team*/, const CreatureData* data)
{
    ...
    m_flags_affixes = normalInfo->flags_affixes;
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            /*
                true if IN COMBAT with a non-GM player victim
                or in combat with no victim at all
            */
            if (isInCombat())
            {
                if (auto v = getVictim())
                    if (auto p = v->GetCharmerOrOwnerPlayerOrPlayerItself())
                        if (p->isGameMaster())
                            bInCombat = false;
                        else bInCombat = true;
                    else bInCombat = true;
...
bool Creature::isCanTrainingOf(Player* player, bool msg) const
{
    ...
    switch (GetCreatureTemplate()->trainer_type)
    {
        ...
        case TRAINER_TYPE_PETS:
            ...
            {
                // Hyjarion: This causing empty gossip menus appeared from city pet trainer for non hunter classes, so I comment these two lines out as I don't know why they exist.
                /*player->PlayerTalkClass->ClearMenus(); 
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    data.spawntimesecs = m_coldrespawnDelay;
    ...
    stmt->setUInt32(index++, m_coldrespawnDelay);//NOT USING COLD HERE CREATES A SELF-REDUCING LOOP TOWARDS ZERO RESPAWN TIME
...
bool Creature::LoadCreatureFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    //TC_LOG_ERROR("sql.sql", "before setting creature respawn: %u, %u, %u %u", m_coldrespawnDelay, data->spawntimesecs, m_respawnDelay, m_respawnTime);
    m_coldrespawnDelay = data->spawntimesecs;
    m_respawnDelay = m_coldrespawnDelay;

    if (data->spawncorpsesecs >= 0)
        m_corpseDelay = 30;
    ...
    //TC_LOG_ERROR("sql.sql", "after setting creature respawn: %u, %u, %u %u", m_coldrespawnDelay, data->spawntimesecs, m_respawnDelay, m_respawnTime);
...
void Creature::ConnectToObjTransport(WorldObject* obj, bool connect)
{
    if (connect)
    {
        if (!obj)
        ...
        obj->CalculateObjPassengerOffset(pass_x, pass_y, pass_z, pass_o);
        DisableSpline();
        m_movementInfo.t_guid = obj->GetGUID();
        m_movementInfo.t_pos.Relocate(pass_x, pass_y, pass_z, pass_o);
        SendTeleportPacket(m_movementInfo.t_pos);
    }
    else
    {
        m_movementInfo.t_guid = 0;
        m_movementInfo.t_pos.Relocate(0.f, 0.f, 0.f, 0.f);
        SendTeleportPacket(m_movementInfo.t_pos);
    }
...
void Creature::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        m_corpseRemoveTime = time(NULL) + GenerateDynamicRespawn(m_corpseDelay);
        if (auto m = GetMap())
            if (m->IsDungeon() || m->IsRaid())
            {
                m_respawnTime = time(NULL) + DAY;
            }
            else
                m_respawnTime = time(NULL) + (GenerateDynamicRespawn(m_coldrespawnDelay + m_corpseDelay));
...
bool Creature::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    if (GetCreatureTemplate()->type == CREATURE_TYPE_MECHANICAL)
        switch (spellInfo->Effects[index].Effect)
        {
            ...
            case SPELL_EFFECT_HEAL_MECHANICAL:
                if (spellInfo->Effects[index].TargetA.GetTarget() == TARGET_UNIT_CASTER)
                    if (spellInfo->Effects[index].TargetB.GetTarget() == NULL || spellInfo->Effects[index].TargetB.GetTarget() == TARGET_UNIT_CASTER)
                        return false;
                //TC_LOG_ERROR("sql.sql", "creature %u is immune to effect %u", GetGUIDLow(), spellInfo->Effects[index].Effect);
                return true;
                break;
            default:
                break;
...
bool Creature::_IsTargetAcceptable(const Unit* target, const bool aggro /*= false*/) const
{
    ...
    /*if (IsFriendlyTo(target)
        || !target->isTargetableForAttack(false, false)
        || (m_vehicle && (IsOnVehicle(target) || m_vehicle->GetBase()->IsOnVehicle(target))))*/
    if (IsFriendlyTo(target))
        return false;
    if (!target->isTargetableForAttack(false, false))
        return false;
...
void Creature::SaveRespawnTime()
{
    ...
    if (auto t = GetCreatureTemplate())
    {
        if (t->Entry == 32153)//Timelost Proto Drake
            override = time(NULL) + (urand(WEEK, (WEEK * 2)));
        else if (t->rank == CREATURE_ELITE_ELITE)
        {
            override = time(NULL) + (urand((MINUTE * 5), (MINUTE * 10)));
        }
        else if (t->rank & CREATURE_ELITE_RARE)
            override = time(NULL) + (urand((HOUR * 6), (HOUR * 24)));
    }

    GetMap()->SaveCreatureRespawnTime(m_DBTableGuid, override? override : m_respawnTime);
}
...
void Creature::AllLootRemovedFromCorpse(bool loot_empty)
{
    if ((isDead() || loot_empty)
        && !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE)
        && GetCreatureTemplate()->SkinLootId
        && LootTemplates_Skinning.HaveLootFor(GetCreatureTemplate()->SkinLootId)
        && hasLootRecipient()
        && !GetSkinLooted())
    {
        SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE);
    }
    else if (GetSkinLooted() || !GetCreatureTemplate()->SkinLootId)//skinning is done or 
    {
        time_t now = time(NULL);
    ...
    public:
        ...
        bool Execute(uint64 /*execTime*/, uint32 /*diff*/)
        {
            if (id < 0)
            {
                ...
                if (auto instance = crea->GetInstanceScript())
                    sCreatureTextMgr->SendBroadcast(crea, id *= -1, whisperGuid, msgType, TEXT_RANGE_MAP);
                else
            ...
            {
                if (auto instance = crea->GetInstanceScript())
                    sCreatureTextMgr->SendChat(crea, id, whisperGuid, msgType, LANG_UNIVERSAL, TEXT_RANGE_MAP);
                else
...
bool Creature::CanBeSubjectOfAffix(uint32 affix)
{
    if (isPet() || isTotem() || (getFaction() == 35) || isTrigger() || isGuardian() || (GetOwner() && GetOwner()->ToPlayer()))
        return false;

    if (auto t = GetCreatureTemplate())
    if (t->type == CREATURE_TYPE_CRITTER)
        return false;

    if (!m_flags_affixes)
        return true;

    bool hasAffixFlag = std::abs(m_flags_affixes) & (1 << (affix - 1));
