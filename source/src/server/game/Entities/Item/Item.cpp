...
Item::~Item()
{
    //TC_LOG_ERROR("sql.sql", "Deleting Item %u", GetGUID());
}

bool confirm_no_other_sets_providing_this_Bonus(uint32 bonus_spell_id, uint32 excluded_setID, Player* player)
{
    ItemSetEntry const* excluded_set = sItemSetStore.LookupEntry(excluded_setID);
    ItemSetEffect* eff = NULL;

    size_t setindex = 0;

    if (player->ItemSetEff.size())
    for (; setindex < player->ItemSetEff.size(); setindex++)
    {
        if (!player)            //null check
            continue;

        if (!bonus_spell_id)    //nullcheck
            continue;   

        if (!excluded_setID)    //null check
            continue;

        if (!player->ItemSetEff[setindex])//null check
            continue;

        if (player->ItemSetEff[setindex]->setid == excluded_setID)
            continue;

        for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; x++)
        {
            if (!player->ItemSetEff[setindex]->spells[x])
                continue;

            for (uint32 z = 0; z < MAX_ITEM_SET_SPELLS; z++)
            {
                if (player->ItemSetEff[setindex]->spells[z] && player->ItemSetEff[setindex]->spells[z]->Id == bonus_spell_id)
                    return false;
            }
        }
    }
...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x)
    {
        ...
        {
            if (!eff->spells[y] || (eff->spells[y] && player->ToUnit() && !player->ToUnit()->HasAura(eff->spells[y]->Id, player->GetGUID()))) // free slot
            {
                ...
                if (confirm_no_other_sets_providing_this_Bonus(spellInfo->Id, eff->setid, player))
    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; x++)
    {
        ...
        for (uint32 z = 0; z < MAX_ITEM_SET_SPELLS; z++)
        {
            if (eff->spells[z] && eff->spells[z]->Id == set->spells[x])
            {
                ...
                if (confirm_no_other_sets_providing_this_Bonus(eff->spells[z]->Id, player->ItemSetEff[setindex]->setid, player))
...
{
    // Account bound bags can be mailed after loot
    if (!mail && m_lootGenerated)
        return false;
...
bool Item::HasEnchantRequiredSkill(const Player* player) const
{
    ...
    {
        if (enchant_slot > ENGINEERING_ENCHANT_SLOT && enchant_slot < PROP_ENCHANTMENT_SLOT_0)    // not holding enchantment id
            continue;
...
bool Item::CanBeTransmogrified() const
{
    ...
    if (proto->Quality == ITEM_QUALITY_LEGENDARY ||
        proto->Quality == ITEM_QUALITY_POOR ||
...
{
    if (!GetOwner())//null check
    {
        TC_LOG_ERROR("misc", "Transmogrification: owner does not exist");
        return false;
    } 
    ...
    if ((proto->AllowableClass & GetOwner()->getClassMask()) == 0 || (proto->AllowableRace & GetOwner()->getORaceMask()) == 0)//If the item is for another race/class
        return false;
...
bool Item::CanTransmogrifyItemWithItem(Item const* transmogrified, Item const* transmogrifier)
{
    ...
    if ((proto1->InventoryType == INVTYPE_RANGED && proto2->InventoryType == INVTYPE_RANGED) || (proto1->InventoryType == INVTYPE_RANGEDRIGHT && proto2->InventoryType == INVTYPE_RANGEDRIGHT))
    ...
    //inventory slots don't match up
    InventoryType offhand_weapons[2]    { INVTYPE_WEAPONOFFHAND, INVTYPE_HOLDABLE };
    bool found[2]                       {  false, false                           };

    for (auto slot_off1 : offhand_weapons)
        for (auto slot_off2 : offhand_weapons)
        {
            if (proto1->InventoryType == slot_off1)
                found[0] = true;

            if (proto2->InventoryType == slot_off2)
                found[1] = true;
        }

    if (found[0] != found[1])//one is an offhand/holdable, but the other is not.
        return false;


    if (proto1->InventoryType != proto2->InventoryType 
        //not a weapon or not offhand or mainhand
        && (proto1->Class != ITEM_CLASS_WEAPON 
    ...
    if ((proto1->InventoryType == INVTYPE_WEAPONMAINHAND && proto2->InventoryType != INVTYPE_WEAPONMAINHAND)
        ||
        (proto1->InventoryType != INVTYPE_WEAPONMAINHAND && proto2->InventoryType == INVTYPE_WEAPONMAINHAND))
        return false;

    if (proto1->InventoryType == INVTYPE_2HWEAPON && proto2->InventoryType == INVTYPE_2HWEAPON)
    {
        switch (proto1->SubClass)
        {
    ...
    if (proto1->InventoryType == proto2->InventoryType)         //same equipment slot
    {
        switch (proto1->InventoryType)
        {
        case INVTYPE_HEAD:
        ...
        case INVTYPE_WRISTS:
        case INVTYPE_HANDS:
        case INVTYPE_CLOAK:                                 //this is a piece of armor
            if (proto1->SubClass != proto2->SubClass)   //material is different
                return false;
            break;
        default:
            break;
