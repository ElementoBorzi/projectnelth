...
struct GameObjectTemplate
{
    ...
    uint32 GetAutoCloseTime() const
    {
        ...
        case GAMEOBJECT_TYPE_TRAPDOOR:      autoCloseTime = trapDoor.autoCloseTime; break;
...
union GameObjectValue
{
    ...
    struct
    {
        uint32 PathProgress;
        uint32 CurrSeg;
        TransportAnimation const* AnimInfo;
    } Transport;
    //29 GAMEOBJECT_TYPE_CAPTURE_POINT
    struct
    {
...
enum GOState
{
    ...
    GO_STATE_TRANSPORT_READY    = 25
};

#define MAX_GO_STATE              26
...
public:
    ...
    bool IsControlableTransport() const;
    bool IsDestructibleBuilding() const;

    void SetControlableTransport(bool state = true);
    ...
    void Refresh();
    void Delete(uint32 delayTime = 0);
    ...
    //void SetTransportState(uint32 stopFrame);
    ...
    float GetStationaryX() const { return GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT ? m_stationaryPosition.GetPositionX() : GetPositionX(); }
    float GetStationaryY() const { return GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT ? m_stationaryPosition.GetPositionY() : GetPositionY(); }
    float GetStationaryZ() const { return GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT ? m_stationaryPosition.GetPositionZ() : GetPositionZ(); }
    float GetStationaryO() const { return GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT ? m_stationaryPosition.GetOrientation() : GetOrientation(); }
    ...
    void SendActivateAnimKit(uint32 anim);
    ...
    //LinearTransport* ToLinearTransport() { if (GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT) return reinterpret_cast<LinearTransport*>(this); else return NULL; }
    Transport* ToLinearTransport() { if (GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT) return reinterpret_cast<Transport*>(this); else return NULL; }
