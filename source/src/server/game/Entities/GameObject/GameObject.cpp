...
GameObject::GameObject() : WorldObject(false), MapObject(), m_model(NULL), m_goValue(), m_AI(NULL)
{
...
void GameObject::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        if (GetGoType() == GAMEOBJECT_TYPE_CHEST)
            toggledState = getLootState() == GO_READY;
        else if (GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
            toggledState = GetDestructibleState() != GO_DESTRUCTIBLE_DESTROYED;
        else if (IsTransport())
            toggledState = true;
        else
            toggledState = GetGoState() == GO_STATE_READY;
        if (m_model)
            ...
            switch (GetDestructibleState())
            {
                case GO_DESTRUCTIBLE_DESTROYED:
                    if (modelData->DestroyedDisplayId)
                        SetDisplayId(modelData->DestroyedDisplayId);
                    break;
                case GO_DESTRUCTIBLE_DAMAGED:
                    if (modelData->DamagedDisplayId)
                        SetDisplayId(modelData->DamagedDisplayId);
                    break;
                case GO_DESTRUCTIBLE_REBUILDING:
                    if (modelData->RebuildingDisplayId)
                        SetDisplayId(modelData->RebuildingDisplayId);
                    break;
            }
        }
    }
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    uint32 goEntry = goinfo->entry;
    if (goinfo->type == GAMEOBJECT_TYPE_TRANSPORT)
    {
        //if (goinfo->entry == 207834) TC_LOG_ERROR("sql.sql", "onyxia elevator recognized as transport.");

        m_updateFlag = (m_updateFlag | UPDATEFLAG_TRANSPORT) & ~UPDATEFLAG_GO_TRANSPORT_POSITION;
        highType = HIGHGUID_MO_TRANSPORT;
        goEntry = 0;
        if (goinfo->transport.startOpen)
            m_updateFlag |= UPDATEFLAG_UNK7;
    }

    Object::_Create(guidlow, goEntry, highType);
    ...
    switch (goinfo->type)
    {
        ...
        case GAMEOBJECT_TYPE_TRAP:
            if (GetGOInfo()->trap.stealthed)
            {
                ...
                m_stealth.AddValue(STEALTH_TRAP, 4);
            }
...
void GameObject::Update(uint32 diff)
{
    ...
    {
        if (IsControlableTransport())
        {
            if (!m_goValue.Transport.AnimInfo)
                return;

            if (GetGoState() == GO_STATE_TRANSPORT_READY)
            {
                if (m_goValue.Transport.CurrSeg == GetGOInfo()->transport.pause)
                    return;
            }
            else if (GetGoState() == GO_STATE_TRANSPORT_SPEC)
            {
                if (m_goValue.Transport.CurrSeg == m_goValue.Transport.AnimInfo->TotalTime)
                {
                    m_goValue.Transport.CurrSeg = GetGOInfo()->transport.startPos;
                    return;
                }
                if (m_goValue.Transport.CurrSeg == GetGOInfo()->transport.startPos)
                    return;
            }

            m_goValue.Transport.PathProgress += diff;
            uint32 timer = m_goValue.Transport.PathProgress % m_goValue.Transport.AnimInfo->TotalTime;
            TransportAnimationEntry const* node = m_goValue.Transport.AnimInfo->GetAnimNode(timer);
            if (node && m_goValue.Transport.CurrSeg != node->TimeSeg)
            {
                m_goValue.Transport.CurrSeg = node->TimeSeg;
                m_goValue.Transport.PathProgress = node->TimeSeg;

                // Seems to be useless because on sniff server use stationary position for every packet
                G3D::Quat rotation = m_goValue.Transport.AnimInfo->GetAnimRotation(timer);
    ...
    switch (m_lootState)
    {
        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    float radius = (float)(goInfo->trap.radius); // TODO rename radius to diameter (goInfo->trap.radius) should be (goInfo->trap.diameter)
                    if (!radius)
                    {
                        ...
                        radius = (((float)(goInfo->trap.radius)) / 2.f) + 0.5f; // TODO rename radius to diameter (goInfo->trap.radius) should be (goInfo->trap.diameter)
                    ...
                    if (owner)                    // hunter trap
                    {
                        ...
                        possibleTargets.remove_if([](WorldObject* target){
                            return target->GetEntry() == 47649;
                        });

                        Unit *realtarget = NULL;
                        for (auto itr = possibleTargets.begin(); itr != possibleTargets.end(); ++itr)
                        if (auto potential_target = (*itr))
                            if (!realtarget || GetExactDist(potential_target) < GetExactDist(realtarget))
                            {
                                //TC_LOG_ERROR("sql.sql", "target %u is %f yards away, with an absolute distance requirement of %f (%f stored in the database)", potential_target->GetGUID(), GetExactDist(potential_target), radius, goInfo->trap.radius);
                                //TC_LOG_ERROR("sql.sql", "trap with spell id %u is active. A target has been found. random number: %u", goInfo->trap.spellId, urand(0, 10));
                                realtarget = *itr;
                            }
                    ...
                    {
                        /*
                        if (GetEntry() == 183957 || GetEntry() == 2561 || GetEntry() == 164639)
                            TC_LOG_ERROR("sql.sql", "hunter trap does not have an owner.");
                        */
                    ...
                    if (ok)
                    {
                        ...
                        if (goInfo->trap.spellId)
                        {
                            ...
                            {
                                //TC_LOG_ERROR("sql.sql", "owner is not a valid attack target of %u", ok->GetGUID());
                                break;
                            }
                            else
                            {
                                // hackfix? not sure... seems blizzard teleport players with high pings or big speed amounts back to trap position on trigger
                                // but only those with small radius

                                //TC_LOG_ERROR("sql.sql", "trap with spell id %u is active. random number: %u", spellInfo->Id, urand(0,10));
                                if (goInfo->trap.spellId == 3355 //hunter freezing trap
                                    || (goInfo->trap.spellId == 45145 && (owner->HasAura(19387) || owner->HasAura(19184)))  //snake trap with entrapment
                                    || (goInfo->trap.spellId == 63487 && (owner->HasAura(19387) || owner->HasAura(19184)))  //ice trap with entrapment
                                    )
                                {
                                    /*
                                        Not the trap's exact center, but the player's location at the time of triggering
                                    */
                                    if (!ok->IsImmunedToSpell(sSpellMgr->GetSpellInfo(goInfo->trap.spellId)))
                                    ok->NearTeleportTo(ok->GetPositionX(), ok->GetPositionY(), ok->GetPositionZ(), ok->GetOrientation());
                                }
                        ...
                        m_cooldownTime = getMSTime() + (goInfo->trap.cooldown ? goInfo->trap.cooldown * IN_MILLISECONDS : uint32(4000));   // template or 4 seconds
                        ...
                        // where to move this?
                        if (goInfo->entry == 184663 || goInfo->entry == 184664)
                        {
                            ok->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
                            ok->RemoveAurasByType(SPELL_AURA_MOD_INVISIBILITY);
                        }
                    }
                }
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            uint32 flood_mod { m_respawnDelayTime };
            uint32 null_1{ 0 };
            uint32 null_2{ 0 };
            uint32 null_3{ 0 };

            sObjectMgr->GenerateFloodResponseValue(FLOOD_RESPONSE_RESPAWN, this, flood_mod, null_1, null_2, null_3);//Adjust respawn time of gameobjects
            m_respawnTime = time(NULL) + flood_mod;
...
void GameObject::Delete(uint32 delayTime /*= 0*/)
{
    if (delayTime)
    {
        if (!m_uiDeleteTime)
            m_uiDeleteTime = delayTime;
        return;
    }
...
// Transport than can be stopped
bool GameObject::IsControlableTransport() const
{
    GameObjectTemplate const* gInfo = GetGOInfo();
    if (!gInfo)
        return false;

    return gInfo->type == GAMEOBJECT_TYPE_TRANSPORT && m_updateFlag & UPDATEFLAG_UNK7;
}

void GameObject::SetControlableTransport(bool state)
{
    GameObjectTemplate const* gInfo = GetGOInfo();

    if (!gInfo || gInfo->type != GAMEOBJECT_TYPE_TRANSPORT)
        return;

    if (state)
    {
        if (gInfo->transport.pause)
            m_updateFlag |= UPDATEFLAG_UNK7;
    }
    else
        m_updateFlag &= ~UPDATEFLAG_UNK7;
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_SPELLCASTER:                   //22
        {
            ...
            bool dismount = true;
            uint32 bgObj = info->entry;
            
            if (bgObj == 195314 || bgObj == 195313 || bgObj == 195315 || bgObj == 195316
                || bgObj == 195326 || bgObj == 195320 || bgObj == 192819)
                dismount = false;
            ...
            if (spellId == 52415 && user->HasAura(52418))
                return;
                    ...
                    switch (info->entry)
                    {
                        ...
                        case 184142:                        // Netherstorm Flag
                            if (bg->GetTypeID(true) == BATTLEGROUND_EY)
                                ...
                                ((BattlegroundEYR *)bg)->EventPlayerClickedOnMiddleFlag(player, this);
                            break;
                    }
                }
...
void GameObject::SendActivateAnimKit(uint32 anim)
{
...
void GameObject::SetGoState(GOState state)
{
    if (GetGoState() == state)
        return;
    sScriptMgr->BeforeGameObjectStateChanged(this, uint32(state));

    // Prevent client crash
    if (IsControlableTransport() && state == GO_STATE_READY)
    {
        //TC_LOG_ERROR("sql.sql", "Prevent client crash check failed");
        return;
    }

    if (state == GO_STATE_TRANSPORT_SPEC)
    {
        if (!IsControlableTransport())
        {
            //TC_LOG_ERROR("sql.sql", "IsControlableTransport check failed");
            SetGoState(GO_STATE_ACTIVE);
            return;
        }

        if (m_goValue.Transport.CurrSeg != m_goInfo->transport.pause)
        {
            //TC_LOG_ERROR("sql.sql", "currseg: %u, pause: %u", m_goValue.Transport.CurrSeg, m_goInfo->transport.pause);
            return;
        }

        uint32 duration = m_goValue.Transport.AnimInfo->TotalTime - m_goInfo->transport.pause;
        SetUInt32Value(GAMEOBJECT_LEVEL, getMSTime() + duration);
    }
    else if (state == GO_STATE_TRANSPORT_READY)
    {
        if (!IsControlableTransport())
        {
            //TC_LOG_ERROR("sql.sql", "GO_STATE_TRANSPORT_READY  IsControlableTransport check failed");
            SetGoState(GO_STATE_READY);
            return;
        }

        if (m_goValue.Transport.CurrSeg)//fix for characters who were mid ring-of-valor during a x-out or crash causing chain crashes
            if (m_goValue.Transport.CurrSeg != m_goValue.Transport.AnimInfo->TotalTime &&
                m_goValue.Transport.CurrSeg != GetGOInfo()->transport.startPos)
            {
                //TC_LOG_ERROR("sql.sql", "m_goValue.Transport.CurrSe check failed");
                return;
            }

        SetUInt32Value(GAMEOBJECT_LEVEL, getMSTime() + m_goInfo->transport.pause);
    }

    //TC_LOG_ERROR("sql.sql", "gameobject %u set to state %u", GetEntry(), state);
    ...
    if (m_model && !IsTransport())
    {
        ...
        if (state == GO_STATE_TRANSPORT_SPEC || state == GO_STATE_TRANSPORT_READY)
            EnableCollision(true);
        else
            EnableCollision(collision);
    }
    sScriptMgr->AfterGameObjectStateChanged(this, uint32(state));
}
