...
void MapManager::LoadTransports()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT guid, entry, name, period, Phasemask, ScriptName FROM transports");
    ...
    do
    {
        ...
        uint32 phaseId = fields[4].GetUInt32();
        uint32 scriptId = sObjectMgr->GetScriptId(fields[5].GetCString());
...
void Transport::ElevatorMoveTo(bool up)
{
    if (GetGoType() != GAMEOBJECT_TYPE_TRANSPORT)
    {
        TC_LOG_FATAL("sql.sql", "void Transport::ElevatorMoveTo being called for gameobject that is not a transport, entry %u", m_goInfo->entry);
        return;
    }

    if (up)
    {
        if (GetGoState() != GO_STATE_READY)//TRANSPORT STATE BEING SET TO THE SAME STATE AS THE CURRENT ONE CAUSES A CLIENT FREEZE
            SetGoState(GO_STATE_READY);
    }
    else
    {
        if (GetGoState() != GO_STATE_ACTIVE) //TRANSPORT STATE BEING SET TO THE SAME STATE AS THE CURRENT ONE CAUSES A CLIENT FREEZE
...
{
    if (GetEntry() == 207834)
        TC_LOG_ERROR("sql.sql", "transport::update");
    ...
    if (GetEntry() == 207834)
        TC_LOG_ERROR("sql.sql", "elevator: timer =  %u", timer);
    ...
    for (;;)
    {
        if (timer >= _currentFrame->ArriveTime)
        {
            ...
            if (timer < _currentFrame->DepartureTime)
            {
                ...
                {
                    if (GetEntry() == 207834)
                        TC_LOG_ERROR("sql.sql", "elevator: pendingstop and go state not ready, setting to ready.");
