...
bool GameEventMgr::StartEvent(uint16 event_id, bool overwrite)
{
    ...
    else
    {
        ...
        // or to schedule another update where the next event will be started
...
void GameEventMgr::LoadFromDB()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loading Game Event Seasonal Quest Relations...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                const_cast<Quest*>(quest)->SetSeasonal(eventEntry);
    ...
    TC_LOG_INFO("server.loading", ">> Loading Game Event Creature Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (!quest)
                {
                    TC_LOG_ERROR("sql.sql", "`game_event_creature_quest` quest id (%u) does not exist in `quest_template`", questId);
                    continue;
                }

                if (!quest->IsSeasonal())
                    TC_LOG_ERROR("sql.sql", "`game_event_creature_quest` quest id (%u) is not a seasonal quest (event: %u)", questId, event_id);

                QuestRelList& questlist = mGameEventCreatureQuests[event_id];
                questlist.push_back(QuestRelation(id, questId));
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event GO Quest Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!quest)
                {
                    TC_LOG_ERROR("sql.sql", "`game_event_gameobject_quest` quest id (%u) does not exist in `quest_template`", questId);
                    continue;
                }

                if (!quest->IsSeasonal())
                    TC_LOG_ERROR("sql.sql", "`game_event_gameobject_quest` quest id (%u) is not a seasonal quest (event: %u)", questId, event_id);
...
void GameEventMgr::ApplyNewEvent(uint16 event_id)
{
    ...
    if (isSystemInit) /// @todo: find a better way to not allow reseting quests after crash
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    for (GuidList::iterator itr = mGameEventCreatureGuids[internal_event_id].begin(); itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            if (auto creature = ObjectAccessor::GetObjectInWorld(MAKE_NEW_GUID(*itr, data->id, HIGHGUID_UNIT), (Creature*)NULL))
    ...
    for (GuidList::iterator itr = mGameEventGameobjectGuids[internal_event_id].begin(); itr != mGameEventGameobjectGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (GameObjectData const* data = sObjectMgr->GetGOData(*itr))
        {
            ...
            if (auto pGameobject = ObjectAccessor::GetObjectInWorld(MAKE_NEW_GUID(*itr, data->id, HIGHGUID_GAMEOBJECT), (GameObject*)NULL))
