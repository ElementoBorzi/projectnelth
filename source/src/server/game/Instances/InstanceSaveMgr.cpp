...
InstanceSave::~InstanceSave()
{
    ...
    // Re-assure an InstanceSaveMgr crash
    m_playerList.clear();
    m_groupList.clear();
    // ASSERT(m_playerList.empty() && m_groupList.empty()); // Re-assure an InstanceSaveMgr crash
}
...
void InstanceSaveManager::_ResetSave(InstanceSaveHashMap::iterator &itr)
{
    ...
    for (InstanceSave::PlayerListType::iterator itr2 = pList.begin(); itr2 != pList.end();)
    {
        Player* player = *itr2;
        ++itr2;
        player->UnbindInstance(save->GetMapId(), save->GetDifficulty(), true, save->IsLfg());
    }

    InstanceSave::GroupListType& gList = save->m_groupList;
    for (InstanceSave::GroupListType::iterator itr2 = gList.begin(); itr2 != gList.end();)
    {
        Group* group = *itr2;
        ++itr2;
        group->UnbindInstance(save->GetMapId(), save->GetDifficulty(), true, save->IsLfg());
    }

    if (!save->m_playerList.empty() || !save->m_groupList.empty())
    {
        TC_LOG_FATAL("misc.instancesave", "InstanceSaveManager::_ResetSave: error while reseting InstanceSave (MapId: %u, Difficulty: %u, IsLfg: %u), still bound to any player or group.", save->GetMapId(), save->GetDifficulty(), save->IsLfg());
        //ASSERT(false);
    }
