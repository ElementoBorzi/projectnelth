...
BattlegroundQueue::~BattlegroundQueue()
{
    ...
    for (int i = 0; i < MAX_BATTLEGROUND_BRACKETS; ++i)
    {
        ...
        {
            for (auto itr = m_QueuedGroups[i][j].begin(); itr!= m_QueuedGroups[i][j].end(); ++itr)
                delete (*itr);
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
    ...
    ginfo->ArenaTeamIdOrRbgLeaderGuid = arenateamid;
    ...
    if (ginfo->Team == HORDE)
        index++;
    if (ginfo->BgTypeId == BATTLEGROUND_RATED_10_VS_10 && ginfo->IsRated)
        index = BG_QUEUE_RBG;
    if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED) && ArenaType == 0 && !isRated)
    ...
    if (isRated && sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE) && ArenaType != 0)
    {
        ...
        if (!isRated && !isPremade && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        {
            if (auto bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
            {
                if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED))
                {
                    ...
                    for (auto itr = m_QueuedGroups[bracketId][BG_QUEUE_MIXED].begin(); itr != m_QueuedGroups[bracketId][BG_QUEUE_MIXED].end(); ++itr)
...
WarGameGroupQueueInfo BattlegroundQueue::AddWarGameGroup(Group* groupA, Group* groupB, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracketId, uint8 ArenaType)
{
    ...
    for (uint8 i = 0; i < 2; ++i)
    {
        ...
        for (auto itr = loopGroup->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            if (i == 0)
                wginfo.TeamA.push_back(member->GetGUID());
            else wginfo.TeamB.push_back(member->GetGUID());
        }
    }

    wginfo.BgTypeId = bgTypeId;
    wginfo.BracketId = bracketId;
    wginfo.JoinTime = getMSTime();
    wginfo.ArenaType = ArenaType;

    m_QueuedWargameGroups.push_back(wginfo);
...
GroupQueueInfo* BattlegroundQueue::AddSoloQueueGroup(std::list<SoloQueueInfo*> playerList, BattlegroundBracketId bracketId, uint32 team)
{
    ...
    ginfo->ArenaTeamIdOrRbgLeaderGuid = urand(50000, 100000);
    ginfo->IsRated = true;
    ginfo->IsInvitedToBGInstanceGUID = 0;
    ginfo->JoinTime = getMSTime();
    ginfo->RemoveInviteTime = 0;
    ...
    for (SoloQueueInfo* playerInfo : playerList)
    {
    ...
    TC_LOG_DEBUG("bg.battleground", "Adding Solo Queue Group to BattlegroundQueue bgTypeId : 6, bracket_id : %u, index : %u", bracketId, index);
...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    // WarGame
    for (std::list<WarGameGroupQueueInfo>::iterator itr = m_InvitedWargameGroups.begin(); itr != m_InvitedWargameGroups.end(); ++itr)
    {
        uint8 team = 0;
        if ((*itr).IsPlayerIn(guid, &team, true))
        {
            ...
            if ((*itr).TeamA.empty() && (*itr).TeamB.empty())
                m_InvitedWargameGroups.erase(itr);
    ...
    for (int32 bracket_id_tmp = MAX_BATTLEGROUND_BRACKETS - 1; bracket_id_tmp >= 0 && bracket_id == -1; --bracket_id_tmp)
    {
        ...
        if (!group->IsRated || group->ArenaType)
        {
            for (uint8 j = 0; j < BG_NORMAL_QUEUE_GROUP_TYPES_COUNT; ++j)
            {
                auto k = m_QueuedGroups[bracket_id_tmp][j].begin();
        ...
        {
            auto k = m_QueuedGroups[bracket_id_tmp][BG_QUEUE_RBG].begin();
            for (; k != m_QueuedGroups[bracket_id_tmp][BG_QUEUE_RBG].end(); ++k)
            {
                if ((*k) == group)
                {
                    bracket_id = bracket_id_tmp;
...
{
    for (std::list<WarGameGroupQueueInfo>::iterator itr = m_InvitedWargameGroups.begin(); itr != m_InvitedWargameGroups.end(); ++itr)
        if ((*itr).IsInvitedToBGInstanceGUID == bgInstanceGuid && (*itr).IsPlayerIn(pl_guid))
            return true;
...
bool BattlegroundQueue::GetInvitedPlayerWarGameGroupInfoData(uint64 guid, WarGameGroupQueueInfo& wginfo, uint8& team)
{
    for (std::list<WarGameGroupQueueInfo>::iterator itr = m_InvitedWargameGroups.begin(); itr != m_InvitedWargameGroups.end();++itr)
    {
        WarGameGroupQueueInfo _wginfo = (*itr);

        std::list<uint64> loopList;
        for (uint8 i = 0; i < 2; ++i)
        {
            loopList = i == 0 ? _wginfo.TeamA : _wginfo.TeamB;
            for (std::list<uint64>::iterator itr2 = loopList.begin(); itr2 != loopList.end();++itr2)
            {
                if (guid == (*itr2))
                {
                    wginfo = _wginfo;
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, uint32 side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        if (bg->isArena() && bg->isRated())
            bg->SetArenaTeamIdForTeam(ginfo->Team, ginfo->ArenaTeamIdOrRbgLeaderGuid);

        uint32 accepttime = bg->isBattleground() && bg->isRated() ? INVITE_ACCEPT_WAIT_TIME * 3 : INVITE_ACCEPT_WAIT_TIME;
        ginfo->RemoveInviteTime = getMSTime() + accepttime;
        ...
        for (auto itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            if (bg->isArena() && bg->isRated())
                ...
                bg->AddEmptyPlayerScoresForGUID(current_guid, ginfo->Team);
            ...
            {
                BattlegroundPlayerStats bps;
                InfoCharEntry info;
                if (sInfoMgr->GetCharInfo(player->GetGUID(), info))
                {
                    bps.personalRating = info.RBGRating;
                    bps.matchMakerRating = info.MMR[3];
                    bps.games = info.RbgPlayed;
                    bps.wins = info.RbgWon;
                }
                bps.team = side;
                bps.joined = true;
                bg->AddRBGPlayer(player->GetGUID(), bps);
            }
            ...
            m_events.AddEvent(removeEvent, m_events.CalculateTime(accepttime));
...
void BattlegroundQueue::InviteWarGameGroupToBG(WarGameGroupQueueInfo* wginfo, Battleground* bg)
{
    uint32 accepttime = INVITE_ACCEPT_WAIT_TIME;

    wginfo->IsInvitedToBGInstanceGUID = bg->GetInstanceID();
    BattlegroundTypeId bgTypeId = bg->GetTypeID();
    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, wginfo->ArenaType);
    BattlegroundBracketId bracket_id = bg->GetBracketId();

    std::list<uint64> loopList;
    for (uint8 i = 0; i < 2; ++i)
    {
        loopList = i == 0 ? wginfo->TeamA : wginfo->TeamB;
        for (std::list<uint64>::iterator itr = loopList.begin(); itr != loopList.end(); ++itr)
        {
            ...
            // set invited player counters
            bg->IncreaseInvitedCount(i);

            player->SetInviteForBattlegroundQueueType(bgQueueTypeId, bg->GetInstanceID());

            // create remind invite events
            BGQueueInviteEvent* inviteEvent = new BGQueueInviteEvent(player->GetGUID(), bg->GetInstanceID(), bgTypeId, bg->GetArenaType(), getMSTime() + accepttime);
            m_events.AddEvent(inviteEvent, m_events.CalculateTime(INVITATION_REMIND_TIME));
            // create automatic remove events
            BGQueueRemoveEvent* removeEvent = new BGQueueRemoveEvent(player->GetGUID(), bg->GetInstanceID(), bgTypeId, bg->GetArenaType(), bgQueueTypeId, getMSTime() + accepttime);
            m_events.AddEvent(removeEvent, m_events.CalculateTime(accepttime));
            ...
            TC_LOG_DEBUG("bg.battleground", "Battleground: invited player %s (%u) to WarGame instance %u queueindex %u bgtype %u",
                player->GetName().c_str(), player->GetGUIDLow(), bg->GetInstanceID(), queueSlot, bg->GetTypeID());

            // send status packet
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, player, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, player->GetBattlegroundQueueJoinTime(bgTypeId), bg->GetArenaType());
            player->GetSession()->SendPacket(&data);
        }
    }
}
...
bool BattlegroundQueue::CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam)
{
    ...
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        ...
        {
            auto itr = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE + i].begin();
...
bool BattlegroundQueue::CheckSkirmishForSameFaction(BattlegroundBracketId bracket_id, uint32 minPlayersPerTeam)
{
    ...
    auto itr_team = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex].begin();
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    if (arenaType == 6)
    {
        BattlegroundWarGameQueueUpdate(bgTypeId, bracket_id);
        return;
    }
    ...
    if (m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].empty() &&
        ...
        m_QueuedGroups[bracket_id][BG_QUEUE_RBG].empty() &&
    ...
    for (BGFreeSlotQueueContainer::iterator itr = bgQueues.begin(); itr != bgQueues.end();)
    {
            ...
            bg->GetStatus() > STATUS_WAIT_QUEUE && bg->GetStatus() < STATUS_WAIT_LEAVE)
        {
            ...
            for (auto citr = m_SelectionPools[TEAM_ALLIANCE].SelectedGroups.begin(); citr != m_SelectionPools[TEAM_ALLIANCE].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->Team);

            for (auto citr = m_SelectionPools[TEAM_HORDE].SelectedGroups.begin(); citr != m_SelectionPools[TEAM_HORDE].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->Team);
    ...
    else if (bgTypeId == BATTLEGROUND_RB)
    {
            ...
            || ((GetNextRandomBattleground() == BATTLEGROUND_AV || GetNextRandomBattleground() == BATTLEGROUND_IC) && time(NULL) > GetRandomBattlegroundDecayTimer()))
        {
            ...
            SetNextRandomBattleground(nextBg);
            if (nextBg == BATTLEGROUND_AV || nextBg == BATTLEGROUND_IC)
                SetRandomBattlegroundDecayTimer(time(NULL) + (sWorld->getIntConfig(CONFIG_BATTLEGROUND_DECAY_TIMER) * MINUTE));
        }

        Battleground* bg_random_template = sBattlegroundMgr->GetBattlegroundTemplate(GetNextRandomBattleground());
        if (!bg_random_template)
        {
            TC_LOG_ERROR("bg.battleground", "Battleground: Update: bg random template not found for %u", bgTypeId);
            return;
        }

        MinPlayersPerTeam = bg_random_template->GetMinPlayersPerTeam();
        MaxPlayersPerTeam = bg_random_template->GetMaxPlayersPerTeam();
    }
    ...
    if (bg_template->isBattleground() && !isRated)
    {
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            Battleground* bg2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, 0, false, GetNextRandomBattleground());
            ...
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
                for (auto citr = m_SelectionPools[TEAM_ALLIANCE + i].SelectedGroups.begin(); citr != m_SelectionPools[TEAM_ALLIANCE + i].SelectedGroups.end(); ++citr)
                    InviteGroupToBG((*citr), bg2, (*citr)->Team);

            bg2->StartBattleground();

            // Select next random battleground
    ...
    if (!isRated)
    {
        ...
        if (CheckNormalMatch(bg_template, bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam)
            ...
            || CheckCrossFactionMatch(bracket_id, bg_template, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            // we successfully created a pool
            Battleground* bg2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, arenaType, false, GetNextRandomBattleground());
            if (!bg2)
            {
                TC_LOG_ERROR("bg.battleground", "BattlegroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
    ...
    else if (bg_template->isArena())
    {
        ...
        if (!arenaRating)
        {
            ...
            {
                auto citr = m_QueuedGroups[bracket_id][queueGroupType].begin();
                while (citr != m_QueuedGroups[bracket_id][queueGroupType].end())
                {
                    ...
                    for (uint8 i = BG_QUEUE_PREMADE_ALLIANCE; i < BG_QUEUE_NORMAL_ALLIANCE && found < 2; i++)
                    {
                        ...
                        auto itr2 = m_QueuedGroups[bracket_id][i].begin();
                        for (; itr2 != m_QueuedGroups[bracket_id][i].end(); ++itr2)
                        {
                            ...
                            if (!(*itr2)->IsInvitedToBGInstanceGUID
                                ...
                                && (*itr_teams[0])->ArenaTeamIdOrRbgLeaderGuid != (*itr2)->ArenaTeamIdOrRbgLeaderGuid)
                            {
                    ...
                    if (found == 2)
                    {
                        ...
                        TC_LOG_DEBUG("bg.battleground", "setting oposite teamrating for team %u to %u", aTeam->ArenaTeamIdOrRbgLeaderGuid, aTeam->OpponentsTeamRating);
                        TC_LOG_DEBUG("bg.battleground", "setting oposite teamrating for team %u to %u", hTeam->ArenaTeamIdOrRbgLeaderGuid, hTeam->OpponentsTeamRating);
    ...
    else if (bg_template->isRatedBattleground())
    {
        auto citr = m_QueuedGroups[bracket_id][BG_QUEUE_RBG].begin();
        while (citr != m_QueuedGroups[bracket_id][BG_QUEUE_RBG].end())
        {
            GroupQueueInfo* front1 = *citr;
            if (front1->IsInvitedToBGInstanceGUID)
            {
            ...
            auto itr2 = m_QueuedGroups[bracket_id][BG_QUEUE_RBG].begin();
            for (; itr2 != m_QueuedGroups[bracket_id][BG_QUEUE_RBG].end(); ++itr2)
            {
                // if group match conditions, then add it to pool
                if (!(*itr2)->IsInvitedToBGInstanceGUID)
                {
                    if ((*itr2)->ArenaMatchmakerRating >= arenaMinRating && (*itr2)->ArenaMatchmakerRating <= arenaMaxRating && CanJoinBattleground((*itr2), bg_template) && (*itr_teams[0])->ArenaTeamIdOrRbgLeaderGuid != (*itr2)->ArenaTeamIdOrRbgLeaderGuid)
                    {
                        itr_teams[found++] = itr2;
                        break;
                    }
            ...
            if (found == 2)
            {
                ...
                Team teamA = ALLIANCE;
                Team teamH = HORDE;
                if (bg_template->isBattleground() && aTeam->Team != hTeam->Team)
                {
                    if (aTeam->Team != ALLIANCE)
                    {
                        teamA = HORDE;
                ...
                InviteGroupToBG(aTeam, arena, teamA);
                InviteGroupToBG(hTeam, arena, teamH);
...
void BattlegroundQueue::BattlegroundWarGameQueueUpdate(BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id)
{
    Battleground* bg_template = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId);
    if (!bg_template)
    {
    ...
    for (std::list<WarGameGroupQueueInfo>::iterator itr = m_QueuedWargameGroups.begin(); itr != m_QueuedWargameGroups.end();)
    {
        WarGameGroupQueueInfo* wginfo = &(*itr);

        if (bgTypeId == wginfo->BgTypeId && bracket_id == wginfo->BracketId)
        {
            Battleground* WarGame;
            if(bgTypeId == BATTLEGROUND_RB)
                WarGame = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, wginfo->ArenaType, false, sBattlegroundMgr->GetRandomBG(bgTypeId), true);
            else WarGame = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, wginfo->ArenaType, false, BATTLEGROUND_TYPE_NONE, true);
            InviteWarGameGroupToBG(wginfo, WarGame);
            WarGame->StartBattleground();

            m_InvitedWargameGroups.push_back(*wginfo);
            m_QueuedWargameGroups.erase(itr++);
        }
        else ++itr;
    }
}

bool BattlegroundQueue::CanJoinBattleground(GroupQueueInfo* ginfo, Battleground const* bgOrTemplate)
{
    // check every member of the group to be able to join
    uint32 memberscount = 0;
    if (sBattlegroundMgr->isTesting())
        return true;
    for (std::map<uint64, PlayerQueueInfo*>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr, ++memberscount)
    {
    ...
    if (bgOrTemplate->isRatedBattleground() && memberscount != bgOrTemplate->GetMinPlayersPerTeam())                // no MinPlayerCount for battlegrounds
        return false;
...
uint32 BattlegroundQueue::GetQueuedGroups() const
{
    ...
    PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), 85);
    if (!bracketEntry)
        return 0;
