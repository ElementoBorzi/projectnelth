...
BattlegroundAV::BattlegroundAV()
{
    ...
    BgCreatures.resize(BG_AV_TOTALNPCS);
...
void BattlegroundAV::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    if (killer)
        ...
        else TC_LOG_ERROR("bg.battleground", "BattlegroundAV::HandleKillPlayer: Killer player not found");
    else TC_LOG_ERROR("bg.battleground", "BattlegroundAV::HandleKillPlayer: Victim player not found");

    UpdateScore(player->GetTeam(), -1);
    //insert quest item (blood or storm crystal) addition here
    ...
    if (killer->GetGUID() != player->GetGUID())
    {
        if (killer->GetTeamId() == TEAM_ALLIANCE) {
            killer->AddItem(17423, 1); //add storm crystal
        }
        else {
            killer->AddItem(17306, 1); //add stormpike soldier's blood
        }
        killer->AddItem(17422, (rand() % 8 + 16)); //add armor scraps
    }
...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    ...
    //Alliance Marshals
    else if (entry == BG_AV_CreatureInfo[AV_NPC_MARSHAL_NORTH][0])
    {
        unit->RemoveAurasDueToSpell(45828);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_MARSHAL_SOUTH][0])
    {
        unit->RemoveAurasDueToSpell(45829);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_MARSHAL_STONE][0])
    {
        unit->RemoveAurasDueToSpell(45830);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_MARSHAL_ICE][0])
    {
        unit->RemoveAurasDueToSpell(45831);
    }
    //Horde Warmasters
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_MARSHAL_ICE][0])
    {
        unit->RemoveAurasDueToSpell(45822);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_MARSHAL_TOWER][0])
    {
        unit->RemoveAurasDueToSpell(45823);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_MARSHAL_WTOWER][0])
    {
        unit->RemoveAurasDueToSpell(45824);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_MARSHAL_ETOWER][0])
    {
        unit->RemoveAurasDueToSpell(45826);
    }
    //End Marshals/Warmasters
    //begin titans
    else if (entry == BG_AV_CreatureInfo[AV_NPC_IVUS][0])
    {
        DelCreature(AV_CPLACE_IVUS);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_LOKHOLAR][0])
    {
        DelCreature(AV_CPLACE_LOKHOLAR);
    }
    //End Titans
    //begin guards for scraps
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_GRAVEDEFENSE0][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_GRAVEDEFENSE1][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_GRAVEDEFENSE2][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_GRAVEDEFENSE3][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_GRAVEDEFENSE0][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_GRAVEDEFENSE1][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_GRAVEDEFENSE2][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_GRAVEDEFENSE3][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_TOWERDEFENSE_0][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_TOWERDEFENSE_0][0])
    {
        killer->AddItem(17422, (rand() % 8 + 2)); //add armor scraps
    }
    //end guards for scraps
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    AV_QUEST_H_BOSS2        = 6801,     //"Lokholar the ice lord"(Horde 1)
        ...
        case AV_QUEST_H_BOSS1:
            m_Team_QuestStatus[team][4] += 5; //Turn in 5 at once
             //m_Team_QuestStatus[team][4] += 200; //test purposes
            break;
    ...
    //check for completed boss quests here

    if (m_Team_QuestStatus[team][4] >= 200) {
        if (player->GetTeamId() == TEAM_ALLIANCE) {
            if (!Ivus_has_been_spawned) {
                TC_LOG_DEBUG("bg.battleground", "BG_AV Quest %i completed but is not interesting at all", questid);
                AddAVCreature(AV_NPC_IVUS, AV_CPLACE_IVUS);
                Creature* creature = GetBGCreature(AV_CPLACE_IVUS);
                if (creature) {
                    YellToAll(creature, "Wicked, wicked, mortals! The forest weeps. The elements recoil at the destruction. Ivus must purge you from this world!", LANG_UNIVERSAL);
                }
                Ivus_has_been_spawned = true;
            }

        }
        else {
            if (!Lokholar_has_been_spawned) {
                AddAVCreature(AV_NPC_LOKHOLAR, AV_CPLACE_LOKHOLAR);
                Creature* creature = GetBGCreature(AV_CPLACE_LOKHOLAR);
                if (creature) {
                    YellToAll(creature, "WHO DARES SUMMON LOKHOLAR? The blood of a thousand Alliance soldiers I shall spill... None shall stand against the might of the Ice Lord!", LANG_UNIVERSAL);
                }
                Lokholar_has_been_spawned = true;
            }
        }
    }
}
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type)
{
    ...
    if (type >= AV_CPLACE_MAX) //static
    {
        ...
        if (BG_AV_StaticCreatureInfo[cinfoid][1] == TEAM_HORDE || BG_AV_StaticCreatureInfo[cinfoid][1] == TEAM_ALLIANCE)
            creature->setFaction(BG_AV_StaticCreatureInfo[cinfoid][1] == TEAM_HORDE ? BG_AV_Factions[1] : BG_AV_Factions[0]);
        isStatic = true;
    }
    else
    {
        ...
        if (BG_AV_CreatureInfo[cinfoid][1] == TEAM_HORDE || BG_AV_CreatureInfo[cinfoid][1] == TEAM_ALLIANCE)
            creature->setFaction(BG_AV_CreatureInfo[cinfoid][1] == TEAM_HORDE ? BG_AV_Factions[1] : BG_AV_Factions[0]);
    }
    if (!creature)
        return NULL;
...
void BattlegroundAV::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        /************************************* MARSHAL/WARMASTER RESPAWN AND PASSIVE VICTORY CHECK *******************************************/
        if (GetBGCreature(60 + AV_CPLACE_MAX)->isAlive()) {//Alliance Boss
            if (!GetBGCreature(60 + AV_CPLACE_MAX)->isInCombat()) { //Alliance Boss
                for (uint8 i = 0; i < 4; i++) {
                    if (m_Nodes[i + 7].State != POINT_DESTROYED) {
                        if (!GetBGCreature(69 + i)->isAlive()) {
                            DelCreature(69 + i);
                            AddAVCreature(14 + i, 69 + i);
                        }
                    }
                }
            }
            else {
                GetBGCreature(60 + AV_CPLACE_MAX)->CallForHelp(50.0f);
            }
        }
        else {
            EndBattleground(HORDE);
        }
        if (GetBGCreature(132 + AV_CPLACE_MAX)->isAlive()) {//Horde Boss
            if (!GetBGCreature(132 + AV_CPLACE_MAX)->isInCombat()) { //Horde Boss
                for (uint8 i = 0; i < 4; i++) {
                    if (m_Nodes[i + 11].State != POINT_DESTROYED) {
                        if (!GetBGCreature(73 + i)->isAlive()) {
                            DelCreature(73 + i);
                            AddAVCreature(18 + i, 73 + i);
                        }
                    }
                }
            }
...
void BattlegroundAV::RemovePlayer(Player* player, uint64 /*guid*/, uint32 /*team*/)
{
   if (!player)
    ...
    player->DestroyItemCount(17306, 100000000, true);
    player->DestroyItemCount(17423, 100000000, true);
    player->DestroyItemCount(17422, 100000000, true);
}
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    if (IsTower(node)) {
        creatureid = (owner == ALLIANCE) ? AV_NPC_A_TOWERDEFENSE_0 : AV_NPC_H_TOWERDEFENSE_0;
    }
    else
    {       
    ...
    if (trigger)
    {
        ...
        trigger->setFaction(owner == ALLIANCE ? BG_AV_Factions[0] : BG_AV_Factions[1]);
        trigger->CastSpell(trigger, SPELL_HONORABLE_DEFENDER_25Y, false);
    }
}
...
{
    Ivus_has_been_spawned = false;
    Lokholar_has_been_spawned = false;
    ...
    if (!AddObject(AV_OPLACE_FirePit_1, BG_AV_OBJECTID_CAMPFIRE, BG_AV_ObjectPos[AV_OPLACE_FirePit_1][0], BG_AV_ObjectPos[AV_OPLACE_FirePit_1][1], BG_AV_ObjectPos[AV_OPLACE_FirePit_1][2], BG_AV_ObjectPos[AV_OPLACE_FirePit_1][3], 0, 0, 0, 0, RESPAWN_IMMEDIATELY)) {
        TC_LOG_ERROR("sql.sql", "BattleGroundAV: Failed to spawn campfire 1");
    }
    if (!AddObject(AV_OPLACE_FirePit_2, BG_AV_OBJECTID_BONFIRE, BG_AV_ObjectPos[AV_OPLACE_FirePit_2][0], BG_AV_ObjectPos[AV_OPLACE_FirePit_2][1], BG_AV_ObjectPos[AV_OPLACE_FirePit_2][2], BG_AV_ObjectPos[AV_OPLACE_FirePit_2][3], 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        TC_LOG_ERROR("sql.sql", "BattleGroundAV: Failed to spawn campfire 2");
    }
    if (!AddObject(AV_OPLACE_FirePit_3, BG_AV_OBJECTID_BONFIRE_SKINNY, BG_AV_ObjectPos[AV_OPLACE_FirePit_3][0], BG_AV_ObjectPos[AV_OPLACE_FirePit_3][1], BG_AV_ObjectPos[AV_OPLACE_FirePit_3][2], BG_AV_ObjectPos[AV_OPLACE_FirePit_3][3], 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        TC_LOG_ERROR("sql.sql", "BattleGroundAV: Failed to spawn campfire 3");
    }
    if (!AddObject(AV_OPLACE_FirePit_4, BG_AV_OBJECTID_BONFIRE_SKINNY, BG_AV_ObjectPos[AV_OPLACE_FirePit_4][0], BG_AV_ObjectPos[AV_OPLACE_FirePit_4][1], BG_AV_ObjectPos[AV_OPLACE_FirePit_4][2], BG_AV_ObjectPos[AV_OPLACE_FirePit_4][3], 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        TC_LOG_ERROR("sql.sql", "BattleGroundAV: Failed to spawn campfire 3");
    }
    GetBGObject(AV_OPLACE_FirePit_1)->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
    GetBGObject(AV_OPLACE_FirePit_2)->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
    //spawn node-objects
...
void BattlegroundAV::ResetBGSubclass()
{
    ...
    for (uint16 i = 0; i < BG_AV_TOTALNPCS; i++)
        if (BgCreatures[i])
            DelCreature(i);
}
