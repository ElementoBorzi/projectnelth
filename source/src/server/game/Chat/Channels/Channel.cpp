...
void Channel::JoinChannel(Player* player, std::string const& pass)
{
    if (GetName() == "gmfeed")
        if (auto s = player->GetSession())
            if (AccountMgr::IsPlayerAccount(s->GetSecurity()))
            {
                TC_LOG_ERROR("sql.sql", "sending");
...
void Channel::SetMode(Player const* player, std::string const& p2n, bool mod, bool set)
{
    ...
    if (!victim || !IsOn(victim) ||
        ...
        ((!AccountMgr::IsGMAccount(sec) || !AccountMgr::IsGMAccount(newp->GetSession()->GetSecurity())) &&
         player->GetOTeam() != newp->GetOTeam() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL)))
    {
        WorldPacket data;
...
void Channel::SetOwner(Player const* player, std::string const& newname)
{
    ...
    if (!victim || !IsOn(victim) ||
        (newp->GetOTeam() != player->GetOTeam() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL)))
    {
        WorldPacket data;
...
bool Channel::Say(uint64 guid, std::string const& what, uint32 lang)
{
    if (what.empty())
        return false;
...
void Channel::CarbonCopy(uint64 guid, std::string const& what)
{
    if (what.empty())
        return;
    ...
    if (GetName() != "gmfeed")
    {
        TC_LOG_ERROR("sql.sql", "Carbon copy called for a channel other than gmfeed.");
        return;
    }
...
void Channel::Invite(Player const* player, std::string const& newname)
{
    ...
    if (newp->GetOTeam() != player->GetOTeam() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
    {
        WorldPacket data;
