        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            if (!AccountMgr::IsPlayerAccount(table[i].SecurityLevel))
            {
                ...
                if (m_session)
                {
                    ...
                    /*
                        Command Logging viewable by .lookup history command <account id>
                    */
                    bool log{ true };
                    std::list<std::string> commands_to_ignore//These commands would flood the living fuck out of the log, and have no reason to be logged themselves.
                    {
                        ".gm",
                        ".whispers",
                        ".mod speed",
                    ...
                    for (auto command : commands_to_ignore)
                        if (log)
                            if (fullcmd.rfind(command, 0) == 0)//Command starts with listed commands (corrected to prevent adding these into the "reason" of a command as an exploit to avoid the log).
                                log = false;
                        
                        if (log)
                        {
                            uint64 log_guid{ 0 };
                            if (auto u = p->GetSelectedUnit())
                            {
                                log_guid = u->GetGUID();

                                if (auto c = u->ToCreature())
                                    log_guid = c->GetDBTableGUIDLow();
                            }

                            auto cmd_typed{ fullcmd };
                            CharacterDatabase.EscapeString(cmd_typed);

                            std::string target_name{ "" };
                            if (auto u = p->GetSelectedUnit())
                                target_name = u->GetName();
                            else
                                target_name = GetLogNameForGuid(sel_guid);

                            CharacterDatabase.EscapeString(target_name);

                            CharacterDatabase.AsyncPQuery("INSERT INTO command_log VALUES (NOW(), %u, '%s', '%s', %u);", m_session->GetAccountId(), cmd_typed.c_str(), target_name.c_str(), log_guid);
                        }
                }
            }
        }
