...
Group::~Group()
{
    ...
    if (m_boundInstances->size())
    for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
        for (auto itr2 = m_boundInstances[i].begin(); itr2 != m_boundInstances[i].end(); ++itr2)
            if (itr2->second.save)
...
bool Group::Create(Player* leader)
{
    ...
    m_layerMask = leader->GetLayerMask();
...
void Group::ConvertToRaid()
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
        if (auto player = ObjectAccessor::FindPlayer(citr->guid))
...
void Group::RemoveInvite(Player* player)
{
    if (player)
    {
        InvitesList::iterator itr = m_invitees.find(player);
...
bool Group::AddMember(Player* player)
{
    ...
    if (player)
    {
        ...
        InstanceGroupBind* bind = GetBoundInstance(player, IsNonPermanentLFGGroup());
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        sInfoMgr->UpdateCharGroup(GUID_LOPART(member.guid), GetLowGUID());
    ...
    if (player)
    {
        ...
        uint32 groupUpdateFlag = GROUP_UPDATE_FULL;
        if (player->GetPet())
            groupUpdateFlag |= GROUP_UPDATE_PET;

        player->SetGroupUpdateFlag(groupUpdateFlag);
...

        {
            ...
            for (auto itr = GetFirstMember(); itr != NULL; itr = itr->next())
            {
    ...
    if (GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBFGroup()) ? 1u : 2u))
    {
        ...
        if (player)
        {
            ...
            // Regular group
            {
                ...
                //TC_LOG_ERROR("sql.sql", "removing player %u from group with %u", player->GetGUID(), GetLeaderGUID());
                player->SetLayerMask(player->GetProperLayerMask(), true, player->GetMap());
        ...
        if (!isBGGroup() && !isBFGroup())
        {
            sInfoMgr->UpdateCharGroup(GUID_LOPART(guid), 0);
...
void Group::BroadcastNewLayerMask(bool from_leader)
{
    std::list<Player*> players;
    GetMembers(players, true);
    //TC_LOG_ERROR("sql.sql", "broadcasting new layer %u", GetLayerMask());

    for (auto p : players)
    {
        if (IsLeader(p->GetGUID()) && from_leader)
            continue;

        p->SetLayerMask(p->GetProperLayerMask(), true, p->GetMap());
        p->UpdateObjectVisibility();
       // TC_LOG_ERROR("sql.sql", "player %u has layer %u", p->GetGUID(), p->GetLayerMask());
    }
}
...
void Group::ChangeLeader(uint64 newLeaderGuid)
{
    ...
    Player* oldLeader = ObjectAccessor::FindPlayer(GetLeaderGUID());
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
        {
            for (auto itr = m_boundInstances[i].begin(); itr != m_boundInstances[i].end();)
            {
    ...
    SetLayerMask(newLeader->GetProperLayerMask());
    BroadcastNewLayerMask();
...
void Group::SendLootRoll(uint64 sourceGuid, uint64 targetGuid, int8 rollNumber, uint8 rollType, Roll const& roll)
{
...
void Group::SendLootRollWon(uint64 sourceGuid, uint64 targetGuid, int8 rollNumber, uint8 rollType, Roll const& roll)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            if (r->totalPlayersRolling > 0)
            {
                ...
                else if (auto go = pLootedObject->ToGameObject())
                {
    ...
    for (std::vector<LootItem>::iterator i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            if (r->totalPlayersRolling > 0)
            {
                ...
                if (auto creature = lootedObject->ToCreature())
                {
                    creature->m_groupLootTimer = 60000;
                    creature->lootingGroupLowGUID = GetLowGUID();
                }
                else if (auto go = lootedObject->ToGameObject())
                {
                    go->m_groupLootTimer = 60000;
                    go->lootingGroupLowGUID = GetLowGUID();
                }
...
{
    TC_LOG_DEBUG("network.opcode", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST)");
...
void Group::LogoutPlayerOutOfRange(Player* player)
{
    if (!player)
        return;

    WorldPacket data(SMSG_PARTY_MEMBER_STATS, 2+4+2);
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(Battleground const* bgOrTemplate, BattlegroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount, uint32 /*MaxPlayerCount*/, bool isRated, uint32 arenaSlot)
{
    ...
    memberscount = 0;
    for (auto itr = GetFirstMember(); itr != NULL; itr = itr->next(), ++memberscount)
    {
        Player* member = itr->getSource();
        ...
        // don't let join to rated bg queue if someone from the group is already in a bg queue
        if (bgOrTemplate->GetTypeID() == BATTLEGROUND_RATED_10_VS_10 && (member->InBattlegroundQueue() || member->isUsingLfg()))
        ...
        if (member->isUsingLfg())
            return ERR_LFG_CANT_USE_BATTLEGROUND;
        // check if member is listed in solo queue
        if (sSoloQueueMgr->IsPlayerInSoloQueue(member))
            return ERR_BATTLEDGROUND_QUEUED_FOR_RATED;
    }

    // only check for MinPlayerCount since MinPlayerCount == MaxPlayerCount for arenas and RBGs
...
void Group::SetRaidDifficulty(Difficulty difficulty, uint32 raidId /*= 0*/)
{
    ...
    for (auto itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
        {
            for (auto itr = m_boundInstances[i].begin(); itr != m_boundInstances[i].end(); ++itr)
            {
                uint32 key = InstanceSaveKey::Create(player->GetMapId(), IsNonPermanentLFGGroup());
...
void Group::ResetInstances(uint8 method, bool isRaid, Player* SendMsgTo)
{
    ...
    if (m_boundInstances)
    if (m_boundInstances->size())
        if (m_boundInstances->find(diff) != m_boundInstances->end())
            if (m_boundInstances[diff].size())
                for (auto itr = m_boundInstances[diff].begin(); itr != m_boundInstances[diff].end(); ++itr)
                {
                    if (InstanceSave* instanceSave = itr->second.save)
                        if (MapEntry const* entry = itr->second.save->GetMapEntry())
                        {
                            if (entry->IsRaid() != isRaid || (!instanceSave->CanReset() && method != INSTANCE_RESET_GROUP_DISBAND))
                            {
                                continue;
                            }
                            ...
                            if (isEmpty || method == INSTANCE_RESET_GROUP_DISBAND || method == INSTANCE_RESET_CHANGE_DIFFICULTY)
                            {
                                ...
                                itr = m_boundInstances[diff].erase(itr);
...
void Group::ProcessBoundSwitch(InstanceSave *save, Difficulty previousDifficulty, Difficulty newDifficulty, std::list<uint32 > &mapToErase, Player *leader)
{
    ...
    if (map)
    {
        ...
        {
            hardMode = instanceScript->GetHardModeDifficulty();
        }
    }

    if (hardMode > 0)
    {
        if (hardMode == RAID_DIFFICULTY_25MAN_HEROIC && ((newDifficulty == RAID_DIFFICULTY_10MAN_NORMAL || newDifficulty == RAID_DIFFICULTY_10MAN_HEROIC)))
            return;

        if (hardMode == RAID_DIFFICULTY_10MAN_HEROIC && ((newDifficulty == RAID_DIFFICULTY_25MAN_NORMAL || newDifficulty == RAID_DIFFICULTY_25MAN_HEROIC)))
            return;
    }
...
void Group::SwitchBoundInstance(uint32 /*mapid*/, Difficulty previousDifficulty, Difficulty newDifficulty)
{
    if (auto leader = ObjectAccessor::FindPlayer(GetLeaderGUID()))
    {
        ...
        for (auto itr = m_boundInstances[previousDifficulty].begin(); itr != m_boundInstances[previousDifficulty].end(); ++itr)
        ...
        {
            auto itr2 = m_boundInstances[previousDifficulty].find(*itr);
            if (itr2 != m_boundInstances[previousDifficulty].end())
                itr2 = m_boundInstances[previousDifficulty].erase(itr2);
        }
    }
...
InstanceGroupBind* Group::GetBoundInstance(Difficulty difficulty, uint32 mapId, bool isLfgId)
{
    ...
    auto itr = m_boundInstances[difficulty].find(key);
    if (itr != m_boundInstances[difficulty].end())
        return &itr->second;
    return NULL;
}

bool Group::IsNonPermanentLFGGroup() const
{
    return isLFGGroup() && !sLFGMgr->IsPermanentLFGDungeon(GetGUID());
}
...
InstanceGroupBind* Group::BindToInstance(InstanceSave* save, bool permanent, bool load)
{
    ...
    uint32 key = InstanceSaveKey::Create(save->GetMapId(), IsNonPermanentLFGGroup());
    ...
    bind.perm = permanent;
    if (!load)
        TC_LOG_DEBUG("maps", "Group::BindToInstance: Group (guid: %u, storage id: %u) is now bound to map %d, instance %d, difficulty %d, isLfg %u",
            GUID_LOPART(GetGUID()), m_dbStoreId, save->GetMapId(), save->GetInstanceId(), save->GetDifficulty(), isLFGGroup());
    return &bind;
}

void Group::UnbindInstance(uint32 mapId, uint8 difficulty, bool unload, bool isLfgId /*= false*/)
{
    uint32 key = InstanceSaveKey::Create(mapId, isLfgId);
    auto itr = m_boundInstances[difficulty].find(key);
    if (itr != m_boundInstances[difficulty].end())
...
void Group::GetMembers(std::list<Player*>& players, bool online)
{
    if (m_memberSlots.size())
        for (auto itr = m_memberSlots.begin(); itr != m_memberSlots.end(); ++itr)
            if (auto p = ObjectAccessor::FindPlayer(itr->guid))
                if ((p->GetSession() && online) || (!p->GetSession() && !online))
                players.push_back(p);
    return;
}
...
void Group::SetEveryoneAssistant(Group* group, bool active)
{
    for (member_witerator itr = m_memberSlots.begin(); itr != m_memberSlots.end(); ++itr)
        if (!group->IsLeader(itr->guid))
            group->ToggleGroupMemberFlag(itr, MEMBER_FLAG_ASSISTANT, active);
}
...
{
    TC_LOG_DEBUG("network.opcode", "Group::MarkerChanged (SMSG_RAID_MARKERS_CHANGED)");
...
void Group::SetMarker(uint8 slot, Position const& destTarget, Unit* caster, SpellInfo const* spell)
{
    DynamicObject* dynObj = ObjectAccessor::GetObjectInWorld(m_markers[slot], (DynamicObject*)NULL);
    if (dynObj)
        dynObj->Remove();

    dynObj = new DynamicObject(false);
    if (!dynObj->CreateDynamicObject(sObjectMgr->GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), caster, spell, destTarget, 0, DYNAMIC_OBJECT_RAID_MARKER))
    {
        delete dynObj;
        return;
    }
    ...
    int32 basePoints = spell->Effects[0].BasePoints;
    Aura* aura = Aura::TryCreate(spell, MAX_EFFECT_MASK, dynObj, caster, &basePoints);
    //dynObj->SetDuration(spell->GetDuration()); equal 0 ?? wtf ?
    dynObj->SetDuration(1200000);
    dynObj->SetFieldNotifyFlag(UF_FLAG_PARTY_MEMBER);
    m_markers[slot] = dynObj->GetGUID();
...
{
    TC_LOG_DEBUG("network.opcode", "Group::RemoveMarker");
...
uint8 Group::GetGuildMembersCount(uint64 guid) const
{
    uint8 count = 0;
    if (auto player = ObjectAccessor::FindPlayer(guid))
    {
        uint32 playergguid = player->GetGuildId();

        if(Map* tempMap = player->GetMap())
        {
            Map::PlayerList const& PlayerList = tempMap->GetPlayers();
            if (!PlayerList.isEmpty())
            {
                for (Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
                    if (auto tplayer = itr->getSource())
                        if(playergguid == tplayer->GetGuildId())
                            count++;
            }
        }
    }
