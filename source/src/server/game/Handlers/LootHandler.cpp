...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_AUTOSTORE_LOOT_ITEM");
    ...
    if (auto player = GetPlayer())
        if (player->isGameMaster())
        {
            ...
            player->GetSession()->DoLootRelease(player->GetGUID());
            return;
        }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_LOOT_MONEY");
    ...
    if (loot)
    {
        ...
        if (shareMoney && player->GetGroup())      //item, pickpocket and players can be looted only single player
        {
            ...
            for (auto i = playersNear.begin(); i != playersNear.end(); ++i)
            {

                if ((*i)->isGameMaster())
                {
                    (*i)->SendEquipError(EQUIP_ERR_CANT_EQUIP_EVER, NULL, NULL);

                    (*i)->GetSession()->DoLootRelease(guid);
                    continue;
                }
                ...
                if (auto guild = sGuildMgr->GetGuildById((*i)->GetGuildId()))
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_LOOT");

    uint64 guid;
    recvData >> guid;

    // Check possible cheat
    if (!GetPlayer()->isAlive() || (!IS_CRE_OR_VEH_GUID(guid) && (!IS_CORPSE_GUID(guid))))
        return;
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_LOOT_RELEASE");
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
    ...
    if (!target)
        ...
        {
            _player->SendEquipError(EQUIP_ERR_CANT_EQUIP_EVER, NULL, NULL);
            _player->SendLootRelease(GetPlayer()->GetLootGUID());
            if (auto g = _player->GetGroup())
                g->RemoveMember(_player->GetGUID(), GROUP_REMOVEMETHOD_LEAVE, NULL, "GameMaster anti-abuse detection");
        }

        if (target->isGameMaster())
        {
            _player->SendLootRelease(GetPlayer()->GetLootGUID());
            target->SendEquipError(EQUIP_ERR_CANT_EQUIP_EVER, NULL, NULL);
            if (auto g = target->GetGroup())
                g->RemoveMember(target->GetGUID(), GROUP_REMOVEMETHOD_LEAVE, NULL, "GameMaster anti-abuse detection");
            return;
        }

    TC_LOG_DEBUG("network.opcode", "WorldSession::HandleLootMasterGiveOpcode (CMSG_LOOT_MASTER_GIVE, 0x02A3) Target = [%s].", target->GetName().c_str());
    ...
    {
        target->SendEquipError(EQUIP_ERR_CANT_EQUIP_EVER, NULL, NULL);
        target->GetSession()->DoLootRelease(lootguid);
    ...
    if (_player->HasPendingBind() || target->HasPendingBind())
        return;
