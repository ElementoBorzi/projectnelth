...
bool LoginQueryHolder::Initialize()
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_GUILD_LEVEL);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_GUILD_SPELLS, stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_LFRLOOTDATA);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_LFR_LOOT_DATA, stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAILS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_MAILS, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAILS_ITEMS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_MAILS_ITEMS, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_DIGSITES);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_DIGSITES, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_CURRENT_ARTEFACTS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_CURRENT_ARTIFACTS, stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_KEYSTONES);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_KEYSTONES, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_ARENA_TITLE_DIST);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_ARENA_REWARD, stmt);
...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        if (IsRequestingNewCharacter())     //We have a premium account asking for client-side space to make a new character.
        {
            if (charCount > 9)
                charCount = 9;
            SetRequestingNewCharacter(false);
        }
        ...
        do
        {
            ...
            TC_LOG_INFO("network.opcode", "Loading char guid %u from account %u.", guidLow, GetAccountId());

            Player::BuildEnumData(result, &dataBuffer, &bitBuffer, this);

            if (GetSecurity() >= sWorld->GetEntranceSecurityLimit())
        ...
        } while (result->NextRow() && (charProcessed < charCount));
    }
    else
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        TC_LOG_ERROR("network.opcode", "Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", createInfo.Class, GetAccountId());
    ...
    if (!raceEntry)
    {
        TC_LOG_ERROR("network.opcode", "Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", createInfo.Race, GetAccountId());
        SendCharCreate(CHAR_CREATE_FAILED);
        return;
    }
    ...
    {
        TC_LOG_ERROR("network.opcode", "Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", Expansion(), GetAccountId(), raceEntry->expansion, createInfo.Race);
    ...
    {
        TC_LOG_ERROR("network.opcode", "Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", Expansion(), GetAccountId(), classEntry->expansion, createInfo.Class);
    ...
    {
        TC_LOG_ERROR("network.opcode", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
    ...
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 2:
        {
            if (result)
            {
                ...
                bool is_upgraded{ premium_HasPerk(ACCOUNT_PERK_50_CHARACTERS)};

                /*
                    Insert Definition of is_upgraded here
                */
                /*
                    TODO: DB CHECK NUMBER OF CHARACTERS, not just client input
                */

                if (is_upgraded)
                {
                    if (createInfo->CharCount > 50)
                    too_many = true;
                }
                else
                {
        ...
        case 3:
        {
            ...
            {
                std::string imp_name = sObjectMgr->GeneratePetName(416);
                CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,    `savetime`,   `abdata`) VALUES (%u,   416,   %u,   4449,   688,   0,   1,   0,   0,   '%s',   0,   0,   282,   72,  1295721046,   '7 2 7 1 7 0 129 3110 1 0 1 0 1 0 6 3 6 1 6 0 ')", pet_id, newChar.GetGUIDLow(), imp_name.c_str());
            ...
            if (newChar.getClass() == CLASS_HUNTER)
            {
                ...
                {
                case RACE_HUMAN:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,  `savetime`,   `abdata`) VALUES (%u,   42717,   %u,   903,   13481,   1,   1,   0,   0,   'Wolf',   0,   0,   192,   0,   1295727347,  '7 2 7 1 7 0 129 2649 129 17253 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_DWARF:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,   `savetime`,    `abdata`) VALUES (%u,   42713,   %u,   822,   13481,   1,   1,   0,   0,   'Bear',   0,   0,   212,   0,   1295727650,   '7 2 7 1 7 0 129 2649 129 16827 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_ORC:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,    `savetime`,    `abdata`) VALUES (%u,   42719,   %u,   744,   13481,   1,   1,   0,   0,   'Boar',   0,   0,   212,   0,    1295727175,   '7 2 7 1 7 0 129 2649 129 17253 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_NIGHTELF:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,   `savetime`,   `abdata`) VALUES (%u,   42718,   %u,    17090,   13481,   1,   1,   0,   0,   'Cat',   0,   0,   192,   0,   1295727501,   '7 2 7 1 7 0 129 2649 129 16827 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_UNDEAD_PLAYER:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,   `savetime`,   `abdata`) VALUES (%u,   51107,   %u,    368,   13481,   1,   1,   0,   0,   'Spider',   0,   0,   202,   0,   1295727821,  '7 2 7 1 7 0 129 2649 129 17253 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_TAUREN:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,    `savetime`,    `abdata`) VALUES (%u,   42720,   %u,    29057,   13481,   1,   1,   0,   0,   'Tallstrider',   0,   0,   192,   0,  1295727912,  '7 2 7 1 7 0 129 2649 129 16827 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_TROLL:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,    `savetime`,   `abdata`) VALUES (%u,   42721,   %u,    23518,   13481,   1,   1,   0,   0,   'Raptor',   0,   0,   192,   0,  1295727987,   '7 2 7 1 7 0 129 2649 129 50498 129 16827 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_GOBLIN:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,   `savetime`,    `abdata`) VALUES (%u,   42715,   %u,   27692,   13481,   1,   1,   0,   0,   'Crab',   0,   0,   212,   0,    1295720595,   '7 2 7 1 7 0 129 2649 129 16827 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_BLOODELF:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,  `savetime`,   `abdata`) VALUES (%u,   42710,   %u,   23515,   13481,   1,   1,   0,   0,   'Dragonhawk',   0,   0,   202,   0,    1295728068,   '7 2 7 1 7 0 129 2649 129 17253 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_DRAENEI:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,   `savetime`,   `abdata`) VALUES (%u,   42712,   %u,   29056,   13481,   1,   1,   0,   0,   'Moth',   0,   0,   192,   0,   1295728128,    '7 2 7 1 7 0 129 2649 129 49966 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                case RACE_WORGEN:
                    CharacterDatabase.PExecute("REPLACE INTO character_pet (`id`,   `entry`,   `owner`,   `modelid`,   `CreatedBySpell`,   `PetType`,   `level`,   `exp`,   `Reactstate`,   `name`,   `renamed`,   `slot`,   `curhealth`,   `curmana`,   `savetime`,   `abdata`) VALUES (%u,   42722,   %u,   30221,   13481,   1,   1,   0,   0,   'Dog',   0,   0,   192,   0,    1295728219,   '7 2 7 1 7 0 129 2649 129 17253 1 0 1 0 6 3 6 1 6 0 ')",   pet_id,   newChar.GetGUIDLow());
                    break;
                }
                newChar.SetTemporaryUnsummonedPetNumber(pet_id);
            }
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("entities.player.character", "Account: %d, IP: %s deleted character: %s, GUID: %u, Level: %u", accountId, GetRemoteAddress().c_str(), name.c_str(), GUID_LOPART(guid), level);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_ERROR("network.opcode", "Player tries to login again, AccountId = %d", GetAccountId());
        return;
    }
    ...
    TC_LOG_DEBUG("network.opcode", "Character (Guid: %u) logging in", GUID_LOPART(playerGuid));

    if (!CharCanLogin(GUID_LOPART(playerGuid)))
    {
        //TC_LOG_ERROR("network.opcode", "Account (%u) can't login with that character (%u).", GetAccountId(), GUID_LOPART(playerGuid));
        ...
        SendPacket(&data);
        SendAuthResponse(CHAR_LOGIN_DISABLED, false, 0);
...
void WorldSession::HandleLoadScreenOpcode(WorldPacket& recvPacket)
{
    ...
    recvPacket >> mapID;
    bool showing = recvPacket.ReadBit();

    if (GetPlayer())
        GetPlayer()->SetPlayerIsInLoadscreen(showing);
    recvPacket.rfinish();
}
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    // Send MOTD
    {
        ...
        SendPacket(&data);
        TC_LOG_DEBUG("network.opcode", "WORLD: Sent motd (SMSG_MOTD)");

        // send server info
        if (sWorld->getIntConfig(CONFIG_ENABLE_SINFO_LOGIN) == 1)
            chH.PSendSysMessage(_FULLVERSION);

        TC_LOG_DEBUG("network.opcode", "WORLD: Sent server info");
    }
    ...
    if (PreparedQueryResult resultGuild = holder->GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_GUILD))
    {
        ...
        if (auto guild = sGuildMgr->GetGuildById(pCurrChar->GetGuildId()))
        {
            ...
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_REACH_LEVEL, 0, 0, 0, NULL, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_REACH_SKILL_LEVEL, 0, 0, 0, NULL, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION, 0, 0, 0, NULL, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GAIN_EXALTED_REPUTATION, 0, 0, 0, NULL, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_EARNED_PVP_TITLE, 0, 0, 0, NULL, pCurrChar);
        }
    }
    ...
    if (pCurrChar->isGMVisible())
        if (!HasAccessFlag(ACCOUNT_FLAG_HEAD_GAMEMASTER) && !pCurrChar->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
    ...
    pCurrChar->LoadPets(pCurrChar->GetCurrentPetSlot());
...
{
    TC_LOG_ERROR("network.opcode", "WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_SHOWING_HELM for %s", _player->GetName().c_str());
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_SHOWING_CLOAK for %s", _player->GetName().c_str());
    recvData.read_skip<uint8>(); // unknown, bool?
...
{
    std::shared_ptr<CharacterRenameInfo> renameInfo(new CharacterRenameInfo());
    ...
    _charRenameCallback.SetParam(renameInfo);
    _charRenameCallback.SetFutureResult(CharacterDatabase.AsyncQuery(stmt));
}

void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::shared_ptr<CharacterRenameInfo const> renameInfo)
{
    if (!result)
    {
    ...
    _queryFuture = CharacterDatabase.CommitTransactionWithFuture(trans);
    _queryCallback = [this, oldName, renameInfo](bool success)
    {
        if (success)
        {
            TC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Character:[%s] (guid:%u) Changed name to: %s",
                GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), GUID_LOPART(renameInfo->Guid), renameInfo->Name.c_str());

            SendCharRename(RESPONSE_SUCCESS, *renameInfo);
            sInfoMgr->UpdateCharBase(GUID_LOPART(renameInfo->Guid), renameInfo->Name);
        }
        else
        {
            TC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Character:[%s] (guid:%u) Change name to: %s failed.",
                GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), GUID_LOPART(renameInfo->Guid), renameInfo->Name.c_str());

            SendCharRename(RESPONSE_FAILURE, *renameInfo);
        }
    };
}
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ALTER_APPEARANCE");
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_DEBUG("network.opcode", "Client sent wrong glyph slot number in opcode CMSG_REMOVE_GLYPH %u", slot);
        return;
    }
...
{
    std::shared_ptr<CharacterCustomizeInfo> customizeInfo(new CharacterCustomizeInfo());
    ...
    stmt->setUInt32(0, GUID_LOPART(customizeInfo->Guid));
    ...
    stmt->setUInt32(2, GUID_LOPART(customizeInfo->Guid));
    ...
    _queryFuture = CharacterDatabase.CommitTransactionWithFuture(trans);
    _queryCallback = [this, customizeInfo](bool success)
    {
        if (success)
        {
            std::string oldName;
            if (sObjectMgr->GetPlayerNameByGUID(customizeInfo->Guid, oldName))
            {
                TC_LOG_INFO("entities.player.character", "Account: %d (IP: %s), Character[%s] (guid:%u) Customized to: %s",
                    GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), GUID_LOPART(customizeInfo->Guid), customizeInfo->Name.c_str());
            }

            sInfoMgr->UpdateCharBase(GUID_LOPART(customizeInfo->Guid), customizeInfo->Name, customizeInfo->Gender);
        ...
        else
        {
            ...
            if (sObjectMgr->GetPlayerNameByGUID(customizeInfo->Guid, oldName))
            {
                TC_LOG_INFO("entities.player.character", "Account: %d (IP: %s), Character[%s] (guid:%u) Customize to: %s failed.",
                    GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), GUID_LOPART(customizeInfo->Guid), customizeInfo->Name.c_str());
            }

            SendCharCustomize(RESPONSE_FAILURE, *customizeInfo);
        }
    };
}
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_EQUIPMENT_SET_SAVE");
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_EQUIPMENT_SET_DELETE");
...
void WorldSession::HandleEquipmentSetUse(WorldPacket& recvData)
{
    ...
    TC_LOG_DEBUG("network.opcode", "CMSG_EQUIPMENT_SET_USE");
...
{
    std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo(new CharacterFactionChangeInfo());
    recvData >> factionChangeInfo->Guid
             >> factionChangeInfo->Name
    ...
    if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
    {
        ...
        if (newTeam == oldTeam)
        {
            SendCharFactionChange(CHAR_CREATE_CHARACTER_SWAP_FACTION, *factionChangeInfo);
            return;
        }
    }
    ...
    if (oldRace != factionChangeInfo->Race)
    {
        ...
        if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
        {
            ...
            if (team == TEAM_ALLIANCE)
            {
                loc.WorldRelocate(0, -8866.601563f, 671.615784f, 97.904839f, 5.318801f);
                zoneId = 1519;
            }
            else
            {
                loc.WorldRelocate(1, 1572.561401f, -4437.027344f, 16.054110f, 1.849459f);
                zoneId = 1637;
            }
            ...
            for (std::map<uint32, uint32>::const_iterator it = sObjectMgr->FactionChange_Reputation.begin(); it != sObjectMgr->FactionChange_Reputation.end(); ++it)
            {
                ...
                if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
                {
                    ...
                    // new base reputation
                    int32 newBaseRep = sObjectMgr->GetBaseReputationOff(sFactionStore.LookupEntry(newReputation), factionChangeInfo->Race, playerClass);
    ...
    _queryCallback = [this, oldRace, factionChangeInfo](bool success)
    {
        if (success)
        {
            sInfoMgr->UpdateCharBase(GUID_LOPART(factionChangeInfo->Guid), factionChangeInfo->Name, factionChangeInfo->Gender, factionChangeInfo->Race);
        ...
        {
            TC_LOG_DEBUG("entities.player", "%s (IP: %s) failed changing race from %u to %u", GetPlayerInfo().c_str(), GetRemoteAddress().c_str(), oldRace, factionChangeInfo->Race);
            SendCharFactionChange(RESPONSE_FAILURE, *factionChangeInfo);
        }
    };
}
...
void WorldSession::SendCharCustomize(ResponseCodes result, CharacterCustomizeInfo const& customizeInfo)
{
    ...
    if (result == RESPONSE_SUCCESS)
    {
        data << uint64(customizeInfo.Guid);
...
void WorldSession::SendCharFactionChange(ResponseCodes result, CharacterFactionChangeInfo const& factionChangeInfo)
{
    ...
    if (result == RESPONSE_SUCCESS)
    {
        data << uint64(factionChangeInfo.Guid);
