...
{
    TC_LOG_DEBUG("network.opcode", "MSG_INSPECT_ARENA_TEAMS");

    uint64 guid;
    recvData >> guid;
    TC_LOG_DEBUG("network.opcode", "Inspect Arena stats (GUID: %u TypeId: %u)", GUID_LOPART(guid), GuidHigh2TypeId(GUID_HIPART(guid)));

    if (auto player = ObjectAccessor::FindPlayer(guid))
    {
        for (uint8 i = 0; i < MAX_NORMAL_ARENA_SLOT; ++i)
        {
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_ARENA_TEAM_QUERY");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_ARENA_TEAM_ROSTER");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_ARENA_TEAM_CREATE");
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_INVITE");
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && player->GetOTeam() != GetPlayer()->GetOTeam())
    {
    ...
    TC_LOG_DEBUG("network.opcode", "WORLD: Sent SMSG_ARENA_TEAM_INVITE");
}

void WorldSession::HandleArenaTeamAcceptOpcode(WorldPacket & /*recvData*/)
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_ACCEPT");                // empty opcode
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && _player->GetOTeam() != sObjectMgr->GetPlayerTeamByGUID(arenaTeam->GetCaptain()))
    {
    ...
    // Add player to team
    if (!
        arenaTeam->AddMember(_player->GetGUID())
    )
    {
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_DECLINE");               // empty opcode
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_LEAVE");
    ...
    // Disallow leave team while in arena queue
    if (_player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_2v2) ||
    ...
    // Disallow leave team while in solo queue
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_DISBAND");
    ...
    if (ArenaTeam* arenaTeam = sArenaTeamMgr->GetArenaTeamById(arenaTeamId))
    {
        ...
        //this line is commented out because it already exists somewhere else and works properly now. - cryptid, 2/24/2021
        //arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_DISBANDED_S, _player->GetGUID(), 3,"A", "B", "C");
        arenaTeam->Disband(this);
        delete arenaTeam;
    }
}
...
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_REMOVE");
    ...
    arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_REMOVE_SSS, _player->GetGUID(), 3, name, arenaTeam->GetName(), _player->GetName());
    //arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_REMOVE_SSS, _player->GetGUID(), 3, "DEBUG_ARG_1", "DEBUG_ARG_2", "DEBUG_ARG_3");
    arenaTeam->DelMember(member->Guid, true);
}

void WorldSession::HandleArenaTeamLeaderOpcode(WorldPacket& recvData)
{
    TC_LOG_DEBUG("network.opcode", "CMSG_ARENA_TEAM_LEADER");
    ...
    /*void BroadcastEvent(
        ...
        std::string const& str2,
        std::string const& str3);*/
        //fired when a new captain is set.
    arenaTeam->BroadcastEvent(
        ERR_ARENA_TEAM_LEADER_CHANGED_SSS, _player->GetGUID(), 3, _player->GetName(), name, arenaTeam->GetName());
}
