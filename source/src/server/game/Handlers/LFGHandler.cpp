...
void BuildQuestReward(WorldPacket& data, Quest const* quest, Player* player)
{
    ...
    data << uint32(player->getLevel() >= DEFAULT_MAX_LEVEL ? 0 : quest->XPValue(player));
    data << uint8(rewCount);
    if (rewCount)
    {
        ...
        for (uint8 i = 0; i < quest->GetRewCurrencyCount(); ++i)
        {
            ...
            if(quest->RewardCurrencyId[i])
            {
                uint32 rewardCurrencyCount = quest->RewardCurrencyCount[i] * LFG_MULTYPLIER;
                // display the max possible valor point amount
                if (quest->RewardCurrencyId[i] == CURRENCY_TYPE_VALOR_POINTS)
                {
                    uint32 currentWeekCap = player->GetCurrencyOnWeek(quest->RewardCurrencyId[i], false);
                    if ((currentWeekCap + rewardCurrencyCount) > CURRENCY_WEEK_CAP_VALOR_POINTS)
                        rewardCurrencyCount = CURRENCY_WEEK_CAP_VALOR_POINTS - currentWeekCap;
                }
                data << uint32(0);
                data << uint32(rewardCurrencyCount);  // currency amount
            }
            data << uint8(true);
        }
    }
...
void WorldSession::HandleLfgSetCommentOpcode(WorldPacket&  recvData)
{
    ...
    std::string comment = recvData.ReadString(msgLen);
...
{
    int8 agree;                                            // Agree to kick player
    recvData >> agree;
    uint64 guid = GetPlayer()->GetGUID();
...
void WorldSession::SendLfgPlayerLockInfo()
{
    ...
    for (lfg::LfgDungeonSet::const_iterator it = randomDungeons.begin(); it != randomDungeons.end(); ++it)
    {
        ...
        uint32 currentValorPoints = _player->GetCurrencyOnWeek(CURRENCY_TYPE_VALOR_POINTS, false);
        uint32 realDungeonId = (dungeonId & 0x00FFFFFF);
        bool isRandom85Heroic = (realDungeonId == 301 || realDungeonId == 434);
        bool forceValorQuest = isRandom85Heroic && currentValorPoints < CURRENCY_WEEK_CAP_VALOR_POINTS;
        ...
        if (reward)
        {
            ...
            if (quest[0])
            {
                ...
                // Lfr Reward check
                if (quest[0]->RewardCurrencyId[0] == CURRENCY_TYPE_VALOR_POINTS)
                    if (currentValorPoints >= CURRENCY_WEEK_CAP_VALOR_POINTS)
                        done = true;

                if ((!isRandom85Heroic && done) || (isRandom85Heroic && !forceValorQuest))
                    actualQuest = quest[1];
                else
                    actualQuest = quest[0];
            }
        ...
        if (actualQuest)
        {
            ...
            data << uint32(currentValorPoints);                             // periodPurseQuantity
            ...
            if (realDungeonId == 416 || realDungeonId == 417)
            {
                DungeonEncounterList const* encounters = sObjectMgr->GetDungeonEncounterList(967, RAID_DIFFICULTY_25MAN_NORMAL);
                for (DungeonEncounterList::const_iterator itr = encounters->begin(); itr != encounters->end(); ++itr)
                {
                    DungeonEncounter const* encounter = *itr;
                    if ((encounter->dbcEntry->encounterIndex >= 5 && realDungeonId == 416) || (encounter->dbcEntry->encounterIndex <= 4 && realDungeonId == 417))
                        continue;

                    if (GetPlayer()->HasLFRLootObject(encounter->creditEntry))
                        completedEncounters |= 1 << encounter->dbcEntry->encounterIndex;
                }
            }
            data << uint32(completedEncounters);                                          // completedEncounters
        }
        else
        {
        ...
        if (isCallToArmEligible)
        {
            ...
            if (roleTank)
            {
                ...
                BuildQuestReward(data, q, GetPlayer());
            }
            else
                data << uint32(0);
            if (roleHeal)
...
void WorldSession::SendLfgRoleCheckUpdate(lfg::LfgRoleCheck const& roleCheck)
{
    ...
    data << uint8(roleCheck.isBeginning ? 1 : 0);
...
{
    TC_LOG_DEBUG(LOG_FILTER_PACKETIO, "SMSG_LFG_UPDATE_LIST %s dungeon entry: %u",
        GetPlayerInfo().c_str(), dungeonEntry);
