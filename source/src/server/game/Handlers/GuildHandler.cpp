...
void WorldSession::HandleGuildQueryOpcode(WorldPacket& recvPacket)
{
    ...
    if (auto guild = sGuildMgr->GetGuildByGuid(guildGuid))
...
void WorldSession::HandleGuildAcceptOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (!GetPlayer()->GetGuildId())
        if (auto guild = sGuildMgr->GetGuildById(GetPlayer()->GetGuildIdInvited()))
...
void WorldSession::HandleGuildBankerActivate(WorldPacket& recvPacket)
{
    ...
    if (go->GetEntry() == 206602 || go->GetEntry() == 206603)
        if (GetPlayer()->GetReputationRank(1168) < REP_FRIENDLY)
        {
            Guild::SendCommandResult(this, GUILD_COMMAND_VIEW_TAB, ERR_GUILD_REP_TOO_LOW);
            return;
        }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_QUERY_GUILD_XP");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GUILD_REQUEST_PARTY_STATE");
...
void WorldSession::HandleGuildRequestMaxDailyXP(WorldPacket& recvPacket)
{
    ...
    if (auto guild = sGuildMgr->GetGuildByGuid(guid))
    {
        if (guild->IsMember(_player->GetGUID()))
        {
...
void WorldSession::HandleGuildSwitchRankOpcode(WorldPacket& recvPacket)
{
    ...
    TC_LOG_DEBUG("network.opcode","WORLD: Received CMSG_GUILD_SWITCH_RANK rank %u up %u", rankId, up);
...
void WorldSession::HandleReplaceGuildLeader(WorldPacket& recvPacket)
{
    if (auto guild = GetPlayer()->GetGuild())
        guild->ReplaceGuildLeader(_player);
}

void WorldSession::HandleGuildRenameRequest(WorldPacket& recvPacket)
{
    TC_LOG_DEBUG("guild", "CMSG_GUILD_CHANGE_NAME_REQUEST");

    //    uint32 length = recvPacket.ReadBits(12);
    //  std::string info = recvPacket.ReadString(length);

    //    TC_LOG_DEBUG("guild", "CMSG_GUILD_CHANGE_NAME_REQUEST [%s]: %s", GetPlayerInfo().c_str(), info.c_str());

    //    if (auto guild = GetPlayer()->GetGuild())
    //    guild->UpdateName(info);
}

void WorldSession::HandleGuildMemberRecipes(WorldPacket& recvPacket)
{
    TC_LOG_DEBUG("guild", "CMSG_QUERY_GUILD_MEMBER_RECIPES");
    ...
    if (auto guild = GetPlayer()->GetGuild())
    {
        TC_LOG_DEBUG("guild", "SMSG_GUILD_MEMBER_RECIPES");

        WorldPacket data(SMSG_GUILD_MEMBER_RECIPES, 8 + 4 + 300 + 4 + 4);
        ...
        data << uint32(skillValue);

        // ====    wtf ? ====
        // data << learnRecipeMask
        for (int i = 0; i < 300; i++)
            data.WriteBit(0);
        // =======
...
void WorldSession::HandleGuildMemberForRecipes(WorldPacket& recvPacket)
{
    TC_LOG_DEBUG("guild", "CMSG_QUERY_GUILD_MEMBERS_FOR_RECIPE");
    ...
    HandleGuildMemberByRecipes(SKILL_BLACKSMITHING);
    HandleGuildMemberByRecipes(SKILL_ALCHEMY);
    HandleGuildMemberByRecipes(SKILL_LEATHERWORKING);
    HandleGuildMemberByRecipes(SKILL_COOKING);
    HandleGuildMemberByRecipes(SKILL_JEWELCRAFTING);
    HandleGuildMemberByRecipes(SKILL_INSCRIPTION);
    HandleGuildMemberByRecipes(SKILL_ENGINEERING);
    HandleGuildMemberByRecipes(SKILL_TAILORING);
    HandleGuildMemberByRecipes(SKILL_ENCHANTING);
}

void WorldSession::HandleGuildMemberByRecipes(uint32 skillId)
{
    Guild *guild = GetPlayer()->GetGuild();
    ...
    TC_LOG_DEBUG("guild", "SMSG_GUILD_MEMBERS_FOR_RECIPE");

    uint32 spellId = GetPlayer()->GetProfesionSpellBySkillId(skillId);
    std::list<ObjectGuid > memberGUIDs /* = guild->GetMemberListBySkillId(skillId) */;

    if (!memberGUIDs.empty())
    {
        WorldPacket data(SMSG_GUILD_MEMBERS_FOR_RECIPE, 4 + 8 * memberGUIDs.size() + 4 + 4);
        data.WriteBits(memberGUIDs.size(), 26);
        for (std::list<ObjectGuid >::const_iterator itr = memberGUIDs.begin(); itr != memberGUIDs.end(); itr++)
            if (ObjectGuid memberGUID = *itr)
            {
...
void WorldSession::HandleGuildRecipes(WorldPacket& recvPacket)
{
    TC_LOG_DEBUG("guild", "CMSG_QUERY_GUILD_RECIPES");
    ...
    /*    if (Guild *guild = GetPlayer()->GetGuild())
    {
        uint16 count = 1;
        uint32 skillId = 182;
        WorldPacket data(SMSG_GUILD_RECIPES, count * (16 + 4 + 300));
        for (int i = 0; i < count; ++i)
        {
            ...
            for (int j = 0; j < 300; ++j)
                ...
                if (SkillLineAbilityEntry const* pAbility = sSkillLineAbilityStore.LookupEntry(j))
                    if (pAbility->skillId == skillId)
                    mask[pAbility->character_points[1] / 8] |= pAbility->character_points[1] & 0x7;

            data.WriteBits(count, 16);
            data << uint32(SKILL_TAILORING);
            for (int j = 0; j < 300; ++j)
                data.WriteBit(mask[j]);
                //data.WriteBit
