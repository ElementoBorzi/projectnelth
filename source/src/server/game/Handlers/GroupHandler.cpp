...
void WorldSession::SendPartyResult(PartyOperation operation, const std::string& member, PartyResult res, uint32 val /* = 0 */, uint64 targetGUID /*= 0*/)
{
    ...
    data << uint64(targetGUID);                             // player who caused error (in some cases).
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_INVITE");
    ...
    // can't group with
    if (!GetPlayer()->isGameMaster() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && GetPlayer()->GetOTeam() != player->GetOTeam())
    {
    ...
    if (group)
    {
        ...
        // not have place
        if (group->IsFull() || (group->isLFRGroup() && group->GetMembersCount() >= 25))
        {
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_INVITE_RESPONSE");
    ...
    if (accept)
    {
        ...
        {
            TC_LOG_ERROR("network.opcode", "HandleGroupAcceptOpcode: player %s(%d) tried to accept an invite to his own group", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
            return;
        }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_UNINVITE_GUID");
    ...
    {
        TC_LOG_ERROR("network.opcode", "WorldSession::HandleGroupUninviteGuidOpcode: leader %s(%d) tried to uninvite himself from the group.",
    ...
    {
        if (res == ERR_PARTY_LFG_BOOT_COOLDOWN_S)
        {
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_UNINVITE");
    ...
    {
        TC_LOG_ERROR("network.opcode", "WorldSession::HandleGroupUninviteOpcode: leader %s(%d) tried to uninvite himself from the group.",
            GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
        return;
    }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_SET_LEADER");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_SET_ROLES");
    ...
    Player *target = ObjectAccessor::FindPlayer(guid2);
    if (!target)
        return;
    int32 role = 0;
    if (GetPlayer()->GetGroup())
    {
        role = GetPlayer()->GetGroup()->GetRaidRoles(guid2);
        GetPlayer()->GetGroup()->SetRaidRoles(guid2, newRole);
        uint8 tmpRole = newRole - 19314926400;
        GetPlayer()->GetGroup()->SetLfgRoles(guid2, tmpRole);
...
void WorldSession::HandleRollPollBegin(WorldPacket& recvData)
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_ROLL_POLL_BEGIN");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_DISBAND");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_LOOT_METHOD");
    ...
    if (auto player = GetPlayer())
        ...
        {
            player->SendEquipError(EQUIP_ERR_CANT_EQUIP_EVER, NULL, NULL);

            if (auto g = player->GetGroup())
                g->RemoveMember(player->GetGUID(), GROUP_REMOVEMETHOD_LEAVE, NULL, "GameMaster anti-abuse detection");
            return;
        }
    ...
    if (auto thismaster = ObjectAccessor::GetUnit(*GetPlayer(), lootMaster))
        if (auto newMaster = thismaster->ToPlayer())
        if (newMaster->isGameMaster())
        {
            GetPlayer()->SendEquipError(EQUIP_ERR_CANT_EQUIP_EVER, NULL, NULL);

            if (auto g = newMaster->GetGroup())
                g->RemoveMember(newMaster->GetGUID(), GROUP_REMOVEMETHOD_LEAVE, NULL, "GameMaster anti-abuse detection");
            return;
        }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received MSG_MINIMAP_PING");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received MSG_RANDOM_ROLL");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received MSG_RAID_TARGET_UPDATE");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_RAID_CONVERT");
    ...
    if (auto s = _player->GetSession())
    {
                ...
                if (auto group = _player->GetGroup())
                {
                    ...
                    {
                        s->SendNotification("|cffff0000Cannot convert party to raid while any party member is in active challenge mode!|r");
                        return;
                    }
                }
    }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_CHANGE_SUB_GROUP");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_SWAP_SUB_GROUP");
    std::string unk1;
    std::string unk2;
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_GROUP_ASSISTANT_LEADER");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_SET_EVERYONE_IS_ASSISTANT ");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received MSG_PARTY_ASSIGNMENT");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received MSG_RAID_READY_CHECK");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_REQUEST_PARTY_MEMBER_STATS");
    uint64 guid;
    recvData >> guid;
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_OPT_OUT_OF_LOOT");
    ...
    {
        if (passOnLoot)
            TC_LOG_ERROR("network.opcode", "CMSG_OPT_OUT_OF_LOOT value<>0 for not-loaded character!");
        return;
    }
...
void WorldSession::HandleClearRaidMarker(WorldPacket& recvData)
{
    ...
    Group* group = GetPlayer()->GetGroup();
    TC_LOG_DEBUG("network.opcode", "Group::HandleClearRaidMarker Slots: %u",slots);

    if (!group)
        return;

    if (slots != 5)
        group->RemoveMarker(slots);
    else
        for(int i=0; i <= 5;++i)
