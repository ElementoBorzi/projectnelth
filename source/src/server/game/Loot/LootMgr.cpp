...
{
    for (auto itr=m_LootTemplates.begin(); itr != m_LootTemplates.end(); ++itr)
        delete itr->second;
...
{
    for (auto i = m_LootTemplates.begin(); i != m_LootTemplates.end(); ++i)
...
uint32 LootStore::LoadAndCollectLootIds(LootIdSet& lootIdSet)
{
    ...
    for (auto tab = m_LootTemplates.begin(); tab != m_LootTemplates.end(); ++tab)
...
{
    for (auto ltItr = m_LootTemplates.begin(); ltItr != m_LootTemplates.end(); ++ltItr)
...
bool LootStoreItem::Roll(bool rate, Object* source, Loot* loot) const
{
    bool someone_can_use{ false };


    auto players = loot->GetLooters();
        for (auto i = players->begin(); i != players->end(); ++i)
            if (auto guid = *i)
                if (auto player = ObjectAccessor::FindPlayer(guid))
                    if (sConditionMgr->IsObjectMeetToConditions(player, conditions))
                    {
                        someone_can_use = true;
                    }

        if (!someone_can_use)
        {
            //TC_LOG_ERROR("sql.sql", "nobody can use item %u", itemid);
            return false;
        }
        ...
        if (chance >= 100.0f)
        {
            ...
            //TC_LOG_ERROR("sql.sql", "positive roll for item %u", itemid);
            return true;
        }
    ...
    float chance_modified{ chance * qualityModifier };

    if (mincountOrRef < 0)
    {
        chance_modified = (chance * (rate ? sWorld->getRate(RATE_DROP_ITEM_REFERENCED) : 1.0f) * qualityModifier);// reference case
        //TC_LOG_ERROR("sql.sql", "ref %f: chance_modified: %f x %f = %f", -mincountOrRef, chance, qualityModifier, chance_modified);
    }
    //else TC_LOG_ERROR("sql.sql", "item %u: chance_modified: %f x (quality rate)%f = %f", itemid, chance, qualityModifier, chance_modified);


    uint32 chance_uint32{ uint32(chance_modified * 10000) };
    uint32 null_1{ 0 };
    uint32 null_2{ 0 };
    uint32 null_3{ 0 };

    if (needs_quest)//Flood control rate adjustment
    {
        sObjectMgr->GenerateFloodResponseValue(FLOOD_RESPONSE_LOOT_QUEST, source, chance_uint32, null_1, null_2, null_3);
    }
    else
    {
        sObjectMgr->GenerateFloodResponseValue(FLOOD_RESPONSE_LOOT_NONQUEST, source, chance_uint32, null_1, null_2, null_3);
    }

    chance_modified = (float(chance_uint32)/float(10000));

    if (chance_modified >= 100.f)
        return true;

    if (chance_modified < 0.f)
        return false;

    //TC_LOG_ERROR("sql.sql", "single-rolling for item %u, chance: %f", itemid, float(chance_modified));
    return roll_chance_f(float(chance_modified));
}
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        TC_LOG_ERROR("sql.sql", "Table '%s' entry %d item %d: group (%u) must be less %u - skipped", store.GetName(), entry, itemid, group, 1 << 7);
        return false;
    }
...
bool Loot::DropItem(Object* source, LootItem& loot)
{
    if (!source)
        return true;

    if (auto u = source->ToUnit())
        if (auto looting_map = u->GetMap())
            if (auto i = u->GetInstanceScript())
                if (!i->ItemDrop(source, loot))
                    return false;
    return true;
}
...
void Loot::AddItem(LootStoreItem const& item)
{
    ...
    auto minmax_random = urand(item.mincountOrRef, item.maxcount);

        if (item.maxcount <= 1)
            minmax_random = 1;

    /*
        Distribute our minmax range into 
    */
    uint32 bases = uint32(float(minmax_random) / float(proto->GetMaxStackSize()));
    /*
        If our number of bases doesn't exactly line up with the number of stacks possible, we add a base.
    */
    if (minmax_random % proto->GetMaxStackSize() || bases == 0)
        bases += 1;

    //TC_LOG_ERROR("sql.sql", "item %u , bases: %u, minmax_random: %u (%u to %u)", item.itemid, bases, minmax_random, item.mincountOrRef, item.maxcount);
    //What category are our stacks going into.
    ...
    uint32 itr_count{std::min(limit, minmax_random)};

    /*
        We can add multiple items for the amount, or we can add stacks.
    */
    for (uint32 i = 0; i < bases; ++i)
    if (lootItems.size() < limit)
    {
        uint8 count{ 0 };
        count = std::min(minmax_random, proto->GetMaxStackSize());

        LootItem generatedLoot(item);
        generatedLoot.count = count;

        //TC_LOG_ERROR("sql.sql", "item %u , count %u", item.itemid, count);

            if (DropItem(loot_source, generatedLoot))
            {
                minmax_random -= count;
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/)
{
    ...
    this->AddLooter(lootOwner->GetGUID());
    if (group)
    {
        std::list<Player*> players;
        group->GetMembers(players, true);
        group->GetMembers(players, false);

        for (auto gm : players)
            AddLooter(gm->GetGUID());
    }
...
void Loot::clear()
{
    for (auto itr = PlayerQuestItems.begin(); itr != PlayerQuestItems.end(); ++itr)
        delete itr->second;
    PlayerQuestItems.clear();

    for (auto itr = PlayerFFAItems.begin(); itr != PlayerFFAItems.end(); ++itr)
        delete itr->second;
    PlayerFFAItems.clear();

    for (auto itr = PlayerNonQuestNonFFAConditionalItems.begin(); itr != PlayerNonQuestNonFFAConditionalItems.end(); ++itr)
        delete itr->second;
...
void Loot::generateMoneyLoot(uint32 minAmount, uint32 maxAmount, Object* center)
{

    if (maxAmount > 0)
    {
        if (maxAmount > 2 && maxAmount == minAmount)//Always ensure a range of variation exists
            minAmount *= 0.8f;
        ...
        sObjectMgr->GenerateFloodResponseValue(FLOOD_RESPONSE_LOOT_MONEY, center, gold, null_0, null_1, null_2);
...
std::set<uint64> const* Loot::GetLooters()
{ 
    return (&PlayersLooting); 
};

void Loot::AddLooter(uint64 GUID)
{
    //TC_LOG_ERROR("sql.sql", "running add looter %u", GUID);
    if (PlayersLooting.find(GUID) == PlayersLooting.end())
    {
        //TC_LOG_ERROR("sql.sql", "adding looter %u", GUID);
        PlayersLooting.insert(GUID);
    }
    //for (auto loot : PlayersLooting) TC_LOG_ERROR("sql.sql", "%u is a looter", loot);
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot& loot, uint16 lootMode) const
{
    ...
    {
        std::vector<LootStoreItem*> temp(possibleLoot.begin(), possibleLoot.end());

            //TC_LOG_ERROR("sql.sql", "temp is %u big, possibleLoot is %u big", temp.size(), possibleLoot.size());

        std::random_shuffle(temp.begin(), temp.end());
        LootStoreItemList possibleLoot_shuffled;
            ...
            if (players->size())
                for (auto i = players->begin(); i != players->end(); ++i)
                    if (!someone_can_use)
                        if (auto guid = *i)
                            if (auto player = ObjectAccessor::FindPlayer(guid))
                                if (sConditionMgr->IsObjectMeetToConditions(player, itr->conditions))
                                {
                                    possibleLoot_shuffled.push_back(itr);
                                    //TC_LOG_ERROR("sql.sql", "adding item %u to list, list is now %u long", itr->itemid, possibleLoot_shuffled.size());
                                }
        }
        ...
        /*
            Count up the number 
        */
        float zero_denominator { 0.f };
        float roll_cap{ 0.0f };
        for (auto itr : possibleLoot_shuffled)
            if (itr->chance == 0.f)
                zero_denominator += 1.f;

        /*
            All the non-zero values get chance priority. ex: kelp has a 5% drop chance, 
            but the fish will have a 0% chance in db, should actually be a 95% chance.
        */
            for (auto itr : possibleLoot_shuffled)
            if (itr->chance != 0.f)
            {
                    roll_cap += fabs(itr->chance);
                    itr->weighted_calc = roll_cap;
                    //TC_LOG_ERROR("sql.sql", "adding %u at %f roll level", itr->itemid, roll_cap);
            }

        /*
            Items with zero chance are distributed into the remaining odds beneath zero percent.
        */
        if (zero_denominator >= 1.f)
        if (roll_cap < 100.f)
        {
            float value_to_distribute{ 100.f - roll_cap };

            for (auto itr : possibleLoot_shuffled)
                if (itr->chance == 0.f)
                {
                    roll_cap += (value_to_distribute/zero_denominator);
                    itr->weighted_calc = roll_cap;
                    //TC_LOG_ERROR("sql.sql", "assigning %f chance to zero-chance loot item: %u", itr->itemid);
                }
        }

        float roll = frand(0.f, std::max(roll_cap, 100.f));

        for (auto itr = possibleLoot_shuffled.begin(); itr != possibleLoot_shuffled.end(); ++itr)   // check each explicitly chanced entry in the template and modify its chance based on quality.
            if (LootStoreItem* item = *itr)
            {
                if (roll <= item->weighted_calc)
                {
                    //TC_LOG_ERROR("sql.sql", "debug f1: %u at %f weight of %f weight, returning item.", item->itemid, item->weighted_calc, roll);
                    return item;
                }
                //else TC_LOG_ERROR("sql.sql", "debug f1: %u at %f weight of %f weight, insufficient", item->itemid, item->weighted_calc, roll);
            }
    }
...
{
    for (auto i = ExplicitlyChanced.begin(); i != ExplicitlyChanced.end(); ++i)
        if ((*i)->needs_quest)
            return true;

    for (auto i = EqualChanced.begin(); i != EqualChanced.end(); ++i)
        if ((*i)->needs_quest)
            return true;
...
{
    for (auto ieItr = ExplicitlyChanced.begin(); ieItr != ExplicitlyChanced.end(); ++ieItr)
    {
    ...
    for (auto ieItr = EqualChanced.begin(); ieItr != EqualChanced.end(); ++ieItr)
    {
        LootStoreItem* item = *ieItr;
        if (item->mincountOrRef < 0)
        {
...
void LootTemplate::CopyConditions(LootItem* li) const
{
    ...
    for (auto _iter = Entries.begin(); _iter != Entries.end(); ++_iter)
    {
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId) const
{
    ...
    if (groupId)                                            // Group reference uses own processing of the group
    {
        ...
        {
            //TC_LOG_ERROR("sql.sql", "group %u not processing because it's bigger than group size %u.", groupId, Groups.size());
        ...
        //TC_LOG_ERROR("sql.sql", "processing (group %u)", groupId);
    ...
    for (auto i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (!item->Roll(rate, loot.loot_source, &loot))
        {
            //TC_LOG_ERROR("sql.sql", "bad roll for item %u", item->itemid);

            continue;                                           // Bad luck for the entry
        }
        //TC_LOG_ERROR("sql.sql", "good roll for item %u", item->itemid);
        ...
        if (item->mincountOrRef < 0)                            // References processing
        {
            ...
            if (!Referenced)
            {
                //TC_LOG_ERROR("sql.sql", "could not find referenced %f",  float(-item->mincountOrRef));
                continue;                                       // Error message already printed at loading stage
            }

            uint32 maxcount = uint32(float(item->maxcount) * sWorld->getRate(RATE_DROP_ITEM_REFERENCED_AMOUNT));
            for (uint32 loop = 0; loop < maxcount; ++loop)      // Ref multiplicator
                Referenced->Process(loot, rate, lootMode);
        }
...
bool LootTemplate::HasQuestDrop(LootTemplateMap const& store, uint8 groupId) const
{
    ...
    for (auto i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        {
            auto Referenced = store.find(-item->mincountOrRef);
...
{
    for (auto ieItr = Entries.begin(); ieItr != Entries.end(); ++ieItr)
    {
        LootStoreItem* item = *ieItr;
        if (item->mincountOrRef < 0)
        {
