...
void Guild::SaveProfession(Player* player, SQLTransaction& trans)
{
    if (!player)
        return;

    if (Member* member = GetMember(player->GetGUID()))
        if (member->IsProfessionChanged())
        {
            member->SaveProfession(trans);
            UpdateMemberData(player, GUILD_MEMBER_DATA_PROFESSIONS, 0);
            member->SetProfessionChanged(false);
        }
}
...
void Guild::Member::ResetValues(bool weekly /* = false*/)
{
    ...
    for (uint8 tabId = 0; tabId <= GUILD_BANK_MAX_TABS; ++tabId)
    {
        ...
        UpdateBankWithdrawValue(trans, tabId, 0);
    }
...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    stmt->setUInt64(++index, m_cashFlowContribution);
    trans->Append(stmt);
...
void Guild::SaveToDB(SQLTransaction& trans)
{
    if (!_needsSave)
        return;
...
void Guild::UpdateMemberData(Player* player, uint8 dataid, uint32 value)
{
    if (Member* member = GetMember(player->GetGUID()))
    {
        switch (dataid)
        {
            ...
            {
                bool hide_zone_update{ false };

                if (player)
                    if (auto s = player->GetSession())
                        if (s->HasAccessFlag(ACCOUNT_FLAG_HEAD_GAMEMASTER))
                            if (player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
                                hide_zone_update = true;
            ...
            case GUILD_MEMBER_DATA_PROFESSIONS:
            {
                ...
                for (auto spellIter = player->GetSpellMap().begin(); spellIter != player->GetSpellMap().end(); ++spellIter)
                {
                    if (count_prof >= 2)
                        break;
...
void Guild::SetProfessionChanged(uint64 playerGuid, bool value)
{
    if (Member* member = GetMember(playerGuid))
        member->SetProfessionChanged(value);
}
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    for (auto itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        bool hide_online_from_roster{ false };
        if (player)
        {

            if (auto player_requesting_roster = session->GetPlayer())
                if (auto roster_member_session = player->GetSession())
                    if (!AccountMgr::IsPlayerAccount(roster_member_session->GetSecurity()))
                        if (!AccountMgr::IsPlayerAccount(session->GetSecurity()))
                        {
                            /*
                                We're dealing with a staff member looking at the staff guild roster, currently iterating through another staff member on the guild roster.
                            */
                            if (roster_member_session->HasAccessFlag(ACCOUNT_FLAG_HEAD_GAMEMASTER))
                                if (player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
                                    hide_online_from_roster = true;
                        }


            if (!hide_online_from_roster)
            {
...
void Guild::SendGuildRankInfo(WorldSession* session) const
{
    ...
    for (uint8 i = 0; i < _GetRanksSize(); i++)
    {
        ...
        rankData << uint32(i == 0 ? uint32(m_bankMoney / 1000) : rankInfo->GetBankMoneyPerDayInGold());
...
void Guild::HandleSetAchievementTracking(WorldSession* session, std::set<uint32> const& achievementIds)
{
    ...
    if (Member* member = GetMember(player->GetGUID()))
    {
        ...
        for (auto achievementId = achievementIds.begin(); achievementId != achievementIds.end(); ++achievementId)
        {
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && pInvitee->GetOTeam() != player->GetOTeam())
    {
...
void Guild::HandleAcceptMember(WorldSession* session)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        player->GetOTeam() != sObjectMgr->GetPlayerTeamByGUID(GetLeaderGUID()))
        return;
...
{
    uint8 rankSize = _GetRanksSize();

    // Cannot remove rank if total count is minimum allowed by the client or is not leader
    if (rankSize <= GUILD_RANKS_MIN_COUNT || rankId >= rankSize || !_IsLeader(session->GetPlayer()))
        return;
    ...
    CharacterDatabase.Execute(stmt);
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_RIGHTS_FOR_RANK_UPD);
    stmt->setUInt32(0, m_id);
    stmt->setUInt8(1, rankId);
    CharacterDatabase.Execute(stmt);
    // Delete rank
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_RANK_UPD);
    stmt->setUInt32(0, m_id);
    stmt->setUInt8(1, rankId);
    CharacterDatabase.Execute(stmt);

    _BroadcastEvent(GE_RANK_DELETED, rankId);
    uint8 oldRank = rankId;
    while (rankId + 1 < rankSize && rankId + 1 < GUILD_RANKS_MAX_COUNT)
    {
        RankInfo info(m_id, rankId, m_ranks[rankId + 1].GetName(), m_ranks[rankId + 1].GetRights(), m_ranks[rankId + 1].GetBankMoneyPerDayInGold());
        m_ranks[rankId] = info;
        rankId++;
    }
    m_ranks.resize(rankSize - 1);
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint64 amount, bool cashFlow /*=false*/)
{
    ...
    if (cashFlow)
    {
        m_cashFlowContribution += amount;
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_CASHFLOW);
        stmt->setUInt64(0, m_cashFlowContribution);
        stmt->setUInt32(1, m_id);
        trans->Append(stmt);
        CharacterDatabase.CommitTransaction(trans);
        return;
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint64 amount, bool repair)
{
   ...
   if (uint64(_GetMemberRemainingMoney(member)) < amount)   // Check if we have enough slot/money today
    ...
    if (!repair)
    {
        if (!player->ModifyMoney((int64)amount))
            return false;
...
void Guild::HandleMemberLogout(WorldSession* session)
{
    ...
    if (Member* member = GetMember(player->GetGUID()))
    {
        ...
        if (session->HasAccessFlag(ACCOUNT_FLAG_HEAD_GAMEMASTER))
            if (player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
                hide_logout = true;
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        data << uint32(tabId);
        if (hasCashFlow)
            data << uint64(m_cashFlowContribution);
        session->SendPacket(&data);
...
bool Guild::LoadFromDB(Field* fields)
{
    ...
    m_cashFlowContribution = fields[19].GetUInt64();
...
void Guild::BroadcastAddonToGuild(WorldSession* session, bool officerOnly, std::string const& msg, std::string const& prefix) const
{
    ...
    {
        WorldPacket data;
        ChatHandler::FillMessageData(&data, session, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD, uint32(LANG_ADDON), NULL, 0, msg.c_str(), NULL, prefix.c_str());
...
bool Guild::AddMember(uint64 guid, uint8 rankId)
{
    ...
    Member* member = new Member(m_id, guid, rankId, true);
    std::string name;
...
{
    auto itr = m_members.find(GUID_LOPART(guid));
...
void Guild::_SwitchRank(uint8 rankId, bool up)
{
    ...
    RankInfo oldRank(m_id, newRankId, m_ranks[rankId].GetName(), m_ranks[rankId].GetRights(), m_ranks[rankId].GetBankMoneyPerDayInGold());
    RankInfo newRank(m_id, rankId, m_ranks[newRankId].GetName(), m_ranks[newRankId].GetRights(), m_ranks[newRankId].GetBankMoneyPerDayInGold());
    m_ranks[rankId] = newRank;
    m_ranks[newRankId] = oldRank;
    ...
    CharacterDatabase.PExecute("REPLACE INTO guild_rank (guildid,rid,rname,rights,BankMoneyPerDay) "
        "VALUES ('%u', '%u', '%s', '%u', '%u')", m_id, rankId, my_escape_string(m_ranks[rankId].GetName()).c_str(),m_ranks[rankId].GetRights(),m_ranks[rankId].GetBankMoneyPerDay());
    CharacterDatabase.PExecute("REPLACE INTO guild_rank (guildid,rid,rname,rights,BankMoneyPerDay) "
        "VALUES ('%u', '%u', '%s', '%u', '%u')", m_id, newRankId, my_escape_string(m_ranks[newRankId].GetName()).c_str(),m_ranks[newRankId].GetRights(),m_ranks[newRankId].GetBankMoneyPerDay());
...
void Guild::SendGuildRanksUpdate(uint64 setterGuid, uint64 targetGuid, uint32 rank)
{
    ...
    TC_LOG_DEBUG("network.opcode", "SMSG_GUILD_RANKS_UPDATE [Broadcast] Target: %u, Issuer: %u, RankId: %u",
...
void Guild::ResetTimes(bool weekly)
{
    ...
    if (weekly)
    {
        ...
        SQLTransaction trans = CharacterDatabase.BeginTransaction();
        _ModifyBankMoney(trans, m_cashFlowContribution, true);
        _LogBankEvent(trans, GUILD_BANK_LOG_CASH_FLOW_DEPOSIT, uint8(0), 0, m_cashFlowContribution);
        m_cashFlowContribution = 0;
        PreparedStatement* stmt2 = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_CASHFLOW);
        stmt2->setUInt64(0, m_cashFlowContribution);
        stmt2->setUInt32(1, m_id);
        trans->Append(stmt2);
        CharacterDatabase.CommitTransaction(trans);
    }
...
void Guild::UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1, uint64 miscValue2, uint64 miscValue3, Unit* unit, Player* player, GameObject* go /*= NULL*/)
{
    _needsSave = true;
    m_achievementMgr.UpdateAchievementCriteria(type, miscValue1, miscValue2, miscValue3, unit, player, go);
}

void Guild::CompletedCriteria(AchievementCriteriaEntry const* entry, Player* referencePlayer)
{
    _needsSave = true;
    m_achievementMgr.CompletedCriteria(entry, referencePlayer);
}
...
void Guild::ReplaceGuildLeader(Player* newleader)
{
    Member* newLead = GetMember(newleader->GetGUID());
    Member* oldLead = GetMember(m_leaderGuid);

    // leader has to be offline for 30 days
    if (oldLead && (oldLead->IsOnline() || ((time(NULL) - oldLead->GetLogoutTime()) / DAY) < 30))
        return;

    if (newLead->GetRankId() < GR_MEMBER) // i think this is custom because the frame is shown on every rank... but maybe better at our players XD
        return;
    ...
    if (newLead && oldLead)
    {
        ...
        _BroadcastEvent(GE_GM_REPLACED, 0, oldLead->GetName().c_str(), newLead->GetName().c_str());
    }
}

void Guild::UpdateName(std::string const& name)
{
    SetName(name);
