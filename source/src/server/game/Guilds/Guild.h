...
private:
    ...
    public:
        Member(uint32 guildId, uint64 guid, uint8 rankId, bool professionChanged = false) :
            m_guildId(guildId),
            m_guid(guid),
            m_zoneId(0),
            m_level(0),
            ...
            m_totalReputation(0),
            m_weekReputation(0),
            m_professionChanged(professionChanged)
        {
        ...
        void SetProfessionChanged(bool value) { m_professionChanged = value; }
        bool IsProfessionChanged() const { return m_professionChanged; }
    ...
    public:
        ...
        int32 GetBankMoneyPerDay() const { return m_bankMoneyPerDay*10000; } // Should be in gold
        int32 GetBankMoneyPerDayInGold() const { return m_bankMoneyPerDay; }
...
public:
    ...
    void ReplaceGuildLeader(Player* newleader);
    ...
    /// if validate returns false, the guild must be disbanded
    bool Validate();
    ...
    void BroadcastEvent(GuildEvents guildEvent, uint64 guid, const char* param1 = NULL, const char* param2 = NULL, const char* param3 = NULL) const
    {
        _BroadcastEvent(guildEvent, guid, param1,param2, param3);
    };
    ...
    void SetProfessionChanged(uint64 playerGuid, bool value);
    ...
    void UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1, uint64 miscValue2, uint64 miscValue3, Unit* unit, Player* player, GameObject* go = NULL);
    void CompletedCriteria(AchievementCriteriaEntry const* entry, Player* referencePlayer);

    void SaveProfession(Player* player, SQLTransaction& trans);
    ...
    {
        if (auto rankInfo = GetRankInfo(rankId))
