...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password)
{
    ...
    uint32 lower_bound = (urand(0, 49) * 10000);
    uint32 upper_bound = lower_bound + 10000;
    stmt->setUInt32(0, lower_bound);
    stmt->setUInt32(1, upper_bound);

    stmt->setString(2, username);
    stmt->setString(3, CalculateShaPassHash(username, password));
...
bool AccountMgr::IsHeadGMAccount(uint32 gmlevel)
{
    return gmlevel >= SEC_HEAD_GM && gmlevel <= SEC_CONSOLE;
}
