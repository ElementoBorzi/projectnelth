...
Battlefield::Battlefield()
{
    ...
    m_LastQueueTimer = 30 * IN_MILLISECONDS;
    m_StartGroupingTimer = 0;
    m_StartGrouping = false;
    StalkerGuid = 0;

    KickPositionA.Relocate(-8827.364f, 626.74f, 94.0f, 3.83f);
    KickPositionA.m_mapId        = 0;
    KickPositionH.Relocate(1569.486f, -4397.64f, 16.2f, 0.44f);
    KickPositionH.m_mapId        = 1;
}
...
void Battlefield::HandlePlayerEnterZone(Player* player, uint32 /*zone*/)
{
    ...
    if (IsWarTime())
    {
        if (!m_PlayersInWar[player->GetTeamId()].count(player->GetGUID())) // player is already in war ?
        {
            if ((m_TypeId != BATTLEFIELD_TB && m_PlayersInWar[player->GetTeamId()].size() + m_InvitedPlayers[player->GetTeamId()].size() < m_MaxPlayer) ||
                (m_TypeId == BATTLEFIELD_TB && m_PlayersInWar[player->GetTeamId()].size() + m_InvitedPlayers[player->GetTeamId()].size() < m_MaxPlayer &&
                 m_PlayersInWar[player->GetTeamId()].size() +  m_InvitedPlayers[player->GetTeamId()].size() <
                 m_PlayersInWar[player->GetOppositeTeamId()].size() + m_InvitedPlayers[player->GetOppositeTeamId()].size() + 1)) // Vacant spaces
                InvitePlayerToWar(player);
            else
            {
                /// @todo Send a packet to announce it to player
                m_PlayersWillBeKick[player->GetTeamId()][player->GetGUID()] = time(NULL) + 30;
...
bool Battlefield::Update(uint32 diff)
{
    ...
    if (IsWarTime())
    {
        if (m_LastQueueTimer <= diff)
        {
            InvitePlayersInQueueToWar();
            m_LastQueueTimer = 5000;
        }
        else
            m_LastQueueTimer -= diff;
        ...
        if (m_uiKickDontAcceptTimer <= diff) // need to be rewritten in a more cleanup way.
        {
            ...
            for (int team = 0; team < 2; team++)
            {
                ...
                {
                    player->GetSession()->SendBfLeaveMessage(m_Guid);
                    m_InvitedPlayers[team].erase(player->GetGUID());
                }
            }
            ...
            for (int team = 0; team < 2; team++)
            {
                ...
                for (PlayerTimerMap::iterator itr = m_PlayersWillBeKick[team].begin(); itr != m_PlayersWillBeKick[team].end(); ++itr)
                    if (itr->second <= now)
                    {
                        if (m_TypeId == BATTLEFIELD_TB)
                            TeleportNonValidPlayers();
                        else
                            KickPlayerFromBattlefield(itr->first);
                        break;
                    }
                if (player)
                    m_PlayersWillBeKick[team].erase(player->GetGUID());
            }
            m_uiKickDontAcceptTimer = 1000;
        }
        else
...
bool Battlefield::IsPlayerInQueue(Player *player)
{
    return m_PlayersInQueue[player->GetTeamId()].find(player->GetGUID()) != m_PlayersInQueue[player->GetTeamId()].end();
}

bool Battlefield::IsPlayerInWar(Player *player)
{
    return m_PlayersInWar[player->GetTeamId()].find(player->GetGUID()) != m_PlayersInWar[player->GetTeamId()].end();
}
...
void Battlefield::TeleportNonValidPlayers()
{
    for (uint8 team = 0; team < 2; ++team)
        for (GuidSet::const_iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
            if (auto player = sObjectAccessor->FindPlayer(*itr))
                if (!IsPlayerInWar(player))
                    player->TeleportTo(player->GetMapId(), -616.94f, 1183.27f, 95.71f, player->GetOrientation());
}
            ...
            if (auto player = sObjectAccessor->FindPlayer(*itr))
            {
                if ((m_TypeId != BATTLEFIELD_TB && m_PlayersInWar[player->GetTeamId()].size() + m_InvitedPlayers[player->GetTeamId()].size() < m_MaxPlayer) ||
                    ...
                    if (InvitePlayerToWar(player))
                        cleanupPl.push_back(player->GetGUID());
            }
        }

        for (std::list<uint64 >::iterator itr = cleanupPl.begin(); itr != cleanupPl.end(); ++itr)
            if (m_PlayersInQueue[team].count(*itr))
            ...
            if (auto player = sObjectAccessor->FindPlayer(*itr))
            {
                ...
                if ((m_TypeId != BATTLEFIELD_TB && m_PlayersInWar[player->GetTeamId()].size() + m_InvitedPlayers[player->GetTeamId()].size() < m_MaxPlayer) ||
                    (m_TypeId == BATTLEFIELD_TB && m_PlayersInWar[player->GetTeamId()].size() + m_InvitedPlayers[player->GetTeamId()].size() < m_MaxPlayer &&
                     ...
                     m_PlayersInWar[player->GetOppositeTeamId()].size() + m_InvitedPlayers[player->GetOppositeTeamId()].size() + 1)) // Vacant spaces
                    InvitePlayerToWar(player);
                else if (!m_PlayersWillBeKick[player->GetTeamId()].count(player->GetGUID()))
                    m_PlayersWillBeKick[player->GetTeamId()][player->GetGUID()] = time(NULL) + 30;
            }
        }
}
...
bool Battlefield::InvitePlayerToWar(Player* player)
{
    ...
    player->GetSession()->SendBfLeaveMessage(m_Guid, BF_LEAVE_REASON_UNK1);
    m_PlayersWillBeKick[player->GetTeamId()].erase(player->GetGUID());
...
{
    if (auto creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
    ...
    if (auto player = sObjectAccessor->FindPlayer(guid))
        if (player->GetZoneId() == GetZoneId())
        {
            if (player->GetTeamId() == TEAM_HORDE)
                player->TeleportTo(KickPositionH);
            else
                player->TeleportTo(KickPositionA);
        }
}
...
{
    player->GetSession()->SendBfLeaveMessage(m_Guid, BF_LEAVE_REASON_UNK2); /* FIX ME message isn't supposed to be send ???*/
...
// Called in WorldSession::HandleBfExitRequest
void Battlefield::AskToLeaveBattle(Player* player)
{
...
{
    if (auto stalker = GetCreature(StalkerGuid))
...
// ******************* Group System *******************
// ****************************************************
void Battlefield::DisbandBFGroup(TeamId TeamId)
{
...
void BfGraveyard::Resurrect()
{
    ...
    for (GuidSet::const_iterator itr = m_ResurrectQueue.begin(); itr != m_ResurrectQueue.end(); ++itr)
    {
        ...
        if (player->IsInWorld())
            if (auto spirit = m_Bf->GetCreature(m_SpiritGuide[m_ControlTeam]))
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o, uint32 mapID = 571)
{
    // Get map object
    Map* map = sMapMgr->CreateBaseMap(mapID); // *vomits*
    if (!map)
        return 0;
    ...
    if (gInfo->type == GAMEOBJECT_TYPE_TRANSPORT)
    {
        ...
        if (!go->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_TRANSPORT), entry, map, PHASEMASK_NORMAL, x, y, z, o, 0, 0, 0, 0, 100, GO_STATE_READY))
        {
...
bool BfCapturePoint::HandlePlayerEnter(Player* player)
{
    ...
    {
        if (auto capturePoint = m_Bf->GetGameObject(m_capturePointGUID))
        {
...
void BfCapturePoint::ResetCapturePointData(GameObject* capturePoint, TeamId team)
{
    ASSERT(capturePoint);

    TC_LOG_DEBUG("bg.battlefield", "Reset capture point %u", capturePoint->GetEntry());
...
{
    TC_LOG_DEBUG("bg.battlefield", "Creating capture point %u", entry);

    // check info existence
    GameObjectTemplate const* goinfo = sObjectMgr->GetGameObjectTemplate(entry);
    if (!goinfo || goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT)
    {
        TC_LOG_ERROR("bg.battlefield", "OutdoorPvP: GO %u is not capture point!", entry);
        return false;
    }
    GameObject *go = m_Bf->SpawnGameObject(entry, x, y, z, o, map);
    return SetCapturePointData(go);
}
...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    if (auto capturePoint = m_Bf->GetGameObject(m_capturePointGUID))
    {
        ...
        for (uint8 team = 0; team < 2; ++team)
        {
            ...
            {
                if (auto player = ObjectAccessor::GetPlayer(*capturePoint, *itr))
                {
