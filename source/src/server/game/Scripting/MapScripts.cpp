...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, const ScriptInfo* scriptInfo) const
{
    ...
    else
    {
        ...
        else
        {
            ...
            else if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR && pDoor->GetGoType() != GAMEOBJECT_TYPE_TRAPDOOR)
                TC_LOG_ERROR("scripts", "%s gameobject is not a door or a trap door (GoType: %u, Entry: %u, GUID: %u).",
