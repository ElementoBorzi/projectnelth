...
WorldSession::WorldSession(uint32 id, WorldSocket* sock, AccountTypes sec, uint8 expansion, time_t mute_time, uint8 mute_type, LocaleConstant locale, uint32 recruiter, bool isARecruiter, uint32 account_flags):
    m_muteTime(mute_time),
    m_muteType(mute_type),
    m_timeOutTime(0),
    _player(NULL),
    m_Socket(sock),
    _security(sec),
    ...
    forceExit(false)
{
    if (sock)
    {
        ...
        if (auto stmt =  LoginDatabase.GetPreparedStatement(LOGIN_INS_IP_MASKING))
        {
            stmt->setString(0, m_Address.c_str());
            LoginDatabase.Execute(stmt);
        }

        if (auto stmt2 = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_MASKING_MASK_FROM_IP))
        {
            stmt2->setString(0, sock->GetRemoteAddress());
            if (auto results = LoginDatabase.Query(stmt2))
                if (auto fields = results->Fetch())
                    m_Address_Masked = fields[0].GetUInt32();
        }
        if (auto stmt3 = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_PREMIUM))
        {
            stmt3->setUInt32(0, GetAccountId());
            stmt3->setUInt32(1, sWorld->getRealmID());
            if (auto results = LoginDatabase.Query(stmt3))
                    do
                    {
                        if (auto fields = results->Fetch())
                        {
                            auto perk_id{ fields[2].GetUInt8() };
                            auto misc_value_1{ fields[3].GetFloat() };
                            auto misc_value_2{ fields[4].GetFloat() };
                            auto misc_value_3{ fields[5].GetFloat() };

                            //TC_LOG_ERROR("sql.sql", "adding perk %u, %f, %f, %f", perk_id, misc_value_1, misc_value_2, misc_value_3);
                            ...
                            */
                            ...
                            premium_AddPerk(perk_id, this_perk);
                        }
                        //else TC_LOG_ERROR("sql.sql", "fields do not exist");

                    } while (results->NextRow());
            //else TC_LOG_ERROR("sql.sql", "account perk results not found.");

        }
       // else TC_LOG_ERROR("sql.sql", "perks statement not found.");
    }
...
void WorldSession::QueuePacket(WorldPacket* new_packet)
{
    ...
    if (m_packetThrottler.MustDiscard(new_packet->GetOpcode(), GetAccountId(), GetRemoteAddress()))
    {
        ...
        const char* codename{ "OPCODE_UNDEFINED" };

        auto opcode = new_packet->GetOpcode();
        
            std::string pre_codename = GetOpcodeNameForLogging(opcode);
                codename = pre_codename.c_str();
        
        std::ostringstream message{ "" };
        auto playername{ "LOGINSCREEN" };

        if (auto p = GetPlayer())
            playername = p->GetName().c_str();
        message << playername << ": " << codename;
        AnnounceToSniffers(opcode, message.str());
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    {
        if (p->needsGMFreeze())
            p->GetPhaseMgr().SetCustomPhase(p->m_pre_gmFreeze_PhaseMask ? p->m_pre_gmFreeze_PhaseMask : 1);
        ...
        if (p->GetDeathTimer())
        {
            p->getHostileRefManager().deleteReferences();
            p->BuildPlayerRepop();
            p->RepopAtGraveyard();
        }
        else if (p->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION))
        {
        ...
        else if (p->HasPendingBind())
        {
            p->RepopAtGraveyard();
            p->SetPendingBind(0, 0);
        }
        ...
        if (!p->m_InstanceValid && !p->isGameMaster())
            p->TeleportTo(p->m_homebindMapId, p->m_homebindX, p->m_homebindY, p->m_homebindZ, p->GetOrientation());

        sOutdoorPvPMgr->HandlePlayerLeaveZone(p, p->GetZoneId());

        for (int i=0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
        {
            if (BattlegroundQueueTypeId bgQueueTypeId = p->GetBattlegroundQueueTypeId(i))
            {
                p->RemoveBattlegroundQueueId(bgQueueTypeId);
                BattlegroundQueue& queue = sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId);
                queue.RemovePlayer(p->GetGUID(), true);
            }
        }

        // Repop at GraveYard or other player far teleport will prevent saving player because of not present map
        // Teleport player immediately for correct player save
        while (p->IsBeingTeleportedFar())
            HandleMoveWorldportAckOpcode();
        ...
        ///- Remove pet
        p->RemovePet(NULL, PET_SLOT_ACTUAL_PET_SLOT, true, true);
        ...
        if (Save)
        {
            ...
            for (int j = BUYBACK_SLOT_START; j < BUYBACK_SLOT_END; ++j)
            {
                ...
                p->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOT_1 + (eslot * 2), 0);
                p->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE_1 + eslot, 0);
                p->SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP_1 + eslot, 0);
            }
            p->SaveToDB();
        }
        ...
        {
            g->SendUpdate();
            g->ResetMaxEnchantingLevel();
            g->LogoutPlayerOutOfRange(p);
        }

        //! Broadcast a logout message to the player's friends
        if (p->isGMVisible())
            if (!HasAccessFlag(ACCOUNT_FLAG_HEAD_GAMEMASTER) && !p->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
        sSocialMgr->SendFriendStatus(p, FRIEND_OFFLINE, p->GetGUIDLow(), true);
        sSocialMgr->RemovePlayerSocial(p->GetGUIDLow());

        //! Call player save scripts before logout
        sScriptMgr->OnPlayerSave(p);

        //! Call script hook before deletion
        sScriptMgr->OnPlayerLogout(p);
        ...
        TC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Logout Character:[%s] (GUID: %u) Level: %d",
            GetAccountId(), GetRemoteAddress().c_str(), p->GetName().c_str(), p->GetGUIDLow(), p->getLevel());
        if (auto _map = p->FindMap())
            _map->RemovePlayerFromMap(p, true);
...
bool WorldSession::AntiCheatNotificationRequest()
{
    if ((last_anticheat_time + 15) > time(NULL))
        return false;

    last_anticheat_time = time(NULL);
    return true;
}
...
void WorldSession::AddSniffer(uint32 sniffer_account) 
{
    if (sniffer_account)
        if (m_sniffers.find(sniffer_account) == m_sniffers.end())
        {
            //TC_LOG_ERROR("sql.sql", "inserting account %u into sniffers of account %u", sniffer_account, GetAccountId());
            m_sniffers.insert(sniffer_account);
        }
       // else TC_LOG_ERROR("sql.sql", "m_sniffer size: %u", m_sniffers.size());
    //else TC_LOG_ERROR("sql.sql", "sniffer_account = %u", sniffer_account);

    //TC_LOG_ERROR("sql.sql", "m_sniffer size: %u", m_sniffers.size());

};

void WorldSession::RemoveSniffer(uint32 sniffer_account)
{
    if (m_sniffers.find(sniffer_account) != m_sniffers.end())
        m_sniffers.erase(sniffer_account);
};

void WorldSession::StartSniffing(uint32 sniffed_account)
{
    if (sniffed_account)
        if (auto session = sWorld->FindSession(sniffed_account))
            {
                session->AddSniffer(GetAccountId());
                SetSniffing(sniffed_account);
            }
};

void WorldSession::StopSniffing()
{
    if (auto sniffed_account = GetSniffing())
            if (auto session = sWorld->FindSession(sniffed_account))
                session->RemoveSniffer(GetAccountId());

    SetSniffing(0);
}

void WorldSession::StopBeingSniffed()
{
    if (m_sniffers.size())
    if (auto this_player = GetPlayer())
        for (uint32 sniffer_account : m_sniffers)
            if (auto session = sWorld->FindSession(sniffer_account))
            {
                if (auto gm = session->GetPlayer())
                    gm->SendSniffReport("You have been disconnected from your current sniff target.");
                session->SetSniffing(0);
            }
}

void WorldSession::CheckAndClearSniff()
{
    if (auto sniffed_account = GetSniffing())
        if (auto session = sWorld->FindSession(sniffed_account))
        {
            //do nothing, sniff target is still present
        }
        else
            StopSniffing();
}

void WorldSession::CheckAndClearSniffers()
{
    if (m_sniffers.size())
        for (uint32 sniffer_account : m_sniffers)
            if (auto session = sWorld->FindSession(sniffer_account))
            {
                //do nothing, sniffer is still present
            }
            else m_sniffers.erase(sniffer_account);
}

void WorldSession::AnnounceToSniffers(Opcodes opcode, std::string const sniff_announcement)
{

    /*
        We're blocking the logs of movement opcodes to reduce spam.
    */
    switch (opcode)
    {
    ...
    if (m_sniffers.size())
        {
            //if (GetAccountId() == 5) TC_LOG_ERROR("sql.sql", "sniffer count: %u", m_sniffers.size());
            for (uint32 sniffer_account : m_sniffers)
                if (auto session = sWorld->FindSession(sniffer_account))
                    if (session->GetSniffing() == GetAccountId())
                    {
                        if (auto gm = session->GetPlayer())
                        {
                            gm->SendSniffReport(sniff_announcement);
                            //TC_LOG_ERROR("sql.sql", "sending report to %u", gm->GetGUID());
                        }
                    }
                    else
                    {
                        RemoveSniffer(sniffer_account);
                    }

                    //else TC_LOG_ERROR("sql.sql", "could not find player for account", sniffer_account);
        }
}
...
void WorldSession::SetAccountData(AccountDataType type, time_t tm, std::string const& data)
{
    ...
    //TC_LOG_ERROR("sql.sql", "WorldSession::SetAccountData - LoadClientConfigRawValues");
    //LoadClientConfigRawValues();
}
