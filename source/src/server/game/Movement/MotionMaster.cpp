...
void MotionMaster::MoveChase(Unit* target, float dist, float angle, uint32 spellId, MovementSlot slot)
{

    // ignore movement request if target not exist
    if (!target || target == _owner || _owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE) || _owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED))
        return;
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        Mutate(new ChaseMovementGenerator<Player>(target, dist, angle, spellId), slot);
    }
    else
    {
        TC_LOG_DEBUG("misc", "Creature (Entry: %u GUID: %u) chase to %s (GUID: %u)",
            ...
            target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
            target->GetTypeId() == TYPEID_PLAYER ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
        Mutate(new ChaseMovementGenerator<Creature>(target, dist, angle, spellId), slot);
    }
}
...
void MotionMaster::MovePoint(uint32 id, float x, float y, float z, bool generatePath, float speed, bool fixedOrientation)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        Mutate(new PointMovementGenerator<Player>(id, x, y, z, generatePath, speed, fixedOrientation), MOTION_SLOT_ACTIVE);
    }
    else
    {
        TC_LOG_DEBUG("misc", "Creature (Entry: %u GUID: %u) targeted point (ID: %u X: %f Y: %f Z: %f)",
            _owner->GetEntry(), _owner->GetGUIDLow(), id, x, y, z);
        Mutate(new PointMovementGenerator<Creature>(id, x, y, z, generatePath, speed, fixedOrientation), MOTION_SLOT_ACTIVE);
    }
}

void MotionMaster::MoveSmoothPath(G3D::Vector3 const* pathPoints, uint32 pathSize, float speed)
{
...
void MotionMaster::ForceMoveLand(uint32 id, Position const& pos)
{
    float x, y, z;
    pos.GetPosition(x, y, z);
...
void MotionMaster::MoveJumpTo(float angle, float speedXY, float speedZ, bool withCollision)
{
    //this function may make players fall below map
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
    ...
    if (withCollision)
    {
        ...
        _owner->MovePositionToFirstCollision(pos, dist, _owner->GetOrientation() + angle);
    }
    else
        _owner->GetClosePoint(pos.m_positionX, pos.m_positionY, pos.m_positionZ, _owner->GetObjectSize(), dist, angle);

    MoveJump(pos.m_positionX, pos.m_positionY, pos.m_positionZ, speedXY, speedZ);
}

void MotionMaster::MoveJump(float x, float y, float z, float speedXY, float speedZ, uint32 id /*= EVENT_JUMP*/)
{
...
void MotionMaster::ForceMoveJump(float x, float y, float z, float speedXY, float speedZ, uint32 id, Unit const* target)
{
    Clear(false);
    _owner->StopMoving();
    _owner->DisableSpline();
    TC_LOG_DEBUG("misc", "Unit (GUID: %u) jump to point (X: %f Y: %f Z: %f)", _owner->GetGUIDLow(), x, y, z);
...
{
    /*
    Vector3 modified_end{ 0.f, 0.f, 0.f };
    bool limit_by_trap{ false };
    uint32 points{ 0 };
    if (_owner->ToPlayer())
    {

        std::list<GameObject*> objs;
        _owner->GetGameObjectListWithEntryInGrid(objs, 183957, 50.f);
        _owner->GetGameObjectListWithEntryInGrid(objs, 2561, 50.f);
        _owner->GetGameObjectListWithEntryInGrid(objs, 164639, 50.f);

        if (objs.size())
            for (auto itr = objs.begin(); itr != objs.end(); ++itr)
                if (auto rock = (*itr))
                {
                    // if (!_owner->IsImmunedToSpellEffect()
                }


        for (auto iter = path.GetPath().begin(); iter != path.GetPath().end(); ++iter)
        if (!limit_by_trap)
        {
            auto point = *iter;

            TC_LOG_ERROR("sql.sql", "Point iteration, %u of %u", points, path.GetPath().size());
            points++;

                Position this_point{ point.x, point.y, point.z, 0.f };

                if (objs.size())
                    for (auto itr = objs.begin(); itr != objs.end(); ++itr)
                        if (auto trap = (*itr))
                            if (auto goInfo = trap->GetGOInfo())
                                if (auto radius = float((goInfo->trap.radius) / 2))
                                    if (trap->GetDistance(this_point) <= radius)
                                    {
                                        limit_by_trap = true;
                                        modified_end = point;
                                    }
    ...
    */
    ...
    Vector3 dest = (/*limit_by_trap ? modified_end : */path.GetActualEndPosition());
...
void MotionMaster::ForceMovePoint(float x, float y, float z, float speed, uint32 id, bool generatePath)
{
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        TC_LOG_DEBUG("misc", "Player (GUID: %u) move point (X: %f Y: %f Z: %f)", _owner->GetGUIDLow(), x, y, z);
        Mutate(new PointMovementGenerator<Player>(id, x, y, z, generatePath, speed), MOTION_SLOT_CONTROLLED);
    }
}
...
void MotionMaster::MoveAroundPoint(Position const& centerPos, float radius, bool clockwise, uint8 numberOfPoints, float z, bool smooth)
{
    ...
    for (uint8 i = 0; i < numberOfPoints; angle += step, ++i)
    {
        ...
        if (z)
            ...
            point.z = _owner->GetPositionZ();
        //        _owner->UpdateAllowedPositionZ(point.x, point.y, point.z); need somes tests
