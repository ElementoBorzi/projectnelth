...
void FleeingMovementGenerator<T>::_setTargetLocation(T* owner)
{
    ...
    if (owner->HasUnitState(UNIT_STATE_ROOT) || owner->HasUnitState(UNIT_STATE_STUNNED))
    {
        return;
    }
...
void FleeingMovementGenerator<T>::_getPoint(T* owner, float &x, float &y, float &z)
{
    ...
    {
        Position pos;
        owner->GetFirstCollisionPosition(pos, 0.f, 0.f);
        x = pos.m_positionX;
        y = pos.m_positionY;
        z = pos.m_positionZ;
...
void FleeingMovementGenerator<Creature>::DoFinalize(Creature* owner)
{
    ...
    if (auto v = owner->getVictim())
        owner->SetTarget(v->GetGUID());
}
