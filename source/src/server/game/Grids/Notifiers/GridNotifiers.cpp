...
void VisibleNotifier::SendToSelf()
{
    ...
    if (Transport* transport = i_player.GetTransport())
        transport->UpdateVisibilityOf(i_visibleNow, vis_guids, i_data, i_player);
    ...
    for (auto it = i_visibleNow.begin(); it != i_visibleNow.end(); ++it)
    ...
    for (auto iter = m.begin(); iter != m.end(); ++iter)
        if (auto p = iter->getSource())
        {
            ...
            if (p->HasSharedVision())
                for (auto i = p->GetSharedVisionList().begin(); i != p->GetSharedVisionList().end(); ++i)
                    if (auto vision = (*i))
                        if (vision->m_seer == p)
...
void VisibleChangesNotifier::Visit(CreatureMapType &m)
{
    for (auto iter = m.begin(); iter != m.end(); ++iter)
        if (iter->getSource()->HasSharedVision())
            for (auto i = iter->getSource()->GetSharedVisionList().begin();
